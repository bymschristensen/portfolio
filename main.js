...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),o=e.querySelector(".cs-headline"),l=e.querySelectorAll(".cs-titles-inner div");a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"});o&&gsap.set(o,{autoAlpha:0,visibility:"hidden"});l.length&&gsap.set(l,{autoAlpha:0,y:20,filter:"blur(10px)"});a&&t.add(()=>gsap.set(a,{visibility:"visible"}),0).to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(o){t.addLabel("headline",.35).add(()=>gsap.set(o,{visibility:"visible",display:"block",autoAlpha:1}),"headline").call(()=>requestAnimationFrame(()=>{const r=splitAndMask(o);animateLines(r.lines).eventCallback("onComplete",()=>safelyRevertSplit(r,o));}),null,"headline")}l.length&&t.to(l,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t}
		};
