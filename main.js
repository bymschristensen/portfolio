	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async function(r){var SEL=".appear-in-line",CH=":scope > *",STAG=0.15,DUR=0.8;function arm(root){if(root.__ailArmed)return;root.__ailArmed=true;var splits=[],batches=[];Array.prototype.forEach.call(root.querySelectorAll(CH),function(child){var cs=getComputedStyle(child),cols=parseInt(cs.columnCount,10)||1,rect=child.getBoundingClientRect();if(cols>1){try{gsap.set(child,{opacity:0});var s=new SplitText(child,{type:"lines",linesClass:"split-line"});splits.push(s);var colW=rect.width/cols,acc=Array.apply(null,Array(cols)).map(function(){return[]});s.lines.forEach(function(line){var left=line.getBoundingClientRect().left-rect.left,idx=Math.min(Math.floor(left/colW),cols-1);acc[idx].push(line);gsap.set(line,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})});acc.forEach(function(arr){batches.push(arr)})}catch(err){gsap.set(child,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"});batches.push([child])}}else{gsap.set(child,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"});batches.push([child])}});var io=new IntersectionObserver(function(entries,obs){entries.forEach(function(entry){if(!entry.isIntersecting)return;obs.unobserve(entry.target);batches.forEach(function(group,i){gsap.to(group,{y:0,opacity:1,filter:"blur(0px)",duration:DUR,ease:"power2.out",delay:i*STAG})});var total=(batches.length-1)*STAG+DUR;gsap.delayedCall(total+0.05,function(){try{splits.forEach(function(s){s.revert()})}catch(e){}})})},{root:null,rootMargin:"0px 0px -10% 0px",threshold:0});CoreUtilities.Observers.addDom(io);io.observe(root);var rect0=root.getBoundingClientRect();if(rect0.top<window.innerHeight&&rect0.bottom>0){io.unobserve(root);batches.forEach(function(group,i){gsap.to(group,{y:0,opacity:1,filter:"blur(0px)",duration:DUR,ease:"power2.out",delay:i*STAG})});var total2=(batches.length-1)*STAG+DUR;gsap.delayedCall(total2+0.05,function(){try{splits.forEach(function(s){s.revert()})}catch(e){}})}var tabs=r.querySelector(".w-tabs");if(tabs){var pane=root.closest(".w-tab-pane");if(pane){var mo=new MutationObserver(function(){if(!pane.classList.contains("w--tab-active"))return;io.observe(root);var rr=root.getBoundingClientRect();if(rr.top<window.innerHeight&&rr.bottom>0){io.unobserve(root);batches.forEach(function(group,i){gsap.to(group,{y:0,opacity:1,filter:"blur(0px)",duration:DUR,ease:"power2.out",delay:i*STAG})});var total3=(batches.length-1)*STAG+DUR;gsap.delayedCall(total3+0.05,function(){try{splits.forEach(function(s){s.revert()})}catch(e){}})}});CoreUtilities.Observers.addDom(mo);mo.observe(pane,{attributes:true,attributeFilter:["class"]})}}}r.querySelectorAll(SEL).forEach(arm);}
			})
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			selected(e=document){const t=e.querySelector(".selected-container"),a=t?.querySelector(".selected-content"),o=Array.from(e.querySelectorAll(".selected-item-outer")),l=gsap.timeline();if(!(t&&a&&o.length))return l;o.forEach(e=>{if(e.__entryDone)return;const t=e.querySelector(".selected-visual"),a=e.querySelector(".selected-item-header .headline-m"),o=e.querySelector(".selected-item-details"),l=e.querySelectorAll(".selected-item-details .body-s");t&&gsap.set(t,{scaleY:0,transformOrigin:"bottom center",opacity:0}),a&&gsap.set(a,{opacity:0}),o&&gsap.set(o,{opacity:0,height:0}),l.length&&gsap.set(l,{opacity:0,y:20,filter:"blur(10px)"})});const r=i=>{const n=()=>{requestAnimationFrame(()=>requestAnimationFrame(i))};if(t.hasAttribute("data-loop-ready")){n();return}const s=()=>{a.removeEventListener("selected:loop-ready",s,!0),n()};a.addEventListener("selected:loop-ready",s,!0),setTimeout(()=>{a.removeEventListener("selected:loop-ready",s,!0),n()},600)};r(()=>{const W=window.innerWidth||document.documentElement.clientWidth,H=window.innerHeight||document.documentElement.clientHeight;const d=o.map(el=>{const r=el.getBoundingClientRect();return{ o:el,r,area:Math.max(0,Math.min(r.right,W)-Math.max(r.left,0))*Math.max(0,Math.min(r.bottom,H)-Math.max(r.top,0)),center:.5*(r.left+r.right)}});let v=d.filter(x=>x.area>1).sort((a,b)=>a.r.left-b.r.left);if(!v.length){const M=.5*W;v=d.slice().sort((a,b)=>Math.abs(a.center-M)-Math.abs(b.center-M)).slice(0,2).sort((a,b)=>a.r.left-b.r.left)}const S=new Set(v.map(x=>x.o));d.forEach(x=>{if(x.o.__entryDone||S.has(x.o))return;const n=x.o.querySelector(".selected-visual"),r=x.o.querySelector(".selected-item-header .headline-m"),i=x.o.querySelector(".selected-item-details"),s=x.o.querySelectorAll(".selected-item-details .body-s");n&&gsap.set(n,{scaleY:1,opacity:1}),r&&gsap.set(r,{opacity:1}),i&&gsap.set(i,{opacity:1,height:"auto"}),s.length&&gsap.set(s,{opacity:1,y:0,filter:"blur(0px)"}),x.o.__entryDone=!0});v.forEach((entry,idx)=>{const el=entry.o;if(el.__entryDone)return;const n=el.querySelector(".selected-visual"),r=el.querySelector(".selected-item-header .headline-m"),i=el.querySelector(".selected-item-details"),s=el.querySelectorAll(".selected-item-details .body-s"),c=.15*idx;n&&l.set(n,{opacity:1},c).to(n,{scaleY:1,duration:.8,ease:"power2.out"},c);r&&l.set(r,{opacity:1},c+.2).call(()=>{if(r.__splitRun)return;r.__splitRun=!0,CoreUtilities.Fonts.ready().then(()=>{const sp=splitAndMask(r);gsap.delayedCall(.15,()=>{animateLines(sp.lines).eventCallback("onComplete",()=>safelyRevertSplit(sp,r))})})},null,c+.2);i&&l.to(i,{opacity:1,height:"auto",duration:.4,ease:"power2.out"},c+.6);s.length&&l.to(s,{opacity:1,y:0,filter:"blur(0px)",duration:.4,ease:"power2.out",stagger:.15},c+.6);el.__entryDone=!0})});return l},
