...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var S=[].slice.call(r.querySelectorAll(".section-single-service"));if(!S.length)return;S.forEach(function(sec){var small=sec.offsetHeight<window.innerHeight;gsap.timeline({scrollTrigger:{trigger:sec,start:small?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}}).to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0)});}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
