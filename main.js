...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),H=e.querySelector(".cs-hero-image"),A=e.querySelector(".cs-headline"),N=e.querySelectorAll(".cs-titles-inner div"),dfr=()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));function splitMask(n){if(!window.SplitText)return null;const ws=getComputedStyle(n).whiteSpace||"normal",pWS=n.style.whiteSpace,pD=n.style.display,wasInline=getComputedStyle(n).display==="inline";n.style.whiteSpace=ws;wasInline&&(n.style.display="block");n.clientWidth;const sp=new SplitText(n,{type:"lines",linesClass:"line",reduceWhiteSpace:!1});sp.__masks=[];sp.lines.forEach(L=>{const rect=L.getBoundingClientRect(),h=Math.ceil(rect.height||L.offsetHeight||0),m=document.createElement("div");m.className="text-mask";m.style.cssText="overflow:hidden;display:block;width:100%;box-sizing:border-box;flex:0 0 100%;align-self:stretch;grid-column:1/-1;clear:both";L.style.whiteSpace=ws;L.style.display="block";L.style.width="100%";L.style.boxSizing="border-box";L.parentNode.insertBefore(m,L);m.appendChild(L);m.style.height=h+"px";sp.__masks.push(m);gsap.set(L,{y:h,opacity:1,transformOrigin:"0 0",willChange:"transform,opacity",force3D:!0})});n.style.whiteSpace=pWS;n.style.display=pD;return sp}H&&gsap.set(H,{autoAlpha:0,y:80,filter:"blur(10px)"}),A&&gsap.set(A,{autoAlpha:0}),N.length&&gsap.set(N,{autoAlpha:0,y:20,filter:"blur(10px)"}),H&&t.to(H,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(A){const orig=A.style.display,inline=getComputedStyle(A).display==="inline";t.addLabel("headline",.35).set(A,{autoAlpha:1,visibility:"visible"},"headline").call(()=>{const run=()=>{try{const sp=splitMask(A);if(!sp){gsap.to(A,{y:0,opacity:1,duration:.6,ease:"power2.out"});return}gsap.to(sp.lines,{y:0,duration:.8,stagger:.08,ease:"power2.out",onComplete:()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:sp.revert).call(sp,sp,A)}catch{}try{sp.__masks&&sp.__masks.forEach(m=>m.remove())}catch{}inline&&(A.style.display="inline");A.style.display=A.style.display||orig}})}catch{gsap.to(A,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(dfr).then(run)},null,"headline")}return N.length&&t.to(N,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
