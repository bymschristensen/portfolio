...
// Entry Animation Controller
	window.EntryGate=function(){const t="data-entry-gated";function e(e){if(e){e.removeAttribute(t),e.style.visibility="";try{gsap.set(e,{autoAlpha:1,clearProps:"opacity"})}catch{e.style.opacity="1"}}}return{arm:function(e){if(e&&"1"!==e.getAttribute(t)){e.setAttribute(t,"1"),e.style.visibility="hidden";try{gsap.set(e,{autoAlpha:0})}catch{e.style.opacity="0"}}},release:e,clear:function(){document.querySelectorAll(`[${t}="1"]`).forEach(e)}}}();
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		async function runEntryFlow(t,e){e=e||{},EntryGate.arm(t),e.withCoverOut&&await TransitionEffects.coverOut(),await InitManager.run(t,{preserveServicePins:!1});const n=runPageEntryAnimations(t),a=n&&n.tl?n.tl:gsap.timeline(),r=n&&"number"==typeof n.entryOffset?n.entryOffset:0;EntryGate.release(t),a&&a.duration&&a.duration()&&(a.play(r),await new Promise((t=>a.eventCallback("onComplete",t)))),await finalizeAfterEntry(t)}
		...
		// Entry Animations
		var EntryAnimations = {};
		EntryAnimations.selected=function(e){e=e||document;var t=e.querySelector(".selected-container"),n=t&&t.querySelector(".selected-content"),i=gsap.timeline();if(!t||!n||!window.gsap)return i;function r(){try{var e=[n].concat([].slice.call(n.querySelectorAll(".selected-item-outer")),[].slice.call(n.querySelectorAll(".appear-in-line")),[].slice.call(n.querySelectorAll(".selected-item-outer .selected-visual")),[].slice.call(n.querySelectorAll(".selected-item-outer .selected-item-header .headline-md")),[].slice.call(n.querySelectorAll(".selected-item-outer .selected-item-details")));e.length&&gsap.set(e,{visibility:"visible",opacity:1,clearProps:"visibility,opacity"}),document.documentElement.removeAttribute("data-preloading")}catch{}}function o(){var e=Math.max(document.documentElement.clientWidth,window.innerWidth||0),t=window.innerHeight||document.documentElement.clientHeight||0,r=[].slice.call(n.querySelectorAll(".selected-item-outer"));if(!r.length)return i;function o(e){if(e.__entryPrepared)return;var t=e.querySelector(".selected-visual"),n=e.querySelector(".selected-item-header .headline-md"),i=e.querySelector(".selected-item-details"),r=e.querySelectorAll(".selected-item-details .body-sm");t&&gsap.set(t,{scaleY:0,transformOrigin:"bottom center",opacity:0,willChange:"transform,opacity"}),n&&gsap.set(n,{opacity:0,willChange:"opacity,transform"}),i&&gsap.set(i,{opacity:0,height:0,willChange:"height,opacity"}),r.length&&gsap.set(r,{opacity:0,y:20,filter:"blur(10px)",willChange:"transform,opacity,filter"}),e.__entryPrepared=1}r.forEach(o);var a=r.map((function(t){var n=t.getBoundingClientRect();return{el:t,rect:n,area:Math.max(0,Math.min(n.right,e)-Math.max(n.left,0))*Math.max(0,Math.min(n.bottom,t)-Math.max(n.top,0)),cx:(n.left+n.right)/2}})),l=a.filter((function(e){return e.area>1})).sort((function(e,t){return e.rect.left-t.rect.left}));if(!l.length){var s=e/2;l=a.slice().sort((function(e,t){return Math.abs(e.cx-s)-Math.abs(t.cx-s)})).slice(0,2).sort((function(e,t){return e.rect.left-t.rect.left}))}var d=new Set(l.map((function(e){return e.el})));a.forEach((function(e){if(e.el.__entryDone||d.has(e.el))return;var t=e.el.querySelector(".selected-visual"),n=e.el.querySelector(".selected-item-header .headline-md"),i=e.el.querySelector(".selected-item-details"),r=e.el.querySelectorAll(".selected-item-details .body-sm");t&&gsap.set(t,{scaleY:1,opacity:1,clearProps:"willChange"}),n&&gsap.set(n,{opacity:1,clearProps:"willChange"}),i&&gsap.set(i,{opacity:1,height:"auto",clearProps:"willChange"}),r.length&&gsap.set(r,{opacity:1,y:0,filter:"blur(0px)",clearProps:"willChange"}),e.el.__entryDone=1})),l.forEach((function(e,t){var n=e.el;if(n.__entryDone)return;var r=n.querySelector(".selected-visual"),o=n.querySelector(".selected-item-header .headline-md"),a=n.querySelector(".selected-item-details"),l=n.querySelectorAll(".selected-item-details .body-sm"),s=.14*t;r&&i.set(r,{opacity:1},s).to(r,{scaleY:1,opacity:1,duration:.75,ease:"power2.out",clearProps:"willChange"},s),o&&i.set(o,{opacity:1},s+.16).call((function(e){var t=CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve();t.then((function(){try{if(window.SplitText&&window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){var t=splitAndMask(e);animateLines(t.lines).eventCallback("onComplete",(function(){safelyRevertSplit(t,e)}))}else gsap.to(e,{y:0,opacity:1,duration:.55,ease:"power2.out"})}catch(t){gsap.to(e,{y:0,opacity:1,duration:.55,ease:"power2.out"})}}))}),[o],s+.16),a&&i.to(a,{opacity:1,height:"auto",duration:.4,ease:"power2.out",clearProps:"willChange"},s+.56),l.length&&i.to(l,{opacity:1,y:0,filter:"blur(0px)",duration:.4,ease:"power2.out",stagger:.12,clearProps:"willChange"},s+.56),n.__entryDone=1}));return i}function a(){return new Promise((function(e){requestAnimationFrame((function(){requestAnimationFrame(e)}))}))}function l(){r();return a().then(o)}if(t.hasAttribute("data-loop-ready"))l();else{var s=function e(){n.removeEventListener("selected:loop-ready",e,!0),t.removeEventListener("selected:loop-ready",e,!0),l()};n.addEventListener("selected:loop-ready",s,!0),t.addEventListener("selected:loop-ready",s,!0),setTimeout(l,520)}return i};
		...
		EntryAnimations.capabilities=function(e,t){e=e||document,t=t||{};var a=!!t.delayHero,o=gsap.timeline(),r=e.querySelector(".showreel-headline")||e.querySelector(".hero-section-showreel .showreel-headline")||e.querySelector(".section-hero .headline-lg"),n=e.querySelector(".showreel-container")||e.querySelector(".hero-section-showreel .showreel-container")||e.querySelector(".approach-mask"),i=e.querySelector(".section-table-of-contents"),s=[].slice.call(e.querySelectorAll(".table-of-contents-item")),l=a?.2:0;if(i&&gsap.set(i,{autoAlpha:0}),r&&gsap.set(r,{autoAlpha:0,visibility:"visible"}),n){var c=!!(window.CSS&&CSS.supports&&CSS.supports("clip-path: polygon(0% 0%,100% 0%,100% 100%,0% 100%)"));gsap.set(n,{autoAlpha:1,visibility:"visible",willChange:"clip-path,transform"}),c?(gsap.set(n,{clipPath:"polygon(100% 100%,100% 100%,100% 100%,100% 100%)"}),o.to(n,{clipPath:"polygon(0% 0%,100% 0%,100% 100%,0% 100%)",duration:1.05,ease:"power2.out",clearProps:"willChange"},l+.55)):(gsap.set(n,{transformOrigin:"100% 100%",scale:0,force3D:!0}),o.to(n,{scale:1,duration:1.05,ease:"power2.out",clearProps:"transformOrigin,willChange"},l+.55))}if(r){o.addLabel("heroStart",l).set(r,{autoAlpha:1},"heroStart").call(function(){var e=window.CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve();e.then(function(){try{if(window.SplitText&&window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){var e=splitAndMask(r);animateLines(e.lines).eventCallback("onComplete",function(){safelyRevertSplit(e,r)})}else gsap.to(r,{y:0,opacity:1,duration:.6,ease:"power2.out"})}catch(e){gsap.to(r,{y:0,opacity:1,duration:.6,ease:"power2.out"})}})},null,"heroStart")}return s.length&&o.from(s,{autoAlpha:0,paddingTop:"6rem",paddingBottom:"6rem",duration:1,ease:"power2.out",stagger:.15},l),i&&o.to(i,{autoAlpha:1,duration:.6,ease:"power2.out"},l),o};
