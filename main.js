...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=document.documentElement,i=e.querySelector(".cs-hero-image"),o=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),r=e=>{if(!e)return null;let t=e.querySelector(".ta-one,[data-animate-headline],h1")||e;for(;t&&t.children&&1===t.children.length&&t.textContent.trim()===t.children[0].textContent.trim();)t=t.children[0];return t},s=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList&&e.classList.remove(t))}catch{}};function l(E){if(!window.SplitText)return null;const W=getComputedStyle(E),A=W.whiteSpace||"normal",P=E.style.whiteSpace,_=E.style.display;E.style.whiteSpace=A,"inline"===getComputedStyle(E).display&&(E.style.display="block"),E.clientWidth;const S=new SplitText(E,{type:"lines",linesClass:"line",reduceWhiteSpace:!1});S.lines.forEach(L=>{const h=L.getBoundingClientRect().height||L.offsetHeight||0,c=document.createElement("div");c.className="text-mask",c.style.cssText="overflow:hidden;display:block;height:"+h+"px",L.style.whiteSpace=A,L.style.display="block",L.parentNode.insertBefore(c,L),c.appendChild(L)}),gsap.set(S.lines,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform,opacity"}),E.style.whiteSpace=P,E.style.display=_;return S}a.classList.add("entry-ready");const d=r(o);o&&(o.style.visibility="visible"),d&&(d.style.visibility="visible"),i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)",overwrite:"auto",immediateRender:!0}),d&&gsap.set(d,{autoAlpha:0,overwrite:"auto",immediateRender:!0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),s(i),s(o),n.forEach(s),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(d){const E=d.style.display,C="inline"===getComputedStyle(d).display;C&&(d.style.display="block");const u=()=>{C&&(d.style.display="inline")},f=()=>gsap.to(d,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete:u}),g=()=>{try{const y=l(d);if(!y){f();return}animateLines(y.lines).eventCallback("onComplete",()=>{(window.safelyRevertSplit?window.safelyRevertSplit:y.revert).call(y,y,d),u()})}catch{f()}};let m=0,p=null;const v=()=>{if(window.SplitText){g();return}if(m++<12){p=setTimeout(v,60);return}f()};t.addLabel("headline",.35).set(d,{autoAlpha:1,visibility:"visible"},"headline").call(()=>{p&&clearTimeout(p),requestAnimationFrame(()=>{CoreUtilities.Fonts.ready().then(v)})},null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
