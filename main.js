	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				namespaces:'*',
				selectors:['.ta-one'],
				init: async r=>{const{splitAndMask:a,safelyRevertSplit:t,animateLines:e}=CoreUtilities.Text,n=".ta-one",o=[...r.querySelectorAll(n)].filter(s=>!s.closest(".cs-headline"));o.forEach(s=>{gsap.set(s,{autoAlpha:0}),t(s._split,s)});const c=CoreUtilities.Observers.addDom(new IntersectionObserver((s,i)=>{s.forEach(d=>{if(!d.isIntersecting)return;const f=d.target;gsap.set(f,{autoAlpha:1});const u=a(f);e(u.lines).eventCallback("onComplete",()=>{t(u,f),f.__taDone=!0}),i&&i.unobserve(f)})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));o.forEach(s=>{if(s.__taDone||s.__taOneDone)return;c.observe(s);const i=s.getBoundingClientRect();if(i.top<innerHeight&&i.bottom>0){c.unobserve(s);gsap.set(s,{autoAlpha:1});const d=a(s);e(d.lines).eventCallback("onComplete",()=>{t(d,s),s.__taDone=!0})}});r.querySelectorAll(".w-tabs .w-tab-pane").forEach(s=>{CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!s.classList.contains("w--tab-active"))return;s.querySelectorAll(n).forEach(f=>{if(f.__taDone||f.__taOneDone||f.closest(".cs-headline"))return;const u=f.getBoundingClientRect();if(u.top<innerHeight&&u.bottom>0){gsap.set(f,{autoAlpha:1});const v=a(f);e(v.lines).eventCallback("onComplete",()=>{t(v,f),f.__taDone=!0})}})})).observe(s,{attributes:!0,attributeFilter:["class"]})})}
			}),
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),a=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),o=e=>{e&&(e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t)),e.style.visibility="visible")},r=e=>{if(!e)return null;let t=e.querySelector(".ta-one,[data-animate-headline],h1")||e;for(;t&&t.children&&1===t.children.length&&t.textContent.trim()===t.children[0].textContent.trim();)t=t.children[0];return t},l=r(a);[a,l,i,...n].forEach(o),i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"}),l&&gsap.set(l,{autoAlpha:0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(l){const e=l.style.display,o="inline"===getComputedStyle(l).display;o&&(l.style.display="block"),t.addLabel("headline",.35).set(l,{autoAlpha:1,display:"block",visibility:"visible"},"headline").call(()=>requestAnimationFrame(()=>{try{const t=splitAndMask(l);animateLines(t.lines).eventCallback("onComplete",()=>{safelyRevertSplit(t,l),l.style.display=e})}catch(t){l.style.display=e,gsap.set(l,{autoAlpha:1})}}),null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
