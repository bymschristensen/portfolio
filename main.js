...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),o=e.querySelectorAll(".cs-titles-inner div"),n=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t))}catch{}};a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"}),i&&gsap.set(i,{autoAlpha:0,visibility:"hidden"}),o.length&&gsap.set(o,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(a),n(i),o.forEach(n),a&&t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),i&&t.addLabel("headline",.35).call(()=>{const t=()=>{try{const e=getComputedStyle(i),t=i.getBoundingClientRect(),n=e.display,s="inline"===n?"inline-block":n;i.style.display=s,i.style.width=Math.max(1,Math.round(t.width))+"px",i.style.height=Math.max(1,Math.round(t.height))+"px",i.style.visibility="visible",i.style.willChange="contents,transform,opacity";if(window.splitAndMask&&window.animateLines){const e=splitAndMask(i);gsap.set(i,{autoAlpha:1});animateLines(e.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:e.revert).call(e,e,i)}catch{}i.style.willChange="",i.style.width="",i.style.height="",i.style.display=n})}else gsap.set(i,{autoAlpha:1}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete(){i.style.willChange="",i.style.width="",i.style.height="",i.style.display=n}})}catch{gsap.set(i,{autoAlpha:1,visibility:"visible"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>requestAnimationFrame(()=>requestAnimationFrame(t))).catch(t)},null,"headline"),o.length&&t.to(o,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t;}
		};
