	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init:async r=>{const L=[...r.querySelectorAll(".accordion-list")];if(!L.length)return;let to;const rf=()=>{clearTimeout(to);to=setTimeout(()=>{try{ScrollTrigger.refresh()}catch(e){}},120)};L.forEach(l=>{const A=[...l.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];if(!A.length)return;A.forEach(it=>{if(it._accBound)return;it._accBound=1;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:null});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":null,duration:.5,onComplete:()=>gsap.set(ct,{maxHeight:"none"})},0);if(isQ&&qi)tl.from(qi,{opacity:0,duration:.4},0);tl.eventCallback("onComplete",rf);tl.eventCallback("onReverseComplete",rf);it._accTL=tl;it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");A.forEach(o=>{if(o!==it){const oh=o.querySelector(".accordion-header"),ot=o._accTL;if(oh&&ot&&oh.classList.contains("accordion-active")&&!ot.isActive()){oh.classList.remove("accordion-active");ot.reverse();}}});open?(gsap.set(ct,{maxHeight:ct.offsetHeight}),tl.reverse()):(gsap.set(ct,{maxHeight:0}),tl.play());h.classList.toggle("accordion-active",!open);});});});}
			}),
