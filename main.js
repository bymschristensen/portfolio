...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(root){if(!gsap||!ScrollTrigger)return;root=root||document;var sections=[].slice.call(root.querySelectorAll(".section-single-service"));if(!sections.length)return;sections.forEach(function(sec){var tl=gsap.timeline({scrollTrigger:{trigger:sec,start:"top top+=80",end:"bottom top",pin:!0,pinSpacing:!0,scrub:!1,invalidateOnRefresh:!0}}),overlay=sec.querySelector(".single-service-overlay");overlay&&(gsap.set(overlay,{opacity:0,pointerEvents:"none"}),tl.to(overlay,{opacity:1,duration:.3,ease:"power2.out"},.2).to(overlay,{opacity:0,duration:.3,ease:"power2.in"},.7))})}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
