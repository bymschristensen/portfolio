	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: function(root){root=root||document;var acc=[...root.querySelectorAll("[data-accordion]")];if(!acc.length)return;const rf=()=>{requestAnimationFrame(()=>requestAnimationFrame(()=>{try{ScrollTrigger.refresh()}catch(e){}}))};acc.forEach(a=>{var head=a.querySelector("[data-accordion-head]"),body=a.querySelector("[data-accordion-body]");if(!head||!body)return;gsap.set(body,{height:0,overflow:"hidden"});a.dataset.open="0";head.addEventListener("click",()=>{var open=a.dataset.open==="1",h=open?0:body.scrollHeight;gsap.to(body,{height:h,duration:.35,ease:"power2.out",onComplete:rf,onReverseComplete:rf});a.dataset.open=open?"0":"1"})})}
			}),
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: function(r){if(!window.gsap||!window.ScrollTrigger)return;r=r||document;var S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;ScrollTrigger.addEventListener("refreshInit",()=>{S.forEach(s=>{s.style.removeProperty("transform")})});S.forEach((sec,i)=>{sec.style.filter="contrast(100%) blur(0px)";ScrollTrigger.create({id:"servicePin:"+i,trigger:sec,start:i===0?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0,anticipatePin:1,end:()=>sec.offsetHeight+innerHeight, onUpdate:self=>{var p=self.progress,c=100-(p*90),b=p*10;sec.style.filter="contrast("+c+"%) blur("+b+"px)"}})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
