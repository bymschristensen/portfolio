	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				stage: "main",
				namespaces: ["archive"],
				selectors: [".section-archive"],
				init: function(r){if(!window.gsap||!window.ScrollTrigger)return;r=r||document;var h=r.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||r.querySelector('[data-barba="container"]')||r;if(h.__archiveBgShiftBound)return;h.__archiveBgShiftBound=1;var s=h.querySelector(".section-archive");if(!s){h.__archiveBgShiftBound=0;return}if(matchMedia("(prefers-reduced-motion: reduce)").matches){h.__archiveBgShiftBound=0;return}var cs=getComputedStyle(s),orig=cs&&cs.backgroundColor?cs.backgroundColor:"rgba(0,0,0,0)",tw=0,st=0;function kill(){try{tw&&tw.kill&&tw.kill()}catch(_){}tw=0}function to(c){kill();tw=gsap.to(s,{backgroundColor:c,duration:.55,ease:"power2.out",overwrite:!0})}function start(){return"top -"+Math.round(innerHeight*.5)+"px"}st=ScrollTrigger.create({trigger:document.body,start:start,onEnter:function(){to("transparent")},onLeaveBack:function(){to(orig)},invalidateOnRefresh:!0});return function(){try{st&&st.kill&&st.kill()}catch(_){}st=0;kill();try{h.__archiveBgShiftBound=0}catch(_){} }}
			})
		);
