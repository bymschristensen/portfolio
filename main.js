	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init:async r=>{const L=[...(r||document).querySelectorAll(".accordion-list")];if(!L.length)return;L.forEach(l=>{const A=[...l.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];A.forEach(it=>{if(it._acc)return;it._acc=1;const h=it.querySelector(".accordion-header"),c=it.querySelector(".accordion-content"),x=it.querySelector(".cross-line-animating"),q=it.classList.contains("accordion-quote");if(!h||!c)return;gsap.set(c,{height:0,opacity:0,overflow:"hidden"});const tl=gsap.timeline({paused:1,defaults:{ease:"power2.out"}}).to(x,{rotation:0,duration:.3},0).to(c,{height:"auto",opacity:1,duration:.45},0);it._tl=tl;h.addEventListener("click",e=>{e.preventDefault();if(tl.isActive())return;const o=h.classList.contains("accordion-active");A.forEach(a=>{if(a!==it&&a._tl&&a.querySelector(".accordion-header")?.classList.contains("accordion-active")){a.querySelector(".accordion-header").classList.remove("accordion-active");a._tl.reverse()}});h.classList.toggle("accordion-active",!o);o?tl.reverse():tl.play()},{passive:!1})})})}
			}),
