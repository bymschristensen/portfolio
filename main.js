...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(t){const e=matchMedia("(min-width:1024px)");if(!e.matches)return;const r=t.querySelector(".hero-section-showreel"),n=r?.querySelector(".showreel-track"),o=n?.querySelector(".showreel-container");if(!r||!n||!o)return;const i=[r.querySelector(".showreel-headline"),r.querySelector(".tableofcontents"),r.querySelector(".toc-outer"),r.querySelector(".showreel-subtext")].filter(Boolean),s=getComputedStyle(o).transform||"none",pfx="none"===s?"":s+" ";let h=window.innerHeight,c=0,l=0,u=1,m=.5,f=!1;function d(t){return t<0?0:t>1?1:t}function a(t,e,r){return t+(e-t)*r}function g(){const t=n.getBoundingClientRect();c=Math.max(1,t.width);const e=o.style.transform;o.style.transform=s;l=Math.max(1,o.getBoundingClientRect().width);o.style.transform=e;u=Math.max(1,c/l)}function v(){const t=r.getBoundingClientRect(),e=t.height-h;if(e<=0)return 0;const n=-t.top/e;return d(n)}function y(){const t=v(),e=a(1,u,t),r=l*e,q=Math.max(0,c-r),x=(m-.5)*q;o.style.transform=pfx+"translate3d("+x+"px,0,0) scale("+e+")";const w=-.45*h*t,k=1-t;i.forEach((t=>{t&&(t.style.transform="translate3d(0,"+w+"px,0)",t.style.opacity=k)}))}function S(){f||(f=!0,requestAnimationFrame(()=>{f=!1,y()}))}function b(t){const e=n.getBoundingClientRect(),r=Math.max(e.left,Math.min(t.clientX,e.right));m=(r-e.left)/e.width,S()}function E(){h=window.innerHeight,g(),S()}function T(){S()}g(),y(),window.addEventListener("scroll",T,{passive:!0}),window.addEventListener("resize",E,{passive:!0}),n.addEventListener("mousemove",b,{passive:!0});const C=new ResizeObserver(()=>E());try{C.observe(n),C.observe(o)}catch(t){}r.__heroShowreelDestroy__=()=>{try{window.removeEventListener("scroll",T)}catch(t){}try{window.removeEventListener("resize",E)}catch(t){}try{n.removeEventListener("mousemove",b)}catch(t){}try{C.disconnect()}catch(t){}try{o.style.transform="none"===s?"":s,i.forEach(t=>{t.style.transform="",t.style.opacity=""})}catch(t){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(t){}const e=document.querySelector(".hero-section-showreel");e&&(e.__heroShowreelDestroy__=null)}}),
			feature({
