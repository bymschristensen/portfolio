	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async R=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const A=e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const n=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0;let o=[];return Promise.race([Promise.all(t.map(e=>{if(n(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),o=new Promise(t=>{const n=()=>{e.removeEventListener("load",n),t()};e.addEventListener("load",n,{once:!0})}),i=new Promise(t=>{let o=0;!function r(){if(n(e)||o++>80)return t();setTimeout(r,40)}()});return Promise.race([t,o,i])})),new Promise(e=>o.push(setTimeout(e,3500)))]).finally(()=>o.forEach(clearTimeout))},H=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),q=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const n=(e.className||"").match(/sv-ratio-(\d+)/);if(n){const e=n[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const o=e.querySelector("img,video");if(o){const e=parseInt(o.getAttribute("width"),10)||o.naturalWidth||o.videoWidth||0,t=parseInt(o.getAttribute("height"),10)||o.naturalHeight||o.videoHeight||0;if(e>0&&t>0)return e/t}return 3/2},T=(e,t)=>{const n=parseFloat(e.getAttribute("data-size"));if(n&&n>0)return Math.max(1,Math.round(t*n/100));const o=(e.className||"").match(/sv-size-(\d+)/);if(o){const e=parseFloat(o[1]);if(e>0)return Math.max(1,Math.round(t*e/100))}const i=e.getBoundingClientRect().height||0;return i>0?Math.round(i):Math.max(1,Math.round(.5*t))},W=()=>Math.max(document.documentElement.clientWidth,window.innerWidth||0),G=[...R.querySelectorAll(".infinite-gallery")];if(!G.length)return;G.forEach(async E=>{const L=E.querySelector(".infinite-gallery-wrapper");if(!L||L.__inited)return;L.__inited=!0;E.setAttribute("data-armed","measure");const C=[...L.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!C.length){E.setAttribute("data-armed","1");return}const k=new Set;let a=!1,r=0,s=0,x=0,d=("right"===(E.dataset.direction||"left").toLowerCase()?-1:1),u=parseFloat(E.dataset.speed)||.6;C.forEach((e,t)=>{const n=q(e);e.dataset.ratio=String(n),e.dataset.key=String(t),e.dataset.revealed="0",e.dataset.revealing="",e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.height="0px",e.style.willChange="height,transform",H?(e.style.aspectRatio=n+" / 1",e.style.removeProperty("width"),e.style.removeProperty("min-width"),e.style.removeProperty("max-width")):(e.style.width="1px",e.style.minWidth="1px",e.style.maxWidth="1px")});const y=()=>{C.forEach(e=>{const t=parseFloat(e.dataset.ratio)||q(e),n=T(e,L.clientHeight);e.dataset.targetH=String(n),H?e.style.aspectRatio=t+" / 1":(()=>{const o=Math.max(1,Math.round(n*t));e.style.width=o+"px",e.style.minWidth=o+"px",e.style.maxWidth=o+"px"})()}),L.style.gap=getComputedStyle(L).gap||"0px"},v=()=>{[...L.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())},b=e=>{const t=e.cloneNode(!0),n=e.dataset.key||"",o=parseFloat(e.dataset.ratio)||q(e),i=+e.dataset.targetH||0;return t.setAttribute("data-clone","1"),t.dataset.key=n,t.dataset.ratio=String(o),t.style.overflow="hidden",t.style.flex="0 0 auto",t.style.willChange="height,transform",t.style.height=k.has(n)&&i>0?i+"px":"0px",H?t.style.aspectRatio=o+" / 1":(()=>{const e=Math.max(1,Math.round(Math.max(1,i)*o));t.style.width=e+"px",t.style.minWidth=e+"px",t.style.maxWidth=e+"px"})(),L.appendChild(t),t},w=()=>{C.forEach(b)},M=()=>{let e=0;return L.childNodes.forEach(t=>{1===t.nodeType&&(e+=(t.getBoundingClientRect().width||0)+(parseFloat(getComputedStyle(L).gap||"0")||0))}),e},S=()=>{v(),w(),w();let e=0;for(;M()<3*W()&&e++<8;)w()},I=()=>{E.setAttribute("data-armed","1");const e=E.getBoundingClientRect(),t=innerHeight||document.documentElement.clientHeight||0;e.bottom>0&&e.top<t&&(a=!0,cancelAnimationFrame(r),r=requestAnimationFrame(O))},O=e=>{if(!a){cancelAnimationFrame(r);return}if((e||0)-s<32){r=requestAnimationFrame(O);return}s=e||performance.now();const t=innerWidth||document.documentElement.clientWidth||0,n=-.05*t,o=1.05*t,i=[...L.querySelectorAll('.service-visual-wrapper:not([data-clone])')].filter(e=>{const t=e.getBoundingClientRect();return t.right>n&&t.left<o}).filter(e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing).sort((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left);if(i.length){const e=gsap.timeline();i.forEach((t,n)=>{const o=+t.dataset.targetH||0,i=t.dataset.key||"",r=parseFloat(t.dataset.ratio)||q(t);t.dataset.revealing="1",e.fromTo(t,{height:0},{height:o,duration:.9,ease:"power2.out",onComplete:()=>{t.style.height="",t.dataset.revealed="1",t.dataset.revealing="",k.add(i),L.querySelectorAll('.service-visual-wrapper[data-clone="1"][data-key="'+i+'"]').forEach(e=>{e.style.height=o+"px";if(!H){const t=Math.max(1,Math.round(o*r));e.style.width=t+"px",e.style.minWidth=t+"px",e.style.maxWidth=t+"px"}}),H||(()=>{const e=Math.max(1,Math.round(o*r));t.style.width=e+"px",t.style.minWidth=e+"px",t.style.maxWidth=e+"px"})()}},.12*n)})}r=requestAnimationFrame(O)},P=(e,t)=>{const n=(t||16.6667)/1e3,o=60*u;x-=d*n*o;const i=e=>e.getBoundingClientRect().width+(parseFloat(getComputedStyle(L).gap||"0")||0);let r=L.firstElementChild,s=0;for(;r&&x<-i(r)&&s++<50;)x+=i(r),L.appendChild(r),r=L.firstElementChild;let l=L.lastElementChild;for(s=0;l&&x>0&&s++<50;)x-=i(l),L.insertBefore(l,L.firstElementChild),l=L.lastElementChild;gsap.set(L,{x:x});const c=(innerWidth||0)/2;L.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const n=t.getBoundingClientRect(),o=(c-(n.left+n.width/2))/((innerWidth)||1);e.style.setProperty("--drift",40*o+"px")})},N=e=>{e.forEach(e=>{e.target===E&&(e.isIntersecting?(a=!0,cancelAnimationFrame(r),r=requestAnimationFrame(O)):(a=!1,cancelAnimationFrame(r)))})},RO=new ResizeObserver(()=>{y(),v(),S(),C.forEach(e=>{const t=e.dataset.key||"",n=+e.dataset.targetH||0;L.querySelectorAll('.service-visual-wrapper[data-clone="1"][data-key="'+t+'"]').forEach(e=>{e.style.height=n+"px"})}),I()});await A(L),y(),S(),I();const io=new IntersectionObserver(N,{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});io.observe(E),CoreUtilities.Observers.addDom(io),CoreUtilities.Observers.addDom({disconnect(){a=!1,cancelAnimationFrame(r)}});const tick=(e,t)=>P(e,t);gsap.ticker.add(tick),CoreUtilities.Observers.addTicker(tick),RO.observe(L),CoreUtilities.Observers.addDom(RO)})}
			})
		);
