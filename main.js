	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				stage: "main",
				namespaces: ["archive"],
				selectors: [".section-archive-playful",".archive-playful-list",".archive-playful-item",".archive-playful-item-data",".archive-playful-item-data-outer",".archive-playful-data-inner",".archive-playful-visuals",".archive-playful-visual-block",".archive-playful-visual-img",".shortcut-link"],
				init: function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__archivePlayfulBound)return;host.__archivePlayfulBound=1;if(!window.gsap||!window.ScrollTrigger)return host.__archivePlayfulBound=0,void 0;var mm=matchMedia("(min-width:1024px)"),pref=matchMedia("(prefers-reduced-motion: reduce)");if(pref.matches)return host.__archivePlayfulBound=0,void 0;var P=host.querySelector(".section-archive-playful");if(!P)return host.__archivePlayfulBound=0,void 0;var unsubs=[],ST=window.ScrollTrigger,gs=window.gsap,trs=[],raf=0;function on(n,e,f,o){n&&n.addEventListener(e,f,o),unsubs.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function visPlayful(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function killAll(){try{trs.forEach(function(t){try{t&&t.kill&&t.kill()}catch(_){}})}catch(_){}trs.length=0;try{gs.globalTimeline&&gs.globalTimeline.call&&gs.globalTimeline.call(function(){})}catch(_){}}function clamp(n,a,b){return n<a?a:n>b?b:n}function items(){return[].slice.call(P.querySelectorAll(".archive-playful-item"))}function getEl(it,sel){return it?it.querySelector(sel):null}function prepItem(it,idx){var v=getEl(it,".archive-playful-visuals"),d=getEl(it,".archive-playful-data-inner"),l=getEl(it,".archive-playful-item-data-outer .shortcut-link");if(v){it.__apFullH||(it.__apFullH=Math.max(1,Math.min((innerHeight||900)*.45,v.scrollHeight||9999)));gs.set(v,{height:0,overflow:"hidden",willChange:"height"})}d&&gs.set(d,{autoAlpha:0,y:10,willChange:"transform,opacity"});l&&gs.set(l,{autoAlpha:0,y:10,willChange:"transform,opacity"});if(idx===0&&v){gs.set(v,{height:it.__apFullH});d&&gs.set(d,{autoAlpha:1,y:0});l&&gs.set(l,{autoAlpha:1,y:0});it.__apForceOpen=1}}function build(it,idx){var v=getEl(it,".archive-playful-visuals"),d=getEl(it,".archive-playful-data-inner"),l=getEl(it,".archive-playful-item-data-outer .shortcut-link"),imgs=[].slice.call(it.querySelectorAll(".archive-playful-visual-img"));if(!v&&!d&&!l)return;var full=it.__apFullH||Math.max(1,Math.min((innerHeight||900)*.45,v?v.scrollHeight||9999:9999));it.__apFullH=full;var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});v&&tl.to(v,{height:full,duration:.45},0).to(v,{height:0,duration:.55},.45);d&&tl.to(d,{autoAlpha:1,y:0,duration:.25,ease:"power2.out"},.18).to(d,{autoAlpha:0,y:-6,duration:.25,ease:"power2.out"},.68);l&&tl.to(l,{autoAlpha:1,y:0,duration:.25,ease:"power2.out"},.22).to(l,{autoAlpha:0,y:-6,duration:.25,ease:"power2.out"},.70);if(imgs.length){imgs.forEach(function(im){try{gs.set(im,{yPercent:10,willChange:"transform"})}catch(_){}});tl.to(imgs,{yPercent:-10,stagger:0, duration:1},0)}var st=ST.create({trigger:it,start:"top 70%",end:"top -10%",scrub:!0,onUpdate:function(self){try{tl.progress(self.progress)}catch(_){}}});trs.push(st);trs.push(tl);if(idx===0){requestAnimationFrame(function(){try{st.refresh();if(window.scrollY<st.start){tl.progress(.45);v&&gs.set(v,{height:full});d&&gs.set(d,{autoAlpha:1,y:0});l&&gs.set(l,{autoAlpha:1,y:0})}}catch(_){}})}}function initAll(){killAll();if(!mm.matches||!visPlayful())return;var list=items();for(var i=0;i<list.length;i++)prepItem(list[i],i);for(i=0;i<list.length;i++)build(list[i],i);try{ST.refresh()}catch(_){}}function sched(){raf||(raf=requestAnimationFrame(function(){raf=0;initAll()}))}function onView(){sched()}function onMQ(){sched()}on(document,"archive:view",onView,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",onMQ):mm.addListener(onMQ)}catch(_){ }unsubs.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",onMQ):mm.removeListener(onMQ)}catch(_){}});requestAnimationFrame(initAll);return function destroy(){try{killAll()}catch(_){ }unsubs.forEach(function(fn){fn()});unsubs.length=0;try{host.__archivePlayfulBound=0}catch(_){}}}
			})
		);
