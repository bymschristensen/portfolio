	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger||!window.gsap)return;const c=r.closest(".barba-container")||r,ds=[...c.querySelectorAll(".cs-details")],ms=[...c.querySelectorAll(".cs-morework")],g=c.querySelector(".cs-gallery"),d=ds[0],m=ms[ms.length-1];if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill(),ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{v!==l&&(l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"}))},getY=()=>{try{const f=ScrollTrigger.getScrollFunc&&ScrollTrigger.getScrollFunc(window);if(f)return f()}catch{}return window.pageYOffset||document.documentElement.scrollTop||0};let md=m.querySelector("[data-cs-morework-trigger]");md||(md=document.createElement("div"),md.setAttribute("data-cs-morework-trigger","1"),md.style.cssText="position:relative;width:100%;height:1px;pointer-events:none;",m.parentNode&&m.parentNode.insertBefore(md,m));const sd=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),refreshPriority:100,invalidateOnRefresh:!0}),sm=ScrollTrigger.create({id:"csbg:more",trigger:md,start:"top bottom",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),refreshPriority:100,invalidateOnRefresh:!0}),sync=()=>{const y=getY();s(y>=sm.start?bd:y>=sd.start?bg:cs)};ScrollTrigger.addEventListener("refresh",sync);const rf=()=>{requestAnimationFrame(()=>ScrollTrigger.refresh()),setTimeout(()=>ScrollTrigger.refresh(),150)};if(!g){rf();sync();return}const imgs=[...g.querySelectorAll("img")];if(!imgs.length){rf();sync();return}let n=0;const done=()=>{if(++n<imgs.length)return;rf();sync()};imgs.forEach(i=>{if(i.complete&&i.naturalWidth>0){done();return}(typeof i.decode=="function"?i.decode().catch(()=>{}):Promise.resolve()).finally(done),i.addEventListener("load",done,{once:!0}),i.addEventListener("error",done,{once:!0})});sync()}
			}),
			feature({
