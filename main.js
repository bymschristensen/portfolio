	...
	window.InitManager = (function () {
		...
		registries.pages.selected.push(
			feature({
				...
				init:async r=>{const t=r.querySelector(".selected-container"),o=t?.querySelector(".selected-content");if(!t||!o||t.__selectedLoopInited)return;t.__selectedLoopInited=!0,o.style.justifyContent="center",o.style.transform="translateZ(0)",o.style.touchAction="pan-y";!function(){if(o.__hardTapGate)return;o.__hardTapGate=1;const C=()=>window.matchMedia&&matchMedia("(pointer:coarse)").matches;let sx=0,sy=0,sy0=0,t0=0,mv=!1,sc=0,scTO=0,armed="",armedUntil=0;const arm=h=>{armed=h||"",armedUntil=Date.now()+520,setTimeout(()=>{Date.now()>armedUntil&&(armed="",armedUntil=0)},560)};window.addEventListener("scroll",()=>{sc=Date.now(),clearTimeout(scTO),scTO=setTimeout(()=>{sc=0},220)},{passive:!0});window.addEventListener("touchstart",e=>{if(!C())return;const t=e.touches&&e.touches[0];t&&(sx=t.clientX,sy=t.clientY,sy0=window.scrollY||0,t0=Date.now(),mv=!1)},{passive:!0,capture:!0});window.addEventListener("touchmove",e=>{if(!C())return;const t=e.touches&&e.touches[0];if(!t)return;const dx=Math.abs(t.clientX-sx),dy=Math.abs(t.clientY-sy);(dx>36||dy>36)&&(mv=!0)},{passive:!0,capture:!0});window.addEventListener("touchcancel",()=>{mv=!0},{passive:!0,capture:!0});o.addEventListener("touchend",e=>{if(!C())return;const el=e.target&&e.target.closest?e.target.closest("a.selected-item-inner"):null;if(!el)return;const dt=Date.now()-t0,sd=Math.abs((window.scrollY||0)-sy0);if(mv||sc||dt>420||sd>12)return;arm(el.getAttribute("href")||el.href||"")},{passive:!0});const kill=e=>{if(!C())return;const el=e.target&&e.target.closest?e.target.closest("a.selected-item-inner"):null;if(!el)return;const h=el.getAttribute("href")||el.href||"";if(Date.now()<armedUntil&&armed&&h&&h===armed){e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation&&e.stopImmediatePropagation(),armed="",armedUntil=0,location.assign(h);return}e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation&&e.stopImmediatePropagation()};document.addEventListener("click",kill,!0),document.addEventListener("pointerup",kill,!0),document.addEventListener("touchend",kill,!0)}();const n=Array.from(o.querySelectorAll(".selected-item-outer"));if(!n.length)return;const a=e=>{const t=getComputedStyle(e);return e.offsetWidth+((parseFloat(t.marginLeft)||0)+(parseFloat(t.marginRight)||0))},s=()=>{const e=Math.max(document.documentElement.clientWidth,window.innerWidth||0),t=window.matchMedia("(max-width:767px)").matches;return Math.round(e*(t?.78:.28))},l=e=>{o.querySelectorAll(".selected-item-outer").forEach(t=>{t._baseW=e,t.style.width=e+"px"})},i=()=>{let e=0;return Array.from(o.children).forEach(t=>{1===t.nodeType&&(e+=a(t))}),e},c=()=>{n.forEach(e=>{const t=e.cloneNode(!0);t.setAttribute("data-clone","true"),o.appendChild(t)})};let d=0;function h(){const e=Array.from(o.children).filter(e=>1===e.nodeType),n=Math.floor(e.length/2);let r=0;for(let t=0;t<n;t++)r+=a(e[t]);const l=a(e[n]);d=-(r+.5*l-.5*t.clientWidth),gsap.set(o,{x:d})}function u(){t.hasAttribute("data-loop-ready")||(t.setAttribute("data-loop-ready","1"),o.dispatchEvent(new CustomEvent("selected:loop-ready",{bubbles:!0})))}!function(){Array.from(o.children).forEach(e=>{1===e.nodeType&&e.hasAttribute("data-clone")&&e.remove()}),l(s()),c(),c();const e=3*t.clientWidth;let n=0;for(;i()<e&&n++<8;)c();l(s())}();let p=0,f=1;const m={t:0},y=gsap.quickTo(m,"t",{duration:.45,ease:"power3.out",onUpdate:()=>{o.querySelectorAll(".selected-item-outer").forEach(e=>{const t=e._baseW||s();e.style.width=t*(1+m.t)+"px"})}});let g=!1;const b=(e,t=16.6667)=>{const n=t/16.6667,r=p+1*f;d-=r*n;let l=o.firstElementChild,x=0;for(;l&&d<-a(l)&&x++<50;)d+=a(l),o.appendChild(l),l=o.firstElementChild;let i=o.lastElementChild;for(x=0;i&&d>0&&x++<50;)d-=a(i),o.insertBefore(i,o.firstElementChild),i=o.lastElementChild;gsap.set(o,{x:d});const c=Math.min(1,Math.abs(r)/70);y((r>=0?.14:-.1)*c),Math.abs(r)<3&&Math.abs(m.t)>.002&&!g&&(g=!0,gsap.to(m,{t:0,duration:1.1,ease:"elastic.out(0.62, 0.32)",onUpdate:()=>y(m.t)})),Math.abs(r)>=3&&(g=!1);const h=o.querySelectorAll(".selected-item-visual");if(h.length){const e=.5*window.innerWidth,t=.5+.5*c;h.forEach(o=>{const n=o.closest(".selected-visual");if(!n)return;const r=n.getBoundingClientRect(),l=(e-(r.left+.5*r.width))/window.innerWidth;o.style.setProperty("--drift",80*l*t+"px")})}p*=Math.pow(.94,n),Math.abs(p)<.01&&(p=0)};gsap.ticker.add(b),CoreUtilities.Observers.addTicker(b),CoreUtilities.Observers.addGsap(Observer.create({target:o,type:"wheel,touch",wheelSpeed:1,tolerance:6,onChange(e){const t=Math.abs(e.deltaX)>=Math.abs(e.deltaY)?e.deltaX:e.deltaY;if(!t)return;const n=e.event.type.includes("touch")?0.34:.08;p+=t*n,p=gsap.utils.clamp(-70,70,p),f=t>0?1:-1}})),h(),u();let v=0;const w=new ResizeObserver(()=>{cancelAnimationFrame(v),v=requestAnimationFrame(()=>{l(s()),y(m.t),h(),u()})});w.observe(t),CoreUtilities.Observers.addDom(w)}
			})
		);
