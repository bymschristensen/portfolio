	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=r.closest(".barba-container")||r;if(!e)return;const d=e.querySelector(".cs-details"),m=e.querySelector(".cs-morework"),g=e.querySelector(".cs-gallery");if(!d||!m)return;try{e.__csbgCleanup&&e.__csbgCleanup()}catch{}try{ScrollTrigger.getById("csbg:watch")?.kill()}catch{}const cs=getComputedStyle(e).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";e.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(e,{backgroundColor:v,overwrite:"auto"})};const chk=()=>{const h=window.innerHeight||document.documentElement.clientHeight||0,dr=d.getBoundingClientRect(),mr=m.getBoundingClientRect();mr.top<=.8*h?s(bd):dr.top<=h?s(bg):s(cs)};const st=ScrollTrigger.create({id:"csbg:watch",trigger:e,start:"top top",end:"bottom bottom",onUpdate:chk,onRefresh:chk,refreshPriority:100});const onR=()=>requestAnimationFrame(chk);window.addEventListener("resize",onR,{passive:!0});const ref=()=>{ScrollTrigger.refresh();requestAnimationFrame(chk)};let to=0;const kick=()=>{clearTimeout(to),to=setTimeout(ref,50)};if(g){const imgs=[...g.querySelectorAll("img")];if(imgs.length){let n=0,done=()=>{if(++n<imgs.length)return;kick(),setTimeout(kick,150),setTimeout(kick,350)};imgs.forEach(i=>{if(i.complete&&i.naturalWidth>0)return done();(typeof i.decode=="function"?i.decode().catch(()=>{}):Promise.resolve()).finally(done),i.addEventListener("load",done,{once:!0}),i.addEventListener("error",done,{once:!0})})}else kick()}else kick();chk();e.__csbgCleanup=()=>{window.removeEventListener("resize",onR);try{st.kill()}catch{}delete e.__csbgCleanup}}
			}),
			feature({
