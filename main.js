	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				stage: "main",
				namespaces: ["archive"],
				selectors: [".section-archive-playful",".section-archive-minimal",".archive-playful-list",".archive-minimal-list",".archive-switch-block",".filters-tab",".modal-filters-item"],
				init: function(root){root=root||document;var main=root.querySelector("main.main")||root;if(main.__archiveCtlBound)return;main.__archiveCtlBound=1;var secPlay=main.querySelector(".section-archive-playful"),secMin=main.querySelector(".section-archive-minimal");if(!secPlay||!secMin)return;var playList=secPlay.querySelector(".archive-playful-list"),minList=secMin.querySelector(".archive-minimal-list");if(!playList||!minList)return;var mq=matchMedia("(min-width:1024px)"),rm=matchMedia("(prefers-reduced-motion: reduce)");function normLabel(t){return t=(t||"").toLowerCase().trim().replace(/&/g," ").replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g,"-"),"apps-web-apps"===t||"apps-webapps"===t||"apps-web"===t?"appswebapps":"e-commerce"===t||"ecommerce"===t?"ecommerce":"platforms-systems"===t||"platforms-and-systems"===t?"platforms-systems":"websites"===t?"websites":"other"===t?"other":"all"===t?"all":t}function idKey(id){return(id||"").replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,"").trim()}function getKeyFromEl(el){if(!el)return"all";var k="";try{k=(el.getAttribute("data-filter")||el.getAttribute("data-category")||el.dataset&&((el.dataset.filter||"")||(el.dataset.category||""))||"").trim()}catch(e){}if(!k&&el.id)k=idKey(el.id);if(!k){var t=(el.getAttribute("aria-label")||"").trim();t||(t=(el.textContent||"").trim());k=t}k=normLabel(k);return k||"all"}function itemCats(it){for(var a=[],n=it.querySelectorAll(".archive-categories .cms-categories,.list-categories .cms-categories,.cms-categories"),i=0;i<n.length;i++){var s=(n[i].textContent||"").trim();s&&a.push(normLabel(s))}return a}function matches(it,k){if("all"===k)return!0;for(var a=itemCats(it),i=0;i<a.length;i++)if(a[i]===k)return!0;return!1}var playItems=[].slice.call(playList.children),minItems=[].slice.call(minList.children),state={view:"playful",cat:"all"};function emit(){try{var ev=new CustomEvent("archive:state",{bubbles:!0,detail:{view:state.view,cat:state.cat}});main.dispatchEvent(ev);document.dispatchEvent(ev)}catch(e){}}function setSections(v){var p="playful"===v;secPlay.style.display=p?"":"none";secMin.style.display=p?"none":"";secPlay.classList.toggle("is-active",p);secMin.classList.toggle("is-active",!p);secPlay.classList.toggle("is-inactive",!p);secMin.classList.toggle("is-inactive",p)}function apply(items,k){for(var i=0;i<items.length;i++){var it=items[i];if(1!==it.nodeType)continue;var show=matches(it,k);it.style.display=show?"":"none";it.setAttribute("aria-hidden",show?"false":"true")}}function allFilterEls(){return[].slice.call(document.querySelectorAll('[id^="archive-filter-"],[id^="nav-archive-filter-"],.filters-tab,.modal-filters-item'))}function syncFilters(){var els=allFilterEls();for(var i=0;i<els.length;i++){var el=els[i],k=getKeyFromEl(el),on=k===state.cat;el.classList.toggle("is-active",on);el.classList.toggle("is-current",on);el.setAttribute("aria-pressed",on?"true":"false")}}function syncToggle(){var block=main.querySelector(".archive-switch-block"),dot=main.querySelector(".archive-switch-dot"),show=mq.matches;if(block)block.style.display=show?"":"none";if(dot)dot.classList.toggle("is-on","minimal"===state.view);if(block)block.setAttribute("aria-pressed","minimal"===state.view?"true":"false")}function setViewNow(v){mq.matches||(v="minimal");state.view=v;setSections(v);syncToggle();apply(playItems,state.cat);apply(minItems,state.cat);emit()}function setCat(k){state.cat=k||"all";syncFilters();apply(playItems,state.cat);apply(minItems,state.cat);emit()}function requestSwap(to){try{var ev=new CustomEvent("archive:swap-request",{bubbles:!0,detail:{from:state.view,to:to}});document.dispatchEvent(ev)}catch(e){}}function setView(v){if(!mq.matches){setViewNow("minimal");return}var to=v,from=state.view;if(to===from){emit();return}if(window.gsap&&!rm.matches){var done=function(e){if(!e||!e.detail||e.detail.to!==to)return;document.removeEventListener("archive:swap-done",done);setViewNow(to)};document.addEventListener("archive:swap-done",done);requestSwap(to);setTimeout(function(){document.removeEventListener("archive:swap-done",done);setViewNow(to)},800)}else setViewNow(to)}function bindAllFilters(){var els=allFilterEls();for(var i=0;i<els.length;i++){var el=els[i];if(!el||el.__archiveBound)return;el.__archiveBound=1;el.addEventListener("click",function(e){e.preventDefault();setCat(getKeyFromEl(this))},{passive:!1})}}bindAllFilters();var switchBlock=main.querySelector(".archive-switch-block");switchBlock&&!switchBlock.__archiveBound&&(switchBlock.__archiveBound=1,switchBlock.addEventListener("click",function(e){e.preventDefault();setView("playful"===state.view?"minimal":"playful")},{passive:!1}));function onMQ(){mq.matches?setViewNow(state.view||"playful"):setViewNow("minimal")}mq.addEventListener?mq.addEventListener("change",onMQ):mq.addListener(onMQ);setCat("all");setViewNow(mq.matches?"playful":"minimal");main.__archiveAPI={setView:setView,setViewNow:setViewNow,setCategory:setCat,getState:function(){return{view:state.view,cat:state.cat}}}}
			}),
			feature({
				...
				namespaces: ["archive"],
				selectors: [".archive-playful-list",".archive-minimal-list"],
				init: function(root){root=root||document;var main=(root.querySelector("main.main")||root);if(main.__arcMotion)return;main.__arcMotion=1;var rm=matchMedia("(prefers-reduced-motion: reduce)");function activeList(view){return"minimal"===view?(main.querySelector(".section-archive-minimal .archive-minimal-list")):(main.querySelector(".section-archive-playful .archive-playful-list"))}function visItems(list){if(!list)return[];return[].slice.call(list.children).filter(function(n){return 1===n.nodeType&&getComputedStyle(n).display!=="none"})}function fadeOut(list){var nodes=visItems(list);if(!nodes.length)return gsap.timeline();return gsap.timeline().to(nodes,{opacity:0,y:12,filter:"blur(10px)",duration:.22,ease:"power2.in",stagger:{each:.02,from:"end"}})}function prepIn(nodes){for(var i=0;i<nodes.length;i++)gsap.set(nodes[i],{opacity:0,y:24,filter:"blur(10px)",willChange:"transform,opacity,filter"})}function fadeIn(list){var nodes=visItems(list);if(!nodes.length)return;prepIn(nodes);gsap.to(nodes,{opacity:1,y:0,filter:"blur(0px)",duration:.65,ease:"power2.out",stagger:.08,clearProps:"willChange"})}var pendingTo=null;document.addEventListener("archive:swap-request",function(e){if(!window.gsap||rm.matches)return;var d=e&&e.detail?e.detail:null;if(!d||!d.to||!d.from)return;pendingTo=d.to;var fromList=activeList(d.from);fadeOut(fromList).eventCallback("onComplete",function(){try{document.dispatchEvent(new CustomEvent("archive:swap-done",{bubbles:!0,detail:{to:d.to,from:d.from}}))}catch(_){} })});document.addEventListener("archive:state",function(e){if(!window.gsap||rm.matches)return;var d=e&&e.detail?e.detail:null;if(!d||!d.view)return;cancelAnimationFrame(main.__arcMotionRaf||0);main.__arcMotionRaf=requestAnimationFrame(function(){if(pendingTo&&d.view===pendingTo){pendingTo=null;fadeIn(activeList(d.view))}})})}
			})
		);
