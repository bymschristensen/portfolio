	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async function(r){if(r=r||document,window.matchMedia&&matchMedia("(prefers-reduced-motion: reduce)").matches||!window.gsap||!window.ScrollTrigger)return;var e=gsap.utils.toArray(".section-single-service",r);if(!e.length)return;e.forEach(function(t,n){t.style.removeProperty("transform"),t.style.removeProperty("filter"),t._ppsST&&(t._ppsST.kill(),t._ppsST=null);var i=t.offsetHeight<window.innerHeight,o="servicePin:"+(t.id||t.getAttribute("data-service-id")||n),s=gsap.timeline({scrollTrigger:{id:o,trigger:t,start:i?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,anticipatePin:1,invalidateOnRefresh:!0}});s.to(t,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0),t._ppsST=s.scrollTrigger}),ScrollTrigger.refresh()}
			}),
			feature({
				id: "productGallery",
				stage: "main",
