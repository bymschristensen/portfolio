...
// Core Utilities
	window.CoreUtilities = (function () {
		...
		const Text={splitAndMask:e=>{if(!e)return null;if(e._originalHTML||(e._originalHTML=e.innerHTML),e._split)return e._split;if(!window.SplitText){gsap.set(e,{autoAlpha:1,clearProps:"yPercent,rotation,willChange"});const t={lines:[e],revert(){}};return e._split=t,t}const t=e.style.whiteSpace,r=e.style.display;e.style.whiteSpace=getComputedStyle(e).whiteSpace||"normal","inline"===getComputedStyle(e).display&&(e.style.display="block"),e.clientWidth;const i=new SplitText(e,{type:"lines",linesClass:"line",reduceWhiteSpace:!0});i.lines.forEach(o=>{const n=o.getBoundingClientRect().height||o.offsetHeight||0,s=document.createElement("div");s.className="text-mask",s.style.cssText="overflow:hidden;display:block;height:"+n+"px",o.style.whiteSpace=getComputedStyle(e).whiteSpace||"normal",o.style.display="block",o.parentNode.insertBefore(s,o),s.appendChild(o)}),gsap.set(i.lines,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform,opacity"}),e.style.whiteSpace=t,e.style.display=r;return e._split=i,i},safelyRevertSplit:(e,t)=>{if(e&&t){try{e.revert&&e.revert()}catch{}t._originalHTML&&(t.innerHTML=t._originalHTML),delete t._split,delete t._originalHTML}},animateLines:e=>gsap.to(e,{yPercent:0,rotation:0,duration:.8,ease:"power2.out",stagger:.08})};
	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async e=>{const t=".appear-in-line",r=":scope > *",o=.15,n=.8;function i(s){if(s.__ailInit)return;s.__ailInit=!0;const l=[],a=[];s.setAttribute("data-ail","1"),[...s.querySelectorAll(r)].forEach(d=>{const c=getComputedStyle(d),u=parseInt(c.columnCount,10)||1,f=d.getBoundingClientRect();function p(g){gsap.set(g,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})}if(u>1&&window.SplitText){try{const g=new SplitText(d,{type:"lines",linesClass:"split-line"});l.push(g);const m=f.width/u,h=Array.from({length:u},()=>[]);g.lines.forEach(y=>{const v=y.getBoundingClientRect().left-f.left,_=Math.min(Math.floor(v/m),u-1);h[_].push(y),p(y)}),h.forEach(y=>a.push(y))}catch{p(d),a.push([d])}}else p(d),a.push([d])});const d=new IntersectionObserver((c,u)=>{c.forEach(f=>{if(!f.isIntersecting)return;u.unobserve(f.target),s.removeAttribute("data-ail"),a.forEach((p,g)=>{gsap.to(p,{y:0,opacity:1,filter:"blur(0px)",duration:n,ease:"power2.out",delay:g*o})});const m=(a.length-1)*o+n;gsap.delayedCall(m+.05,()=>{try{l.forEach(p=>p.revert())}catch{}})})},{root:null,rootMargin:"0px 0px -10% 0px",threshold:0});CoreUtilities.Observers.addDom(d),d.observe(s);const c=s.getBoundingClientRect();if(c.top<innerHeight&&c.bottom>0){d.unobserve(s),s.removeAttribute("data-ail"),a.forEach((u,f)=>{gsap.to(u,{y:0,opacity:1,filter:"blur(0px)",duration:n,ease:"power2.out",delay:f*o})});const u=(a.length-1)*o+n;gsap.delayedCall(u+.05,()=>{try{l.forEach(f=>f.revert())}catch{}})}const u=e.querySelector(".w-tabs");if(u){const f=s.closest(".w-tab-pane");f&&CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!f.classList.contains("w--tab-active"))return;d.observe(s);const p=s.getBoundingClientRect();if(p.top<innerHeight&&p.bottom>0){d.unobserve(s),s.removeAttribute("data-ail"),a.forEach((g,m)=>{gsap.to(g,{y:0,opacity:1,filter:"blur(0px)",duration:n,ease:"power2.out",delay:m*o})});const g=(a.length-1)*o+n;gsap.delayedCall(g+.05,()=>{try{l.forEach(m=>m.revert())}catch{}})}}).observe(f,{attributes:!0,attributeFilter:["class"]})}}e.querySelectorAll(t).forEach(i)}
			}),
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=document.documentElement,i=e.querySelector(".cs-hero-image"),o=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),r=e=>{if(!e)return null;let t=e.querySelector(".ta-one,[data-animate-headline],h1")||e;for(;t&&t.children&&t.children.length===1&&t.textContent.trim()===t.children[0].textContent.trim();)t=t.children[0];return t},s=e=>{e&&(e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t)))};a.classList.add("entry-ready");const l=r(o);o&&(o.style.visibility="visible"),l&&(l.style.visibility="visible"),i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)",overwrite:"auto",immediateRender:!0}),l&&gsap.set(l,{autoAlpha:0,overwrite:"auto",immediateRender:!0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),s(i),s(o),n.forEach(s),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(l){const e=l.style.display,c="inline"===getComputedStyle(l).display;c&&(l.style.display="block"),t.addLabel("headline",.35).set(l,{autoAlpha:1,visibility:"visible"},"headline").call(()=>requestAnimationFrame(()=>{CoreUtilities.Fonts.ready().then(()=>{if(!window.SplitText){gsap.to(l,{y:0,opacity:1,duration:.6,ease:"power2.out"}),l.style.display=c?"inline":e;return}try{const i=splitAndMask(l);animateLines(i.lines).eventCallback("onComplete",()=>{safelyRevertSplit(i,l),l.style.display=c?"inline":e})}catch{i&&gsap.set(l,{autoAlpha:1}),l.style.display=c?"inline":e}}))},null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
