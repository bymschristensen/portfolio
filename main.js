	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			feature({
				...
				stage: "early",
				namespaces: ["archive"],
				selectors: [".filters-counter",".nav-counter-filters","[data-counter]",".section-archive-playful",".section-archive-minimal",".archive-playful-list",".archive-minimal-list",".archive-switch-block",".archive-switch",".archive-switch-dot",'[id^="archive-filter-"]','[id^="nav-archive-filter-"]',".archive-playful-item",".archive-minimal-item",".archive-categories .cms-categories"],
				init: function(root){
					root=root||document;
					...
					(function(){var d=el.doc,r=0,mo=0,ids=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];function all(){var p=el.listP?[].slice.call(el.listP.querySelectorAll(".archive-playful-item")):[],m=el.listM?[].slice.call(el.listM.querySelectorAll(".archive-minimal-item")):[];return p&&p.length?p:m&&m.length?m:[]}function cats(it){if(it.__asCatsAll)return it.__asCatsAll;for(var a=["all"],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var t=(n[i].textContent||"").trim();t&&(t=normCat(t))&&a.indexOf(t)===-1&&a.push(t)}return it.__asCatsAll=a,a}function compute(){var items=all(),c={all:0,appswebapps:0,websites:0,ecommerce:0,"platforms-systems":0,other:0};c.all=items.length;for(var i=0;i<items.length;i++){var set=new Set(cats(items[i]));set.forEach(function(k){k!=="all"&&k in c&&(c[k]+=1)})}return c}function ensure(btn){if(!btn)return null;var s=btn.querySelector(".filters-counter")||btn.querySelector(".nav-counter-filters")||btn.querySelector("[data-counter]");return s||(s=document.createElement("span"),s.className=btn.id&&btn.id.indexOf("nav-")===0?"nav-counter-filters":"filters-counter",btn.appendChild(s)),s}function render(c){for(var i=0;i<ids.length;i++){var id=ids[i],btn=d.getElementById(id);if(!btn)continue;var key=baseId(id),s=ensure(btn);s&&(s.textContent="("+(c[key]??0)+")")}}function now(){r=0;try{render(compute())}catch(_){}}function sched(){r||(r=requestAnimationFrame(now))}mod.updateCounts=sched;sched();try{mo=new MutationObserver(sched);el.listP&&mo.observe(el.listP,{childList:!0,subtree:!0});el.listM&&mo.observe(el.listM,{childList:!0,subtree:!0});unsubs.push(function(){try{mo&&mo.disconnect&&mo.disconnect()}catch(_){}})}catch(_){}on(document,"archive:view",sched,{passive:!0});on(document,"archive:filter:applied",sched,{passive:!0});on(window,"load",sched,{once:!0,passive:!0})})();
					
					// Motion Module
					(function(){if(!window.gsap){mod.scheduleMotionRefresh=function(){};mod.resetMotion=function(){};mod.destroyMotion=function(){};return}var io=0,raf=0,run=0,tries=0,max=45,STAG=.08,DUR=.75,MIN_AFTER=3,MIN_PAD=.06,MIN_DUR=.6,pending=0,pendWhy="",seq=0,lockUntil=0,PMQ=matchMedia("(min-width:1024px)"),PLAY_AFTER=2,PLAY_MAX=7,playLocked=0;function cleanup(){try{io&&io.disconnect&&io.disconnect()}catch(e){}io=0;try{raf&&cancelAnimationFrame(raf)}catch(e){}raf=0;pending=0;pendWhy="";playLocked=0}unsubs.push(cleanup);mod.destroyMotion=cleanup;function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(e){return!0}}function view(){try{return(mod.getView&&mod.getView())||""}catch(e){return""}}function scope(){try{var v=view();if(v==="playful"&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(v==="minimal"&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}return el.host||host}function items(sc){return[].slice.call((sc||scope()).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function isMin(sc){try{return!!(sc&&sc.classList&&sc.classList.contains("section-archive-minimal"))}catch(e){return!1}}function isPlay(sc){try{return!!(sc&&sc.classList&&sc.classList.contains("section-archive-playful"))}catch(e){return!1}}function minVis(sc){return sc&&(sc.querySelector(".archive-minimal-item-visual")||sc.querySelector(".archive-minimal-visual")||sc.querySelector(".archive-minimal-visual-wrap"))||null}function rectTop(it){try{return it.getBoundingClientRect().top}catch(e){return 1e9}}function inView(it){if(!can(it)||it.__almDone)return!1;var r=it.getBoundingClientRect();return r.top<(innerHeight||0)&&r.bottom>0}function prep(it){if(!it)return;it.__almRun=run;it.__almDone=0;try{gsap.killTweensOf(it)}catch(e){}gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"})}function doneSet(it){if(!it||it.__almDone||!can(it))return;it.__almDone=1;try{gsap.killTweensOf(it)}catch(e){}gsap.set(it,{y:0,autoAlpha:1,filter:"blur(0px)",clearProps:"y,filter,willChange,opacity,visibility"})}function reveal(it,d){if(!it||it.__almDone||!can(it))return;it.__almRun!==run&&prep(it);it.__almDone=1;gsap.to(it,{delay:d||0,y:0,autoAlpha:1,filter:"blur(0px)",duration:DUR,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange,opacity,visibility"})}function prepV(v){if(!v)return;try{gsap.killTweensOf(v)}catch(e){}gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}function showVInstant(v){if(!v)return;try{gsap.killTweensOf(v)}catch(e){}gsap.set(v,{autoAlpha:1,filter:"blur(0px)",clearProps:"filter,willChange,opacity,visibility"})}function revealV(sc,bd,why){var v=minVis(sc);if(!v)return;var vv=view()||"minimal",k="__almVisualSeen_"+vv;if(why!=="view"&&v[k]){showVInstant(v);return}prepV(v);gsap.to(v,{delay:bd||0,autoAlpha:1,filter:"blur(0px)",duration:MIN_DUR,ease:"power2.out",overwrite:!0,onComplete:function(){try{v[k]=1}catch(_){}} ,clearProps:"filter,willChange,opacity,visibility"})}function plan(list){var live=[],i=0;for(i=0;i<list.length;i++)can(list[i])&&live.push(list[i]);live.sort(function(a,b){return rectTop(a)-rectTop(b)});var vis=[],out=[];for(i=0;i<live.length;i++)(inView(live[i])?vis:out).push(live[i]);if(!vis.length)return{p:[],s:live};var last=vis[vis.length-1],lastTop=rectTop(last),below=[];for(i=0;i<out.length;i++){var t=out[i];if(rectTop(t)>=lastTop)below.push(t)}below.sort(function(a,b){return rectTop(a)-rectTop(b)});var extra=below.slice(0,MIN_AFTER),p=vis.concat(extra),set=new Set(p),s=[];for(i=0;i<live.length;i++){var it=live[i];set.has(it)||s.push(it)}return{p:p,s:s}}function planPlay(list){var live=[],i=0;for(i=0;i<list.length;i++)can(list[i])&&live.push(list[i]);live.sort(function(a,b){return rectTop(a)-rectTop(b)});var vis=[],out=[];for(i=0;i<live.length;i++)(inView(live[i])?vis:out).push(live[i]);if(!vis.length)return{p:[],s:live};var last=vis[vis.length-1],lastTop=rectTop(last),below=[];for(i=0;i<out.length;i++){var t=out[i];if(rectTop(t)>=lastTop)below.push(t)}below.sort(function(a,b){return rectTop(a)-rectTop(b)});var extra=below.slice(0,PLAY_AFTER),p=vis.concat(extra);p=p.slice(0,PLAY_MAX);var set=new Set(p),s=[];for(i=0;i<live.length;i++){var it=live[i];set.has(it)||s.push(it)}return{p:p,s:s}}function startIO(list){try{io&&io.disconnect&&io.disconnect()}catch(e){}io=0;if("IntersectionObserver"in window){io=new IntersectionObserver(function(es){for(var i=0;i<es.length;i++){var t=es[i].target;if(es[i].isIntersecting)doneSet(t)}},{threshold:.12,rootMargin:"0px 0px -10% 0px"});for(var i=0;i<list.length;i++)try{io.observe(list[i])}catch(e){}}}function runOnce(sc,why){var now=Date.now();if(now<lockUntil)return;lockUntil=now+340;var list=items(sc);if(!list.length){if(tries++<max)return requestAnimationFrame(function(){runOnce(sc,why)});return}tries=0;var v=view(),play=PMQ.matches&&v==="playful"&&isPlay(sc);if(play&&playLocked&&why!=="load"&&why!=="view"&&why!=="filter")return;run++;var i=0;if(play){for(i=0;i<list.length;i++)can(list[i])&&prep(list[i])}else{for(i=0;i<list.length;i++)can(list[i])&&prep(list[i])}var p=play?planPlay(list):plan(list),prim=p.p||[],sec=p.s||[];for(i=0;i<prim.length;i++)reveal(prim[i],STAG*i);var base=STAG*prim.length+.02;gsap.delayedCall(base,function(){for(var j=0;j<sec.length;j++)doneSet(sec[j])});if(isMin(sc)){var idx=Math.max(0,Math.min(MIN_AFTER,prim.length)-1),dd=STAG*idx+MIN_PAD;revealV(sc,dd,why)}startIO(list);if(play)playLocked=1}function pr(w){return w==="view"?3:w==="load"?2:w==="filter"?1:0}function sched(why){seq++;var my=seq;why=why||"auto";if(why==="view"||why==="filter"||why==="load")playLocked=0;if(!pending){pending=1;pendWhy=why;raf=requestAnimationFrame(function(){pending=0;raf=0;runOnce(scope(),pendWhy);if(my!==seq)return})}else{pr(why)>pr(pendWhy)&&(pendWhy=why)}}mod.resetMotion=function(){tries=0;sched("load")};mod.scheduleMotionRefresh=function(why){sched(why||"auto")};on(document,"archive:view",function(){sched("view")},{passive:!0});on(document,"archive:filter:applied",function(){sched("filter")},{passive:!0});requestAnimationFrame(function(){sched("load")})})();
			...
			feature({
				...
				init: function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apxLiveBound)return;host.__apxLiveBound=1;if(!window.gsap){host.__apxLiveBound=0;return}var gs=window.gsap,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches){host.__apxLiveBound=0;return}var P=host.querySelector(".section-archive-playful");if(!P){host.__apxLiveBound=0;return}var un=[],items=[],raf=0,waitRaf=0,buildRaf=0,active=!1,waitSeq=0;function on(n,e,f,o){if(!n)return;n.addEventListener(e,f,o);un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(_){return!0}}function pickDataParts(it){var pd=qs(it,".archive-playful-data-product"),pe=qs(it,".archive-playful-data-expertise");if(!pd.length&&!pe.length){pd=qs(it,".archive-playful-data-inner .headline-xxs");pe=qs(it,".archive-playful-data-inner .body-sm")}return pd.concat(pe)}function pickLink(it){return q(it,".archive-playful-item-data-outer .case-study-url")||q(it,".archive-playful-item-data .shortcut-link")}function kill(){active=!1;raf&&cancelAnimationFrame(raf);raf=0;waitRaf&&cancelAnimationFrame(waitRaf);waitRaf=0;for(var i=0;i<items.length;i++){var d=items[i];try{d&&d.tl&&d.tl.kill&&d.tl.kill()}catch(_){}}items.length=0}function build(){kill();if(!mm.matches||!vis())return;var list=[].slice.call(P.querySelectorAll(".archive-playful-item")).filter(can);if(!list.length)return;for(var i=0;i<list.length;i++){var it=list[i],v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickDataParts(it),ln=pickLink(it);try{imgs.length&&gs.set(imgs,{willChange:"transform"});v&&gs.set(v,{willChange:"height",overflow:"hidden"});di&&gs.set(di,{willChange:"margin"});parts.length&&gs.set(parts,{willChange:"opacity,height"});ln&&gs.set(ln,{willChange:"opacity,height",overflow:"hidden"})}catch(_){}var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.fromTo(imgs,{yPercent:-15},{yPercent:15,duration:1},0);parts.length&&tl.to(parts,{opacity:0,duration:.08},.75);di&&tl.to(di,{marginTop:0,marginBottom:0,duration:.08},.75);parts.length&&tl.to(parts,{height:0,duration:.08},.80);ln&&tl.to(ln,{opacity:0,duration:.08},.80);ln&&tl.to(ln,{height:0,duration:.08},.85);imgs.length&&tl.to(imgs,{scale:1.5,duration:.20},.80);v&&tl.to(v,{height:0,duration:.20},.80);items.push({it:it,tl:tl})}waitForEntry()}function clamp(n){return n<0?0:n>1?1:n}function tick(){if(!active){raf=0;return}raf=requestAnimationFrame(tick);var vh=innerHeight||0;if(!vh)return;var endExtra=.07*vh;for(var i=0;i<items.length;i++){var d=items[i],it=d.it,tl=d.tl;if(!it||!tl||!can(it))continue;var r;try{r=it.getBoundingClientRect()}catch(_){continue}if(r.bottom<-200||r.top>vh+200)continue;var denom=vh+endExtra,p=(vh-r.top)/denom;p=clamp(p);try{tl.progress(p)}catch(_){}}}function readyNow(){var vh=innerHeight||0;if(!vh)return!1;for(var i=0;i<items.length;i++){var it=items[i].it;if(!it||!can(it))continue;var r;try{r=it.getBoundingClientRect()}catch(_){continue}if(r.bottom<-50||r.top>vh+50)continue;try{if(it.__almDone!==1)return!1}catch(_){return!1}}return!0}function waitForEntry(){waitSeq++;var my=waitSeq;active=!1;raf&&cancelAnimationFrame(raf);raf=0;waitRaf&&cancelAnimationFrame(waitRaf);waitRaf=0;var t0=Date.now();(function loop(){if(my!==waitSeq)return;waitRaf=requestAnimationFrame(loop);if(!mm.matches||!vis())return;var ok=readyNow();if(ok||Date.now()-t0>1400){waitRaf&&cancelAnimationFrame(waitRaf);waitRaf=0;active=!0;tick()}})()}function syncNow(){if(!mm.matches||!vis()){kill();return}build()}function schedBuild(){if(buildRaf)return;buildRaf=requestAnimationFrame(function(){buildRaf=0;syncNow()})}function sync(){schedBuild()}on(document,"archive:view",sync,{passive:!0});on(document,"archive:filter:applied",sync,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sync):mm.addListener(sync)}catch(_){}un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sync):mm.removeListener(sync)}catch(_){}});requestAnimationFrame(function(){setTimeout(sync,0)});return function destroy(){try{kill()}catch(_){}buildRaf&&cancelAnimationFrame(buildRaf);buildRaf=0;for(var i=0;i<un.length;i++)un[i]();un.length=0;try{host.__apxLiveBound=0}catch(_){}}}
			}),
			feature({
