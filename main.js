	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;let base=getComputedStyle(c).backgroundColor;const setBG=t=>gsap.to(c,{backgroundColor:t,duration:.6,ease:"power1.inOut",overwrite:"auto"}),BG="var(--colors--background)",BDR="var(--colors--border)";try{ScrollTrigger.getById("csbg1")?.kill();ScrollTrigger.getById("csbg2")?.kill()}catch{}const onRefresh=()=>{base=getComputedStyle(c).backgroundColor};ScrollTrigger.addEventListener("refreshInit",onRefresh);ScrollTrigger.create({id:"csbg1",trigger:d,start:"top bottom-=15%",onEnter:()=>setBG(BG),onLeaveBack:()=>setBG(base),invalidateOnRefresh:!0});ScrollTrigger.create({id:"csbg2",trigger:m,start:"top bottom-=15%",onEnter:()=>setBG(BDR),onLeaveBack:()=>setBG(BG),invalidateOnRefresh:!0});}
			}),
			feature({
