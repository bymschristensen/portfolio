...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async e=>{const h=e.querySelector(".hero-section-showreel"),o=h&&h.querySelector(".showreel-outer"),c=h&&h.querySelector(".showreel-outer .container"),r=h&&h.querySelector(".showreel-container"),n=h&&h.querySelector(".showreel-headline"),t=h&&h.querySelector(".showreel-subtext"),l=h&&h.querySelector(".toc-outer"),a=[n,t,l].filter(Boolean);if(!h||!o||!c||!r)return;const s=(e,t,o)=>e+(t-e)*o,i=e=>Math.max(0,Math.min(1,e));let d=window.innerHeight,m=1,u=.5;const p=getComputedStyle(r).transform||"none",f="none"===p?"":p+" ";let g=0;function y(){const e=r.style.transform;r.style.transform=p;const h=r.getBoundingClientRect().width||1;r.style.transform=e;return h}function v(){const e=h.getBoundingClientRect(),o=e.height-d;return o<=0?0:i((-e.top)/o)}function x(){const e=c.getBoundingClientRect();g=e.width||1}function E(){m=function(){const e=r.style.transform;r.style.transform=p;const h=r.getBoundingClientRect().width||1;r.style.transform=e;return Math.max(1,g/(h||1))}()}function S(){const e=v(),o=s(1,m,e),n=y()*o,t=Math.max(0,n-g),l=(u-.5)*t*-1;r.style.transform=`${f}translate3d(${l}px,0,0) scale(${o})`;const a=-.45*d,w=1-e;for(let e=0;e<a.length;e++){const h=a[e];h.style.transform=`translate3d(0, ${s(0,a,e)}px, 0)`.replace("a",(-.45*d));h.style.opacity=w}}function _(){d=window.innerHeight,x(),E(),S()}function b(e){const h=c.getBoundingClientRect(),o=Math.max(h.left,Math.min(e.clientX,h.right));u=(o-h.left)/(h.width||1)}x(),E(),S();let T=!1;const C=()=>{T||(T=!0,requestAnimationFrame(()=>{T=!1,S()}))};window.addEventListener("scroll",C,{passive:!0}),window.addEventListener("resize",_,{passive:!0}),h.addEventListener("mousemove",b,{passive:!0}),document.__capHeroAll={offScroll:()=>window.removeEventListener("scroll",C),offResize:()=>window.removeEventListener("resize",_),offMove:()=>h.removeEventListener("mousemove",b),media:r,copy:a,base:p}},destroy:async()=>{const e=document.__capHeroAll;if(!e)return;try{e.offScroll&&e.offScroll()}catch{}try{e.offResize&&e.offResize()}catch{}try{e.offMove&&e.offMove()}catch{}try{e.media&&(e.media.style.transform=e.base||"");(e.copy||[]).forEach(e=>{e.style.transform="";e.style.opacity=""})}catch{}document.__capHeroAll=null}),
			feature({
