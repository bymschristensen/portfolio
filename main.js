...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:r=>{if(!gsap||!ScrollTrigger)return;r=r||document;let L=!1;document.addEventListener("click",e=>{if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){L=!0;setTimeout(()=>{L=!1;try{ScrollTrigger.refresh()}catch(_){}} ,350);}});const S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;S.forEach((sec,i)=>{const small=sec.offsetHeight<innerHeight,st=i===0?"top top":small?"top top":"bottom bottom";gsap.timeline({scrollTrigger:{trigger:sec,start:st,end:"bottom top",pin:!0,pinSpacing:!1,scrub:1,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:self=>{if(L)self.progress(self.progress());}}}).to(sec,{filter:"contrast(10%) blur(10px)",ease:"none",startAt:{filter:"contrast(100%) blur(0px)"}});});}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
