		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: async function (root) {
					...
					// ---- State bucket (one per root) ----
					var S = root.__archiveMinimalExperience || (root.__archiveMinimalExperience = {});
					if (S._bound) return;
					S._bound = 1;
					
					// ---- Selectors ----
					S.sel = {
						section: ".section-archive-minimal",
						list: ".archive-minimal-list",
						item: ".archive-minimal-item",
						hiddenImg: "img.archive-minimal-item-hidden-img",
						stickyImg: ".archive-minimal-visual-img"
					};
					
					// ---- Media queries ----
					// Hover logic only for >=1024px fine pointer and no reduced motion.
					S.mqDesktop = window.matchMedia("(min-width: 1024px) and (pointer: fine) and (prefers-reduced-motion: no-preference)");
					...
					// ---- Core nodes ----
					S.section = $(S.sel.section) || root;
					S.list = $(S.sel.list, S.section) || S.section;
					S.stickyImg = $(S.sel.stickyImg, root);
					
					// If Minimal view isn't present on this page state, cleanly exit.
					// (Feature remains bound, but does nothing; safe for view toggles.)
					if (!S.list || !S.stickyImg) {
						S.destroy = function () {
							S._bound = 0;
							try { delete root.__archiveMinimalExperience; } catch (e) {}
						};
						return;
					}
					
					// -----------------------------
					// Sub-module: Step 1 baseline
					// -----------------------------
					function initVisualBaseline() {
						var M = {};
						var stickyImg = S.stickyImg;
						var list = S.list;
						
						// Prevent double-binding just in case init() runs again in same root
						if (stickyImg.__ame_v1) return null;
						stickyImg.__ame_v1 = 1;
						
						var currentKey = "";
						var hoverActive = false;
						
						function getVisibleItems() { return $$(S.sel.item, list).filter(isVisible); }
						function getHiddenImgFromItem(item) { return item ? $(S.sel.hiddenImg, item) : null; }
						function keyFromImg(img) { return (img && (img.currentSrc || img.src)) || ""; }
						...
						function applyImg(img) {
							...
							var key = keyFromImg(img);
							if (key && key === currentKey) return true;
							
							// Copy responsive attrs (helps prevent blur)
							var srcset = img.getAttribute("srcset");
							var sizes = img.getAttribute("sizes");
							
							stickyImg.alt = img.alt || "";
							
							if (srcset) stickyImg.setAttribute("srcset", srcset);
							else stickyImg.removeAttribute("srcset");
							
							if (sizes) stickyImg.setAttribute("sizes", sizes);
							else stickyImg.removeAttribute("sizes");
						...
						function setToFirstVisible() {
							...
							applyImg(getHiddenImgFromItem(first));
						}
						
						function onEnter(e) {
							if (!S.mqDesktop.matches) return;
							var img = getHiddenImgFromItem(e.currentTarget);
							if (!img) return;
							hoverActive = true;
							applyImg(img);
						}
						...
						function onLeave() {
							if (!S.mqDesktop.matches) return;
							hoverActive = false;
							// Keep last hovered (intentional, stable)
						}
						
						function bindHover() {
							var items = $$(S.sel.item, list);
							for (var i = 0; i < items.length; i++) {
								var it = items[i];
								if (it.__ame_v1h) continue;
								it.__ame_v1h = 1;
								it.addEventListener("mouseenter", onEnter, { passive: true });
								it.addEventListener("mouseleave", onLeave, { passive: true });
							}
						}
					
						function refresh() {
							// Step 1 rule: on list change, snap to first visible unless hovering (desktop).
							if (!(S.mqDesktop.matches && hoverActive)) setToFirstVisible();
							bindHover();
						}
						...
						// Observe list changes (Webflow filtering often toggles class/style)
						...
						// Expose minimal hooks for later steps + external calls
						M.refresh = refresh;
						M.setToFirstVisible = setToFirstVisible;
						...
						M.destroy = function () {
							...
							for (var i = 0; i < items.length; i++) {
								...
								it.removeEventListener("mouseleave", onLeave);
								it.__ame_v1h = 0;
							}
							
							stickyImg.__ame_v1 = 0;
							try { delete stickyImg.__ame_v1; } catch (e) {}
						};
					...
					// Init modules
					S.modules = S.modules || {};
					S.modules.visualBaseline = initVisualBaseline();
					
					// Unified destroy for this feature
					S.destroy = function () {
						try {
							if (S.modules && S.modules.visualBaseline && S.modules.visualBaseline.destroy) {
								S.modules.visualBaseline.destroy();
							}
						} catch (e) {}
					...
					// (Optional) expose for manual refresh calls from your filter logic
					// e.g. root.__archiveMinimalExperience.refresh()
					S.refresh = function () { try { S.modules && S.modules.visualBaseline && S.modules.visualBaseline.refresh && S.modules.visualBaseline.refresh(); } catch (e) {} };
				}
			})
		);
