	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async r=>{const sel='.appear-in-line';r.querySelectorAll(sel).forEach(b=>{if(b.__ail)return;b.__ail=!0;const groups=[],splits=[];[...b.querySelectorAll(':scope > *:not(:empty)')].forEach(el=>{const cs=getComputedStyle(el),cols=parseInt(cs.columnCount,10)||1,rect=el.getBoundingClientRect();if(cols>1){const sp=new SplitText(el,{type:'lines',linesClass:'split-line'});splits.push(sp);const w=rect.width/cols,colsArr=Array.from({length:cols},()=>[]);sp.lines.forEach(L=>{const x=L.getBoundingClientRect().left-rect.left,idx=Math.min(cols-1,Math.max(0,Math.floor(x/w)));colsArr[idx].push(L);gsap.set(L,{y:100,opacity:0,filter:'blur(10px)',willChange:'transform,opacity'});});colsArr.forEach(col=>groups.push(col));}else{gsap.set(el,{y:100,opacity:0,filter:'blur(10px)',willChange:'transform,opacity'});groups.push([el]);}});const reveal=()=>{if(b.__ailDone)return;groups.forEach((g,i)=>gsap.to(g,{y:0,opacity:1,filter:'blur(0px)',duration:.8,ease:'power2.out',delay:.12*i}));gsap.delayedCall(.12*(groups.length-1)+.85,()=>{splits.forEach(s=>s.revert());b.__ailDone=!0;});};const io=CoreUtilities.Observers.addDom(new IntersectionObserver((ents,obs)=>{ents.forEach(e=>{if(e.isIntersecting){obs.unobserve(b);reveal();}})},{root:null,rootMargin:'0px 0px -12% 0px',threshold:0}));io.observe(b);const rect=b.getBoundingClientRect();if(rect.top<innerHeight&&rect.bottom>0){io.unobserve(b);reveal();}});}
			}),
			...
			feature({
				...
				init: async root=>{const ns=root.dataset?.barbaNamespace||root.getAttribute("data-barba-namespace")||"",scope=document;scope.querySelectorAll(".nav-primary-wrap").forEach(wrap=>{if(wrap._navInit)return;wrap._navInit=!0;const btnMenu=wrap.querySelector(".nav-button-menu"),label=wrap.querySelector(".nav-button-text"),phone=wrap.querySelector(".phone-number"),minis=wrap.querySelectorAll(".button-minimal-darkmode"),links=wrap.querySelectorAll(".menu-link"),lineText=wrap.querySelector(".ta-one-menu"),shell=wrap.querySelector(".menu-wrapper"),menuBox=wrap.querySelector(".menu-container"),btnFilter=wrap.querySelector(".nav-button-filter"),filterBox=wrap.querySelector(".filters-container"),filterItems=wrap.querySelectorAll(".modal-filters-item"),fLine1=wrap.querySelector(".filter-line-1"),fLine2=wrap.querySelector(".filter-line-2"),fTuner=wrap.querySelectorAll(".filter-tuner"),fCaption=wrap.querySelector(".modal-filters-caption");if(!(btnMenu&&label&&phone&&minis.length&&links.length&&lineText&&shell&&menuBox))return;label.dataset.orig=label.textContent||"Menu";shell.style.display="none";menuBox.style.display="none";filterBox&&(filterBox.style.display="none");let sp=null;const tlMenu=gsap.timeline({paused:!0}).call(()=>{sp=splitAndMask(lineText),animateLines(sp.lines).eventCallback("onComplete",()=>{safelyRevertSplit(sp,lineText),sp=null})},null,0).from(phone,{opacity:0,duration:.4},">").from(minis,{opacity:0,duration:.4,stagger:.15},"<").from(links,{opacity:0,yPercent:240,duration:.45,stagger:.12},"<").to(label,{text:"Close",duration:.25},"<");tlMenu.eventCallback("onReverseComplete",()=>{label.textContent=label.dataset.orig});let tlFilter=null;if(btnFilter&&filterBox&&filterItems.length){if(ns==="archive"){btnFilter.style.display="flex";gsap.to(btnFilter,{opacity:1,duration:.2})}else{gsap.to(btnFilter,{opacity:0,duration:.2,onComplete:()=>btnFilter.style.display="none"})}tlFilter=gsap.timeline({paused:!0}).to(filterBox,{opacity:1,duration:.4,ease:"power2.out"},0).to(fTuner,{opacity:0,duration:.15},"<").to(fLine1,{rotation:45,transformOrigin:"center",duration:.35},"<").to(fLine2,{rotation:-45,marginTop:"-4px",transformOrigin:"center",duration:.35},"<").from(fCaption,{opacity:0,duration:.5},"<").from(filterItems,{opacity:0,duration:.8,stagger:.2},"<")}wrap._menuTimeline=tlMenu;wrap._filterTimeline=tlFilter;wrap._menuButton=btnMenu;wrap._filterButton=btnFilter;let state=null,fallback=null;const lock=b=>{document.body.style.overflow=b?"hidden":""},arm=(fn)=>{fallback&&fallback.kill&&fallback.kill(),fallback=gsap.delayedCall(.55,fn)},hideMenu=()=>{menuBox&&(menuBox.style.display="none",menuBox.removeAttribute("data-open"));shell&&(shell.style.display="none",shell.removeAttribute("data-open"));lock(!1);state=null},hideFilter=()=>{filterBox&&(filterBox.style.display="none",filterBox.removeAttribute("data-open"));shell&&(shell.style.display="none",shell.removeAttribute("data-open"));lock(!1);state=null},clearCbs=()=>{tlMenu&&tlMenu.eventCallback("onReverseComplete",null),tlFilter&&tlFilter.eventCallback("onReverseComplete",null)};btnMenu&&tlMenu&&btnMenu.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if(state==="filter"&&tlFilter){clearCbs(),arm(hideFilter),tlFilter.eventCallback("onReverseComplete",()=>{fallback?.kill?.(),hideFilter()}),tlFilter.timeScale(2).reverse(),gsap.delayedCall(.01,()=>{state="menu",lock(!0),shell&&(shell.style.display="flex",shell.setAttribute("data-open","1")),menuBox&&(menuBox.style.display="flex",menuBox.setAttribute("data-open","1")),tlMenu.timeScale(1).play(0)});return}if(state==="menu"){clearCbs(),arm(hideMenu),tlMenu.eventCallback("onReverseComplete",()=>{fallback?.kill?.(),hideMenu()}),tlMenu.timeScale(2).reverse();return}state="menu",lock(!0),shell&&(shell.style.display="flex",shell.setAttribute("data-open","1")),menuBox&&(menuBox.style.display="flex",menuBox.setAttribute("data-open","1")),tlMenu.timeScale(1).play(0)},{passive:!1});btnFilter&&tlFilter&&btnFilter.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if(state==="menu"&&tlMenu){clearCbs(),arm(hideMenu),tlMenu.eventCallback("onReverseComplete",()=>{fallback?.kill?.(),hideMenu()}),tlMenu.timeScale(2).reverse(),gsap.delayedCall(.01,()=>{state="filter",lock(!0),shell&&(shell.style.display="flex",shell.setAttribute("data-open","1")),filterBox&&(filterBox.style.display="flex",filterBox.setAttribute("data-open","1")),tlFilter.timeScale(1).play(0)});return}if(state==="filter"){clearCbs(),arm(hideFilter),tlFilter.eventCallback("onReverseComplete",()=>{fallback?.kill?.(),hideFilter()}),tlFilter.timeScale(2).reverse();return}state="filter",lock(!0),shell&&(shell.style.display="flex",shell.setAttribute("data-open","1")),filterBox&&(filterBox.style.display="flex",filterBox.setAttribute("data-open","1")),tlFilter.timeScale(1).play(0)},{passive:!1});tlMenu&&(tlMenu.eventCallback("onStart",()=>NavigationManager.setLock("menu",!0)),tlMenu.eventCallback("onComplete",()=>NavigationManager.setLock("menu",!1)),tlMenu.eventCallback("onReverseComplete",()=>NavigationManager.setLock("menu",!1)));tlFilter&&(tlFilter.eventCallback("onStart",()=>NavigationManager.setLock("filter",!0)),tlFilter.eventCallback("onComplete",()=>NavigationManager.setLock("filter",!1)),tlFilter.eventCallback("onReverseComplete",()=>NavigationManager.setLock("filter",!1)))});NavigationManager.attachMenuLocks(document);}
			}),
