		...
		registries.pages.archive.push(
			feature({
				...
				init: function(root){
					...
					// Motion Module
					(function(){if(!window.gsap){mod.scheduleMotionRefresh=function(){};return}var io=0,scan=0,raf=0,t=0,run=0,tries=0,max=45,STAG=.08,MIN_VIS_AFTER=3,MIN_VIS_PAD=.06,MIN_VIS_DUR=.6,lastKick=0,KICK_GAP=140,visDoneRun=0;function cleanup(){try{io&&io.disconnect&&io.disconnect()}catch(_){}io=0;try{scan&&cancelAnimationFrame(scan)}catch(_){}scan=0;try{clearTimeout(t)}catch(_){}t=0}unsubs.push(cleanup);function canRun(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(_){return!0}}function activeScope(){try{var v=mod.getView&&mod.getView();if(v==="playful"&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(v==="minimal"&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){ }try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){ }return el.host||host}function items(scope){return[].slice.call((scope||activeScope()).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function isMinScope(sc){try{return sc&&sc.classList&&sc.classList.contains("section-archive-minimal")}catch(_){return!1}}function minVisual(sc){if(!sc)return null;return sc.querySelector(".archive-minimal-item-visual")||sc.querySelector(".archive-minimal-visual")||sc.querySelector(".archive-minimal-visual-wrap")||null}function prepOne(it){if(!it)return;it.__almRun=run;it.__almDone=0;try{gsap.killTweensOf(it)}catch(_){}gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"})}function prepMinVisual(v){if(!v)return;try{gsap.killTweensOf(v)}catch(_){}gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}function revealOne(it,d){if(!it||it.__almDone||!canRun(it))return;it.__almRun!==run&&prepOne(it);it.__almDone=1;gsap.to(it,{delay:d||0,y:0,autoAlpha:1,filter:"blur(0px)",duration:.75,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange,opacity,visibility"})}function revealMinVisual(sc,bd){var v=minVisual(sc);if(!v||visDoneRun===run)return;visDoneRun=run;prepMinVisual(v);gsap.to(v,{delay:bd||0,autoAlpha:1,filter:"blur(0px)",duration:MIN_VIS_DUR,ease:"power2.out",overwrite:!0,clearProps:"filter,willChange,opacity,visibility"})}function revealBatch(sc,els){els.sort(function(a,b){return a.getBoundingClientRect().top-b.getBoundingClientRect().top});for(var i=0;i<els.length;i++)revealOne(els[i],STAG*i);if(isMinScope(sc)){var idx=Math.max(0,Math.min(MIN_VIS_AFTER,els.length)-1),d=STAG*idx+MIN_VIS_PAD;revealMinVisual(sc,d)}}function inView(it){if(!canRun(it)||it.__almDone)return!1;var r=it.getBoundingClientRect();return r.top<(innerHeight||0)&&r.bottom>0}function forceScan(sc,list,f){var c=0;function tick(){c++;for(var h=[],i=0;i<list.length;i++)inView(list[i])&&h.push(list[i]);h.length&&revealBatch(sc,h);c<(f||30)?scan=requestAnimationFrame(tick):scan=0}scan=requestAnimationFrame(tick)}function start(sc,list){cleanup();if("IntersectionObserver"in window){io=new IntersectionObserver(function(e){for(var h=[],i=0;i<e.length;i++)e[i].isIntersecting&&h.push(e[i].target);h.length&&revealBatch(sc,h)},{threshold:.12,rootMargin:"0px 0px -10% 0px"});for(var i=0;i<list.length;i++)try{io.observe(list[i])}catch(_){}}forceScan(sc,list,30)}function resetAndRun(scope){var sc=scope||activeScope(),list=items(sc);if(!list.length){if(tries++<max)return requestAnimationFrame(function(){resetAndRun(sc)});return}tries=0;run++;visDoneRun=0;for(var i=0;i<list.length;i++)canRun(list[i])&&prepOne(list[i]);if(isMinScope(sc))prepMinVisual(minVisual(sc));start(sc,list)}mod.resetMotion=function(){tries=0;resetAndRun(activeScope())};mod.scheduleMotionRefresh=function(){var n=Date.now();if(n-lastKick<KICK_GAP)return;lastKick=n;if(raf)return;raf=requestAnimationFrame(function(){raf=0;tries=0;resetAndRun(activeScope());try{clearTimeout(t)}catch(_){}t=setTimeout(function(){tries=0;resetAndRun(activeScope())},160)})};on(document,"archive:view",mod.scheduleMotionRefresh,{passive:!0});on(document,"archive:filter:applied",mod.scheduleMotionRefresh,{passive:!0});requestAnimationFrame(mod.scheduleMotionRefresh)})();
