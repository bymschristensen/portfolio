...
!function(){if(["portfolio-13-ec6f18.webflow.io"].includes(location.hostname)){const t="gh:portfolio:main:sha";try{const e=Date.now();let o=null;try{o=JSON.parse(sessionStorage.getItem(t)||"null")}catch{}if(o&&o.sha&&e-(o.t||0)<12e4)return console.info("[BOOT] latest main commit (cached):",o.sha,o.date||"");const a=new AbortController,n=setTimeout((()=>{try{a.abort()}catch{}}),3e3);fetch("https://api.github.com/repos/bymschristensen/portfolio/commits/main",{signal:a.signal,cache:"no-store",headers:{Accept:"application/vnd.github+json"}}).then((t=>t.ok?t.json():Promise.reject(t))).then((e=>{const o=Array.isArray(e)?e[0]:e,a=(o&&o.sha?o.sha:"").slice(0,7),n=new Date(o?.commit?.author?.date||""),s=new Date(n.getTime()+324e5).toISOString().replace("T"," ").replace("Z","");a&&(sessionStorage.setItem(t,JSON.stringify({sha:a,date:s,t:Date.now()})),console.info("[BOOT] latest main commit:",a,s,"(JST)"))})).catch((()=>{})).finally((()=>clearTimeout(n)))}catch{}}}();
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),l=[...e.querySelectorAll(".cs-titles-inner div")],s=l[0]||null,r=l.slice(1),n=o=>{if(!o)return;try{o.removeAttribute("data-entry-hidden"),o.removeAttribute("hidden"),o.removeAttribute("aria-hidden"),o.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(c=>o.classList&&o.classList.remove(c))}catch{}};a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"});i&&gsap.set(i,{autoAlpha:0,visibility:"hidden"});l.length&&gsap.set(l,{autoAlpha:0,y:20,filter:"blur(10px)"});n(a);n(i);l.forEach(n);const d=.6,h=.06;/* earlier headline start */a&&t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:d,ease:"power2.out"},0);if(i){t.addLabel("headline",h).call(()=>{const o=()=>{try{if(window.splitAndMask&&window.animateLines){const u=splitAndMask(i);gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"});const m=((u.lines&&u.lines.length?u.lines.length:3)*.08+.8)|0?((u.lines.length||3)*.08+.8):.8;/* subtitle slightly before headline finishes */s&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.45,ease:"power2.out"},"headline+="+Math.max(0,m-.45));animateLines(u.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:u.revert).call(u,u,i)}catch{}"inline"===getComputedStyle(i).display&&(i.style.display="inline")})}else{gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"}),s&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.45,ease:"power2.out"},"headline+=.25")}}catch{gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"}),s&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.45,ease:"power2.out"},"headline+=.25")}};const g=(CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready&&CoreUtilities.Fonts.ready())||Promise.resolve();const p=new Promise(f=>setTimeout(f,200));/* race fonts to avoid lag */Promise.race([g,p]).then(()=>{requestAnimationFrame(()=>requestAnimationFrame(o))}).catch(o)},null,"headline")}r.length&&t.to(r,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.55,ease:"power2.out",stagger:.05},h+.6);return t}
		};
