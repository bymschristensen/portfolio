...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveController",stage:"early",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-switch-block",".archive-switch",".archive-switch-dot"],init:function(root){root=root||document;var KEY="archiveView",mq=matchMedia("(min-width: 1024px)"),playful=root.querySelector(".section-archive-playful"),minimal=root.querySelector(".section-archive-minimal"),btn=root.querySelector(".archive-switch-block"),sw=root.querySelector(".archive-switch"),dot=root.querySelector(".archive-switch-dot");if(!playful||!minimal||root.__archiveViewBound)return;root.__archiveViewBound=1;var clickTargets=[btn,sw].filter(Boolean);function HIDE_ALL(){try{window.gsap&&gsap.set&&gsap.set([playful,minimal],{display:"none"})}catch(e){}playful.style.display="none";minimal.style.display="none"}function stored(){var v=localStorage.getItem(KEY);return v==="playful"||v==="minimal"?v:null}function bpDefault(){return mq.matches?"playful":"minimal"}function itemsFor(v){return v==="playful"?playful.querySelectorAll(".archive-playful-item"):minimal.querySelectorAll(".archive-minimal-item")}function setUI(v){var p=v==="playful";if(btn){btn.dataset.view=v;btn.setAttribute("aria-pressed",String(p));btn.classList.toggle("is-playful",p);btn.classList.toggle("is-minimal",!p)}if(sw){sw.dataset.view=v;sw.classList.toggle("is-playful",p);sw.classList.toggle("is-minimal",!p)}if(dot){dot.dataset.view=v;dot.classList.toggle("is-playful",p);dot.classList.toggle("is-minimal",!p)}}function prepItems(v){if(!window.gsap)return;var its=[].slice.call(itemsFor(v)||[]);its.forEach(function(it){it.__almDone=1;gsap.killTweensOf(it);gsap.set(it,{y:18,autoAlpha:0,filter:"blur(10px)",willChange:"transform,opacity,filter"})});return its}function animateIn(v){var sec=v==="playful"?playful:minimal;if(!window.gsap){HIDE_ALL();sec.style.display="block";setUI(v);return}HIDE_ALL();sec.style.display="block";setUI(v);gsap.killTweensOf(sec);gsap.set(sec,{autoAlpha:1});var its=prepItems(v);gsap.timeline({defaults:{ease:"power3.out"}}).to(its,{y:0,autoAlpha:1,filter:"blur(0px)",duration:.6,stagger:.06,overwrite:!0,clearProps:"y,filter,willChange"},0).add(function(){try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(e){}try{root.dispatchEvent(new CustomEvent("archive:view",{bubbles:!0,detail:{view:v}}))}catch(e){}},">")}function apply(v,meta){HIDE_ALL();var sec=v==="playful"?playful:minimal;sec.style.display="block";try{window.gsap&&gsap.set&&gsap.set(sec,{autoAlpha:1,filter:"none",clearProps:"opacity,visibility"})}catch(e){}setUI(v);if(meta==="user")try{localStorage.setItem(KEY,v)}catch(e){}try{root.dispatchEvent(new CustomEvent("archive:view",{bubbles:!0,detail:{view:v}}))}catch(e){}}function swap(toV){if(!window.gsap){apply(toV,"user");return}var cur=(btn&&btn.dataset.view)||(sw&&sw.dataset.view)||((playful.style.display!=="none")?"playful":"minimal");if(cur===toV)return;var curSec=cur==="playful"?playful:minimal,nextSec=toV==="playful"?playful:minimal;setUI(toV);try{localStorage.setItem(KEY,toV)}catch(e){}gsap.killTweensOf([curSec,nextSec]);var its=prepItems(toV);gsap.timeline({defaults:{ease:"power2.out"}}).to(curSec,{autoAlpha:0,filter:"blur(10px)",duration:.22,overwrite:!0},0).add(function(){curSec.style.display="none";nextSec.style.display="block";gsap.set(nextSec,{autoAlpha:1,filter:"none"})},">").to(its,{y:0,autoAlpha:1,filter:"blur(0px)",duration:.6,stagger:.06,overwrite:!0,clearProps:"y,filter,willChange"},">-=0.02").add(function(){try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(e){}try{root.dispatchEvent(new CustomEvent("archive:view",{bubbles:!0,detail:{view:toV}}))}catch(e){}},">")}function onClick(e){e&&e.preventDefault&&e.preventDefault();var cur=(btn&&btn.dataset.view)||(sw&&sw.dataset.view)||((playful.style.display!=="none")?"playful":"minimal");swap(cur==="playful"?"minimal":"playful")}function onMQ(){if(stored())return;apply(bpDefault(),"auto");animateIn(bpDefault())}HIDE_ALL();var s=stored(),v=s||bpDefault();apply(v,s?"stored":"auto");animateIn(v);mq.addEventListener?mq.addEventListener("change",onMQ):mq.addListener(onMQ);clickTargets.forEach(function(t){t&&t.addEventListener("click",onClick,{passive:!1})});return root.__archiveViewDestroy=function(){try{mq.removeEventListener?mq.removeEventListener("change",onMQ):mq.removeListener(onMQ)}catch(e){}clickTargets.forEach(function(t){try{t&&t.removeEventListener("click",onClick)}catch(e){}});root.__archiveViewBound=0;root.__archiveViewDestroy=null}}}),
