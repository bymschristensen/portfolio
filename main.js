	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger||!window.gsap)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework"),g=c.querySelector(".cs-gallery");if(!d||!m)return;try{ScrollTrigger.getById("csbg:driver")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{v!==l&&(l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"}))};const imgs=[...((g||c).querySelectorAll("img"))];await new Promise(res=>{if(!imgs.length)return res();let n=0;const done=()=>{++n>=imgs.length&&res()};imgs.forEach(im=>{if(im.complete&&im.naturalWidth>0)return done();(typeof im.decode=="function"?im.decode().catch(()=>{}):Promise.resolve()).finally(done),im.addEventListener("load",done,{once:!0}),im.addEventListener("error",done,{once:!0})})});const calc=()=>{const y=window.pageYOffset||document.documentElement.scrollTop||0,vh=window.innerHeight||document.documentElement.clientHeight||0,dt=d.getBoundingClientRect().top+y,mt=m.getBoundingClientRect().top+y;return{y,vh,dt,mt}};let T=calc();const apply=()=>{const y=window.pageYOffset||document.documentElement.scrollTop||0,vh=window.innerHeight||document.documentElement.clientHeight||0;return y+vh>=T.mt-.2*vh?s(bd):y+vh>=T.dt?s(bg):s(cs)};ScrollTrigger.create({id:"csbg:driver",trigger:document.documentElement,start:0,end:"max",onUpdate:apply,onRefresh:()=>{T=calc();apply()},refreshPriority:200});requestAnimationFrame(()=>{ScrollTrigger.refresh();apply()});setTimeout(()=>{ScrollTrigger.refresh();apply()},250);setTimeout(()=>{ScrollTrigger.refresh();apply()},800)}
			}),
			feature({
