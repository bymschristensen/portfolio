	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework"),g=c.querySelector(".cs-gallery");if(!d||!m)return;try{c.__csbgCleanup&&c.__csbgCleanup()}catch{}try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})};const sd=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),invalidateOnRefresh:!0,refreshPriority:100}),sm=ScrollTrigger.create({id:"csbg:more",trigger:m,start:"top 80%",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),invalidateOnRefresh:!0,refreshPriority:110});const sync=()=>{const y=window.scrollY||document.documentElement.scrollTop||0;s(y>=sm.start?bd:y>=sd.start?bg:cs)};ScrollTrigger.addEventListener("refresh",sync);const ref=()=>{ScrollTrigger.refresh();requestAnimationFrame(sync)};let t1=0,t2=0;const kick=()=>{clearTimeout(t1);clearTimeout(t2);ref();t1=setTimeout(ref,150);t2=setTimeout(ref,450)};const onR=()=>kick();window.addEventListener("resize",onR,{passive:!0});c.__csbgCleanup=()=>{window.removeEventListener("resize",onR);ScrollTrigger.removeEventListener("refresh",sync);try{sd.kill()}catch{}try{sm.kill()}catch{}delete c.__csbgCleanup};if(g){const imgs=[...g.querySelectorAll("img")];if(imgs.length){let n=0;const done=()=>{if(++n<imgs.length)return;kick()};imgs.forEach(i=>{if(i.complete&&i.naturalWidth>0)return done();(typeof i.decode=="function"?i.decode().catch(()=>{}):Promise.resolve()).finally(done),i.addEventListener("load",done,{once:!0}),i.addEventListener("error",done,{once:!0})})}else kick()}else kick();sync()}
			}),
			feature({
