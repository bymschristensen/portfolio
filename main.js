...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=document.documentElement,i=e.querySelector(".cs-hero-image"),o=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),r=e=>{if(!e)return null;let t=e.querySelector(".ta-one,[data-animate-headline],h1")||e;for(;t&&t.children&&1===t.children.length&&t.textContent.trim()===t.children[0].textContent.trim();)t=t.children[0];return t},s=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList&&e.classList.remove(t))}catch{}};a.classList.add("entry-ready");const l=r(o);o&&(o.style.visibility="visible"),l&&(l.style.visibility="visible"),i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)",overwrite:"auto",immediateRender:!0}),l&&gsap.set(l,{autoAlpha:0,overwrite:"auto",immediateRender:!0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),s(i),s(o),n.forEach(s),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(l){const d=l.style.display,c="inline"===getComputedStyle(l).display;c&&(l.style.display="block");const u=()=>{c&&(l.style.display="inline")},h=()=>{gsap.to(l,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete:u})},p=()=>{try{const m=splitAndMask(l);animateLines(m.lines).eventCallback("onComplete",()=>{safelyRevertSplit(m,l),u()})}catch{h()}};let f=0,g=null;const y=()=>{if(window.SplitText){p();return}if(f++<12){g=setTimeout(y,60);return}h()};t.addLabel("headline",.35).set(l,{autoAlpha:1,visibility:"visible"},"headline").call(()=>{cancelTimeout();function cancelTimeout(){if(g){clearTimeout(g);g=null}}requestAnimationFrame(()=>{CoreUtilities.Fonts.ready().then(y)})},null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
