...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				id: "archiveDisableEmptyCaseStudies",
				stage: "main",
				namespaces: ["archive"],
				selectors: [".archive-minimal-list",".archive-minimal-item",".archive-minimal-item-link"],
				init:async function(root){root=root||document;var S=root.__archiveDisableEmptyCS||(root.__archiveDisableEmptyCS={});if(S._bound)return;S._bound=1;var list=root.querySelector(".archive-minimal-list");if(!list){S.destroy=function(){S._bound=0;try{delete root.__archiveDisableEmptyCS}catch(e){}};return}function hasCase(it){if(!it)return!0;var v=(it.getAttribute("has_case_study")||it.getAttribute("data-has_case_study")||"").toLowerCase();if(!v)return!0;return v==="yes"||v==="true"||v==="1"}function setState(it){if(!it)return;var a=it.querySelector(".archive-minimal-item-link");if(!a)return;var ok=hasCase(it);if(ok){a.classList.remove("is-disabled");try{a.removeAttribute("aria-disabled")}catch(e){}try{a.removeAttribute("tabindex")}catch(e){}}else{a.classList.add("is-disabled");try{a.setAttribute("aria-disabled","true")}catch(e){}try{a.setAttribute("tabindex","-1")}catch(e){}}}function sync(){try{var items=list.querySelectorAll(".archive-minimal-item");for(var i=0;i<items.length;i++)setState(items[i])}catch(e){}}function onClick(e){try{var it=e.target&&e.target.closest?e.target.closest(".archive-minimal-item"):null;if(!it||!list.contains(it))return;if(!hasCase(it)){e.preventDefault();e.stopPropagation()}}catch(err){}}sync();list.addEventListener("click",onClick,!0);var mo=new MutationObserver(function(){sync()});try{mo.observe(list,{subtree:!0,childList:!0,attributes:!0,attributeFilter:["class","style","has_case_study","data-has_case_study"]})}catch(e){}S.destroy=function(){try{mo.disconnect()}catch(e){}try{list.removeEventListener("click",onClick,!0)}catch(e){}S._bound=0;try{delete root.__archiveDisableEmptyCS}catch(e){}};}
			}),
			feature({
