...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async t=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=[...t.querySelectorAll(".infinite-gallery")];if(!e.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),a=t=>{let e=0;const r=parseFloat(getComputedStyle(t).gap||"0")||0;return t.childNodes.forEach(t=>{1===t.nodeType&&(e+=(t.getBoundingClientRect().width||0)+r)}),e},i=t=>{const e=parseFloat(t.getAttribute("data-ratio"));if(e&&e>0)return e;const r=(t.className||"").match(/sv-ratio-(\d+)/);if(r){const t=r[1];if("169"===t)return 16/9;if("133"===t||"43"===t)return 4/3;if("11"===t)return 1}const a=t.querySelector("img,video");if(a){const t=parseInt(a.getAttribute("width"),10)||a.naturalWidth||a.videoWidth||0,e=parseInt(a.getAttribute("height"),10)||a.naturalHeight||a.videoHeight||0;if(t>0&&e>0)return t/e}return 1.5},s=(t,e)=>{const r=parseFloat(t.getAttribute("data-size")),a=(t.className||"").match(/sv-size-(\d+)/),i=r||(a?parseFloat(a[1]):0),s=e.clientHeight||0;if(i>0&&s>0)return Math.max(1,Math.round(s*i/100));const o=t.style.height;t.style.height="";const n=parseFloat(getComputedStyle(t).height)||t.getBoundingClientRect().height||0;return t.style.height=o,n>8?Math.round(n):s>0?Math.round(.6*s):360};e.forEach(async t=>{const e=t.querySelector(".infinite-gallery-wrapper");if(!e||e.__inited)return;e.__inited=!0;const o="right"===(t.dataset.direction||"left").toLowerCase()?-1:1,n=parseFloat(t.dataset.speed)||.6;t.setAttribute("data-armed","measure");const l=[...e.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!l.length)return void t.setAttribute("data-armed","1");await(t=>{const e=[...t.querySelectorAll("img,video")];if(!e.length)return Promise.resolve();const r=t=>t.naturalWidth>0&&t.naturalHeight>0||t.videoWidth>0&&t.videoHeight>0,a=[];return Promise.race([Promise.all(e.map(t=>{if(r(t))return Promise.resolve();const e="function"==typeof t.decode?t.decode().catch(()=>{}):Promise.resolve(),a=new Promise(e=>{const r=()=>{t.removeEventListener("load",r),e()};t.addEventListener("load",r,{once:!0})}),i=new Promise(e=>{let a=0;!function i(){if(r(t)||a++>80)return e();setTimeout(i,40)}()});return Promise.race([e,a,i])})),new Promise(t=>a.push(setTimeout(t,3500)))]).finally(()=>a.forEach(clearTimeout))})(e),l.forEach((t,a)=>{const o=i(t),n=s(t,e),l=Math.max(1,Math.round(n*o));t.dataset.key=String(a),t.dataset.ratio=String(o),t.dataset.targetH=String(n),t.dataset.revealed=t.dataset.revealed||"0",t.style.flex="0 0 auto",t.style.minWidth=l+"px",t.style.maxWidth=l+"px",r&&(t.style.aspectRatio=o+" / 1")});const d=()=>{[...e.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(t=>t.remove())},c=t=>{const a=t.cloneNode(!0),s=t.dataset.key||"",o=parseFloat(t.dataset.ratio)||i(t),n=+t.dataset.targetH||0,l=Math.max(1,Math.round(n*o));return a.setAttribute("data-clone","1"),a.dataset.key=s,a.style.flex="0 0 auto",a.style.minWidth=l+"px",a.style.maxWidth=l+"px",r&&(a.style.aspectRatio=o+" / 1"),e.appendChild(a),a},h=()=>l.forEach(c),u=()=>{d(),h(),h();let t=0;const r=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;a(e)<r&&t++<8;)h()};u(),t.setAttribute("data-armed","1");let p=0;const m=(t,r)=>{p-=o*((r||16.6667)/1e3)*(60*n);const a=parseFloat(getComputedStyle(e).gap||"0")||0,i=t=>(t.getBoundingClientRect().width||0)+a;let s=e.firstElementChild,l=0;for(;s&&p<-i(s)&&l++<50;)p+=i(s),e.appendChild(s),s=e.firstElementChild;let d=e.lastElementChild,c=0;for(;d&&p>0&&c++<50;)p-=i(d),e.insertBefore(d,e.firstElementChild),d=e.lastElementChild;gsap.set(e,{x:p});const h=(innerWidth||0)/2;e.querySelectorAll(".service-visual").forEach(t=>{const e=t.closest(".service-visual-wrapper");if(!e)return;const r=e.getBoundingClientRect(),a=(h-(r.left+r.width/2))/(innerWidth||1);t.style.setProperty("--drift",40*a+"px")})};gsap.ticker.add(m),CoreUtilities.Observers.addTicker(m);let f=!1;const g=()=>{if(f)return;f=!0;const e=((e=7)=>{const r=t.getBoundingClientRect(),a=r.left,i=r.right,s=r.top,o=r.bottom,n=(a+i)/2,d=l.map(t=>{const e=t.getBoundingClientRect(),r=Math.max(0,Math.min(e.right,i)-Math.max(e.left,a))*Math.max(0,Math.min(e.bottom,o)-Math.max(e.top,s)),l=(e.left+e.right)/2;return{el:t,rect:e,area:r,dist:Math.abs(l-n)}}),c=d.filter(t=>t.area>1).sort((t,e)=>t.rect.left-e.rect.left);if(c.length){const t=c.slice().sort((t,e)=>t.dist-e.dist).slice(0,e),r=new Set(t.map(t=>t.el));return d.filter(t=>r.has(t.el)).sort((t,e)=>t.rect.left-e.rect.left).map(t=>t.el)}return d.sort((t,e)=>t.dist-e.dist).slice(0,e).sort((t,e)=>t.rect.left-e.rect.left).map(t=>t.el)})(7);e.forEach((t,e)=>{if("1"===t.dataset.revealed)return;const r=+t.dataset.targetH||Math.round(t.getBoundingClientRect().height)||320;gsap.fromTo(t,{height:0,overflow:"hidden"},{height:r,duration:.9,ease:"power2.out",onComplete:()=>{t.style.height="",t.style.overflow="",t.dataset.revealed="1"},delay:.1*e})})},y=new IntersectionObserver(e=>{e.forEach(e=>{e.target===t&&e.isIntersecting&&(g(),y.unobserve(t))})},{root:null,threshold:.05,rootMargin:"0px 0px -5% 0px"});y.observe(t),CoreUtilities.Observers.addDom(y);const v=new ResizeObserver(()=>{d(),u(),l.forEach(t=>{const a=t.dataset.key||"",o=parseFloat(t.dataset.ratio)||i(t),n=+t.dataset.targetH||s(t,e),l=Math.max(1,Math.round(n*o));t.style.minWidth=l+"px",t.style.maxWidth=l+"px",r&&(t.style.aspectRatio=o+" / 1"),e.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${a}"]`).forEach(t=>{t.style.minWidth=l+"px",t.style.maxWidth=l+"px",r&&(t.style.aspectRatio=o+" / 1")})})});v.observe(e),CoreUtilities.Observers.addDom(v)})}
			})
		);
