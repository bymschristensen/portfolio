	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async R=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const U=e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const n=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0;let o=[];return Promise.race([Promise.all(t.map(e=>{if(n(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),o=new Promise(r=>{const t=()=>{e.removeEventListener("load",t),r()};e.addEventListener("load",t,{once:!0})}),i=new Promise(r=>{let t=0;!function c(){if(n(e)||t++>80)return r();setTimeout(c,40)}()});return Promise.race([t,o,i])})),new Promise(e=>o.push(setTimeout(e,3500)))]).finally(()=>o.forEach(clearTimeout))},S=(CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1"))===!0,A=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const n=(e.className||"").match(/sv-ratio-(\d+)/);if(n){const e=n[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const o=e.querySelector("img,video");if(o){const e=parseInt(o.getAttribute("width"),10)||o.naturalWidth||o.videoWidth||0,t=parseInt(o.getAttribute("height"),10)||o.naturalHeight||o.videoHeight||0;if(e>0&&t>0)return e/t}return 3/2},H=(e,t)=>{const n=parseFloat(e.getAttribute("data-size"));if(n&&n>0)return Math.max(1,Math.round(t*n/100));const o=(e.className||"").match(/sv-size-(\d+)/);if(o){const e=parseFloat(o[1]);if(e>0)return Math.max(1,Math.round(t*e/100))}const r=e.getBoundingClientRect().height||0;return r>0?Math.round(r):Math.max(1,Math.round(.5*t))},g=e=>Math.max(document.documentElement.clientWidth,window.innerWidth||0),L=[...R.querySelectorAll(".infinite-gallery")];if(!L.length)return;L.forEach(E=>{const W=E.querySelector(".infinite-gallery-wrapper");if(!W||W.__inited)return;W.__inited=!0;E.setAttribute("data-armed","measure");const C=[...W.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!C.length){E.setAttribute("data-armed","1");return}const k=new Set;let i=!1,a=0,l=0,c=0,d=("right"===(E.dataset.direction||"left").toLowerCase()?-1:1),u=parseFloat(E.dataset.speed)||.6;/* 1) SYNC FREEZE (no flash) */C.forEach((e,t)=>{const n=A(e);e.dataset.ratio=String(n),e.dataset.key=String(t),e.dataset.revealed="0",e.dataset.revealing="",e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.height="0px",e.style.willChange="height,transform",S?(e.style.aspectRatio=n+" / 1",e.style.removeProperty("width"),e.style.removeProperty("min-width"),e.style.removeProperty("max-width")):(e.style.width="1px",e.style.minWidth="1px",e.style.maxWidth="1px")});/* helpers */const q=()=>{C.forEach(e=>{const t=parseFloat(e.dataset.ratio)||A(e),n=H(e,W.clientHeight);e.dataset.targetH=String(n);if(S){e.style.aspectRatio=t+" / 1"}else{const o=Math.max(1,Math.round(n*t));e.style.width=o+"px",e.style.minWidth=o+"px",e.style.maxWidth=o+"px"}})},x=()=>{C.forEach(e=>{const t=e.cloneNode(!0),n=e.dataset.key||"",o=parseFloat(e.dataset.ratio)||A(e),r=+e.dataset.targetH||0;t.setAttribute("data-clone","1"),t.dataset.key=n,t.dataset.ratio=String(o),t.style.overflow="hidden",t.style.flex="0 0 auto",t.style.willChange="height,transform",t.style.height=k.has(n)&&r>0?r+"px":"0px",S?(t.style.aspectRatio=o+" / 1"):(()=>{const e=Math.max(1,Math.round(Math.max(1,r)*o));t.style.width=e+"px",t.style.minWidth=e+"px",t.style.maxWidth=e+"px"})(),W.appendChild(t)})},y=()=>{[...W.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())},v=()=>{let e=0;return W.childNodes.forEach(t=>{1===t.nodeType&&(e+=(t.getBoundingClientRect().width||0)+(parseFloat(getComputedStyle(t.parentElement||t).gap||"0")||0))}),e},b=()=>{y(),x(),x();let e=0;for(;v()<3*g()&&e++<8;)x()},T=()=>{E.setAttribute("data-armed","1");const e=E.getBoundingClientRect(),t=innerHeight||document.documentElement.clientHeight||0;e.bottom>0&&e.top<t&&(i=!0,cancelAnimationFrame(a),a=requestAnimationFrame(M))},M=t=>{if(!i){cancelAnimationFrame(a);return}if((t||0)-l<32){a=requestAnimationFrame(M);return}l=t||performance.now();const n=innerWidth||document.documentElement.clientWidth||0,o=-.05*n,r=1.05*n,s=[...W.querySelectorAll('.service-visual-wrapper:not([data-clone])')].filter(e=>{const t=e.getBoundingClientRect();return t.right>o&&t.left<r}).filter(e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing).sort((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left);if(s.length){const e=gsap.timeline();s.forEach((t,n)=>{const o=+t.dataset.targetH||0,r=t.dataset.key||"",s=parseFloat(t.dataset.ratio)||A(t);t.dataset.revealing="1",e.to(t,{height:o,duration:.9,ease:"power2.out",onStart(){/* ensure hidden before anim even if CSS raced */t.style.height="0px"},onComplete:()=>{t.style.height="",t.dataset.revealed="1",t.dataset.revealing="",k.add(r),W.querySelectorAll('.service-visual-wrapper[data-clone="1"][data-key="'+r+'"]').forEach(e=>{e.style.height=o+"px";if(!S){const t=Math.max(1,Math.round(o*s));e.style.width=t+"px",e.style.minWidth=t+"px",e.style.maxWidth=t+"px"}});if(!S){const e=Math.max(1,Math.round(o*s));t.style.width=e+"px",t.style.minWidth=e+"px",t.style.maxWidth=e+"px"}}},.12*n)})}a=requestAnimationFrame(M)},P=(e,t)=>{const n=(t||16.6667)/1e3,o=60*u;c-=d*n*o;let r=W.firstElementChild,s=0,gp=m=>m.getBoundingClientRect().width+(parseFloat(getComputedStyle(m.parentElement||m).gap||"0")||0);for(;r&&c<-gp(r)&&s++<50;)c+=gp(r),W.appendChild(r),r=W.firstElementChild;let i=W.lastElementChild;s=0;for(;i&&c>0&&s++<50;)c-=gp(i),W.insertBefore(i,W.firstElementChild),i=W.lastElementChild;gsap.set(W,{x:c});const a=(innerWidth||0)/2;W.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const n=t.getBoundingClientRect(),o=(a-(n.left+n.width/2))/((innerWidth)||1);e.style.setProperty("--drift",40*o+"px")})},I=t=>{t.forEach(t=>{t.target===E&&(t.isIntersecting?(i=!0,cancelAnimationFrame(a),a=requestAnimationFrame(M)):(i=!1,cancelAnimationFrame(a)))})},O=new ResizeObserver(()=>{q(),y(),b(),C.forEach(e=>{const t=e.dataset.key||"",n=+e.dataset.targetH||0;W.querySelectorAll('.service-visual-wrapper[data-clone="1"][data-key="'+t+'"]').forEach(e=>{e.style.height=n+"px"})}),T()});(async()=>{await U(W);q();b();T();const e=new IntersectionObserver(I,{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});e.observe(E),CoreUtilities.Observers.addDom(e),CoreUtilities.Observers.addDom({disconnect(){i=!1,cancelAnimationFrame(a)}});const t=(e,t)=>P(e,t);gsap.ticker.add(t),CoreUtilities.Observers.addTicker(t),O.observe(W),CoreUtilities.Observers.addDom(O)})()})
			})
		);
