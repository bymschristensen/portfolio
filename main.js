	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const m=gsap.matchMedia();m.add("(min-width: 1024px)",(()=>{const sec=r.querySelector('[data-imgscroll="photos"]'),bio=r.querySelector('[data-imgscroll="bio"]'),copy=r.querySelector('[data-imgscroll="copy"]'),stats=r.querySelector('[data-imgscroll="stats"]'),headline=r.querySelector(".bio-text .headline-md");if(!sec||!bio||!copy||!stats||!headline)return;const all=[...sec.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(parseInt(a.getAttribute("data-photo"),10)||0)-(parseInt(b.getAttribute("data-photo"),10)||0)),p1=all.find(e=>e.getAttribute("data-photo")==="1"),p2=all.find(e=>e.getAttribute("data-photo")==="2"),p3=all.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const rect=e=>e.getBoundingClientRect(),reset=()=>gsap.set([p1,p2,p3],{x:0,y:0,scale:1,willChange:"transform"}),targetBase=()=>{const s=rect(stats),h=rect(headline);return{left:s.left,top:h.top}},delta=(el,base,o={})=>{const a=rect(el);return{x:base.left-a.left+(o.dx||0),y:base.top-a.top+(o.dy||0),scale:o.scale==null?1:o.scale}};let tl;const build=()=>{reset();const base=targetBase(),d1=delta(p1,base,{scale:.42}),d2=delta(p2,base,{dx:18,dy:22,scale:.38}),d3=delta(p3,base,{dx:44,dy:56,scale:.36});tl&&tl.kill();tl=gsap.timeline({defaults:{ease:"none"},scrollTrigger:{trigger:sec,start:"center center",end:"+=140%",scrub:1,anticipatePin:1,invalidateOnRefresh:!0}});tl.addLabel("g",0).to(p3,d3,"g").to(p2,d2,"g+=0.06").to(p1,d1,"g+=0.12")};build();const ro=new ResizeObserver(()=>ScrollTrigger.refresh());[sec,bio,copy,stats,headline].forEach(el=>el&&ro.observe(el));return()=>{try{ro.disconnect()}catch{}try{tl&&tl.kill()}catch{}}}))}
			}),
			feature({
