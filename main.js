...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy:e=>{const t=gsap.timeline(),h=e.querySelector(".cs-headline"),i=e.querySelector(".cs-hero-image"),s=e.querySelectorAll(".cs-titles-inner div"),u=o=>{if(!o)return;try{o.removeAttribute("data-entry-hidden"),o.removeAttribute("hidden"),o.removeAttribute("aria-hidden"),o.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(c=>o.classList&&o.classList.remove(c))}catch{}};i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"});s.length&&gsap.set(s,{autoAlpha:0,y:20,filter:"blur(10px)"});u(i);u(h);s.forEach(u);i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(h){t.addLabel("headline",.35).call(()=>{const r=()=>{try{const D=getComputedStyle(h).display,W=h.clientHeight;h.style.minHeight=W+"px";gsap.set(h,{autoAlpha:1});if(window.splitAndMask&&window.animateLines){const sp=splitAndMask(h);animateLines(sp.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:sp.revert).call(sp,sp,h)}catch{}h.style.minHeight="";h.style.display=D})}else{gsap.to(h,{opacity:1,duration:.2});h.style.minHeight="";h.style.display=D}}catch{h.style.minHeight=""}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>requestAnimationFrame(()=>requestAnimationFrame(r))).catch(r)},null,"headline")}s.length&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t}
		};
