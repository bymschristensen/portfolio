...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(e){const t=matchMedia("(min-width:1024px)");if(!t.matches)return;const r=e.querySelector(".hero-section-showreel"),n=r?.querySelector(".showreel-track"),o=n?.querySelector(".showreel-container");if(!r||!n||!o)return;const i=[r.querySelector(".showreel-headline"),r.querySelector(".tableofcontents"),r.querySelector(".toc-outer"),r.querySelector(".showreel-subtext")].filter(Boolean),s=getComputedStyle(o).transform||"none",a="none"===s?"":s+" ";let c=window.innerHeight,l=0,d=0,u=1,h=.5,f=!1;function p(e){return e<0?0:e>1?1:e}function m(e,t,r){return e+(t-e)*r}function g(){const e=n.getBoundingClientRect();l=Math.max(1,e.width);const t=o.style.transform;o.style.transform=s;d=Math.max(1,o.getBoundingClientRect().width);o.style.transform=t;u=Math.max(1,l/d)}function v(){const e=r.getBoundingClientRect(),t=e.height-c;if(t<=0)return 0;const n=-e.top/t;return p(n)}function y(){const e=v(),t=1+(u-1)*e,r=d*t,a=Math.max(0,l-r),c=(h-.5)*a;o.style.transform=a+"translate3d("+c+"px,0,0) scale("+t+")";const s=-.42*c*e,a1=1-e;for(let e=0;e<i.length;e++){const t=i[e];t.style.transform="translate3d(0,"+s+"px,0)",t.style.opacity=a1}}function w(){f||(f=!0,requestAnimationFrame(()=>{f=!1,y()}))}function x(e){const t=n.getBoundingClientRect(),r=Math.max(t.left,Math.min(e.clientX,t.right));h=(r-t.left)/t.width,w()}function b(){c=window.innerHeight,g(),w()}function E(){w()}g(),y(),window.addEventListener("scroll",E,{passive:!0}),window.addEventListener("resize",b,{passive:!0}),n.addEventListener("mousemove",x,{passive:!0});const C=new ResizeObserver(()=>{b()});try{C.observe(n),C.observe(o)}catch(e){}r.__heroShowreelDestroy__=()=>{try{window.removeEventListener("scroll",E)}catch(e){}try{window.removeEventListener("resize",b)}catch(e){}try{n.removeEventListener("mousemove",x)}catch(e){}try{C.disconnect()}catch(e){}try{o.style.transform="none"===s?"":s,i.forEach(e=>{e.style.transform="",e.style.opacity=""})}catch(e){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(e){}const e=document.querySelector(".hero-section-showreel");e&&(e.__heroShowreelDestroy__=null)}}),
			feature({
