	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!window.gsap||!window.ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capPin)return;e._capPin=1;gsap.set(e,{filter:"contrast(100%) blur(0px)"});const n=s[i+1]||null,t=e.offsetHeight<window.innerHeight,o=t?"top top":"bottom bottom";ScrollTrigger.create({id:"servicePin:"+i,trigger:e,start:o,endTrigger:n||e,end:n?"top top":"bottom top",pin:!0,pinSpacing:!0,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:a=>{const l=Math.max(0,Math.min(1,a.progress)),c=Math.max(0,Math.min(1,(l-.15)/.85)),f=100-90*c,b=10*c;gsap.set(e,{filter:`contrast(${f}%) blur(${b}px)`})},onRefresh:a=>{const l=Math.max(0,Math.min(1,a.progress)),c=Math.max(0,Math.min(1,(l-.15)/.85)),f=100-90*c,b=10*c;gsap.set(e,{filter:`contrast(${f}%) blur(${b}px)`})},onKill:()=>gsap.set(e,{clearProps:"filter"})})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
