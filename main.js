...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(e){const t=matchMedia("(min-width:1024px)");if(!t.matches)return;const r=e.querySelector(".hero-section-showreel"),n=r?.querySelector(".showreel-track"),o=n?.querySelector(".showreel-container");if(!r||!n||!o)return;const i=[r.querySelector(".showreel-headline"),r.querySelector(".tableofcontents"),r.querySelector(".toc-outer"),r.querySelector(".showreel-subtext")].filter(Boolean),s=getComputedStyle(o).transform||"none",p="none"===s?"":s+" ";let a=window.innerHeight,c=1,l=1,u=1,m=.5,f=.5,d=0,y=0,_=0,b=!1;function h(e){return e<0?0:e>1?1:e}function g(e,t,r){return e+(t-e)*r}function v(){const e=n.getBoundingClientRect();c=Math.max(1,e.width);const t=o.style.transform;o.style.transform="none";l=Math.max(1,o.getBoundingClientRect().width);o.style.transform=t;u=Math.max(1,c/l)}function w(){const e=r.getBoundingClientRect(),t=e.height-a;if(t<=0)return 0;return h(-e.top/t)}function S(){const e=w(),t=g(1,u,e),s=l*t;const p=Math.max(0,c-s);y=(f-.5)*p;const m=Math.max(-p/2,Math.min(p/2,y));d+=(m-d)*.14,_+=(t-_)*.14,o.style.transform=pfx()+d+"px,0,0) scale("+_+")";const x=-.45*a*e,k=1-e;i.forEach((e=>{e&&(e.style.transform="translate3d(0,"+x+"px,0)",e.style.opacity=k)}))}function pfx(){return p+"translate3d("}function T(){b||(b=!0,requestAnimationFrame(()=>{b=!1,S()}))}function E(e){const t=n.getBoundingClientRect(),r=Math.max(t.left,Math.min(e.clientX,t.right));m=(r-t.left)/t.width,T()}function C(){a=window.innerHeight,v(),T()}function M(){T()}v(),S(),n.addEventListener("mousemove",E,{passive:!0}),addEventListener("scroll",M,{passive:!0}),addEventListener("resize",C,{passive:!0});const q=new ResizeObserver(C);try{q.observe(n),q.observe(o)}catch(e){}let L=0;!function A(){L=requestAnimationFrame(A),f+=.18*(m-f),T()}();r.__heroShowreelDestroy__=()=>{try{cancelAnimationFrame(L)}catch(e){}try{removeEventListener("scroll",M)}catch(e){}try{removeEventListener("resize",C)}catch(e){}try{n.removeEventListener("mousemove",E)}catch(e){}try{q.disconnect()}catch(e){}try{o.style.transform="none"===s?"":s,i.forEach(e=>{e.style.transform="",e.style.opacity=""})}catch(e){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(e){}const t=document.querySelector(".hero-section-showreel");t&&(t.__heroShowreelDestroy__=null)}}),
			feature({
