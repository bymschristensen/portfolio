	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger||matchMedia("(prefers-reduced-motion: reduce)").matches)return;const P=r.querySelector('[data-imgscroll="photos"]'),B=r.querySelector('[data-imgscroll="bio"]'),C=r.querySelector('[data-imgscroll="copy"]'),S=r.querySelector('[data-imgscroll="stats"]'),H=r.querySelector(".bio-text .headline-md");if(!P||!B||!C||!S||!H)return;const W=[...P.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(parseInt(a.getAttribute("data-photo"),10)||0)-(parseInt(b.getAttribute("data-photo"),10)||0));if(W.length<3)return;const p1=W.find(e=>e.getAttribute("data-photo")==="1"),p2=W.find(e=>e.getAttribute("data-photo")==="2"),p3=W.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const R=e=>e.getBoundingClientRect(),reset=()=>{gsap.set([p1,p2,p3],{x:0,y:0,scale:1,transformOrigin:"50% 50%",zIndex:i=>200-(parseInt(W[i].getAttribute("data-photo"),10)||0)});[p1,p2,p3].forEach((w,i)=>{const img=w.querySelector(".bio-photo");if(!img)return;const startYP=i===0?-55:-60;gsap.set(img,{xPercent:-50,yPercent:startYP,scale:1.1,force3D:!0})})},anchor=()=>{const s=R(S),h=R(H);return{left:s.left,top:h.top}},delta=(el,t,o={dx:0,dy:0,scale:1})=>{const a=R(el);return{x:t.left-a.left+(o.dx||0),y:t.top-a.top+(o.dy||0),scale:o.scale==null?1:o.scale}};let tl;function build(){reset();const A=anchor();const a3=delta(p3,A,{dx:44,dy:56,scale:.36}),a2=delta(p2,A,{dx:18,dy:22,scale:.38}),a1=delta(p1,A,{dx:0,dy:0,scale:Math.max(.79,.42)});tl&&tl.kill();tl=gsap.timeline({defaults:{ease:"power3.out"},scrollTrigger:{trigger:P,start:"top+=15% top",end:"+=140%",scrub:!0,anticipatePin:1,invalidateOnRefresh:!0}});tl.addLabel("g",0);tl.to(p3,{x:a3.x,y:a3.y,scale:a3.scale,onStart:()=>p3.classList.add("is-gathering")},"g");tl.to(p2,{x:a2.x,y:a2.y,scale:a2.scale,onStart:()=>p2.classList.add("is-gathering")},"g+=0.08");tl.to(p1,{x:a1.x,y:a1.y,scale:a1.scale,onStart:()=>p1.classList.add("is-gathering")},"g+=0.16");const i3=p3.querySelector(".bio-photo"),i2=p2.querySelector(".bio-photo"),i1=p1.querySelector(".bio-photo");i3&&tl.to(i3,{yPercent:-50,force3D:!0,duration:.8},"g");i2&&tl.to(i2,{yPercent:-50,force3D:!0,duration:.8},"g+=0.08");i1&&tl.to(i1,{yPercent:-50,force3D:!0,duration:.8},"g+=0.16")}build();const ro=new ResizeObserver(()=>build());ro.observe(document.documentElement)}
			}),
			feature({
