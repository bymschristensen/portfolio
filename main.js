...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(!matchMedia("(min-width:1024px)").matches)return;const sec=root.querySelector(".hero-section-showreel"),trk=sec?.querySelector(".showreel-track"),box=trk?.querySelector(".showreel-container");if(!sec||!trk||!box)return;const copy=[sec.querySelector(".showreel-headline"),sec.querySelector(".tableofcontents"),sec.querySelector(".toc-outer"),sec.querySelector(".showreel-subtext")].filter(Boolean),orig=getComputedStyle(box).transform||"none",prefix=orig==="none"?"":orig+" ";let vh=window.innerHeight,trackW=1,baseW=1,minS=.65,maxS=1,curS=minS,tgtS=minS,cNorm=.7,cSmooth=.7,curX=0,tgtX=0,rafId=0;function clamp01(v){return v<0?0:v>1?1:v}function lerp(a,b,t){return a+(b-a)*t}function measure(){const r=trk.getBoundingClientRect();trackW=r.width||1;const prev=box.style.transform;box.style.transform="none";baseW=box.getBoundingClientRect().width||1;box.style.transform=prev;if(trackW<10||baseW<10)requestAnimationFrame(measure)}function progress(){const r=sec.getBoundingClientRect(),span=r.height-vh*.7;if(span<=0)return 0;const raw=(-r.top-vh*.1)/span;return clamp01(raw)}function apply(){const prog=progress();tgtS=lerp(minS,maxS,prog);const scaled=baseW*tgtS,room=Math.max(0,trackW-scaled);cSmooth+=.15*(cNorm-cSmooth);const cx=cSmooth*trackW,anchor=.35;let left=cx-anchor*scaled;left<0&&(left=0);left>room&&(left=room);const offset=left+scaled/2-trackW/2;tgtX=offset;curX+=.16*(tgtX-curX);curS+=.18*(tgtS-curS);box.style.transform=prefix+"translate3d("+curX+"px,0,0) scale("+curS+")";const lift=-.45*vh*prog,op=1-prog;for(let i=0;i<copy.length;i++){const el=copy[i];if(!el)continue;el.style.transform="translate3d(0,"+lift+"px,0)";el.style.opacity=op}}function loop(){rafId=requestAnimationFrame(loop);apply()}function onMove(ev){const r=trk.getBoundingClientRect(),x=Math.max(r.left,Math.min(ev.clientX,r.right));cNorm=(x-r.left)/(r.width||1)}function onResize(){vh=window.innerHeight;measure()}measure();apply();trk.addEventListener("mousemove",onMove,{passive:!0});addEventListener("resize",onResize,{passive:!0});const ro=new ResizeObserver(onResize);try{ro.observe(trk),ro.observe(box)}catch(e){}loop();sec.__heroShowreelDestroy__=()=>{try{cancelAnimationFrame(rafId)}catch(e){}try{removeEventListener("resize",onResize)}catch(e){}try{trk.removeEventListener("mousemove",onMove)}catch(e){}try{ro.disconnect()}catch(e){}try{box.style.transform=orig==="none"?"":orig;for(let i=0;i<copy.length;i++){const el=copy[i];el&&(el.style.transform="",el.style.opacity="")}}catch(e){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(e){}const sec=document.querySelector(".hero-section-showreel");sec&&(sec.__heroShowreelDestroy__=null)}}),
			feature({
