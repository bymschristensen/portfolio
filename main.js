	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init:async r=>{const s=".appear-in-line",c=":scope > *";r.querySelectorAll(s).forEach(t=>{if(t.__ailInit)return;t.__ailInit=!0;const e=[],n=[];Array.from(t.querySelectorAll(c)).forEach(o=>{const i=getComputedStyle(o),a=parseInt(i.columnCount,10)||1,l=o.getBoundingClientRect();if(a>1){const i=new SplitText(o,{type:"lines",linesClass:"split-line"});e.push(i);const s=l.width/a,r=Array.from({length:a},()=>[]);i.lines.forEach(o=>{const e=o.getBoundingClientRect().left-l.left,i=Math.min(a-1,Math.max(0,Math.floor(e/s)));r[i].push(o),gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})}),r.forEach(o=>n.push(o))}else gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),n.push([o])});const o=()=>{if(t.__ailDone)return;n.forEach((t,e)=>gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:.8,ease:"power2.out",delay:.12*e}));const r=.12*(n.length-1)+.8;gsap.delayedCall(r+.05,()=>{e.forEach(t=>t.revert()),t.__ailDone=!0})},i=CoreUtilities.Observers.addDom(new IntersectionObserver((e,r)=>{e.forEach(e=>{(e.isIntersecting||e.intersectionRatio>0)&&(r.unobserve(t),o())})},{root:null,rootMargin:"0px 0px -1% 0px",threshold:[0,.01]}));t._appearData={splits:e,groups:n,observer:i},i.observe(t);const a=t.getBoundingClientRect();a.top<innerHeight&&a.bottom>0&&(i.unobserve(t),o());const l=r.querySelector(".w-tabs")?t.closest(".w-tab-pane"):null;l&&CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!l.classList.contains("w--tab-active"))return;i.observe(t);const e=t.getBoundingClientRect();e.top<innerHeight&&e.bottom>0&&(i.unobserve(t),o())})).observe(l,{attributes:!0,attributeFilter:["class"]})})}
			}),
			...
			feature({
				...
				init: async root => {
					...
					scope.querySelectorAll(".nav-primary-wrap").forEach((e=>{if(e._menuInit)return;e._menuInit=!0;const t=e.querySelector(".nav-button-menu"),n=e.querySelector(".nav-button-text"),o=e.querySelector(".phone-number"),r=e.querySelectorAll(".button-minimal-darkmode"),l=e.querySelectorAll(".menu-link"),a=e.querySelector(".ta-one-menu");if(!(t&&n&&o&&r.length&&l.length&&a))return;let u;n.dataset.orig=n.textContent;const i=gsap.timeline({paused:!0}).call((()=>{u=splitAndMask(a),animateLines(u.lines).eventCallback("onComplete",(()=>{safelyRevertSplit(u,a),u=null}))}),null,0).from(o,{opacity:0,duration:.5},">").from(r,{opacity:0,duration:.5,stagger:.2},"<").from(l,{opacity:0,yPercent:240,duration:.5,stagger:.2},"<").to(n,{text:"Close",duration:.3},"<");i.eventCallback("onReverseComplete",(()=>{n.textContent=n.dataset.orig})),e._menuTimeline=i,e._menuButton=t}));
					scope.querySelectorAll(".nav-primary-wrap").forEach((e=>{if(e._filterInit)return;e._filterInit=!0;const t=e.querySelector(".nav-button-filter"),r=e.querySelector(".filters-container"),o=e.querySelectorAll(".filter-tuner"),i=e.querySelector(".filter-line-1"),a=e.querySelector(".filter-line-2"),n=e.querySelector(".modal-filters-caption"),l=e.querySelectorAll(".modal-filters-item"),s=scope.querySelector(".menu-filter-hover"),c=scope.querySelectorAll(".menu-filter-image");if(!t||!r||!l.length)return;if("archive"===ns?(t.style.display="flex",gsap.to(t,{opacity:1,duration:.2})):(gsap.to(t,{opacity:0,duration:.2,onComplete:()=>t.style.display="none"}),r.style.display="none"),s&&c.length){gsap.set(s,{xPercent:-50,yPercent:-50,scale:0});const e=gsap.quickTo(s,"x",{duration:2.6,ease:"expo"}),t=gsap.quickTo(s,"y",{duration:2.6,ease:"expo"}),r=r=>{e(r.pageX),t(r.pageY)};window.addEventListener("mousemove",r,{passive:!0}),CoreUtilities.Observers.addDom({disconnect(){window.removeEventListener("mousemove",r)}});const o=gsap.timeline({paused:!0}).to(s,{scale:1,opacity:1,rotation:0,duration:.5,ease:"power1.inOut"});l.forEach(((e,t)=>{e.addEventListener("mouseover",(()=>{c[t]?.classList.add("active"),o.play()})),e.addEventListener("mouseout",(()=>{o.reverse(),c[t]?.classList.remove("active")}))}))}const u=gsap.timeline({paused:!0}).to(r,{opacity:1,duration:.4,ease:"power2.out"},0).to(o,{opacity:0,duration:.15},"<").to(i,{rotation:45,transformOrigin:"center",duration:.35},"<").to(a,{rotation:-45,marginTop:"-4px",transformOrigin:"center",duration:.35},"<").from(n,{opacity:0,duration:.5},"<").from(l,{opacity:0,duration:.8,stagger:.2},"<");l.forEach((e=>{e.addEventListener("click",(t=>{t.preventDefault();const r=e.id.replace("nav-archive-filter-",""),o=document.getElementById(`archive-filter-${r}`);o&&o.click(),u.timeScale(3).reverse()}))})),e._filterTimeline=u,e._filterButton=e.querySelector(".nav-button-filter")}));
					scope.querySelectorAll(".nav-primary-wrap").forEach((e=>{if(e._navTriggersBound)return;e._navTriggersBound=!0;const l=e._menuButton,t=e._filterButton,n=e._menuTimeline,r=e._filterTimeline,o=e.querySelector(".menu-wrapper"),i=e.querySelector(".menu-container"),s=e.querySelector(".filters-container");if(!n&&!r)return;let a=null;const y=()=>{i&&(i.style.display="none"),o&&(o.style.display="none"),document.body.style.overflow="",a=null},d=()=>{s&&(s.style.display="none"),o&&(o.style.display="none"),document.body.style.overflow="",a=null};n&&n.eventCallback("onReverseComplete",y),r&&r.eventCallback("onReverseComplete",d),l&&n&&l.addEventListener("click",(()=>{if("filter"===a&&r)return r.timeScale(2).reverse(),void r.eventCallback("onReverseComplete",(()=>{d(),a="menu",document.body.style.overflow="hidden",o&&(o.style.display="flex"),i&&(i.style.display="flex"),n.timeScale(1).play(0),r.eventCallback("onReverseComplete",d)}));"menu"!==a?(a="menu",document.body.style.overflow="hidden",o&&(o.style.display="flex"),i&&(i.style.display="flex"),n.timeScale(1).play(0)):n.timeScale(2).reverse()})),t&&r&&t.addEventListener("click",(()=>{if("menu"===a&&n)return n.timeScale(2).reverse(),void n.eventCallback("onReverseComplete",(()=>{y(),a="filter",document.body.style.overflow="hidden",o&&(o.style.display="flex"),s&&(s.style.display="flex"),r.timeScale(1).play(0),n.eventCallback("onReverseComplete",y)}));"filter"!==a?(a="filter",document.body.style.overflow="hidden",o&&(o.style.display="flex"),s&&(s.style.display="flex"),r.timeScale(1).play(0)):r.timeScale(2).reverse()}))}));
					NavigationManager.attachMenuLocks(document);
				}
			}),
			...
			feature({
				...
				init: async root => {const btn = document.querySelector('.nav-button-close-case');if (!btn) return;const path = (location.pathname || '/').replace(/\/+$/, '');const looksLikeCase = /^\/archive\/[^/]+$/.test(path);const hasCsBits = !!root.querySelector('.cs-hero-image') || !!root.querySelector('.cs-headline') || !!root.querySelector('.cs-gallery-inner');if (looksLikeCase || hasCsBits) {btn.style.display = 'flex';btn.style.pointerEvents = 'auto';btn.setAttribute('aria-hidden', 'false');btn.setAttribute('aria-label', btn.getAttribute('aria-label') || 'Close case study');window.gsap ? gsap.to(btn, { opacity: 1, duration: .2 }) : (btn.style.opacity = '1');} else {const hide = () => { btn.style.display = 'none'; btn.style.pointerEvents = 'none'; };btn.setAttribute('aria-hidden', 'true');window.gsap ? gsap.to(btn, { opacity: 0, duration: .2, onComplete: hide }) : (btn.style.opacity = '0', hide());}}
			}),
			...
			feature({
				...
				init: async root => {const el = document.querySelector(".theme-switch");if (!el || el._bound) return;el._bound = true;const cur = document.documentElement.getAttribute("data-theme");el.classList.toggle("dark", cur === "dark");el.addEventListener("click", () => {const next = document.documentElement.getAttribute("data-theme") === "dark" ? "light" : "dark";document.documentElement.setAttribute("data-theme", next);localStorage.setItem("theme", next);el.classList.toggle("dark", next === "dark");});}
			}),
