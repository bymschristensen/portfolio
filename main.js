...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveFiltersController",stage:"early",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-playful-item",".archive-minimal-item","#archive-filter-all","#archive-filter-appswebapps","#archive-filter-websites","#archive-filter-ecommerce","#archive-filter-platforms-systems","#archive-filter-other","#nav-archive-filter-all","#nav-archive-filter-appswebapps","#nav-archive-filter-websites","#nav-archive-filter-ecommerce","#nav-archive-filter-platforms-systems","#nav-archive-filter-other"],init:function(r){r=r||document;var K="archiveFilter",m=r.querySelector("main.main")||r;if(m.__afBound)return;m.__afBound=1;var I=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];function g(i){return r.querySelector("#"+i)||document.getElementById(i)}function n(s){return(s||"").replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,"").trim()}function c(s){s=(s||"").toLowerCase().trim().replace(/&/g," ").replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g,"-");return"apps-web-apps"===s||"apps-webapps"===s||"apps-web"===s?"appswebapps":"e-commerce"===s||"ecommerce"===s?"ecommerce":"platforms-systems"===s||"platforms-and-systems"===s?"platforms-systems":"websites"===s?"websites":"other"===s?"other":"all"===s?"all":s||"all"}function st(){try{var v=localStorage.getItem(K);return v?c(v):"all"}catch(_){return"all"}}function sv(v){try{localStorage.setItem(K,c(v))}catch(_){}}function ui(cat){for(var i=0;i<I.length;i++){var e=g(I[i]);if(!e)continue;var a=c(n(I[i]))===cat;e.classList.toggle("is-active",a),e.classList.toggle("w--current",a),a?e.setAttribute("aria-current","true"):e.removeAttribute("aria-current")}}function cats(it){for(var o=[],d=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<d.length;i++){var t=(d[i].textContent||"").trim();t&&o.push(c(t))}for(var s={},r=[],i=0;i<o.length;i++)s[o[i]]||(s[o[i]]=1,r.push(o[i]));return r}function all(){return[].slice.call(r.querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function prep(it){if(!window.gsap||!gsap.set)return;it.__almDone=0,gsap.killTweensOf(it),gsap.set(it,{y:18,autoAlpha:0,filter:"blur(10px)",willChange:"transform,opacity,filter"})}function apply(cat,meta){cat=c(cat),ui(cat),"user"===meta&&sv(cat);for(var a=all(),i=0;i<a.length;i++){var it=a[i],ok="all"===cat;if(!ok)for(var cc=cats(it),j=0;j<cc.length;j++)if(cc[j]===cat){ok=1;break}ok?(it.style.display==="none"&&prep(it),it.style.display="",it.removeAttribute("aria-hidden")):(it.style.display="none",it.setAttribute("aria-hidden","true"))}try{r.dispatchEvent(new CustomEvent("archive:filter",{bubbles:!0,detail:{filter:cat}}))}catch(_){}try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(_){}}function clk(e){e&&e.preventDefault&&e.preventDefault(),apply(c(n(this.id)),"user")}for(var i=0;i<I.length;i++){var el=g(I[i]);el&&el.addEventListener("click",clk,{passive:!1})}function onView(){apply(st(),"sync")}r.addEventListener("archive:view",onView),apply(st(),"init"),m.__afDestroy=function(){try{r.removeEventListener("archive:view",onView)}catch(_){}for(var i=0;i<I.length;i++){var el=g(I[i]);try{el&&el.removeEventListener("click",clk)}catch(_){}}m.__afBound=0,m.__afDestroy=null}}}),
