	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init:async function(root){root=root||document;if(!window.matchMedia)return;var gs=window.gsap,rm=matchMedia("(prefers-reduced-motion: reduce)"),mq=matchMedia("(min-width: 1024px) and (pointer: fine)");var S=root.__archiveMinimalExperience||(root.__archiveMinimalExperience={});if(S._bound)return;S._bound=1;S.sel={section:".section-archive-minimal",list:".archive-minimal-list",item:".archive-minimal-item",hiddenImg:"img.archive-minimal-item-hidden-img",stickyImg:".archive-minimal-visual-img",viewport:".archive-minimal-visual-viewport",stack:".archive-minimal-visual-stack"};function $(q,c){return(c||root).querySelector(q)}function $$(q,c){return[].slice.call((c||root).querySelectorAll(q))}function isVisible(el){if(!el)return!1;var cs=getComputedStyle(el);if(!cs||"none"===cs.display||"hidden"===cs.visibility)return!1;return null!==el.offsetParent}S.section=$(S.sel.section)||root;S.list=$(S.sel.list,S.section)||S.section;S.base=$(S.sel.stickyImg,root);S.viewport=$(S.sel.viewport,root);S.stack=$(S.sel.stack,root);if(!S.list||!S.base){S.destroy=function(){S._bound=0;try{delete root.__archiveMinimalExperience}catch(e){}};return}S.stack||(S.stack=S.base.parentElement||S.base);S.viewport||(S.viewport=S.stack.parentElement||S.stack);try{getComputedStyle(S.viewport).overflow!=="hidden"&&(S.viewport.style.overflow="hidden")}catch(e){}try{S.stack.style.position=S.stack.style.position||"relative"}catch(e){}try{S.base.style.position=S.base.style.position||"absolute",S.base.style.inset=S.base.style.inset||"0",S.base.style.width=S.base.style.width||"100%",S.base.style.height=S.base.style.height||"100%",S.base.style.zIndex=S.base.style.zIndex||"1"}catch(e){}var CAP=12,lastKey="";function keyFromImg(img){return img&&(img.currentSrc||img.src)||""}function srcFromImg(img){return img&&(img.currentSrc||img.src)||""}function applyAttrs(dst,src){if(!dst||!src)return!1;var s=srcFromImg(src);if(!s)return!1;dst.alt=src.alt||"";var ss=src.getAttribute("srcset"),sz=src.getAttribute("sizes");ss?dst.setAttribute("srcset",ss):dst.removeAttribute("srcset");sz?dst.setAttribute("sizes",sz):dst.removeAttribute("sizes");dst.src=s;return!0}function hiddenFromItem(it){return it?$(S.sel.hiddenImg,it):null}function firstVisible(){return $$(S.sel.item,S.list).filter(isVisible)[0]||null}function clearStack(){try{var kids=[].slice.call(S.stack.children);for(var i=0;i<kids.length;i++){var n=kids[i];if(n&&n.__ameLayer){try{gs&&gs.killTweensOf&&gs.killTweensOf(n)}catch(e){}try{n.remove()}catch(e){}}}}catch(e){}}function setBase(img){if(!img)return;var k=keyFromImg(img);if(k&&k===lastKey)return;applyAttrs(S.base,img);lastKey=k;try{gs&&gs.set&&gs.set(S.base,{clearProps:"transform,filter,opacity,visibility"})}catch(e){}}function pushLayer(img){if(!img)return;var k=keyFromImg(img);if(k&&k===lastKey)return;var ok=srcFromImg(img);if(!ok)return;lastKey=k;if(!gs||rm.matches){setBase(img);return}var layer=new Image();layer.__ameLayer=1;try{layer.setAttribute("aria-hidden","true")}catch(e){}applyAttrs(layer,img);layer.decoding="async";layer.loading="eager";try{layer.style.position="absolute",layer.style.inset="0",layer.style.width="100%",layer.style.height="100%",layer.style.objectFit=getComputedStyle(S.base).objectFit||"cover",layer.style.zIndex="2"}catch(e){}try{S.stack.appendChild(layer)}catch(e){}try{gs.set(layer,{yPercent:-100,scale:1.12,transformOrigin:"50% 0%",willChange:"transform"})}catch(e){}try{gs.to(layer,{yPercent:0,scale:1,duration:.9,ease:"expo.inOut",overwrite:!0,onComplete:function(){try{var layers=[].slice.call(S.stack.querySelectorAll("img.__ameLayer, img[data-ame-layer], img"));}catch(e){}try{var all=[].slice.call(S.stack.children).filter(function(n){return n&&n.__ameLayer});if(all.length>CAP){for(var i=0;i<all.length-CAP;i++){try{all[i].remove()}catch(e){}}}}catch(e){}}})}catch(e){try{layer.remove()}catch(_){ }setBase(img)}}function bindHover(){var items=$$(S.sel.item,S.list);for(var i=0;i<items.length;i++){var it=items[i];if(it.__ameH)continue;it.__ameH=1;it.addEventListener("mouseenter",function(ev){if(!mq.matches)return;var im=hiddenFromItem(ev.currentTarget);im&&pushLayer(im)},{passive:!0})}}function refresh(){clearStack();setBase(hiddenFromItem(firstVisible()));bindHover()}bindHover();setBase(hiddenFromItem(firstVisible()));var mo=new MutationObserver(function(){refresh()});try{mo.observe(S.list,{subtree:!0,childList:!0,attributes:!0,attributeFilter:["style","class"]})}catch(e){}S.refresh=function(){try{refresh()}catch(e){}};S.destroy=function(){try{mo.disconnect()}catch(e){}try{clearStack()}catch(e){}var items=$$(S.sel.item,S.list);for(var i=0;i<items.length;i++){items[i].__ameH=0}S._bound=0;try{delete root.__archiveMinimalExperience}catch(e){}};}
