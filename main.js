console.info("[BOOT] build:", new Date().toISOString());
	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async (root)=>{const SEL=".appear-in-line",CHILD=":scope > *:not(:empty)",STEP=.15,DUR=.8,prep=e=>gsap.set(e,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),hydrate=box=>{if(box.__ailInit)return;box.__ailInit=!0;const groups=[],splits=[];[...box.querySelectorAll(CHILD)].forEach(node=>{const cs=getComputedStyle(node),cols=parseInt(cs.columnCount,10)||1,rect=node.getBoundingClientRect();if(cols>1&&window.SplitText){try{const sp=new SplitText(node,{type:"lines",linesClass:"split-line"});splits.push(sp);const colW=rect.width/cols,bkts=Array.from({length:cols},()=>[]);sp.lines.forEach(L=>{const x=L.getBoundingClientRect().left-rect.left,idx=Math.min(cols-1,Math.max(0,Math.floor(x/colW)));bkts[idx].push(L);prep(L)});bkts.forEach(col=>groups.push(col))}catch{prep(node);groups.push([node])}}else{prep(node);groups.push([node])}});const reveal=()=>{if(box.__ailDone)return;groups.forEach((g,i)=>gsap.to(g,{y:0,opacity:1,filter:"blur(0px)",duration:DUR,ease:"power2.out",delay:i*STEP}));const total=(groups.length-1)*STEP+DUR;gsap.delayedCall(total+.05,()=>{try{splits.forEach(s=>s.revert())}catch{}box.__ailDone=!0})};const io=CoreUtilities.Observers.addDom(new IntersectionObserver((ents,obs)=>{for(const ent of ents){if(!ent.isIntersecting)continue;obs.unobserve(box);reveal()}},{root:null,rootMargin:"0px 0px -10% 0px",threshold:0}));io.observe(box);const first=box.getBoundingClientRect();if(first.top<innerHeight&&first.bottom>0){io.unobserve(box);reveal()}const pane=box.closest(".w-tab-pane");if(pane){CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!pane.classList.contains("w--tab-active"))return;io.observe(box);const r=box.getBoundingClientRect();if(r.top<innerHeight&&r.bottom>0){io.unobserve(box);reveal()}})).observe(pane,{attributes:!0,attributeFilter:["class"]})}};root.querySelectorAll(SEL).forEach(hydrate)}
			}),
