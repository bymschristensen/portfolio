...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),d=document.documentElement,i=e.querySelector(".cs-hero-image"),a=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),r=e=>{if(!e)return null;let t=e.querySelector(".ta-one,[data-animate-headline],h1")||e;for(;t&&t.children&&1===t.children.length&&t.textContent.trim()===t.children[0].textContent.trim();)t=t.children[0];return t},o=e=>{e&&(e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t)))};d.classList.add("entry-ready");const l=r(a);a&&(a.style.visibility="visible"),l&&(l.style.visibility="visible"),i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)",overwrite:"auto",immediateRender:!0}),l&&gsap.set(l,{autoAlpha:0,overwrite:"auto",immediateRender:!0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),o(i),o(a),n.forEach(o),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(l){const e=l.style.display,s="inline"===getComputedStyle(l).display;s&&(l.style.display="block"),t.addLabel("headline",.35).set(l,{autoAlpha:1,visibility:"visible"},"headline").call(()=>requestAnimationFrame(()=>{try{const t=splitAndMask(l);animateLines(t.lines).eventCallback("onComplete",()=>{safelyRevertSplit(t,l),l.style.display=e})}catch(t){l.style.display=e,gsap.set(l,{autoAlpha:1})}}),null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
