	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||document.querySelector(".barba-container")||r;if(!c)return;const d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;if(c.__csbgInit)return;c.__csbgInit=1;try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})},mk=(sec,key)=>{let t=sec.querySelector(`:scope>[data-csbg="${key}"]`);return t||(t=document.createElement("div"),t.setAttribute("data-csbg",key),t.style.cssText="position:absolute;left:0;top:0;width:1px;height:1px;pointer-events:none;opacity:0;",sec.style.position||(sec.style.position="relative"),sec.prepend(t)),t},rf=()=>requestAnimationFrame(()=>ScrollTrigger.refresh());ScrollTrigger.create({id:"csbg:details",trigger:mk(d,"details"),start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),invalidateOnRefresh:!0,refreshPriority:100});ScrollTrigger.create({id:"csbg:more",trigger:mk(m,"more"),start:"top 80%",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),invalidateOnRefresh:!0,refreshPriority:110});rf(),setTimeout(rf,160),setTimeout(rf,520),setTimeout(rf,1100);const g=c.querySelector(".cs-gallery");if(g){const imgs=[...g.querySelectorAll("img")];if(imgs.length){let n=0;const done=()=>{if(++n<imgs.length)return;rf(),setTimeout(rf,160)};imgs.forEach(im=>{if(im.complete&&im.naturalWidth>0){done();return}(typeof im.decode=="function"?im.decode().catch(()=>{}):Promise.resolve()).finally(done),im.addEventListener("load",done,{once:!0}),im.addEventListener("error",done,{once:!0})})}if("ResizeObserver"in window){let h=g.getBoundingClientRect().height,ro=new ResizeObserver(()=>{const nh=g.getBoundingClientRect().height;if(Math.abs(nh-h)>1){h=nh,rf(),setTimeout(rf,160)}});try{ro.observe(g)}catch{}setTimeout(()=>{try{ro.disconnect()}catch{}},8e3);return()=>{try{ro.disconnect()}catch{}}}}}
			}),
			feature({
