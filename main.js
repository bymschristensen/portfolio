...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var S=[].slice.call(r.querySelectorAll(".section-single-service"));if(!S.length)return;var freeze=!1;document.addEventListener("click",function(e){if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){freeze=!0;setTimeout(function(){freeze=!1;ScrollTrigger.refresh()},350)}});S.forEach(function(sec){var tl=gsap.timeline({scrollTrigger:{trigger:sec,start:"top top",end:"bottom top",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0,onUpdate:function(self){if(freeze)self.progress(self.progress())}}});tl.to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0)});}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
