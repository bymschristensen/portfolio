	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(e){e=e||document;var t="archiveView",i=matchMedia("(min-width: 1024px)"),a=e.querySelector(".section-archive-playful"),l=e.querySelector(".section-archive-minimal"),r=e.querySelector(".archive-switch-block"),s=e.querySelector(".archive-switch"),n=e.querySelector(".archive-switch-dot");if(!a||!l||e.__archiveViewBound)return;e.__archiveViewBound=1;var c=[r,s].filter(Boolean),o=null,tl=null,lock=0,LOCK_MS=120,IN_D=.6,OUT_D=.22,STAG=.06,BLUR_IN="blur(10px)",BLUR_OUT="blur(0px)";function u(){try{window.gsap&&gsap.set&&gsap.set([a,l],{display:"none"})}catch(_){}a.style.display="none",l.style.display="none"}function m(){try{var e2=localStorage.getItem(t);return"playful"===e2||"minimal"===e2?e2:null}catch(_){return null}}function f(){return i.matches?m()||"playful":"minimal"}function p(e2){var t2="playful"===e2;r&&(r.dataset.view=e2,r.setAttribute("aria-pressed",String(t2)),r.classList.toggle("is-playful",t2),r.classList.toggle("is-minimal",!t2)),s&&(s.dataset.view=e2,s.classList.toggle("is-playful",t2),s.classList.toggle("is-minimal",!t2)),n&&(n.dataset.view=e2,n.classList.toggle("is-playful",t2),n.classList.toggle("is-minimal",!t2))}function g(e2){try{document.dispatchEvent(new CustomEvent("archive:view",{bubbles:!0,detail:{view:e2}}))}catch(_){}}function h(e2){u();var t2="playful"===e2?a:l;t2.style.display="block";try{window.gsap&&gsap.set&&gsap.set(t2,{autoAlpha:1,filter:"none",clearProps:"opacity,visibility"})}catch(_){}p(e2),g(e2);try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(_){}}function v(e2){if(!window.gsap)return{items:[],vis:[]};var t2="playful"===e2?a:l,i2="playful"===e2?".archive-playful-item":".archive-minimal-item",r2="playful"===e2?".archive-playful-item-visual":".archive-minimal-item-visual",items=[].slice.call(t2.querySelectorAll(i2)||[]),vis=[];return items.forEach(function(el){gsap.killTweensOf(el),gsap.set(el,{y:18,autoAlpha:0,filter:BLUR_IN,willChange:"transform,opacity,filter"});var vEl=el.querySelector(r2);vEl&&(vis.push(vEl),gsap.killTweensOf(vEl),gsap.set(vEl,{autoAlpha:0,filter:BLUR_IN,willChange:"opacity,filter"}))}),{items:items,vis:vis}}function animIn(view){if(!window.gsap)return;var pack=v(view);if(!pack.items.length&&!pack.vis.length)return;tl&&tl.kill&&tl.kill();tl=gsap.timeline({defaults:{ease:"power3.out"}});pack.items.length&&tl.to(pack.items,{y:0,autoAlpha:1,filter:BLUR_OUT,duration:IN_D,stagger:STAG,overwrite:!0,clearProps:"y,filter,willChange"},0);pack.vis.length&&tl.to(pack.vis,{autoAlpha:1,filter:BLUR_OUT,duration:IN_D,stagger:STAG,overwrite:!0,clearProps:"filter,willChange"},0);tl.add(function(){try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(_){}g(view)})}function B(view){if(!window.gsap)return h(view),void function(vw){try{localStorage.setItem(t,vw)}catch(_){}}(view);if(lock)return;lock=1,setTimeout(function(){lock=0},LOCK_MS);var cur=(r&&r.dataset.view)||(s&&s.dataset.view)||("none"!==a.style.display?"playful":"minimal");if(cur===view)return;var from="playful"===cur?a:l,to="playful"===view?a:l;p(view);try{localStorage.setItem(t,view)}catch(_){}tl&&tl.kill&&tl.kill(),gsap.killTweensOf([from,to]),u(),from.style.display="block",to.style.display="none";tl=gsap.timeline({defaults:{ease:"power2.out"}}).set(from,{display:"block",autoAlpha:1,filter:"none"},0).to(from,{autoAlpha:0,filter:BLUR_IN,duration:OUT_D,overwrite:!0},0).add(function(){from.style.display="none",to.style.display="block",gsap.set(to,{autoAlpha:1,filter:"none"})}).add(function(){animIn(view)},0)}function w(e2){e2&&e2.preventDefault&&e2.preventDefault();if(i.matches){var nxt=("playful"===(r&&r.dataset.view||s&&s.dataset.view||("none"!==a.style.display?"playful":"minimal"))?"minimal":"playful");B(nxt)}else h("minimal")}function k(){var v2=f();h(v2),window.gsap?animIn(v2):S()}function S(){if(o)return;var e2=0;o=setInterval(function(){if(window.gsap){clearInterval(o),o=null,animIn(f())}else e2+=50,e2>=2000&&(clearInterval(o),o=null)},50)}u();var initV=f();h(initV),window.gsap?animIn(initV):S(),c.forEach(function(el){el&&el.addEventListener("click",w,{passive:!1})}),i.addEventListener?i.addEventListener("change",k):i.addListener(k),e.__archiveViewDestroy=function(){try{o&&(clearInterval(o),o=null)}catch(_){}try{tl&&tl.kill&&tl.kill(),tl=null}catch(_){}try{i.removeEventListener?i.removeEventListener("change",k):i.removeListener(k)}catch(_){}c.forEach(function(el){try{el&&el.removeEventListener("click",w)}catch(_){}}),e.__archiveViewBound=0,e.__archiveViewDestroy=null}}
			}),
			feature({
