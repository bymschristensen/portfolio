...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero||hero.__hsBound)return;var track=hero.querySelector(".showreel-track"),box=hero.querySelector(".showreel-container");if(!track||!box)return;hero.__hsBound=1;box.style.transformOrigin="center center";box.style.willChange="transform";var S={x:0,scale:1,px:0,hasMove:!1,raf:0,last:0,w:0,tw:0,lock:!1},OFFSET=20,FREQ=10,SC_MIN=.96,SC_MAX=1.10,EDGE=1.1;var setX=gsap.quickSetter(box,"x","px"),setS=gsap.quickSetter(box,"scale");function measure(){S.w=box.getBoundingClientRect().width||box.offsetWidth||0;S.tw=track.getBoundingClientRect().width||track.offsetWidth||0}function limit(){return Math.max(0,(S.tw-S.w*S.scale)/2)}function wantCenterFromCursor(cx){var r=track.getBoundingClientRect();var leftDesired=(cx-r.left)+OFFSET+(S.w*S.scale)/2;var center=leftDesired-(S.tw/2);var L=limit();return Math.max(-L,Math.min(L,center))}function onMove(e){S.hasMove=!0;S.px=e.clientX}function onScroll(){var r=track.getBoundingClientRect(),vh=innerHeight||document.documentElement.clientHeight,c=r.top+r.height/2,d=Math.abs(c-(vh/2)),t=1-Math.min(1,d/(vh*.75));var target=SC_MIN+t*(SC_MAX-SC_MIN);S.scale+=.25*(target-S.scale);S.lock=(S.w*S.scale>=S.tw-1)}function onResize(){measure()}function loop(ts){if(!S.last)S.last=ts;var dt=Math.min(.05,(ts-S.last)/1e3);S.last=ts;var L=limit();var targetX=S.lock?0:(!S.hasMove?0:(()=>{var raw=wantCenterFromCursor(S.px);var n=L?raw/L:0;var soft=Math.tanh(n*EDGE)/Math.tanh(EDGE);return soft*L})());var alpha=1-Math.exp(-FREQ*dt);S.x+=alpha*(targetX-S.x);setX(S.x);setS(S.scale);S.raf=requestAnimationFrame(loop)}measure();onScroll();addEventListener("pointermove",onMove,{passive:!0});addEventListener("scroll",onScroll,{passive:!0});addEventListener("resize",onResize,{passive:!0});S.raf=requestAnimationFrame(loop);hero.__hsDestroy=function(){try{removeEventListener("pointermove",onMove)}catch(_){ }try{removeEventListener("scroll",onScroll)}catch(_){ }try{removeEventListener("resize",onResize)}catch(_){ }if(S.raf)try{cancelAnimationFrame(S.raf)}catch(_){ }}} ,destroy:async function(root){root=root||document;var h=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{h&&h.__hsDestroy&&h.__hsDestroy()}catch(_){}}}),
			feature({
