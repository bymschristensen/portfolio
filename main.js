...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),l=e.querySelector(".cs-hero-image"),a=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div");l&&gsap.set(l,{autoAlpha:0,y:80,filter:"blur(10px)"}),a&&gsap.set(a,{autoAlpha:0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),l&&t.to(l,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),a&&t.addLabel("headline",.35).set(a,{autoAlpha:1,display:"block"},"headline").call(()=>{const e=a;if(!window.SplitText){gsap.to(e,{opacity:1,y:0,duration:.6,ease:"power2.out"});return}const t=getComputedStyle(e),l=t.whiteSpace||"normal",a=e.style.whiteSpace,n=e.style.display;"inline"===getComputedStyle(e).display&&(e.style.display="block"),e.style.whiteSpace=l;const o=new SplitText(e,{type:"lines",linesClass:"line",reduceWhiteSpace:!1});o.lines.forEach(t=>{const l=t.getBoundingClientRect().height||t.offsetHeight||0,s=document.createElement("div");s.className="text-mask",s.style.cssText="overflow:hidden;display:block;height:"+l+"px",t.style.whiteSpace=getComputedStyle(e).whiteSpace||"normal",t.style.display="block",t.parentNode.insertBefore(s,t),s.appendChild(t)}),gsap.set(o.lines,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform,opacity"}),animateLines(o.lines).eventCallback("onComplete",()=>{(window.safelyRevertSplit?window.safelyRevertSplit:o.revert).call(o,o,e),e.style.whiteSpace=a,e.style.display=n||""})},null,"headline"),n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t}
		};
