	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init:function(r){if(!window.gsap)return;r=r||document;if(r.__arRevealBound)return;r.__arRevealBound=1;var e=window.matchMedia&&matchMedia("(prefers-reduced-motion: reduce)").matches,t=r.querySelectorAll(".archive-minimal-item,.archive-playful-item"),i=r.querySelectorAll(".archive-minimal-item-visual,.archive-playful-item-visual");if(!t.length)return;gsap.killTweensOf([t,i]);gsap.set(t,{autoAlpha:0,y:18,clearProps:"transform"});i.length&&gsap.set(i,{autoAlpha:0,y:0,scale:.98,filter:"blur(10px)",transformOrigin:"50% 50%"});function n(){var r=Array.prototype.slice.call(t).filter(function(r){var e=r.getBoundingClientRect();return e.bottom>0&&e.top<window.innerHeight*.92});r.length||(r=[t[0]]);var n=gsap.timeline({defaults:{ease:"power3.out"},onComplete:function(){o&&o.disconnect()}});if(e){gsap.set(r,{autoAlpha:1,y:0});i.length&&gsap.set(i,{autoAlpha:1,scale:1,filter:"blur(0px)"});return}n.to(r,{autoAlpha:1,y:0,duration:.65,stagger:.06},0);i.length&&n.to(i,{autoAlpha:1,scale:1,filter:"blur(0px)",duration:.85},.22)}var o=new IntersectionObserver(function(r){for(var e=0;e<r.length;e++)if(r[e].isIntersecting)return n()}, {threshold:.15,rootMargin:"0px 0px -10% 0px"});o.observe(t[0])}
			}),
			feature({
