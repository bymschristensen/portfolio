...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async e=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=!!(CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),i=e=>parseFloat(getComputedStyle(e).gap||"0")||0,o=e=>{const t=i(e);let r=0;return e.childNodes.forEach(e=>{1===e.nodeType&&(r+=(e.getBoundingClientRect().width||0)+t)}),r},n=e=>{[...e.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())},s=e=>{const t=parseFloat(e.dataset.ratio)||(e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const r=(e.className||"").match(/sv-ratio-(\d+)/);if(r){const e=r[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const i=e.querySelector("img,video");if(i){const e=parseInt(i.getAttribute("width"),10)||i.naturalWidth||i.videoWidth||0,t=parseInt(i.getAttribute("height"),10)||i.naturalHeight||i.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5})(e);if(e.dataset.ratio=String(t),r)e.style.aspectRatio=`${t} / 1`,e.style.width="";else{const r=e.getBoundingClientRect().height||parseFloat(getComputedStyle(e).height)||0,i=r>0?Math.max(1,Math.round(r*t)):0;e.style.width=i?`${i}px`:""}e.style.flex="0 0 auto",e.style.overflow="hidden",e.style.willChange="transform"},a=(e,t)=>{const r=e.cloneNode(!0);return r.setAttribute("data-clone","1"),r.dataset.key=e.dataset.key||"",s(r),t.appendChild(r),r};t.forEach(async e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0;const r=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!r.length)return;e.setAttribute("data-armed","measure"),await(e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const r=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,i=[],o=t.map(e=>{if(r(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),i=new Promise(t=>{const r=()=>{e.removeEventListener("load",r),t()};e.addEventListener("load",r,{once:!0})}),o=new Promise(t=>{let i=0;!function o(){if(r(e)||i++>80)return t();setTimeout(o,40)}()});return Promise.race([t,i,o])}),n=new Promise(e=>i.push(setTimeout(e,3500)));return Promise.race([Promise.all(o),n]).finally(()=>i.forEach(clearTimeout))})(t),r.forEach((e,t)=>{e.dataset.key=String(t),s(e)});const l="right"===(e.dataset.direction||"left").toLowerCase()?-1:1,c=parseFloat(e.dataset.speed)||.6,d=()=>{n(t),r.forEach(e=>a(e,t)),r.forEach(e=>a(e,t));let e=0;const i=3*Math.max(document.documentElement.clientWidth,innerWidth||0);for(;o(t)<i&&e++<8;)r.forEach(e=>a(e,t))};d(),e.setAttribute("data-armed","1");let h=0;const u=(e,r=16.6667)=>{h-=l*((r||16.6667)/1e3*60)*c;const o=i(t),n=e=>(e.getBoundingClientRect().width||0)+o;let s=t.firstElementChild,a=0;for(;s&&h<-n(s)&&a++<50;)h+=n(s),t.appendChild(s),s=t.firstElementChild;let d=t.lastElementChild,u=0;for(;d&&h>0&&u++<50;)h-=n(d),t.insertBefore(d,t.firstElementChild),d=t.lastElementChild;gsap.set(t,{x:h});const f=(innerWidth||0)/2;t.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),i=(f-(r.left+r.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*i+"px")})};gsap.ticker.add(u),CoreUtilities.Observers.addTicker(u);let f=!1;CoreUtilities.Observers.addDom(new IntersectionObserver((t,i)=>{for(const o of t){if(o.target!==e||!o.isIntersecting||f)continue;f=!0;r.filter(e=>{const t=e.getBoundingClientRect();return t.bottom>0&&t.top<innerHeight&&t.right>0&&t.left<innerWidth}).forEach((e,t)=>{const r=e.getBoundingClientRect().height||parseFloat(getComputedStyle(e).height)||0;r&&(e.style.height="0px",gsap.to(e,{height:r,duration:.9,ease:"power2.out",delay:.12*t,onComplete:()=>{e.style.height=""}}))}),i.unobserve(e)}},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0})).observe(e);const g=new ResizeObserver(()=>{r.forEach(s),n(t),d()});g.observe(t),CoreUtilities.Observers.addDom(g)})}
			})
		);
