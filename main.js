		...
		registries.pages.archive.push(
			feature({
				...
				init: function(root){
					...
					// Filters Module
					(function(){var KEY=state.KEY_FILTER||"archiveFilter",doc=el.doc,ids=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"],mo=0,raf=0;function setActive(cat){for(var i=0;i<ids.length;i++){var id=ids[i],x=doc.getElementById(id);if(!x)continue;var onv=baseId(id)===cat;x.classList.toggle("is-active",onv);try{x.setAttribute("aria-current",onv?"true":"false")}catch(_){}try{x.setAttribute("aria-pressed",onv?"true":"false")}catch(_){}}}function stored(){try{var v=localStorage.getItem(KEY);return v?normCat(v):null}catch(_){return null}}function getAll(){return[].slice.call((el.host||host).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function readCats(it){if(it.__asCats)return it.__asCats;for(var a=["all"],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var t=(n[i].textContent||"").trim();t&&(t=normCat(t))&&-1===a.indexOf(t)&&a.push(t)}return it.__asCats=a,a}function applyOnce(cat){cat=normCat(cat||"all")||"all";state.filter=cat;setActive(cat);for(var items=getAll(),shown=0,i=0;i<items.length;i++){var it=items[i],cats=readCats(it),show="all"===cat||-1!==cats.indexOf(cat);it.style.display=show?"":"none";show&&shown++}return{cat:cat,shown:shown}}function emit(cat){try{doc.dispatchEvent(new CustomEvent("archive:filter:applied",{bubbles:!0,detail:{filter:cat}}))}catch(_){}}function apply(cat,meta){var res=applyOnce(cat);if("all"!==res.cat&&0===res.shown){try{localStorage.removeItem(KEY)}catch(_){}res=applyOnce("all")}if("user"===meta)try{localStorage.setItem(KEY,res.cat)}catch(_){}emit(res.cat);}function scheduleReapply(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;applyOnce(state.filter||stored()||"all")})}function onClick(ev){var t=ev&&ev.target&&ev.target.closest?ev.target.closest('[id^="archive-filter-"],[id^="nav-archive-filter-"]'):null;if(!t||!t.id)return;ev.preventDefault&&ev.preventDefault();apply(baseId(t.id),"user")}function onExternal(ev){try{var d=ev&&ev.detail||{},cat=d.filter||d.category||d.id||"";cat=baseId(String(cat||""));cat&&apply(cat,"user")}catch(_){}}function onView(){apply(stored()||"all","auto")}mod.applyFilter=function(cat,save){apply(cat,save?"user":"auto")};mod.getFilter=function(){return state.filter||stored()||"all"};on(doc,"click",onClick,!0);on(doc,"archive:filter",onExternal,{passive:!0});on(doc,"archive:view",onView,{passive:!0});apply(stored()||"all","auto");try{mo=new MutationObserver(scheduleReapply);el.listP&&mo.observe(el.listP,{childList:!0,subtree:!0});el.listM&&mo.observe(el.listM,{childList:!0,subtree:!0});unsubs.push(function(){try{mo&&mo.disconnect&&mo.disconnect()}catch(_){}})}catch(_){}})();

					// Motion Module & Scheduler
					(function(){if(!window.gsap){mod.scheduleMotionRefresh=function(){};return}var io=null,scan=0,raf=0,t=0,run=0;function cleanup(){try{io&&io.disconnect&&io.disconnect()}catch(_){}io=null;try{scan&&cancelAnimationFrame(scan)}catch(_){}scan=0;try{clearTimeout(t)}catch(_){}t=0}unsubs.push(cleanup);function canRun(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&("none"===s.display||"hidden"===s.visibility))}catch(_){return!0}}function activeScope(){try{var v=mod.getView&&mod.getView();if(v==="playful"&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(v==="minimal"&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){ }try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){ }return el.host||host}function items(scope){return[].slice.call((scope||activeScope()).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function prepOne(it){if(!it)return;it.__almRun=run;it.__almDone=0;try{gsap.killTweensOf(it)}catch(_){}gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"});var v=it.classList.contains("archive-minimal-item")?(it.querySelector(".archive-minimal-item-visual")||it.querySelector(".archive-minimal-visual")||it.querySelector(".archive-minimal-visual-wrap")):null;if(v){try{gsap.killTweensOf(v)}catch(_){}gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}}function revealOne(it,delay){if(!it||it.__almDone||!canRun(it))return;it.__almRun!==run&&prepOne(it);it.__almDone=1;gsap.to(it,{delay:delay||0,y:0,autoAlpha:1,filter:"blur(0px)",duration:.75,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange"});var v=it.classList.contains("archive-minimal-item")?(it.querySelector(".archive-minimal-item-visual")||it.querySelector(".archive-minimal-visual")||it.querySelector(".archive-minimal-visual-wrap")):null;v&&gsap.to(v,{delay:(delay||0)+.06,autoAlpha:1,filter:"blur(0px)",duration:.6,ease:"power2.out",overwrite:!0,clearProps:"filter,willChange"})}function revealBatch(els){els.sort(function(a,b){return a.getBoundingClientRect().top-b.getBoundingClientRect().top});els.forEach(function(it,i){revealOne(it,.08*i)})}function inView(it){if(!canRun(it)||it.__almDone)return!1;var r=it.getBoundingClientRect();return r.top<(innerHeight||0)&&r.bottom>0}function forceScan(list,frames){var f=0;function tick(){f++;var hits=[];for(var i=0;i<list.length;i++)inView(list[i])&&hits.push(list[i]);hits.length&&revealBatch(hits);f<(frames||30)?scan=requestAnimationFrame(tick):scan=0}scan=requestAnimationFrame(tick)}function start(list){cleanup();if("IntersectionObserver"in window){io=new IntersectionObserver(function(ents){var hits=[];ents.forEach(function(ent){ent.isIntersecting&&hits.push(ent.target)});hits.length&&revealBatch(hits)},{threshold:.12,rootMargin:"0px 0px -10% 0px"});list.forEach(function(it){try{io.observe(it)}catch(_){}})}forceScan(list,30)}function resetAndRun(scope){var list=items(scope||activeScope());run++;list.forEach(function(it){canRun(it)&&prepOne(it)});start(list)}mod.resetMotion=function(){resetAndRun(activeScope())};mod.scheduleMotionRefresh=function(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;resetAndRun(activeScope());try{clearTimeout(t)}catch(_){}t=setTimeout(function(){resetAndRun(activeScope())},140)})};on(document,"archive:view",mod.scheduleMotionRefresh,{passive:!0});on(document,"archive:filter:applied",mod.scheduleMotionRefresh,{passive:!0});mod.scheduleMotionRefresh()})();
					(function(){var raf=0;function active(){try{var v=mod.getView&&mod.getView();if("playful"===v&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if("minimal"===v&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){}try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){}return el.host||host}function run(){try{window.__archiveListMotion&&__archiveListMotion.resetAndRun&&__archiveListMotion.resetAndRun(active())}catch(_){}}mod.resetMotion=run;mod.scheduleMotionRefresh=function(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;run()})};on(document,"archive:view",mod.scheduleMotionRefresh,{passive:!0});on(document,"archive:filter:applied",mod.scheduleMotionRefresh,{passive:!0})})();
