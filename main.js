...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){const mq=matchMedia("(min-width:1024px) and (pointer:fine)");if(!mq.matches)return;const section=root.querySelector(".hero-section-showreel");if(!section)return;const track=section.querySelector(".showreel-track"),box=section.querySelector(".showreel-container");if(!track||!box)return;box.style.setProperty("--hero-showreel-x","0px");box.style.setProperty("--hero-showreel-scale","1");const heroBits=[section.querySelector(".showreel-headline"),section.querySelector(".tableofcontents"),section.querySelector(".toc-outer"),section.querySelector(".showreel-subtext")].filter(Boolean);let trackRect=track.getBoundingClientRect(),boxRect=box.getBoundingClientRect(),trackW=Math.max(1,trackRect.width),boxW=Math.max(1,boxRect.width);const state={pos:.7,targetPos:.7,scrollScale:1,vel:0,scale:1};function measure(){trackRect=track.getBoundingClientRect();boxRect=box.getBoundingClientRect();trackW=Math.max(1,trackRect.width);boxW=Math.max(1,boxRect.width)}function clamp01(v){return v<0?0:v>1?1:v}function updateScrollScale(){const vh=window.innerHeight||document.documentElement.clientHeight||1,r=section.getBoundingClientRect(),start=.8*vh,end=.1*vh;let p=(start-r.top)/(start-end);p=clamp01(p);state.scrollScale=.96+.26*p}let lastRaw=null;function onPointerMove(ev){if(!trackRect)return;const x=ev.clientX,left=trackRect.left,right=trackRect.right;let local=x<left?0:x>right?trackW:x-left;const minCenter=boxW/2,maxCenter=Math.max(minCenter,trackW-boxW/2);let center=local;center<minCenter&&(center=minCenter);center>maxCenter&&(center=maxCenter);const pos=(center-minCenter)/Math.max(1,maxCenter-minCenter);state.targetPos=clamp01(pos);const raw=trackW?local/trackW:0;null==lastRaw&&(lastRaw=raw);const delta=raw-lastRaw;lastRaw=raw;state.vel=.85*state.vel+.15*Math.abs(delta)}let rafId;function loop(){rafId=requestAnimationFrame(loop);state.pos+=(state.targetPos-state.pos)*.16;const minCenter=boxW/2,maxCenter=Math.max(minCenter,trackW-boxW/2),center=minCenter+(maxCenter-minCenter)*state.pos,offset=center-trackW/2,speedScale=1+Math.min(.09,.9*state.vel),targetScale=state.scrollScale*speedScale;state.scale+=(targetScale-state.scale)*.18;box.style.setProperty("--hero-showreel-x",offset.toFixed(2)+"px");box.style.setProperty("--hero-showreel-scale",state.scale.toFixed(3));const copyP=state.scrollScale-.96,yCopy=-.4*(window.innerHeight||0)*copyP,op=1-1.2*copyP,clampedOp=op<0?0:op>1?1:op;heroBits.forEach(el=>{el.style.transform="translate3d(0,"+yCopy+"px,0)";el.style.opacity=clampedOp})}function onScroll(){updateScrollScale()}function onResize(){measure();updateScrollScale()}measure();updateScrollScale();track.addEventListener("mousemove",onPointerMove,{passive:!0});window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});const ro=new ResizeObserver(()=>{measure()});try{ro.observe(track);ro.observe(box)}catch(e){}loop();section.__heroShowreelDestroy__=function(){try{cancelAnimationFrame(rafId)}catch(e){}try{track.removeEventListener("mousemove",onPointerMove)}catch(e){}try{window.removeEventListener("scroll",onScroll)}catch(e){}try{window.removeEventListener("resize",onResize)}catch(e){}try{ro.disconnect()}catch(e){}try{box.style.removeProperty("--hero-showreel-x");box.style.removeProperty("--hero-showreel-scale");heroBits.forEach(el=>{el.style.transform="";el.style.opacity=""})}catch(e){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(e){}const s=document.querySelector(".hero-section-showreel");s&&(s.__heroShowreelDestroy__=null)}}),
			feature({
