	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||document.querySelector(".barba-container")||r;if(!c||c.__csbgInit)return;c.__csbgInit=1;const d=c.querySelector(".cs-details");let m=[...c.querySelectorAll(".cs-morework")].filter(e=>e&&e.getClientRects().length&&getComputedStyle(e).display!=="none"&&getComputedStyle(e).visibility!=="hidden"&&e.getBoundingClientRect().height>2).sort((a,b)=>(a.getBoundingClientRect().top+(window.pageYOffset||0))-(b.getBoundingClientRect().top+(window.pageYOffset||0))).pop()||c.querySelector(".cs-morework");if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const set=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})},y=()=>window.pageYOffset||document.documentElement.scrollTop||document.body.scrollTop||0;let stD=null,stM=null;const rf=(()=>{let t=0;return()=>{if(t)return;t=requestAnimationFrame(()=>{t=0;try{ScrollTrigger.refresh()}catch{}})}})();const sync=()=>{const p=y(),a=stD&&typeof stD.start=="number"?stD.start:1/0,b=stM&&typeof stM.start=="number"?stM.start:1/0;set(p>=b?bd:p>=a?bg:cs)};stD=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>set(bg),onLeaveBack:()=>set(cs),invalidateOnRefresh:!0,refreshPriority:100,onRefresh:sync});stM=ScrollTrigger.create({id:"csbg:more",trigger:m,start:"top 80%",onEnter:()=>set(bd),onLeaveBack:()=>set(bg),invalidateOnRefresh:!0,refreshPriority:110,onRefresh:sync});sync();const g=c.querySelector(".cs-gallery");let ro=null,offLoad=null;if(g&&"ResizeObserver"in window){let h=g.getBoundingClientRect().height;ro=new ResizeObserver(()=>{const nh=g.getBoundingClientRect().height;if(Math.abs(nh-h)>2){h=nh;rf()}});try{ro.observe(g)}catch{}}if(g){const imgs=[...g.querySelectorAll("img")];if(imgs.length){let n=0,done=()=>{if(++n<imgs.length)return;rf()};imgs.forEach(im=>{if(im.complete&&im.naturalWidth>0){done();return}(typeof im.decode=="function"?im.decode().catch(()=>{}):Promise.resolve()).finally(done),im.addEventListener("load",done,{once:!0}),im.addEventListener("error",done,{once:!0})})}}offLoad=()=>rf();window.addEventListener("load",offLoad,{once:!0});rf();return()=>{try{window.removeEventListener("load",offLoad)}catch{}try{ro&&ro.disconnect()}catch{}try{stD&&stD.kill();stM&&stM.kill()}catch{}try{delete c.__csbgInit}catch{}}}
			}),
			feature({
