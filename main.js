...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero||hero.__hsBound)return;var track=hero.querySelector(".showreel-track"),container=hero.querySelector(".showreel-container");if(!track||!container)return;hero.__hsBound=1;container.style.setProperty("--hero-showreel-x","0px");container.style.setProperty("--hero-showreel-scale","1");var S={x:0,vx:0,scale:1,nx:0,scrollScale:1,raf:0,t:0},K=.06,D=.18,SL=.14,CURSOR_RANGE=.06,SCROLL_MIN=.96,SCROLL_MAX=1.08,baseW=container.offsetWidth||container.clientWidth||0;function clamp01(v){return v<0?0:v>1?1:v}function easeEdge(n){var s=n<0?-1:1,a=Math.min(1,Math.abs(n));return s*Math.sin(a*Math.PI/2)}function limitForScale(sc){var tw=track.clientWidth||0;var lim=(tw-baseW*sc)/2;return lim>0?lim:0}function onMove(e){var r=track.getBoundingClientRect();if(!r.width)return;var nx=(e.clientX-r.left)/r.width*2-1;S.nx=nx<-1?-1:nx>1?1:nx}function onScroll(){var r=track.getBoundingClientRect(),vh=window.innerHeight||document.documentElement.clientHeight||1,mid=r.top+r.height/2,dist=Math.abs(mid-vh/2),f=1-Math.min(1,dist/(vh*.7));S.scrollScale=SCROLL_MIN+f*(SCROLL_MAX-SCROLL_MIN)}function onResize(){baseW=container.offsetWidth||container.clientWidth||baseW}function loop(ts){if(!S.t)S.t=ts;var dt=Math.min(.05,(ts-S.t)/1e3);S.t=ts;var targetScale=S.scrollScale*(1+Math.abs(S.nx)*CURSOR_RANGE);S.scale+= (targetScale-S.scale)*SL;var lim=limitForScale(S.scale);var tx=easeEdge(S.nx)*lim;var a=(tx-S.x)*K-S.vx*D;S.vx+=a;S.x+=S.vx;if(S.x<-lim){S.x=-lim;S.vx=S.vx<0?S.vx*.2:S.vx}if(S.x>lim){S.x=lim;S.vx=S.vx>0?S.vx*.2:S.vx}container.style.setProperty("--hero-showreel-x",S.x.toFixed(2)+"px");container.style.setProperty("--hero-showreel-scale",S.scale.toFixed(3));S.raf=requestAnimationFrame(loop)}onScroll();window.addEventListener("pointermove",onMove,{passive:!0});window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});S.raf=requestAnimationFrame(loop);hero.__hsDestroy=function(){try{window.removeEventListener("pointermove",onMove)}catch(_){}try{window.removeEventListener("scroll",onScroll)}catch(_){}try{window.removeEventListener("resize",onResize)}catch(_){}if(S.raf)try{cancelAnimationFrame(S.raf)}catch(_){}try{container.style.removeProperty("--hero-showreel-x");container.style.removeProperty("--hero-showreel-scale")}catch(_){}hero.__hsBound=0}},destroy:async function(root){root=root||document;var h=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{if(h&&h.__hsDestroy)h.__hsDestroy()}catch(_){}}}),
			feature({
