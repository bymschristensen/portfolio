...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async e=>{if(!window.gsap||matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1 / 1")),i=e=>{const t=parseFloat(e.dataset.ratio);if(t&&t>0)return t;const r=parseFloat(e.getAttribute("data-ratio"));if(r&&r>0)return r;const i=(e.className||"").match(/sv-ratio-(\d+)/);if(i){const e=i[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const n=e.querySelector("img,video");if(n){const e=parseInt(n.getAttribute("width"),10)||n.naturalWidth||n.videoWidth||0,t=parseInt(n.getAttribute("height"),10)||n.naturalHeight||n.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5},n=e=>{const t=i(e);e.dataset.ratio=String(t);if(r)e.style.aspectRatio=t+" / 1",e.style.width="";else{const r=e.getBoundingClientRect().height||parseFloat(getComputedStyle(e).height)||0,i=r>0?Math.max(1,Math.round(r*t)):0;e.style.width=i?i+"px":""}e.style.flex="0 0 auto",e.style.overflow="hidden",e.style.willChange="transform"},o=e=>{try{window.CoreUtilities&&CoreUtilities.Observers&&CoreUtilities.Observers.addDom(e)}catch{}},a=e=>{try{window.CoreUtilities&&CoreUtilities.Observers&&CoreUtilities.Observers.addTicker(e)}catch{}};t.forEach(e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0,e.setAttribute("data-armed","0");const r=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!r.length){e.setAttribute("data-armed","1");return}e.setAttribute("data-armed","measure");const l=new Set;r.forEach(((e,t)=>{e.style.height="",n(e),e.dataset.key=String(t),e.dataset.revealed="0",e.style.height="0px",e.style.overflow="hidden"}));const s="right"===(e.dataset.direction||"left").toLowerCase(),d=parseFloat(e.dataset.speed)||.6,c=parseFloat(getComputedStyle(t).gap||"0")||0,h=e=>e.getBoundingClientRect().width+c;function u(){r.forEach((e=>{const r=e.cloneNode(!0),o=e.dataset.key||"";r.setAttribute("data-clone","1"),r.dataset.key=o,n(r),r.style.height=l.has(o)&&e.dataset.targetH?e.dataset.targetH+"px":"0px",r.style.overflow="hidden",t.appendChild(r)}))}function f(){let e=0;return t.childNodes.forEach((t=>{1===t.nodeType&&(e+=h(t))})),e}function p(){[...t.querySelectorAll('.service-visual-wrapper[data-clone]')].forEach((e=>e.remove())),u(),u();let e=0;for(;f()<3*t.clientWidth&&e++<8;)u()}p(),e.setAttribute("data-armed","1");let m=0,v=!1,g=0;function w(r){if(!v)return void cancelAnimationFrame(m);if(r-g<80)return void(m=requestAnimationFrame(w));g=r||performance.now();const o=window.innerWidth,a=-.05*o,n=1.05*o,s=[...t.querySelectorAll(".service-visual-wrapper")].filter((e=>{const t=e.getBoundingClientRect();return t.right>a&&t.left<n})).filter((e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing)).sort(((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left));if(s.length){const r=gsap.timeline();s.forEach(((o,a)=>{let s=parseFloat(o.dataset.targetH||"0");if(!s||s<=0){const e=i(o),t=o.getBoundingClientRect().width||parseFloat(getComputedStyle(o).width)||0;s=e>0&&t>0?t/e:0,o.dataset.targetH=String(s)}const d=o.dataset.key||"";if(!s)return;o.dataset.revealing="1",r.to(o,{height:s,duration:.9,ease:"power2.out",onComplete:()=>{o.style.height="",o.dataset.revealed="1",o.dataset.revealing="",l.add(d),t.querySelectorAll('.service-visual-wrapper[data-clone][data-key="'+d+'"]').forEach((e=>{e.style.height=s+"px"}))}},.12*a)}))}m=requestAnimationFrame(w)}const y=new IntersectionObserver((t=>{t.forEach((t=>{t.target===e&&(t.isIntersecting?v||(v=!0,cancelAnimationFrame(m),m=requestAnimationFrame(w)):(v=!1,cancelAnimationFrame(m)))}))}),{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});y.observe(e),o(y);let x=0,b=s?-1:1,k=60*d,E=(e,r)=>{const i=(r||16.6667)/1e3;let n=t.firstElementChild,o=0;x-=b*k*i;for(;n&&x<-h(n)&&o++<50;)x+=h(n),t.appendChild(n),n=t.firstElementChild;let a=t.lastElementChild;for(o=0;a&&x>0&&o++<50;)x-=h(a),t.insertBefore(a,t.firstElementChild),a=t.lastElementChild;gsap.set(t,{x:x});const l=window.innerWidth||1,s=l/2;[...t.querySelectorAll(".service-visual")].forEach((e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),i=(s-(r.left+r.right)/2)/l;e.style.setProperty("--drift",40*i+"px")}))};gsap.ticker.add(E),a(E);const q=new ResizeObserver((()=>{p()}));q.observe(t),o(q)})}})
		);
