	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{const g=e=>e&&"string"==typeof e?e.trim():"",W=e=>e?.closest(".w-tabs")||r.querySelector(".w-tabs"),L=(e,t)=>e?.querySelector('.w-tab-link[data-w-tab="'+t+'"]')||[...(e?.querySelectorAll(".w-tab-link")||[])].find(e=>(e.textContent||"").trim().toLowerCase()===t.toLowerCase())||null,P=(e,t)=>e?.querySelector('.w-tab-pane[data-w-tab="'+t+'"]')||[...(e?.querySelectorAll(".w-tab-pane")||[])].find(e=>((e.getAttribute("data-w-tab")||"").trim().toLowerCase())===t.toLowerCase())||null,tw=a=>new Promise(s=>{let n=0;!function o(){if(n++>=a)return s();requestAnimationFrame(o)}()}),scrollToEl=e=>{try{const t=(e.getBoundingClientRect().top+window.pageYOffset)-24;window.scrollTo({top:t,behavior:"smooth"})}catch{try{e.scrollIntoView({block:"start",behavior:"smooth"})}catch{}}},kickAnim=e=>{try{e?.querySelectorAll?.(".appear-in-line").forEach(t=>t.__ailReplay&&t.__ailReplay())}catch{}},handle=(t,e)=>{const a=W(t);if(!a)return;const d=g(t.dataset.tab)||g(t.getAttribute("data-tab"))||g((t.getAttribute("href")||"").replace(/^#/,""))||null,h=(t.getAttribute("href")||"").trim(),S=g(t.dataset.scroll)||(h.startsWith("#")?h:null),ln=d?L(a,d):null,pn=d?P(a,d):null,tr=S?document.querySelector(S):null,vis=!!(tr&&tr.offsetParent!==null),delay=40;e?.preventDefault?.(),e?.stopPropagation?.();if(tr&&vis){scrollToEl(tr);ln&&setTimeout(()=>{ln.click();setTimeout(()=>kickAnim(pn),60)},delay)}else{ln&&ln.click();tr&&tw(2).then(()=>{scrollToEl(tr);setTimeout(()=>kickAnim(pn),60)})}},onClick=e=>{const t=e.target.closest('[data-tab]:not([data-w-tab])');t&&handle(t,e)};r.__tjDelegated||(r.__tjDelegated=!0,r.addEventListener("click",onClick,{capture:!0,passive:!1}))}
			})
		);
