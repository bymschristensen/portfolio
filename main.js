	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger||!window.gsap)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework"),g=c.querySelector(".cs-gallery");if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})};const sd=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),refreshPriority:100}),sm=ScrollTrigger.create({id:"csbg:more",trigger:m,start:"top bottom",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),refreshPriority:100});const y=()=>{const t=ScrollTrigger.scroll();s(t>=sm.start?bd:t>=sd.start?bg:cs)};ScrollTrigger.addEventListener("refresh",y);if(!g){ScrollTrigger.refresh();return}const i=[...g.querySelectorAll("img")];if(!i.length){ScrollTrigger.refresh();return}let n=0;const o=()=>{if(++n<i.length)return;requestAnimationFrame(()=>ScrollTrigger.refresh()),setTimeout(()=>ScrollTrigger.refresh(),150)};i.forEach(e=>{if(e.complete&&e.naturalWidth>0){o();return}(typeof e.decode=="function"?e.decode().catch(()=>{}):Promise.resolve()).finally(o),e.addEventListener("load",o,{once:!0}),e.addEventListener("error",o,{once:!0})})}
			}),
			feature({
