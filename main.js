	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var S=[].slice.call(r.querySelectorAll(".section-single-service"));if(!S.length)return;var P=[];S.forEach(function(sec){var small=sec.offsetHeight<window.innerHeight,t=gsap.timeline({scrollTrigger:{trigger:sec,start:small?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}}).to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0);t.scrollTrigger&&P.push(t.scrollTrigger)});var pausePins=function(){P.forEach(function(st){st.animation&&st.animation.pause()})},resumePins=function(){P.forEach(function(st){st.animation&&st.animation.play()})};document.addEventListener("click",function(e){if(e.target.closest(".accordion")||e.target.closest("[data-accordion]"))pausePins()});window.addEventListener("scroll",function(){resumePins()},{once:!0,passive:!0});}
			}),
			feature({
				id: "productGallery",
				stage: "main",
