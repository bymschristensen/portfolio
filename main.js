	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const mm=gsap.matchMedia();mm.add("(min-width: 1024px)",(()=>{const sec=r.querySelector('[data-imgscroll="photos"]');if(!sec)return;const all=[...sec.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(+a.getAttribute("data-photo")||0)-(+b.getAttribute("data-photo")||0)),p1=all.find(e=>e.getAttribute("data-photo")==="1"),p2=all.find(e=>e.getAttribute("data-photo")==="2"),p3=all.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const sel='img.bio-photo, img, .bio-photo';const i1=p1.querySelector(sel)||p1.querySelector("img"),i2=p2.querySelector(sel)||p2.querySelector("img"),i3=p3.querySelector(sel)||p3.querySelector("img");function prime(img,yPct){if(!img)return;gsap.killTweensOf(img);gsap.set(img,{x:0,y:0,force3D:!0});gsap.set(img,{xPercent:-50,yPercent:yPct,scale:1.1,willChange:"transform",force3D:!0})}prime(i1,-55);prime(i2,-60);prime(i3,-60);const tl=gsap.timeline({defaults:{ease:"none",overwrite:"auto"},scrollTrigger:{trigger:sec,start:"top 35%",end:"+=140%",scrub:1,anticipatePin:1,invalidateOnRefresh:!0,onRefreshInit(){prime(i1,-55);prime(i2,-60);prime(i3,-60)}}});i3&&tl.to(i3,{yPercent:-50,force3D:!0},"0");i2&&tl.to(i2,{yPercent:-50,force3D:!0},"0+=0.04");i1&&tl.to(i1,{yPercent:-50,force3D:!0},"0+=0.08")}))}
			}),
			feature({
