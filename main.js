...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),img=e.querySelector(".cs-hero-image"),hd=e.querySelector(".cs-headline"),subs=e.querySelectorAll(".cs-titles-inner div"),mkVisible=o=>{if(!o)return;try{o.removeAttribute("data-entry-hidden");o.removeAttribute("hidden");o.removeAttribute("aria-hidden");o.style.visibility="visible";["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(c=>o.classList&&o.classList.remove(c))}catch{}};img&&gsap.set(img,{autoAlpha:0,y:80,filter:"blur(10px)"});hd&&gsap.set(hd,{autoAlpha:0});subs.length&&gsap.set(subs,{autoAlpha:0,y:20,filter:"blur(10px)"});mkVisible(img);mkVisible(hd);subs.forEach(mkVisible);img&&t.to(img,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(hd){const origDisp=hd.style.display,wasInline=getComputedStyle(hd).display==="inline";t.addLabel("headline",.35).set(hd,{autoAlpha:1,display:"block"},"headline").call(()=>{const run=()=>{try{if(!window.splitAndMask||!window.animateLines){gsap.to(hd,{y:0,opacity:1,duration:.6,ease:"power2.out"});wasInline&&(hd.style.display="inline");origDisp&&(hd.style.display=origDisp);return}const s=splitAndMask(hd);animateLines(s.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:s.revert).call(s,s,hd)}catch{}wasInline&&(hd.style.display="inline");origDisp&&(hd.style.display=origDisp)})}catch{gsap.to(hd,{y:0,opacity:1,duration:.6,ease:"power2.out"});wasInline&&(hd.style.display="inline");origDisp&&(hd.style.display=origDisp)}};Promise.resolve(CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready&&CoreUtilities.Fonts.ready()).then(()=>{requestAnimationFrame(()=>requestAnimationFrame(run))}).catch(run)},null,"headline")}subs.length&&t.to(subs,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t}
		};
