	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async function(t){t.querySelectorAll(".appear-in-line").forEach((function(t){if(t._ail)return;const e=[],o=[];function n(){o.forEach(((t,e)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:.8,ease:"power2.out",delay:.15*e})}));const t=.15*(o.length-1)+.8;gsap.delayedCall(t+.05,(()=>{e.forEach((t=>{try{t.revert()}catch{}}))}))}Array.from(t.querySelectorAll(":scope > *")).forEach((t=>{const n=getComputedStyle(t),r=parseInt(n.columnCount,10)||1,i=t.getBoundingClientRect();if(r>1&&window.SplitText)try{gsap.set(t,{opacity:0});const n=new SplitText(t,{type:"lines",linesClass:"split-line"});e.push(n);const s=i.width/r,a=Array.from({length:r},(()=>[]));n.lines.forEach((t=>{const e=t.getBoundingClientRect().left-i.left,o=Math.min(Math.floor(e/s),r-1);a[o].push(t),gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})})),a.forEach((t=>o.push(t)))}catch{gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),o.push([t])}else gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),o.push([t])}));const r=new IntersectionObserver(((t,e)=>{t.forEach((t=>{t.isIntersecting&&(e.unobserve(t.target),n())}))}),{root:null,rootMargin:"0px 0px -10% 0px",threshold:0});CoreUtilities.Observers.addDom(r),r.observe(t);const i=t.getBoundingClientRect();i.top<window.innerHeight&&i.bottom>0&&(r.unobserve(t),n());const s=t.closest(".w-tab-pane");if(s){const e=new MutationObserver((()=>{if(!s.classList.contains("w--tab-active"))return;r.observe(t);const e=t.getBoundingClientRect();e.top<window.innerHeight&&e.bottom>0&&(r.unobserve(t),n())}));CoreUtilities.Observers.addDom(e),e.observe(s,{attributes:!0,attributeFilter:["class"]})}t._ail={splits:e,groups:o,io:r}}))}
			}),
