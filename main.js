...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(r){if(!matchMedia("(min-width:1024px)").matches)return;const e=r.querySelector(".hero-section-showreel"),t=e?.querySelector(".showreel-track"),n=t?.querySelector(".showreel-container");if(!e||!t||!n)return;const o=[e.querySelector(".showreel-headline"),e.querySelector(".tableofcontents"),e.querySelector(".toc-outer"),e.querySelector(".showreel-subtext")].filter(Boolean),a=getComputedStyle(n).transform||"none",i="none"===a?"":a+" ";let l=window.innerHeight,c=1,s=1,u=.7,d=1,m=.8,f=m,p=0,h=1,g=!1,y=0;function v(e){return e<0?0:e>1?1:e}function x(e,t,n){return e+(t-e)*n}function b(){const r=t.getBoundingClientRect();c=Math.max(1,r.width);const e=n.style.transform;n.style.transform="none",s=Math.max(1,n.getBoundingClientRect().width),n.style.transform=e;const o=Math.min(1,c/s);d=o,u=Math.max(.5,o*.65)}function w(){const r=e.getBoundingClientRect(),t=r.height-l*.6;if(t<=0)return 0;const n=(-r.top-l*.1)/t;return v(n)}function E(){const r=w(),e=x(u,d,r),a=s*e,f=Math.max(0,c-a),k=.3,S=x(p,((()=>{const r=m*c;let e=r-k*a;e<0&&(e=0),e>f&&(e=f);const t=e+a/2-c/2;return t})()),.18);p+=.18*(m-p),h+=.18*(e-h),y+=.16*(S-y),n.style.transform=i+"translate3d("+y+"px,0,0) scale("+h+")";const T=-.45*l*r,P=1-r;o.forEach((e=>{e&&(e.style.transform="translate3d(0,"+T+"px,0)",e.style.opacity=P)}))}function k(){g||(g=!0,requestAnimationFrame((()=>{g=!1,E()})))}function S(r){const e=t.getBoundingClientRect(),n=Math.max(e.left,Math.min(r.clientX,e.right));m=(n-e.left)/e.width,k()}function T(){l=window.innerHeight,b(),k()}function P(){k()}b(),E(),t.addEventListener("mousemove",S,{passive:!0}),addEventListener("scroll",P,{passive:!0}),addEventListener("resize",T,{passive:!0});const C=new ResizeObserver(T);try{C.observe(t),C.observe(n)}catch(r){}(function A(){y=requestAnimationFrame(A),f+=.1*(m-f),k()})();e.__heroShowreelDestroy__=()=>{try{cancelAnimationFrame(y)}catch(r){}try{removeEventListener("scroll",P)}catch(r){}try{removeEventListener("resize",T)}catch(r){}try{t.removeEventListener("mousemove",S)}catch(r){}try{C.disconnect()}catch(r){}try{n.style.transform="none"===a?"":a,o.forEach((r=>{r&&(r.style.transform="",r.style.opacity="")}))}catch(r){}}},destroy:async function(){try{document.querySelector(".hero-section-showreel")?.__heroShowreelDestroy__?.()}catch(r){}const e=document.querySelector(".hero-section-showreel");e&&(e.__heroShowreelDestroy__=null)}}),
			feature({
