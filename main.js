...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(d){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;d=d||document;var s=d.querySelector(".hero-section-showreel");if(!s||s.__hsBound)return;var tr=s.querySelector(".showreel-track"),ct=s.querySelector(".showreel-container");if(!tr||!ct)return;s.__hsBound=1;ct.style.transformOrigin="50% 50%";ct.style.willChange="transform";ct.style.transform="translate3d(0,0,0) scale(1)";var st={x:0,s:1,px:null,ts:1,last:0,raf:0,w:0,tw:0,lock:!1,hasMove:!1},EDGE=1.1,NUDGE=60,SX0=.96,SX1=1.1;function meas(){var a=ct.getBoundingClientRect(),b=tr.getBoundingClientRect();st.w=a.width||ct.offsetWidth||0;st.tw=b.width||tr.offsetWidth||0}function slack(){return Math.max(0,(st.tw-st.w*st.s)/2)}function shaped(x){var m=slack();if(!m)return 0;var n=x/m,u=Math.tanh(n*EDGE)/Math.tanh(EDGE);return u*m}function mapX(cx){var tb=tr.getBoundingClientRect(),rel=(cx-tb.left)+20+st.w*st.s/2-(st.tw/2);var m=slack();rel=Math.max(-m,Math.min(m,rel));return shaped(rel)}function onMove(e){st.hasMove=!0;st.px=e.clientX}function onView(){var tb=tr.getBoundingClientRect(),vh=(innerHeight||document.documentElement.clientHeight||1),c=tb.top+tb.height/2,dist=Math.abs(c-vh/2),t=1-Math.min(1,dist/(vh*.75));st.ts=SX0+t*(SX1-SX0);st.lock=st.w*st.s>=st.tw-1}function apply(){ct.style.transform="translate3d("+st.x+"px,0,0) scale("+st.s+")"}function loop(t){st.last||(st.last=t);var dt=Math.min(.05,(t-st.last)/1e3);st.last=t;meas();var iw=(innerWidth||document.documentElement.clientWidth||1),mid=.5*iw,px=st.px==null?mid:st.px,nudge=((mid-px)/iw)*NUDGE,target=st.lock?0:(st.hasMove?mapX(px)+nudge:0),eX=1-Math.exp(-10*dt),eS=1-Math.exp(-6*dt);st.s+=eS*(st.ts-st.s);st.x+=eX*(target-st.x);apply();st.raf=requestAnimationFrame(loop)}meas();onView();addEventListener("pointermove",onMove,{passive:!0});addEventListener("scroll",onView,{passive:!0});addEventListener("resize",meas,{passive:!0});st.raf=requestAnimationFrame(loop);s.__hsDestroy=function(){try{removeEventListener("pointermove",onMove)}catch(e){}try{removeEventListener("scroll",onView)}catch(e){}try{removeEventListener("resize",meas)}catch(e){}try{st.raf&&cancelAnimationFrame(st.raf)}catch(e){}}},destroy:async function(r){r=r||document;var s=r.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{s&&s.__hsDestroy&&s.__hsDestroy()}catch(e){}}}),
			feature({
