	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{const f=a=>a&&"string"==typeof a?a.trim():null,t=a=>a?.closest(".w-tabs")||r.querySelector(".w-tabs"),n=(a,t)=>a?.querySelector('.w-tab-link[data-w-tab="'+t+'"]')||[...a?.querySelectorAll(".w-tab-link")||[]].find(a=>(a.textContent||"").trim().toLowerCase()===t.toLowerCase())||null,o=(a,t)=>a?.querySelector('.w-tab-pane[data-w-tab="'+t+'"]')||[...a?.querySelectorAll(".w-tab-pane")||[]].find(a=>((a.getAttribute("data-w-tab")||"").trim().toLowerCase())===t.toLowerCase())||null,e=a=>new Promise(r=>{let t=0;!function n(){if(t++>=a)return r();requestAnimationFrame(n)}()}),c=a=>{try{const r=a.getBoundingClientRect().top+window.pageYOffset-24;window.scrollTo({top:r,behavior:"smooth"})}catch{try{a.scrollIntoView({block:"start",behavior:"smooth"})}catch{}}},l=a=>{try{a?.querySelectorAll?.(".appear-in-line").forEach(a=>a.__ailReplay&&a.__ailReplay())}catch{}},i=(a,r)=>{const i=t(a);if(!i)return;const s=f(a.dataset.tab)||f(a.getAttribute("data-tab"))||f(a.getAttribute("href")||"").replace(/^#?/,"")||null,u=(a.getAttribute("href")||"").trim(),d=f(a.dataset.scroll)||(u.startsWith("#")?u:null),y=s?n(i,s):null,h=s?o(i,s):null,p=d?document.querySelector(d):null,b=!!(p&&p.offsetParent!==null),k=40;r?.preventDefault?.(),r?.stopPropagation?.(),p&&b?(c(p),y&&setTimeout(()=>{y.click(),setTimeout(()=>l(h),60)},k)):(y&&y.click(),p&&e(2).then(()=>{c(p),setTimeout(()=>l(h),60)}))};[...r.querySelectorAll('[data-tab]:not([data-w-tab])')].forEach(a=>{a.__tjBound||(a.__tjBound=!0,a.addEventListener("click",r=>i(a,r),{passive:!1}))})}
			})
		);
