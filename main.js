	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;try{ScrollTrigger.getAll().forEach(st=>{const id=String(st.vars&&st.vars.id||"");const tr=st.vars&&st.vars.trigger;(id.indexOf("csbg")===0||tr===d||tr===m||tr?.closest?.(".cs-details")||tr?.closest?.(".cs-morework"))&&st.kill()})}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})};const ensure=(el,key)=>{let p=el.querySelector(`[data-csbg="${key}"]`);return p||(p=document.createElement("div"),p.setAttribute("data-csbg",key),p.style.cssText="position:relative;width:100%;height:1px;",el.insertBefore(p,el.firstChild)),p};ScrollTrigger.create({id:"csbg:details",trigger:ensure(d,"details"),start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),invalidateOnRefresh:!0,refreshPriority:100});ScrollTrigger.create({id:"csbg:more",trigger:ensure(m,"more"),start:"top 80%",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),invalidateOnRefresh:!0,refreshPriority:110});let raf=0;const ref=()=>{cancelAnimationFrame(raf),raf=requestAnimationFrame(()=>{ScrollTrigger.refresh()})};ref();setTimeout(ref,160);setTimeout(ref,520);setTimeout(ref,1100);const g=c.querySelector(".cs-gallery");if(g&&"ResizeObserver"in window){let h=g.getBoundingClientRect().height,ro=new ResizeObserver(()=>{const nh=g.getBoundingClientRect().height;if(Math.abs(nh-h)>1){h=nh;ref();setTimeout(ref,160)}});try{ro.observe(g)}catch{}setTimeout(()=>{try{ro.disconnect()}catch{}},8000);return()=>{try{ro.disconnect()}catch{}}}
			}),
			feature({
