	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async root=>{const lists=[...root.querySelectorAll(".accordion-list")];if(!lists.length)return;const ns=root?.dataset?.barbaNamespace||root.getAttribute?.("data-barba-namespace")||"";let to;const scheduleRefresh=item=>{if(!window.ScrollTrigger)return;clearTimeout(to);to=setTimeout(()=>{try{if(ns==="capabilities"){const sec=item.closest(".section-single-service");if(!sec)return;const all=ScrollTrigger.getAll?ScrollTrigger.getAll():[];all.forEach(st=>{if(st&&st.trigger===sec){try{st.refresh();}catch(e){}}});}else{ScrollTrigger.refresh();}}catch(e){}},120);};lists.forEach(list=>{const items=[...list.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];if(!items.length)return;items.forEach(it=>{if(it._accordionBound)return;it._accordionBound=!0;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:void 0});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":void 0,duration:.5,onComplete:()=>{gsap.set(ct,{maxHeight:"none"});}},0);if(isQ&&qi)tl.from(qi,{opacity:0,duration:.4},0);const notify=()=>scheduleRefresh(it);tl.eventCallback("onComplete",notify);tl.eventCallback("onReverseComplete",notify);it._accordionTimeline=tl;it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");items.forEach(other=>{if(other===it)return;const oh=other.querySelector(".accordion-header");const ot=other._accordionTimeline;if(!oh||!ot)return;if(oh.classList.contains("accordion-active")&&!ot.isActive()){oh.classList.remove("accordion-active");ot.reverse();}});if(open){gsap.set(ct,{maxHeight:ct.offsetHeight});tl.reverse();}else{gsap.set(ct,{maxHeight:0});tl.play(0);}h.classList.toggle("accordion-active",!open);});});});}
			}),
			feature({
				id: "productGallery",
				stage: "main",
