...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archivePlayfulExperience",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".archive-playful-item",".archive-playful-visuals",".archive-playful-visual-img",".archive-playful-data-inner",".archive-playful-data-product",".archive-playful-data-expertise",".archive-playful-item-data-outer .case-study-url"],init:function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apx2Bound)return;host.__apx2Bound=1;if(!window.gsap||!window.ScrollTrigger){host.__apx2Bound=0;return}var gs=window.gsap,ST=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches){host.__apx2Bound=0;return}var P=host.querySelector(".section-archive-playful");if(!P){host.__apx2Bound=0;return}var un=[],tr=[],raf=0,lockRaf=0,locked=!1;function on(n,e,f,o){if(!n)return;n.addEventListener(e,f,o);un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function kill(){for(var i=0;i<tr.length;i++)try{tr[i]&&tr[i].kill&&tr[i].kill()}catch(_){ }tr.length=0;locked=!1}function pickDataParts(it){var pd=qs(it,".archive-playful-data-product"),pe=qs(it,".archive-playful-data-expertise");if(!pd.length&&!pe.length){pd=qs(it,".archive-playful-data-inner .headline-xxs");pe=qs(it,".archive-playful-data-inner .body-sm")}return pd.concat(pe)}function pickLink(it){return q(it,".archive-playful-item-data-outer .case-study-url")||q(it,".archive-playful-item-data .shortcut-link")}function lockStarts(){if(locked||lockRaf)return;lockRaf=requestAnimationFrame(function(){lockRaf=0;try{for(var i=0;i<tr.length;i++){var st=tr[i];if(!st||st._apxLocked)continue;st._apxLocked=1;st._apxStart=st.start;st._apxEnd=st.end;st.vars.start=st._apxStart;st.vars.end=st._apxEnd}}catch(_){ }try{for(i=0;i<tr.length;i++){st=tr[i];st&&st.refresh&&st.refresh()}}catch(_){ }locked=!0})}function build(){kill();if(!mm.matches||!vis())return;var items=[].slice.call(P.querySelectorAll(".archive-playful-item"));if(!items.length)return;for(var i=0;i<items.length;i++){var it=items[i],v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickDataParts(it),ln=pickLink(it);try{imgs.length&&gs.set(imgs,{willChange:"transform"});v&&gs.set(v,{willChange:"height",overflow:"hidden"});di&&gs.set(di,{willChange:"margin"});parts.length&&gs.set(parts,{willChange:"opacity,height"});ln&&gs.set(ln,{willChange:"opacity,height",overflow:"hidden"})}catch(_){ }var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.fromTo(imgs,{yPercent:-15},{yPercent:15,duration:1},0);parts.length&&tl.to(parts,{opacity:0,duration:.08},.70);di&&tl.to(di,{marginTop:0,marginBottom:0,duration:.08},.70);parts.length&&tl.to(parts,{height:0,duration:.08},.75);ln&&tl.to(ln,{opacity:0,duration:.08},.75);ln&&tl.to(ln,{height:0,duration:.08},.80);imgs.length&&tl.to(imgs,{scale:1.5,duration:.20},.80);v&&tl.to(v,{height:0,duration:.20},.80);tr.push(ST.create({trigger:it,start:"top 100%",end:"top 0%",scrub:!0,animation:tl}))}try{ST.refresh()}catch(_){ }locked=!1;lockStarts()}function sched(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;build()})}function sync(){sched()}on(document,"archive:view",sync,{passive:!0});on(document,"archive:filter:applied",sync,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sync):mm.addListener(sync)}catch(_){ }un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sync):mm.removeListener(sync)}catch(_){}});try{ST.addEventListener("refresh",function(){locked=!1;lockStarts()})}catch(_){ }requestAnimationFrame(function(){setTimeout(build,0)});return function destroy(){try{kill()}catch(_){ }for(var i=0;i<un.length;i++)un[i]();un.length=0;try{host.__apx2Bound=0}catch(_){}}}}) 
		);
