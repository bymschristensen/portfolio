...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.caseStudy=function(e){var t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),n=e.querySelector(".cs-headline"),a=n&&(n.querySelector("h1, .headline-xl, .headline-lg, .headline, [data-split], [role='heading'][aria-level='1']")||[].slice.call(n.querySelectorAll("*")).find((function(e){return(e.textContent||"").trim().length&&"none"!==getComputedStyle(e).display}))||n),l=e.querySelector(".cs-titles-inner .headline-m"),r=function(e){if(e)try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach((function(t){e.classList&&e.classList.remove(t)}))}catch(e){}};if(i&&(gsap.set(i,{autoAlpha:0,y:100,filter:"blur(10px)"}),r(i),t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.7,ease:"power2.out"},0)),a){gsap.set(a,{autoAlpha:0,visibility:"hidden",display:"block"}),r(a),l&&(gsap.set(l,{autoAlpha:0,y:20,filter:"blur(10px)"}),r(l));var o=window.CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready&&CoreUtilities.Fonts.ready()||Promise.resolve(),s=new Promise((function(e){var t=0;!function i(){return window.SplitText&&window.animateLines||++t>40?e():void setTimeout(i,50)}()}));t.addLabel("h",.15).call((function(){Promise.race([o,s]).then((function(){requestAnimationFrame((function(){requestAnimationFrame((function(){var e;if(window.splitAndMask)e=splitAndMask(a);else{var i=document.createElement("div"),n=document.createElement("div");i.className="text-mask",i.style.overflow="hidden",n.className="split-line",n.innerHTML=a.innerHTML,i.appendChild(n),a.innerHTML="",a.appendChild(i),gsap.set(n,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform"}),e={lines:[n],revert:function(){try{a.innerHTML=n.innerHTML}catch(e){}}}}var r=e&&e.lines&&e.lines.length?e.lines:[a],o=.8+.08*Math.max(0,r.length-1);gsap.set(a,{autoAlpha:1,visibility:"visible"}),l&&t.to(l,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.5,ease:"power2.out"},"h+"+Math.max(0,o-.2)),(window.animateLines?animateLines(r):gsap.to(r,{yPercent:0,rotation:0,duration:o,ease:"power2.out",stagger:.08})).eventCallback("onComplete",(function(){try{(window.safelyRevertSplit?window.safelyRevertSplit:e.revert).call(e,e,a)}catch(e){}try{"inline"===getComputedStyle(a).display&&(a.style.display="inline")}catch(e){}}))}))}))}))}),null,"h")}return t};
