...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.info=function(e){e=e||document;var t=gsap.timeline(),n=e.querySelector(".hero-section-text h1"),i=e.querySelector(".hero-subtext"),o=[...e.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((e,t)=>(+e.getAttribute("data-photo")||0)-(+t.getAttribute("data-photo")||0));n&&(gsap.set(n,{autoAlpha:0}),t.addLabel("h",0).set(n,{autoAlpha:1},"h").call((function(){(CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve()).then((function(){if(window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){var e=splitAndMask(n);animateLines(e.lines).eventCallback("onComplete",(function(){safelyRevertSplit(e,n)}))}else gsap.to(n,{y:0,opacity:1,duration:.6,ease:"power2.out"})}))}),null,"h"));i&&(gsap.set(i,{autoAlpha:0,y:20,filter:"blur(10px)"}),t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},"h+=0.48"));if(o.length){var r=o.map((e=>{var t=e.getBoundingClientRect().height||e.offsetHeight||parseFloat(getComputedStyle(e).height)||0;return Math.max(0,Math.round(t))}));gsap.set(o,{height:"0%",opacity:0});t.to(o,{opacity:1,duration:.1,ease:"none"},"h+=0.48").to(o,{height:(e=>r[e]?r[e]+"px":"100%"),duration:1.4,ease:"power2.inOut",stagger:.15,clearProps:"height"},"<+=0.02")}return t};
