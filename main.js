	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async R=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const G=[...R.querySelectorAll(".infinite-gallery")];if(!G.length)return;const W=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),VW=()=>Math.max(document.documentElement.clientWidth,window.innerWidth||0),tw=n=>{let s=0,g=parseFloat(getComputedStyle(n).gap||"0")||0;n.childNodes.forEach(e=>{1===e.nodeType&&(s+=(e.getBoundingClientRect().width||0)+g)});return s},ratio=w=>{const a=parseFloat(w.getAttribute("data-ratio"));if(a&&a>0)return a;const m=(w.className||"").match(/sv-ratio-(\d+)/);if(m){const t=m[1];if("169"===t)return 16/9;if("133"===t||"43"===t)return 4/3;if("11"===t)return 1}const media=w.querySelector("img,video");if(media){const ww=parseInt(media.getAttribute("width"),10)||media.naturalWidth||media.videoWidth||0,hh=parseInt(media.getAttribute("height"),10)||media.naturalHeight||media.videoHeight||0;if(ww>0&&hh>0)return ww/hh}return 1.5},targetH=(w,track)=>{const a=parseFloat(w.getAttribute("data-size")),m=(w.className||"").match(/sv-size-(\d+)/),pct=a||(m?parseFloat(m[1]):0),th=track.clientHeight||0;if(pct>0&&th>0)return Math.max(1,Math.round(th*pct/100));const prev=w.style.height;w.style.height="";const meas=parseFloat(getComputedStyle(w).height)||w.getBoundingClientRect().height||0;w.style.height=prev;return meas>8?Math.round(meas):th>0?Math.round(.6*th):360},waitMedia=scope=>{const media=[...scope.querySelectorAll("img,video")];if(!media.length)return Promise.resolve();const ok=m=>m.naturalWidth>0&&m.naturalHeight>0||m.videoWidth>0&&m.videoHeight>0,ts=[];return Promise.race([Promise.all(media.map(m=>{if(ok(m))return Promise.resolve();const dec="function"==typeof m.decode?m.decode().catch(()=>{}):Promise.resolve(),onl=new Promise(res=>{const h=()=>{m.removeEventListener("load",h),res()};m.addEventListener("load",h,{once:!0})}),poll=new Promise(res=>{let i=0;!function loop(){if(ok(m)||i++>80)return res();setTimeout(loop,40)}()});return Promise.race([dec,onl,poll])})),new Promise(res=>ts.push(setTimeout(res,3500)))]).finally(()=>ts.forEach(clearTimeout))};G.forEach(async gal=>{const track=gal.querySelector(".infinite-gallery-wrapper");if(!track||track.__inited)return;track.__inited=!0;gal.setAttribute("data-armed","measure");const items=[...track.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!items.length){gal.setAttribute("data-armed","1");return}await waitMedia(track);const revealed=new Set,dir=("right"===(gal.dataset.direction||"left").toLowerCase()?-1:1),speed=parseFloat(gal.dataset.speed)||.6;items.forEach((el,idx)=>{const r=ratio(el),h=targetH(el,track),w=Math.max(1,Math.round(h*r));el.dataset.key=String(idx),el.dataset.ratio=String(r),el.dataset.targetH=String(h),el.dataset.revealed="0",el.style.overflow="hidden",el.style.flex="0 0 auto",el.style.willChange="height,transform",el.style.minWidth=w+"px",el.style.maxWidth=w+"px",W&&(el.style.aspectRatio=r+" / 1"),el.style.height="0px"});const removeClones=()=>[...track.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(n=>n.remove()),cloneOf=src=>{const c=src.cloneNode(!0),k=src.dataset.key||"",r=parseFloat(src.dataset.ratio)||ratio(src),h=+src.dataset.targetH||0,w=Math.max(1,Math.round(h*r));c.setAttribute("data-clone","1"),c.dataset.key=k,c.style.overflow="hidden",c.style.flex="0 0 auto",c.style.willChange="height,transform",c.style.minWidth=w+"px",c.style.maxWidth=w+"px",W&&(c.style.aspectRatio=r+" / 1"),c.style.height="0px",track.appendChild(c);return c},appendOnce=()=>{items.forEach(cloneOf)},ensureFill=()=>{removeClones(),appendOnce(),appendOnce();let guard=0;const need=3*VW();for(;tw(track)<need&&guard++<8;)appendOnce()};ensureFill();gal.setAttribute("data-armed","1");let x=0;const driftTick=(t,dtMs)=>{const dt=(dtMs||16.6667)/1e3,pxps=60*speed;x-=dir*dt*pxps;const gap=parseFloat(getComputedStyle(track).gap||"0")||0,segW=n=>(n.getBoundingClientRect().width||0)+gap;let first=track.firstElementChild,g=0;for(;first&&x<-segW(first)&&g++<50;)x+=segW(first),track.appendChild(first),first=track.firstElementChild;let last=track.lastElementChild;for(g=0;last&&x>0&&g++<50;)x-=segW(last),track.insertBefore(last,track.firstElementChild),last=track.lastElementChild;gsap.set(track,{x});const cx=(innerWidth||0)/2;track.querySelectorAll(".service-visual").forEach(node=>{const wrap=node.closest(".service-visual-wrapper");if(!wrap)return;const r=wrap.getBoundingClientRect(),o=(cx-(r.left+r.width/2))/((innerWidth)||1);node.style.setProperty("--drift",40*o+"px")})};gsap.ticker.add(driftTick),CoreUtilities.Observers.addTicker(driftTick);let revealedOnce=!1;const initialReveal=()=>{if(revealedOnce)return;revealedOnce=!0;const L=-.05*VW(),R_=1.05*VW();const originals=[...track.querySelectorAll('.service-visual-wrapper:not([data-clone])')];const onscreen=originals.filter(el=>{const r=el.getBoundingClientRect();return r.right>L&&r.left<R_}).sort((a,b)=>a.getBoundingClientRect().left-b.getBoundingClientRect().left);const offscreen=originals.filter(el=>!onscreen.includes(el));offscreen.forEach(el=>{const h=+el.dataset.targetH||0,key=el.dataset.key||"";el.style.height="",el.dataset.revealed="1",revealed.add(key);track.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${key}"]`).forEach(c=>c.style.height=h+"px")});if(onscreen.length){const tl=gsap.timeline();onscreen.forEach((el,i)=>{const h=+el.dataset.targetH||0,key=el.dataset.key||"";tl.fromTo(el,{height:0},{height:h,duration:.9,ease:"power2.out",onComplete:()=>{el.style.height="",el.dataset.revealed="1",revealed.add(key),track.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${key}"]`).forEach(c=>c.style.height=h+"px")}},.12*i)})}else{originals.forEach(el=>{const h=+el.dataset.targetH||0,key=el.dataset.key||"";el.style.height="",el.dataset.revealed="1",revealed.add(key),track.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${key}"]`).forEach(c=>c.style.height=h+"px")})}};const io=new IntersectionObserver(ents=>{ents.forEach(ent=>{if(ent.target!==gal)return;ent.isIntersecting?(initialReveal(),io.unobserve(gal)):0})},{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});io.observe(gal),CoreUtilities.Observers.addDom(io);const ro=new ResizeObserver(()=>{removeClones(),ensureFill(),items.forEach(el=>{const k=el.dataset.key||"",r=parseFloat(el.dataset.ratio)||ratio(el),h=+el.dataset.targetH||0,w=Math.max(1,Math.round(h*r));el.style.minWidth=w+"px",el.style.maxWidth=w+"px",W&&(el.style.aspectRatio=r+" / 1"),track.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${k}"]`).forEach(c=>{c.style.minWidth=w+"px",c.style.maxWidth=w+"px",W&&(c.style.aspectRatio=r+" / 1")})})});ro.observe(track),CoreUtilities.Observers.addDom(ro)})}
			})
		);
