	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=r.querySelectorAll(".infinite-gallery");if(!e.length)return;const t=async(n,o=3500)=>{const s=[...n.querySelectorAll("img")];if(!s.length)return;const i=e=>e.naturalWidth>0&&e.naturalHeight>0,a=[];try{await Promise.race([Promise.all(s.map(e=>{if(i(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),n=new Promise(t=>{const n=()=>{e.removeEventListener("load",n),t()};e.addEventListener("load",n,{once:!0})}),o=new Promise(e=>{let t=0;const n=()=>{if(i(e)||t++>80)return e();setTimeout(n,40)};n()});return Promise.race([t,n,o])})),new Promise(e=>a.push(setTimeout(e,o)))])}finally{a.forEach(clearTimeout)}};e.forEach(e=>{const n=e.querySelector(".infinite-gallery-wrapper");if(!n||n.__inited)return;n.__inited=!0,e.setAttribute("data-armed","0");const o=[...n.querySelectorAll(".service-visual-wrapper")];if(!o.length){e.setAttribute("data-armed","1");return}const s=new Set;let i=!1,a=0,l=0,c=0;const d=("right"===(e.dataset.direction||"left").toLowerCase()?-1:1),u=parseFloat(e.dataset.speed)||.6,f=t=>{const e=(t.getBoundingClientRect().width||0)+(parseFloat(getComputedStyle(n).gap||"0")||0);return e},p=()=>{n.querySelectorAll('.service-visual-wrapper[data-clone="1"]').forEach(e=>e.remove())},g=()=>{o.forEach((e,t)=>{e.style.height="";const r=e.getBoundingClientRect().width||e.offsetWidth||0;let a=parseFloat(getComputedStyle(e).height)||e.getBoundingClientRect().height||0;if(a<=0){const t=e.querySelector("img");t&&t.naturalWidth>0&&t.naturalHeight>0&&r>0&&(a=t.naturalHeight/t.naturalWidth*r)}const l=String(t);e.dataset.key=l,e.dataset.targetH=String(Math.max(0,Math.round(a))),e.style.minWidth=r+"px",e.style.maxWidth=r+"px",e.style.overflow="hidden",s.has(l)?(e.style.height="",e.dataset.revealed="1",e.dataset.revealing=""):(e.style.height="0px",e.dataset.revealed="0",e.dataset.revealing="")})},h=()=>{o.forEach(e=>{const t=e.dataset.key||"",o=+e.dataset.targetH||0,r=e.cloneNode(!0);r.setAttribute("data-clone","1"),r.dataset.key=t,r.style.minWidth=e.style.minWidth,r.style.maxWidth=e.style.maxWidth,r.style.height=o>0?o+"px":"0px",r.style.overflow="hidden",n.appendChild(r)})},m=()=>{let e=0;return n.childNodes.forEach(t=>{1===t.nodeType&&(e+=f(t))}),e},y=()=>{p(),h(),h();let t=0;for(;m()<3*e.clientWidth&&t++<8;)h()},v=()=>{e.setAttribute("data-armed","1");const t=e.getBoundingClientRect(),n=innerHeight||document.documentElement.clientHeight||0;t.bottom>0&&t.top<n&&(i=!0,cancelAnimationFrame(a),a=requestAnimationFrame(w))},w=t=>{if(!i){cancelAnimationFrame(a);return}if((t||0)-l<80){a=requestAnimationFrame(w);return}l=t||performance.now();const r=innerWidth||document.documentElement.clientWidth||0,o=-.05*r,s=1.05*r,c=[...n.querySelectorAll('.service-visual-wrapper:not([data-clone])')].filter(e=>{const t=e.getBoundingClientRect();return t.right>o&&t.left<s}).filter(e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing).sort((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left);if(c.length){const t=gsap.timeline();c.forEach((e,r)=>{const o=+e.dataset.targetH||0,i=e.dataset.key||"";e.dataset.revealing="1",t.to(e,{height:o,duration:.9,ease:"power2.out",onComplete:()=>{e.style.height="",e.dataset.revealed="1",e.dataset.revealing="",s.add(i),n.querySelectorAll('.service-visual-wrapper[data-clone][data-key="'+i+'"]').forEach(e=>{e.style.height=o+"px"})}},.12*r)})}a=requestAnimationFrame(w)},b=(e,t)=>{const r=(t||16.6667)/1e3,o=60*u;c-=d*r*o;let s=n.firstElementChild,p=0;for(;s&&c<-f(s)&&p++<50;)c+=f(s),n.appendChild(s),s=n.firstElementChild;let g=n.lastElementChild;p=0;for(;g&&c>0&&p++<50;)c-=f(g),n.insertBefore(g,n.firstElementChild),g=n.lastElementChild;gsap.set(n,{x:c});const h=(innerWidth||0)/2;n.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),o=(h-(r.left+r.width/2))/((innerWidth)||1);e.style.setProperty("--drift",40*o+"px")})},k=t=>{t.forEach(t=>{t.target===e&&(t.isIntersecting?(i=!0,cancelAnimationFrame(a),a=requestAnimationFrame(w)):(i=!1,cancelAnimationFrame(a)))})},x=new ResizeObserver(()=>{g(),p(),y(),s.forEach(e=>{const t=o.find(t=>t.dataset.key===e),r=t&&+t.dataset.targetH||0;n.querySelectorAll('.service-visual-wrapper[data-clone][data-key="'+e+'"]').forEach(e=>e.style.height=r+"px")}),v()});(async()=>{await t(n),g(),y(),v();const r=new IntersectionObserver(k,{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});r.observe(e),CoreUtilities.Observers.addDom(r),CoreUtilities.Observers.addDom({disconnect(){i=!1,cancelAnimationFrame(a)}});const o=(e,t)=>b(e,t);gsap.ticker.add(o),CoreUtilities.Observers.addTicker(o),x.observe(n),CoreUtilities.Observers.addDom(x)})()})}
			})
		);
