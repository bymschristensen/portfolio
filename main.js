	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init:function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apxBound)return;host.__apxBound=1;if(!window.gsap||!window.ScrollTrigger)return void(host.__apxBound=0);var gs=window.gsap,ST=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches)return void(host.__apxBound=0);var P=host.querySelector(".section-archive-playful");if(!P)return void(host.__apxBound=0);var un=[],trigs=[],tls=[],built=0;function on(n,e,f,o){n&&n.addEventListener(e,f,o),un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function items(){return[].slice.call(P.querySelectorAll(".archive-playful-item"))}function kill(){for(var i=0;i<trigs.length;i++)try{trigs[i]&&trigs[i].kill&&trigs[i].kill()}catch(_){}for(i=0;i<tls.length;i++)try{tls[i]&&tls[i].kill&&tls[i].kill()}catch(_){}trigs.length=0;tls.length=0;built=0}function prep(it){if(!it||it.__apxPrep)return;it.__apxPrep=1;var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),prod=q(it,".archive-playful-data-inner .archive-playful-data-product"),exp=q(it,".archive-playful-data-inner .archive-playful-data-expertise"),ln=q(it,".archive-playful-item-data-outer .case-study-url");try{v&&(v.style.overflow="hidden")}catch(_){}v&&gs.set(v,{clipPath:"inset(0% 0% 0% 0%)",willChange:"clip-path,transform"});imgs.length&&gs.set(imgs,{yPercent:-15,scale:1,transformOrigin:"50% 50%",willChange:"transform",force3D:!0});di&&gs.set(di,{y:0,willChange:"transform"});(prod||exp)&&gs.set([prod,exp],{autoAlpha:1,scaleY:1,transformOrigin:"50% 0%",willChange:"transform,opacity"});ln&&gs.set(ln,{autoAlpha:1,scaleY:1,transformOrigin:"50% 0%",willChange:"transform,opacity"})}function makeTL(it){var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),prod=q(it,".archive-playful-data-inner .archive-playful-data-product"),exp=q(it,".archive-playful-data-inner .archive-playful-data-expertise"),ln=q(it,".archive-playful-item-data-outer .case-study-url"),t30=.70,t25=.75,t22=.78,t20=.80,t17=.83,t12=.88,t0=1,tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.to(imgs,{yPercent:15,duration:1},0);di&&tl.to(di,{y:-16,duration:t22-t30},t30);(prod||exp)&&tl.to([prod,exp],{autoAlpha:0,duration:t22-t30},t30);(prod||exp)&&tl.to([prod,exp],{scaleY:0,duration:t17-t25},t25);ln&&tl.to(ln,{autoAlpha:0,duration:t17-t25},t25);ln&&tl.to(ln,{scaleY:0,duration:t12-t20},t20);imgs.length&&tl.to(imgs,{scale:1.5,duration:t0-t20},t20);v&&tl.to(v,{clipPath:"inset(0% 0% 100% 0%)",duration:t0-t20},t20);return tl}function build(){if(built)return;var list=items();if(!list.length)return;for(var i=0;i<list.length;i++){var it=list[i];prep(it);var tl=makeTL(it);tls.push(tl);trigs.push(ST.create({trigger:it,start:"top 100%",end:"top 0%",scrub:!0,animation:tl,invalidateOnRefresh:!0}))}built=1;try{ST.refresh()}catch(_){}}function enable(){if(!mm.matches||!vis())return disable();build();for(var i=0;i<trigs.length;i++)try{trigs[i]&&trigs[i].enable&&trigs[i].enable()}catch(_){}}function disable(){for(var i=0;i<trigs.length;i++)try{trigs[i]&&trigs[i].disable&&trigs[i].disable()}catch(_){}}function sync(){mm.matches&&vis()?enable():disable()}function onMQ(){sync()}function onView(){sync();try{ST.refresh()}catch(_){}}on(document,"archive:view",onView,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",onMQ):mm.addListener(onMQ)}catch(_){}un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",onMQ):mm.removeListener(onMQ)}catch(_){}});requestAnimationFrame(sync);return function destroy(){try{kill()}catch(_){ }un.forEach(function(fn){fn()});un.length=0;try{host.__apxBound=0}catch(_){}}}
			})
		);
