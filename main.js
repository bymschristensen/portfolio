	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				namespaces: ['info'],
    			selectors: [],
				init: async r=>{if(window.__recoJumpInstalled)return;window.__recoJumpInstalled=!0;const i=()=>((document.querySelector('[data-barba="container"]')?.dataset?.barbaNamespace)||"")==="info",a=()=>{const t=document.querySelector(".w-tabs");if(!t)return;const e=t.querySelector('.w-tab-link[data-w-tab="Recommendations"]')||[...t.querySelectorAll(".w-tab-link")].find(t=>(t.textContent||"").trim().toLowerCase()==="recommendations");e&&e.click()},s=()=>{const t=document.getElementById("details")||document.querySelector(".section-details");if(!t)return;const e=t.getBoundingClientRect().top+window.pageYOffset-24;window.scrollTo({top:e,behavior:"smooth"})},c=t=>{if(!i())return;const e=t.target&&(t.target.tagName==="A"?t.target:t.target.closest?.("a"));if(!e||e.classList.contains("w-tab-link"))return;const n=e.id?.startsWith?.("recommendationsOpen")||((e.getAttribute("href")||"").replace(location.origin,"").endsWith("#details"));if(!n)return;t.preventDefault(),t.stopPropagation(),t.stopImmediatePropagation?.(),a(),requestAnimationFrame(()=>requestAnimationFrame(s))};document.addEventListener("click",c,!0);const h=()=>{if(!i())return;location.hash==="#details"&&(a(),requestAnimationFrame(()=>requestAnimationFrame(s)))};h(),window.addEventListener("hashchange",h,{passive:!0});}
			})
		);
