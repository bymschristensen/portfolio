		...
		registries.pages.archive.push(
			feature({
				...
				init: function(root){
					...
					// Motion Module
					(function(){if(!window.gsap){mod.scheduleMotionRefresh=function(){};mod.resetMotion=function(){};mod.destroyMotion=function(){};return}var io=0,scan=0,raf=0,to=0,run=0,tries=0,max=45,STAG=.08,STAGF=.015,DUR=.75,DURF=.35,MIN_AFTER=3,MIN_PAD=.06,MIN_DUR=.6,last=0,GAP=140;function cleanup(){try{io&&io.disconnect&&io.disconnect()}catch(e){}io=0;try{scan&&cancelAnimationFrame(scan)}catch(e){}scan=0;try{to&&clearTimeout(to)}catch(e){}to=0}unsubs.push(cleanup);mod.destroyMotion=cleanup;function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(e){return!0}}function scope(){try{var v=mod.getView&&mod.getView();if(v==="playful"&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(v==="minimal"&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}return el.host||host}function items(sc){return[].slice.call((sc||scope()).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function isMin(sc){try{return!!(sc&&sc.classList&&sc.classList.contains("section-archive-minimal"))}catch(e){return!1}}function minVis(sc){return sc&&(sc.querySelector(".archive-minimal-item-visual")||sc.querySelector(".archive-minimal-visual")||sc.querySelector(".archive-minimal-visual-wrap"))||null}function prep(it){if(!it)return;it.__almRun=run;it.__almDone=0;try{gsap.killTweensOf(it)}catch(e){}gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"})}function prepV(v){if(!v)return;try{gsap.killTweensOf(v)}catch(e){}gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}function showVInstant(v){if(!v)return;try{gsap.killTweensOf(v)}catch(e){}gsap.set(v,{autoAlpha:1,filter:"blur(0px)",clearProps:"willChange,opacity,visibility"})}function reveal(it,d,dur,ease){if(!it||it.__almDone||!can(it))return;it.__almRun!==run&&prep(it);it.__almDone=1;gsap.to(it,{delay:d||0,y:0,autoAlpha:1,filter:"blur(0px)",duration:dur||DUR,ease:ease||"power3.out",overwrite:!0,clearProps:"y,filter,willChange,opacity,visibility"})}function revealV(sc,bd){var v=minVis(sc);if(!v)return;if(v.__almVisualSeen){showVInstant(v);return}prepV(v);gsap.to(v,{delay:bd||0,autoAlpha:1,filter:"blur(0px)",duration:MIN_DUR,ease:"power2.out",overwrite:!0,onComplete:function(){try{v.__almVisualSeen=1}catch(_){}} ,clearProps:"filter,willChange,opacity,visibility"})}function rectTop(it){try{return it.getBoundingClientRect().top}catch(e){return 1e9}}function inView(it){if(!can(it)||it.__almDone)return!1;var r=it.getBoundingClientRect();return r.top<(innerHeight||0)&&r.bottom>0}function planReveal(list){var live=[];for(var i=0;i<list.length;i++)can(list[i])&&live.push(list[i]);live.sort(function(a,b){return rectTop(a)-rectTop(b)});var vis=[],out=[];for(i=0;i<live.length;i++)(inView(live[i])?vis:out).push(live[i]);if(!vis.length)return{primary:[],secondary:out};var last=vis[vis.length-1],lastTop=rectTop(last),below=[];for(i=0;i<out.length;i++){var t=out[i];if(rectTop(t)>=lastTop)below.push(t)}below.sort(function(a,b){return rectTop(a)-rectTop(b)});var extra=below.slice(0,MIN_AFTER);var prim=vis.concat(extra);var primSet=new Set(prim);var sec=[];for(i=0;i<live.length;i++){var it=live[i];primSet.has(it)||sec.push(it)}return{primary:prim,secondary:sec}}function runPlan(sc,plan,withVisual){var p=plan.primary||[],s=plan.secondary||[];for(var i=0;i<p.length;i++)reveal(p[i],STAG*i,DUR,"power3.out");var base=STAG*p.length;for(i=0;i<s.length;i++)reveal(s[i],base+STAGF*i,DURF,"power2.out");if(withVisual&&isMin(sc)){var idx=Math.max(0,Math.min(MIN_AFTER,p.length)-1),dd=STAG*idx+MIN_PAD;revealV(sc,dd)}}function startIO(sc,list){cleanup();if("IntersectionObserver"in window){io=new IntersectionObserver(function(es){for(var i=0;i<es.length;i++){var t=es[i].target;if(es[i].isIntersecting)reveal(t,0,DURF,"power2.out")}},{threshold:.12,rootMargin:"0px 0px -10% 0px"});for(var i=0;i<list.length;i++)try{io.observe(list[i])}catch(e){}}}function hard(sc,reason){var list=items(sc);if(!list.length){if(tries++<max)return requestAnimationFrame(function(){hard(sc,reason)});return}tries=0;run++;for(var i=0;i<list.length;i++)can(list[i])&&prep(list[i]);var withVisual=isMin(sc)&&reason!=="filter";if(isMin(sc)){var v=minVis(sc);v&&v.__almVisualSeen&&(showVInstant(v),withVisual=0)}var plan=planReveal(list);runPlan(sc,plan,!!withVisual);startIO(sc,list)}function soft(sc,reason){var list=items(sc);if(!list.length)return;var withVisual=isMin(sc)&&reason!=="filter";if(isMin(sc)){var v=minVis(sc);v&&v.__almVisualSeen&&(showVInstant(v),withVisual=0)}var plan=planReveal(list);runPlan(sc,plan,!!withVisual);startIO(sc,list)}mod.resetMotion=function(){tries=0;hard(scope(),"hard")};mod.scheduleMotionRefresh=function(reason){var n=Date.now();if(n-last<GAP)return;last=n;if(raf)return;raf=requestAnimationFrame(function(){raf=0;tries=0;hard(scope(),reason||"auto");try{to&&clearTimeout(to)}catch(e){}to=setTimeout(function(){soft(scope(),reason||"auto")},180)})};on(document,"archive:view",function(){mod.scheduleMotionRefresh("view")},{passive:!0});on(document,"archive:filter:applied",function(){mod.scheduleMotionRefresh("filter")},{passive:!0});requestAnimationFrame(function(){mod.scheduleMotionRefresh("load")})})();
