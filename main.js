...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero||hero.__hsBound)return;var track=hero.querySelector(".showreel-track"),container=hero.querySelector(".showreel-container");if(!track||!container)return;hero.__hsBound=1;container.style.setProperty("--hero-showreel-x","0px");container.style.setProperty("--hero-showreel-scale","1");var S={x:0,scale:1,targetX:0,targetScale:1,raf:0,normX:0,scrollScale:1},LERP=.15,CURSOR_RANGE=.08,SCROLL_MIN=.94,SCROLL_MAX=1.06;function range(sc){var tw=track.clientWidth||0,cw=container.clientWidth||0;return Math.abs((tw-cw*sc)/2)}function clamp(x,sc){var r=range(sc);return x<-r?-r:x>r?r:x}function tick(){var tx=S.targetX,ts=S.targetScale;S.x+=(tx-S.x)*LERP;S.scale+=(ts-S.scale)*LERP;container.style.setProperty("--hero-showreel-x",S.x.toFixed(2)+"px");container.style.setProperty("--hero-showreel-scale",S.scale.toFixed(3));if(Math.abs(tx-S.x)>.2||Math.abs(ts-S.scale)>.002){S.raf=requestAnimationFrame(tick)}else S.raf=0}function apply(){var sc=S.scrollScale*(1+Math.abs(S.normX)*CURSOR_RANGE);S.targetScale=sc;S.targetX=clamp(S.normX*range(sc),sc);S.raf||(S.raf=requestAnimationFrame(tick))}function onMove(e){var r=track.getBoundingClientRect();if(!r.width)return;var nx=(e.clientX-r.left)/r.width*2-1;nx<-1?nx=-1:nx>1&&(nx=1);S.normX=nx;apply()}function onLeave(e){if(e&&e.relatedTarget)return;S.normX=0;apply()}function onScroll(){var r=track.getBoundingClientRect(),vh=window.innerHeight||document.documentElement.clientHeight||1,mid=r.top+r.height/2,dist=Math.abs(mid-vh/2),f=1-Math.min(1,dist/(vh*.7));S.scrollScale=SCROLL_MIN+f*(SCROLL_MAX-SCROLL_MIN);apply()}function onResize(){apply()}onScroll();window.addEventListener("pointermove",onMove,{passive:!0});document.addEventListener("mouseout",onLeave,{passive:!0});window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});hero.__hsDestroy=function(){try{window.removeEventListener("pointermove",onMove)}catch(_){}try{document.removeEventListener("mouseout",onLeave)}catch(_){}try{window.removeEventListener("scroll",onScroll)}catch(_){}try{window.removeEventListener("resize",onResize)}catch(_){}if(S.raf)try{cancelAnimationFrame(S.raf)}catch(_){}try{container.style.removeProperty("--hero-showreel-x"),container.style.removeProperty("--hero-showreel-scale")}catch(_){}hero.__hsBound=0}},destroy:async function(root){root=root||document;var h=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{h&&h.__hsDestroy&&h.__hsDestroy()}catch(_){}}}),
			feature({
