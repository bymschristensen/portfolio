...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),o=e.querySelectorAll(".cs-titles-inner div"),n=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t))}catch{}};a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"}),i&&gsap.set(i,{autoAlpha:0,visibility:"hidden"}),o.length&&gsap.set(o,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(a),n(i),o.forEach(n),a&&t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),i&&t.addLabel("headline",.35).call(()=>{const t=()=>{try{const s=i.parentElement;let r=null;if(s){const l=i.cloneNode(!0);l.style.cssText="position:absolute;left:-99999px;top:-99999px;visibility:hidden;pointer-events:none;display:block;white-space:pre-wrap;",document.body.appendChild(l),r=l.offsetHeight||i.offsetHeight||null,l.remove(),r&&(s.__oldMinH=s.style.minHeight||"",s.style.minHeight=r+"px")}if(window.splitAndMask&&window.animateLines){const l=splitAndMask(i);gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"}),animateLines(l.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:l.revert).call(l,l,i)}catch{}if(s&&s.__oldMinH!==undefined){s.style.minHeight=s.__oldMinH,delete s.__oldMinH}const d=getComputedStyle(i).display;"inline"===d&&(i.style.display="inline")})}else gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete:()=>{s&&s.__oldMinH!==undefined&&(s.style.minHeight=s.__oldMinH,delete s.__oldMinH)}})}catch{gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>requestAnimationFrame(()=>requestAnimationFrame(t))).catch(t)},null,"headline"),o.length&&t.to(o,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t;}
		};
