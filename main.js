...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async function(root){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;if(!window.matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero)return;hero.__heroShowreelDestroy__&&function(){try{hero.__heroShowreelDestroy__()}catch(e){}}();var container=hero.querySelector(".showreel-container");if(!container)return;var state={x:0,scale:1,targetX:0,targetScale:1,raf:0},MAX_OFFSET_PX=60,SCALE_RANGE=.06,LERP=.09;function animate(){var tx=state.targetX,ts=state.targetScale;state.x+=(tx-state.x)*LERP;state.scale+=(ts-state.scale)*LERP;container.style.setProperty("--hero-showreel-x",state.x.toFixed(2)+"px");container.style.setProperty("--hero-showreel-scale",state.scale.toFixed(3));if(Math.abs(tx-state.x)>.1||Math.abs(ts-state.scale)>.001){state.raf=requestAnimationFrame(animate)}else state.raf=0}function setTarget(normX){state.targetX=normX*MAX_OFFSET_PX;state.targetScale=1+Math.abs(normX)*SCALE_RANGE;state.raf||(state.raf=requestAnimationFrame(animate))}function onPointerMove(e){var rect=hero.getBoundingClientRect(),x=e.clientX-rect.left,nx=x/rect.width*2-1;nx<-1&&(nx=-1);nx>1&&(nx=1);setTarget(nx)}function onPointerLeave(){setTarget(0)}hero.addEventListener("pointermove",onPointerMove);hero.addEventListener("pointerleave",onPointerLeave);hero.__heroShowreelDestroy__=function(){try{hero.removeEventListener("pointermove",onPointerMove)}catch(e){}try{hero.removeEventListener("pointerleave",onPointerLeave)}catch(e){}try{state.raf&&cancelAnimationFrame(state.raf)}catch(e){}state.raf=0;try{container.style.removeProperty("--hero-showreel-x"),container.style.removeProperty("--hero-showreel-scale")}catch(e){}hero.__heroShowreelDestroy__=null}},destroy:async function(){try{var s=document.querySelector(".hero-section-showreel");s&&s.__heroShowreelDestroy__&&s.__heroShowreelDestroy__()}catch(e){}}}),
			feature({
