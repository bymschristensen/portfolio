	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;const cs=getComputedStyle(c).backgroundColor;try{ScrollTrigger.getById("csbg:mid")?.kill();ScrollTrigger.getById("csbg:end")?.kill()}catch{};c.style.backgroundColor=cs;c.style.transition="background-color .6s ease";const set=v=>gsap.set(c,{backgroundColor:v,overwrite:"auto"});ScrollTrigger.create({id:"csbg:mid",trigger:d,start:"top bottom-=15%",endTrigger:m,end:"top bottom-=15%",onEnter:()=>set("var(--colors--background)"),onLeaveBack:()=>set(cs),anticipatePin:1,fastScrollEnd:1});ScrollTrigger.create({id:"csbg:end",trigger:m,start:"top bottom-=15%",endTrigger:c,end:"bottom bottom",onEnter:()=>set("var(--colors--border)"),onLeaveBack:()=>set("var(--colors--background)"),anticipatePin:1,fastScrollEnd:1});}
			}),
			feature({
