...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async R=>{const S=R.querySelector(".hero-section-showreel"),F=S&&(S.querySelector(".showreel-outer .container")||S.querySelector(".showreel-inner")||S),V=S&&S.querySelector(".showreel-container"),C=[S&&S.querySelector(".showreel-headline"),S&&S.querySelector(".showreel-subtext"),S&&S.querySelector(".toc-outer")].filter(Boolean);if(!S||!F||!V)return;const L=(a,b,c)=>a+(b-a)*c,P=e=>Math.max(0,Math.min(1,e));let H=window.innerHeight,k=.5,B=0,W=0,z=0;const T=getComputedStyle(V).transform||"none",X="none"===T?"":T+" ";function q(){const t=V.style.transform;V.style.transform=T;const w=V.getBoundingClientRect().width||1;V.style.transform=t;return w}function N(){const r=F.getBoundingClientRect();B=r.width||1}function Y(){W=Math.max(1,(B||1)/((z||1)))}function U(){const r=S.getBoundingClientRect(),h=r.height-H;return h<=0?0:P((-r.top)/h)}function Z(){const p=U(),s=L(1,W,p),cur=z*s,room=Math.max(0,B-cur),dx=(k-.5)*room;V.style.transform=`${X}translate3d(${dx}px,0,0) scale(${s})`;const lift=-.45*H*p,op=1-p;for(let i=0;i<C.length;i++){const el=C[i];el&&(el.style.transform=`translate3d(0,${lift}px,0)`,el.style.opacity=op)}}function G(e){const r=F.getBoundingClientRect(),x=Math.max(r.left,Math.min(e.clientX,r.right));k=(x-r.left)/(r.width||1)}function J(){H=window.innerHeight;N();z=q();Y();Raf()}function Raf(){f||(f=!0,requestAnimationFrame(()=>{f=!1,Z()}))}z=q();N();Y();let f=!1;const onScroll=()=>Raf();window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",J,{passive:!0});S.addEventListener("mousemove",G,{passive:!0});const ro=new ResizeObserver(J);try{ro.observe(F),ro.observe(V)}catch{}Z();document.__capHero&&document.__capHero.kill?.();document.__capHero={kill(){try{window.removeEventListener("scroll",onScroll)}catch{}try{window.removeEventListener("resize",J)}catch{}try{S.removeEventListener("mousemove",G)}catch{}try{ro.disconnect()}catch{}try{V.style.transform=T==="none"?"":T;C.forEach(el=>{el.style.transform="";el.style.opacity=""})}catch{}}}},destroy:async()=>{try{document.__capHero?.kill?.()}catch{}document.__capHero=null}),
			feature({
