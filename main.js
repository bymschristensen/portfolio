...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async function(t){if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=Array.from(t.querySelectorAll(".infinite-gallery"));if(0===e.length)return;const n=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1 / 1"));function i(t){const e=parseFloat(getComputedStyle(t).gap||"0");return Number.isFinite(e)?e:0}function r(t){const e=i(t);let n=0;return t.childNodes.forEach(function(t){if(1===t.nodeType){const i=t.getBoundingClientRect().width||0;n+=i+e}}),n}function o(t){Array.from(t.querySelectorAll('.service-visual-wrapper[data-clone="1"]')).forEach(function(t){try{t.remove()}catch(t){}})}function a(t){const e=parseFloat(t.dataset.ratio)||function(t){const e=parseFloat(t.getAttribute("data-ratio"));if(Number.isFinite(e)&&e>0)return e;const n=(t.className||"").match(/sv-ratio-(\d+)/);if(n){const t=n[1];if("169"===t)return 16/9;if("133"===t||"43"===t)return 4/3;if("11"===t)return 1}const i=t.querySelector("img,video");if(i){const t=parseInt(i.getAttribute("width"),10)||i.naturalWidth||i.videoWidth||0,e=parseInt(i.getAttribute("height"),10)||i.naturalHeight||i.videoHeight||0;if(t>0&&e>0)return t/e}return 1.5}(t);if(t.dataset.ratio=String(e),n)t.style.aspectRatio=e+" / 1",t.style.width="";else{const n=t.getBoundingClientRect().height||parseFloat(getComputedStyle(t).height)||0,i=n>0?Math.max(1,Math.round(n*e)):0;t.style.width=i?i+"px":""}t.style.flex="0 0 auto",t.style.overflow="hidden",t.style.willChange="transform"}function s(t,e){const n=t.cloneNode(!0);return n.setAttribute("data-clone","1"),n.dataset.key=t.dataset.key||"",a(n),e.appendChild(n),n}e.forEach(async function(t){const e=t.querySelector(".infinite-gallery-wrapper");if(!e||e.__inited)return;e.__inited=!0;const n=Array.from(e.querySelectorAll(".service-visual-wrapper:not([data-clone])"));if(!n.length)return;t.setAttribute("data-armed","measure"),await function(t){const e=Array.from(t.querySelectorAll("img,video"));if(!e.length)return Promise.resolve();function n(t){return t.naturalWidth>0&&t.naturalHeight>0||t.videoWidth>0&&t.videoHeight>0}const i=e.map(function(t){if(n(t))return Promise.resolve();const e="function"==typeof t.decode?t.decode().catch(function(){}):Promise.resolve(),i=new Promise(function(e){t.addEventListener("load",function n(){t.removeEventListener("load",n),e()},{once:!0})}),r=new Promise(function(e){let i=0;!function r(){if(n(t)||i++>80)return e();setTimeout(r,40)}()});return Promise.race([e,i,r])});let r=0;const o=new Promise(function(t){r=window.setTimeout(t,3500)});return Promise.race([Promise.all(i),o]).finally(function(){clearTimeout(r)})}(e),n.forEach(function(t,e){t.dataset.key=String(e),a(t)});const c="right"===(t.dataset.direction||"left").toLowerCase()?-1:1,l=parseFloat(t.dataset.speed)||.6;function u(){o(e),n.forEach(function(t){s(t,e)}),n.forEach(function(t){s(t,e)});const t=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);let i=0;for(;r(e)<t&&i++<8;)n.forEach(function(t){s(t,e)})}u(),t.setAttribute("data-armed","1");let d=0;function f(t,n){const r=Number.isFinite(n)&&n>0?n:16.6667;d-=c*(r/1e3)*60*l;const o=i(e);function a(t){return(t.getBoundingClientRect().width||0)+o}let s=e.firstElementChild,u=0;for(;s&&d<-a(s)&&u++<50;)d+=a(s),e.appendChild(s),s=e.firstElementChild;let f=e.lastElementChild;for(u=0;f&&d>0&&u++<50;)d-=a(f),e.insertBefore(f,e.firstElementChild),f=e.lastElementChild;gsap.set(e,{x:d});const h=(window.innerWidth||0)/2;e.querySelectorAll(".service-visual").forEach(function(t){const e=t.closest(".service-visual-wrapper");if(!e)return;const n=e.getBoundingClientRect(),i=(h-(n.left+n.width/2))/(window.innerWidth||1);t.style.setProperty("--drift",String(40*i)+"px")})}gsap.ticker.add(f),CoreUtilities.Observers.addTicker(f);let h=!1;CoreUtilities.Observers.addDom(new IntersectionObserver(function(e,i){e.forEach(function(e){if(e.target!==t)return;if(!e.isIntersecting)return;if(h)return;h=!0;const r=parseInt(t.getAttribute("data-reveal"),10),o=Number.isFinite(r)&&r>0?r:7,a=window.innerWidth||document.documentElement.clientWidth||0,s=window.innerHeight||document.documentElement.clientHeight||0,c=.12*a,l=a/2,u=n.map(function(t){const e=t.getBoundingClientRect();return{el:t,rect:e,area:Math.max(0,Math.min(e.right,a)-Math.max(e.left,0))*Math.max(0,Math.min(e.bottom,s)-Math.max(e.top,0)),dist:Math.abs((e.left+e.right)/2-l),inView:e.right>-c&&e.left<a+c&&e.bottom>0&&e.top<s?1:0}});let d=u.filter(function(t){return 1===t.inView}).sort(function(t,e){return t.dist-e.dist}).slice(0,o);if(d.length<o){const t=u.filter(function(t){return 0===t.inView}).sort(function(t,e){return t.dist-e.dist}).slice(0,o-d.length);d=d.concat(t)}const f=new Set(d.map(function(t){return t.el}));u.forEach(function(t){if(!f.has(t.el)){try{gsap.killTweensOf(t.el)}catch(t){}t.el.style.height=""}}),d.forEach(function(t,e){const n=t.rect.height||parseFloat(getComputedStyle(t.el).height)||0;n&&(t.el.style.height="0px",gsap.to(t.el,{height:n,duration:.9,ease:"power2.out",delay:.12*e,onComplete:function(){t.el.style.height=""}}))}),i.unobserve(t)})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0})).observe(t);const m=new ResizeObserver(function(){n.forEach(a),o(e),u()});m.observe(e),CoreUtilities.Observers.addDom(m)})}
			})
		);
