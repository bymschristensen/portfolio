...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archivePlayfulExperience",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".archive-playful-item",".archive-playful-visuals",".archive-playful-visual-img",".archive-playful-data-inner",".archive-playful-data-product",".archive-playful-data-expertise",".archive-playful-item-data-outer .case-study-url"],init:function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apx2Bound)return;host.__apx2Bound=1;if(!window.gsap||!window.ScrollTrigger)return void(host.__apx2Bound=0);var gs=window.gsap,ST=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches)return void(host.__apx2Bound=0);var P=host.querySelector(".section-archive-playful");if(!P)return void(host.__apx2Bound=0);var un=[],tr=[],raf=0,itemsData=[],ri=0,rr=0;function on(n,e,f,o){n&&n.addEventListener(e,f,o),un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function kill(){for(var i=0;i<tr.length;i++)try{tr[i]&&tr[i].kill&&tr[i].kill()}catch(_){ }tr.length=0;itemsData.length=0}function pickDataParts(it){var pd=qs(it,".archive-playful-data-product"),pe=qs(it,".archive-playful-data-expertise");if(!pd.length&&!pe.length){pd=qs(it,".archive-playful-data-inner .headline-xxs");pe=qs(it,".archive-playful-data-inner .body-sm")}return pd.concat(pe)}function pickLink(it){return q(it,".archive-playful-item-data-outer .case-study-url")||q(it,".archive-playful-item-data .shortcut-link")}function bindRefreshHooks(){if(ri||rr)return;ri=function(){try{for(var i=0;i<itemsData.length;i++){var d=itemsData[i];d&&d.st&&(d._p=d.st.progress)}for(i=0;i<itemsData.length;i++){d=itemsData[i];d&&d.tl&&d.tl.progress(0)} }catch(_){}};rr=function(){try{for(var i=0;i<itemsData.length;i++){var d=itemsData[i];if(!d||!d.tl||!d.st)continue;d.tl.progress(d.st.progress)} }catch(_){}};ST.addEventListener("refreshInit",ri);ST.addEventListener("refresh",rr)}function unbindRefreshHooks(){try{ri&&ST.removeEventListener("refreshInit",ri)}catch(_){ }try{rr&&ST.removeEventListener("refresh",rr)}catch(_){ }ri=0;rr=0}function build(){kill();if(!mm.matches||!vis())return;var items=[].slice.call(P.querySelectorAll(".archive-playful-item"));if(!items.length)return;bindRefreshHooks();for(var i=0;i<items.length;i++){var it=items[i],v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickDataParts(it),ln=pickLink(it);try{imgs.length&&gs.set(imgs,{willChange:"transform"});v&&gs.set(v,{willChange:"height",overflow:"hidden"});di&&gs.set(di,{willChange:"margin"});parts.length&&gs.set(parts,{willChange:"opacity,height"});ln&&gs.set(ln,{willChange:"opacity,height",overflow:"hidden"})}catch(_){ }var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.fromTo(imgs,{yPercent:-15},{yPercent:15,duration:1},0);(parts.length||di)&&tl.to(parts,{opacity:0,duration:.08},.70),di&&tl.to(di,{marginTop:0,marginBottom:0,duration:.08},.70);(parts.length||ln)&&tl.to(parts,{height:0,duration:.08},.75),ln&&tl.to(ln,{opacity:0,duration:.08},.75);ln&&tl.to(ln,{height:0,duration:.08},.80);(imgs.length||v)&&tl.to(imgs,{scale:1.5,duration:.20},.80),v&&tl.to(v,{height:0,duration:.20},.80);var st=ST.create({trigger:it,start:"top 100%",end:"top 0%",scrub:!0,animation:tl,invalidateOnRefresh:!0});tr.push(st);itemsData.push({it:it,st:st,tl:tl,_p:0})}try{ST.refresh()}catch(_){}}function sched(){raf||(raf=requestAnimationFrame(function(){raf=0;build()}))}function sync(){sched()}on(document,"archive:view",sync,{passive:!0});on(document,"archive:filter:applied",sync,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sync):mm.addListener(sync)}catch(_){ }un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sync):mm.removeListener(sync)}catch(_){}});requestAnimationFrame(function(){setTimeout(build,0)});return function destroy(){try{kill()}catch(_){ }try{unbindRefreshHooks()}catch(_){ }un.forEach(function(fn){fn()});un.length=0;try{host.__apx2Bound=0}catch(_){}}}})}
		);
