...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){var t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),s=e.querySelector(".cs-titles-inner .headline-m"),n=function(o){if(!o)return;try{o.removeAttribute("data-entry-hidden"),o.removeAttribute("hidden"),o.removeAttribute("aria-hidden"),o.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(function(r){o.classList&&o.classList.remove(r)})}catch{}};function l(){var r=(window.CoreUtilities&&window.CoreUtilities.Fonts&&window.CoreUtilities.Fonts.ready)?window.CoreUtilities.Fonts.ready():Promise.resolve();var d=new Promise(function(c){var u=0;(function f(){if(window.SplitText)return c();u++<40?setTimeout(f,50):c()})()});return Promise.race([r,d])}a&&(gsap.set(a,{autoAlpha:0,y:100,filter:"blur(10px)"}),n(a),t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.75,ease:"power2.out"},0));if(i){gsap.set(i,{autoAlpha:0,visibility:"hidden"}),n(i),s&&(gsap.set(s,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(s));t.addLabel("h",.15).call(function(){l().then(function(){requestAnimationFrame(function(){requestAnimationFrame(function(){var o=splitAndMask(i),r=o.lines&&o.lines.length?o.lines:[i],d=.8+.08*Math.max(0,r.length-1);gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"});s&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.5,ease:"power2.out"},"h+"+Math.max(0,d-.2));animateLines(r).eventCallback("onComplete",function(){try{(window.safelyRevertSplit?window.safelyRevertSplit:o.revert).call(o,o,i)}catch{}try{"inline"===getComputedStyle(i).display&&(i.style.display="inline")}catch{}})})})})},null,"h")}return t}
		};
