...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async function(R){var S=R.querySelector(".hero-section-showreel");if(!S)return;var F=S.querySelector(".showreel-outer .container")||S.querySelector(".showreel-inner")||S,V=S.querySelector(".showreel-container"),C=[S.querySelector(".showreel-headline"),S.querySelector(".showreel-subtext"),S.querySelector(".toc-outer")].filter(Boolean);if(!F||!V)return;function L(a,b,c){return a+(b-a)*c}function P(e){return Math.max(0,Math.min(1,e))}var H=window.innerHeight,k=.5,B=0,W=1,z=0,T=(getComputedStyle(V).transform||"none"),X=T==="none"?"":T+" ";function q(){var t=V.style.transform;V.style.transform=T;var w=V.getBoundingClientRect().width||1;V.style.transform=t;return w}function N(){var r=F.getBoundingClientRect();B=r.width||1}function Y(){W=Math.max(1,(B||1)/(z||1))}function U(){var r=S.getBoundingClientRect(),h=r.height-H;return h<=0?0:P((-r.top)/h)}function Z(){var p=U(),s=L(1,W,p),cur=z*s,room=Math.max(0,B-cur),dx=(k-.5)*room;V.style.transform=X+"translate3d("+dx+"px,0,0) scale("+s+")";var lift=-.45*H*p,op=1-p,i=0;for(;i<C.length;i++){var el=C[i];if(el){el.style.transform="translate3d(0,"+lift+"px,0)";el.style.opacity=op}}}function G(e){var r=F.getBoundingClientRect(),cx=Math.max(r.left,Math.min((e.clientX||0),r.right));k=(cx-r.left)/(r.width||1)}function J(){H=window.innerHeight;N();z=q();Y();Raf()}var f=!1;function Raf(){if(f)return;f=!0;requestAnimationFrame(function(){f=!1;Z()})}z=q();N();Y();window.addEventListener("scroll",Raf,{passive:!0});window.addEventListener("resize",J,{passive:!0});S.addEventListener("pointermove",G,{passive:!0});S.addEventListener("mousemove",G,{passive:!0});var ro;try{ro=new ResizeObserver(J);ro.observe(F);ro.observe(V)}catch(e){}Z();if(document.__capHero&&document.__capHero.kill){document.__capHero.kill()}document.__capHero={kill:function(){try{window.removeEventListener("scroll",Raf)}catch(e){}try{window.removeEventListener("resize",J)}catch(e){}try{S.removeEventListener("pointermove",G)}catch(e){}try{S.removeEventListener("mousemove",G)}catch(e){}try{ro&&ro.disconnect()}catch(e){}try{V.style.transform=T==="none"?"":T;var i=0;for(;i<C.length;i++){C[i].style.transform="";C[i].style.opacity=""}}catch(e){}}}},destroy:async function(){try{if(document.__capHero&&document.__capHero.kill){document.__capHero.kill()}}catch(e){}document.__capHero=null}),
			feature({
