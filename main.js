...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=[...r.querySelectorAll(".infinite-gallery")];if(!e.length)return;const t=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),o=e=>parseFloat(getComputedStyle(e).gap||"0")||0,a=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const o=(e.className||"").match(/sv-ratio-(\d+)/);if(o){const e=o[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const r=e.querySelector("img,video");if(r){const e=parseInt(r.getAttribute("width"),10)||r.naturalWidth||r.videoWidth||0,t=parseInt(r.getAttribute("height"),10)||r.naturalHeight||r.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5},n=(e,r)=>{const t=parseFloat(e.getAttribute("data-size")),o=r.closest(".section-single-service"),a=o?o.clientHeight||innerHeight:innerHeight;return t&&a?Math.max(1,Math.round(a*t/100)):Math.round(.6*(a||innerHeight))},i=e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const o=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,r=[];return Promise.race([Promise.all(t.map((e=>{if(o(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),r=new Promise((t=>{const o=()=>{e.removeEventListener("load",o),t()};e.addEventListener("load",o,{once:!0})})),a=new Promise((t=>{let r=0;!function n(){if(o(e)||r++>80)return t();setTimeout(n,40)}()}));return Promise.race([t,r,a])}))),new Promise((e=>r.push(setTimeout(e,3500))))]).finally((()=>r.forEach(clearTimeout)))},s=e=>{const t=o(e);let a=0;return e.childNodes.forEach((e=>{1===e.nodeType&&(a+=(e.getBoundingClientRect().width||0)+t)})),a},l=e=>{[...e.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach((e=>e.remove()))},c=(e,r)=>{const o=e.cloneNode(!0);o.setAttribute("data-clone","1"),o.dataset.key=e.dataset.key||"";const i=parseFloat(e.dataset.ratio)||a(e),s=+e.dataset.targetH||0,l=Math.max(1,Math.round(s*i));return o.style.overflow="hidden",o.style.flex="0 0 auto",o.style.willChange="height,transform",o.style.minWidth=l+"px",o.style.maxWidth=l+"px",t&&(o.style.aspectRatio=i+" / 1"),o.style.height=s?s+"px":"",r.appendChild(o),o};e.forEach((async e=>{const d=e.querySelector(".infinite-gallery-wrapper");if(!d||d.__inited)return;d.__inited=!0;const h=[...d.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!h.length)return;e.setAttribute("data-armed","measure"),await i(d),h.forEach(((e,t)=>{const o=a(e),i=n(e,d),s=Math.max(1,Math.round(i*o));e.dataset.key=String(t),e.dataset.ratio=String(o),e.dataset.targetH=String(i),e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.willChange="height,transform",e.style.minWidth=s+"px",e.style.maxWidth=s+"px",t&&(e.style.aspectRatio=o+" / 1"),e.style.height=i+"px"}));const u=("right"===(e.dataset.direction||"left").toLowerCase()?-1:1),g=parseFloat(e.dataset.speed)||.6,m=()=>{l(d),h.forEach((e=>c(e,d))),h.forEach((e=>c(e,d)));let e=0;const t=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;s(d)<t&&e++<8;)h.forEach((e=>c(e,d)))};m(),e.setAttribute("data-armed","1");let f=0;const v=(e,t=16.6667)=>{const a=(t||16.6667)/1e3*60;f-=u*a*g;const n=o(d),i=e=>((e.getBoundingClientRect().width||0)+n);let s=d.firstElementChild,l=0;for(;s&&f<-i(s)&&l++<50;)f+=i(s),d.appendChild(s),s=d.firstElementChild;let c=d.lastElementChild,p=0;for(;c&&f>0&&p++<50;)f-=i(c),d.insertBefore(c,d.firstElementChild),c=d.lastElementChild;gsap.set(d,{x:f});const y=(innerWidth||0)/2;d.querySelectorAll(".service-visual").forEach((e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const o=t.getBoundingClientRect(),a=(y-(o.left+o.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*a+"px")}))};gsap.ticker.add(v),CoreUtilities.Observers.addTicker(v);let p=!1;const y=CoreUtilities.Observers.addDom(new IntersectionObserver((t=>{t.forEach((t=>{if(t.target!==e||!t.isIntersecting||p)return;p=!0;const o=0,a=innerHeight,n=0,i=innerWidth,s=h.map((e=>{const t=e.getBoundingClientRect(),r=Math.max(0,Math.min(t.bottom,a)-Math.max(t.top,o))*Math.max(0,Math.min(t.right,i)-Math.max(t.left,n)),l=Math.abs((t.top+t.bottom)/2-innerHeight/2);return{w:e,area:r,dist:l}})).filter((e=>e.area>1)).sort(((e,t)=>e.dist-t.dist)).slice(0,7).map((e=>e.w));s.forEach(((t,o)=>{const a=+t.dataset.targetH||t.clientHeight||0;a&&(gsap.set(t,{height:0}),gsap.to(t,{height:a,duration:.9,ease:"power2.out",delay:.12*o,onComplete:()=>{t.style.height=a+"px";const e=t.dataset.key||"";e&&d.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${e}"]`).forEach((t=>t.style.height=a+"px"))}}))})),y.unobserve(e)}))}),{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));y.observe(e);const x=new ResizeObserver((()=>{l(d),h.forEach((e=>{const t=parseFloat(e.dataset.ratio)||a(e),o=n(e,d);e.dataset.targetH=String(o);const i=Math.max(1,Math.round(o*t));e.style.minWidth=i+"px",e.style.maxWidth=i+"px",t&&(e.style.aspectRatio=t+" / 1"),e.style.height=o+"px"})),m()}));x.observe(d),CoreUtilities.Observers.addDom(x)}))}
			})
		);
