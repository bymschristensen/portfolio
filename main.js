	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				stage: 'main',
				namespaces: '*', 
				selectors: ['.cs-details', '.cs-morework'],
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})};const mk=(sec,key)=>{let t=sec.querySelector(`:scope>[data-csbg="${key}"]`);return t||(t=document.createElement("div"),t.setAttribute("data-csbg",key),t.style.cssText="position:absolute;left:0;top:0;width:1px;height:1px;pointer-events:none;opacity:0;",sec.style.position=sec.style.position||"relative",sec.prepend(t)),t};const ref=()=>requestAnimationFrame(()=>ScrollTrigger.refresh());ScrollTrigger.create({id:"csbg:details",trigger:mk(d,"details"),start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),invalidateOnRefresh:!0,refreshPriority:100});ScrollTrigger.create({id:"csbg:more",trigger:mk(m,"more"),start:"top 80%",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),invalidateOnRefresh:!0,refreshPriority:110});ref();setTimeout(ref,160);setTimeout(ref,520);setTimeout(ref,1100);const g=c.querySelector(".cs-gallery");if(g&&"ResizeObserver"in window){let h=g.getBoundingClientRect().height,ro=new ResizeObserver(()=>{const nh=g.getBoundingClientRect().height;if(Math.abs(nh-h)>1){h=nh;ref();setTimeout(ref,160)}});try{ro.observe(g)}catch{}setTimeout(()=>{try{ro.disconnect()}catch{}},8000);return()=>{try{ro.disconnect()}catch{}}}}
		   	}),
			feature({
