	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: function(r){if(!window.gsap||!window.ScrollTrigger)return;r=r||document;var S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;S.forEach((sec,i)=>{sec.style.filter="contrast(100%) blur(0px)";ScrollTrigger.create({id:"servicePin:"+i,trigger:sec,start:i===0?"top top":"bottom bottom",end:()=>sec.offsetHeight+innerHeight*0.25,pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0,anticipatePin:1,onUpdate:self=>{var p=self.progress||0;c=100-90*p;b=10*p;sec.style.filter="contrast("+c+"%) blur("+b+"px)";}})});ScrollTrigger.addEventListener("refreshInit",()=>{S.forEach(sec=>sec.style.removeProperty("transform"))});}
			}),
			feature({
				id: "productGallery",
				stage: "main",
