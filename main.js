	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async r=>{const ns=r.dataset?.barbaNamespace||r.getAttribute("data-barba-namespace")||"",D=document;D.querySelectorAll(".nav-primary-wrap").forEach(w=>{if(w.__navReady)return;w.__navReady=!0;const btn=w.querySelector(".nav-button-menu"),label=w.querySelector(".nav-button-text"),phone=w.querySelector(".phone-number"),mins=w.querySelectorAll(".button-minimal-darkmode"),links=w.querySelectorAll(".menu-link"),lineText=w.querySelector(".ta-one-menu"),wrap=w.querySelector(".menu-wrapper"),box=w.querySelector(".menu-container"),fBtn=w.querySelector(".nav-button-filter"),fBox=w.querySelector(".filters-container"),fItems=w.querySelectorAll(".modal-filters-item"),fCap=w.querySelector(".modal-filters-caption"),fl1=w.querySelector(".filter-line-1"),fl2=w.querySelector(".filter-line-2");if(!(btn&&label&&phone&&mins.length&&links.length&&lineText&&wrap&&box))return;const lock=e=>{document.body.style.overflow=e?"hidden":""},showMenu=()=>{wrap.style.display="flex",box.style.display="flex",wrap.setAttribute("data-open","1"),box.setAttribute("data-open","1"),lock(!0)},hardHideMenu=()=>{box&&(box.style.display="none",box.removeAttribute("data-open")),wrap&&(wrap.style.display="none",wrap.removeAttribute("data-open")),label&&label.dataset&&label.dataset.orig&&(label.textContent=label.dataset.orig),lock(!1)},showFilter=()=>{wrap.style.display="flex",wrap.setAttribute("data-open","1"),fBox&&(fBox.style.display="flex",fBox.setAttribute("data-open","1")),lock(!0)},hardHideFilter=()=>{fBox&&(fBox.style.display="none",fBox.removeAttribute("data-open")),wrap.style.display="none",wrap.removeAttribute("data-open"),lock(!1)};wrap.style.display="none",box.style.display="none",fBox&&(fBox.style.display="none"),label.dataset.orig=label.textContent||"Menu";let split=null;const tl=gsap.timeline({paused:!0}).call(()=>{split=splitAndMask(lineText),animateLines(split.lines).eventCallback("onComplete",()=>{safelyRevertSplit(split,lineText),split=null})},null,0).from(phone,{opacity:0,duration:.4},">").from(mins,{opacity:0,duration:.4,stagger:.15},"<").from(links,{opacity:0,yPercent:240,duration:.45,stagger:.12},"<").to(label,{text:"Close",duration:.25},"<");tl.eventCallback("onReverseComplete",hardHideMenu);let guardFn=null;const armGuard=(t,hide)=>{guardFn&&(gsap.ticker.remove(guardFn),guardFn=null),guardFn=()=>{if(!t.reversed())return;(t.time()<=.02||t.progress()<=.02||!t.isActive())&&(hide(),gsap.ticker.remove(guardFn),guardFn=null)},gsap.ticker.add(guardFn),gsap.delayedCall(.65,()=>{t.reversed()&&(hide(),guardFn&&(gsap.ticker.remove(guardFn),guardFn=null))})};let state=null;btn.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),"menu"===state?(tl.timeScale(2).reverse(),armGuard(tl,hardHideMenu)):"filter"===state?(tlf&&tlf.timeScale(2).reverse(),armGuard(tlf,hardHideFilter),gsap.delayedCall(.02,()=>{state="menu",showMenu(),tl.timeScale(1).play(0)})):(state="menu",showMenu(),tl.timeScale(1).play(0))},{passive:!1});let tlf=null;fBtn&&fBox&&fItems.length&&(ns==="archive"?(fBtn.style.display="flex",gsap.to(fBtn,{opacity:1,duration:.2})):gsap.to(fBtn,{opacity:0,duration:.2,onComplete:()=>fBtn.style.display="none"}),tlf=gsap.timeline({paused:!0}).to(fBox,{opacity:1,duration:.4,ease:"power2.out"},0).to(fl1,{rotation:45,transformOrigin:"center",duration:.35},"<").to(fl2,{rotation:-45,marginTop:"-4px",transformOrigin:"center",duration:.35},"<").from(fCap,{opacity:0,duration:.5},"<").from(fItems,{opacity:0,duration:.8,stagger:.2},"<"),tlf.eventCallback("onReverseComplete",hardHideFilter),fBtn.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),"filter"===state?(tlf.timeScale(2).reverse(),armGuard(tlf,hardHideFilter)):"menu"===state?(tl.timeScale(2).reverse(),armGuard(tl,hardHideMenu),gsap.delayedCall(.02,()=>{state="filter",showFilter(),tlf.timeScale(1).play(0)})):(state="filter",showFilter(),tlf.timeScale(1).play(0))},{passive:!1}));D.addEventListener("keydown",ev=>{"Escape"===ev.key&&("menu"===state?(tl.timeScale(2).reverse(),armGuard(tl,hardHideMenu)):"filter"===state&&tlf&&(tlf.timeScale(2).reverse(),armGuard(tlf,hardHideFilter)))});w._menuTimeline=tl,w._filterTimeline=tlf}),NavigationManager.attachMenuLocks(document);}
			}),
