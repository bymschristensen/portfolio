	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				//init: async root=>{const L=[...root.querySelectorAll(".accordion-list")];if(!L.length)return;let to;const rf=()=>{clearTimeout(to),to=setTimeout(function(){try{ScrollTrigger.refresh()}catch{}},120)};L.forEach(list=>{const items=[...list.querySelectorAll(".accordion-subservice, .accordion-mindset, .accordion-quote")];if(!items.length)return;items.forEach(it=>{if(it._accordionBound)return;it._accordionBound=!0;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:void 0});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":void 0,duration:.5,onComplete:function(){gsap.set(ct,{maxHeight:"none"})}},0);isQ&&qi&&tl.from(qi,{opacity:0,duration:.4},0);tl.eventCallback("onComplete",rf);tl.eventCallback("onReverseComplete",rf);it._accordionTimeline=tl;it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");items.forEach(o=>{if(o!==it){const oh=o.querySelector(".accordion-header"),ot=o._accordionTimeline;oh&&ot&&oh.classList.contains("accordion-active")&&!ot.isActive()&&(oh.classList.remove("accordion-active"),ot.reverse())}});open?(gsap.set(ct,{maxHeight:ct.offsetHeight}),tl.reverse()):(gsap.set(ct,{maxHeight:0}),tl.play());h.classList.toggle("accordion-active",!open)})})})}
				init: async root=>{const L=[...root.querySelectorAll(".accordion-list")];if(!L.length)return;let to;const rf=()=>{clearTimeout(to);to=setTimeout(()=>{try{ScrollTrigger.refresh()}catch(e){}},120)};L.forEach(list=>{const items=[...list.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];if(!items.length)return;items.forEach(it=>{if(it._accBound)return;it._accBound=1;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:null});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":null,duration:.5,onComplete:()=>{gsap.set(ct,{maxHeight:"none"})}},0);if(isQ&&qi)tl.from(qi,{opacity:0,duration:.4},0);tl.eventCallback("onComplete",rf);tl.eventCallback("onReverseComplete",rf);it._accTL=tl;it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");items.forEach(o=>{if(o!==it){const oh=o.querySelector(".accordion-header"),ot=o._accTL;if(oh&&ot&&oh.classList.contains("accordion-active")&&!ot.isActive()){oh.classList.remove("accordion-active");ot.reverse();}}});open?(gsap.set(ct,{maxHeight:ct.offsetHeight}),tl.reverse()):(gsap.set(ct,{maxHeight:0}),tl.play());h.classList.toggle("accordion-active",!open);});});});}
			}),
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var s=[].slice.call(r.querySelectorAll(".section-single-service"));if(!s.length)return;var p=ScrollTrigger._ppsPins||[];ScrollTrigger._ppsPins=p;s.forEach(function(e){var t=e.offsetHeight<window.innerHeight,l=gsap.timeline({scrollTrigger:{trigger:e,start:t?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}}).to(e,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0);l.scrollTrigger&&p.indexOf(l.scrollTrigger)===-1&&p.push(l.scrollTrigger)});if(ScrollTrigger._ppsHooksInstalled)return;ScrollTrigger._ppsHooksInstalled=!0;ScrollTrigger.addEventListener("refreshInit",function(){var r=ScrollTrigger._ppsPins||[];r.forEach(function(e){e&&(e._ppsPrevProgress=e.progress)})});ScrollTrigger.addEventListener("refresh",function(){var r=ScrollTrigger._ppsPins||[];r.forEach(function(e){if(!e||!e.animation)return;var t=e._ppsPrevProgress;if(null==t)return;e.animation.progress(t)})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
