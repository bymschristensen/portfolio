	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async(root)=>{
					const scope=document;scope.querySelectorAll('.nav-primary-wrap').forEach(wrap=>{if(wrap._navInit)return;wrap._navInit=!0;const btn=wrap.querySelector('.nav-button-menu');const label=wrap.querySelector('.nav-button-text');const phone=wrap.querySelector('.phone-number');const minis=wrap.querySelectorAll('.button-minimal-darkmode');const links=wrap.querySelectorAll('.menu-link');const lineText=wrap.querySelector('.ta-one-menu');const menuWrapper=wrap.querySelector('.menu-wrapper');const menuContainer=wrap.querySelector('.menu-container');if(!(btn&&label&&phone&&minis.length&&links.length&&lineText&&menuWrapper&&menuContainer))return;btn.style.pointerEvents='auto';menuWrapper.style.display='none';menuContainer.style.display='none';label.dataset.orig=label.textContent||'Menu';let split=null;const tl=gsap.timeline({paused:!0}).call(()=>{split=splitAndMask(lineText);animateLines(split.lines).eventCallback('onComplete',()=>{safelyRevertSplit(split,lineText);split=null;});},null,0).from(phone,{opacity:0,duration:.4},'>').from(minis,{opacity:0,duration:.4,stagger:.15},'<').from(links,{opacity:0,yPercent:240,duration:.45,stagger:.12},'<').to(label,{text:'Close',duration:.25},'<');tl.eventCallback('onReverseComplete',()=>{label.textContent=label.dataset.orig;});let open=false;const openMenu=()=>{document.body.style.overflow='hidden';menuWrapper.style.display='flex';menuContainer.style.display='flex';tl.timeScale(1).play(0);open=true;};const closeMenu=()=>{tl.timeScale(1.8).reverse();document.body.style.overflow='';open=false;};btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();open?closeMenu():openMenu();},{passive:false});wrap._menuTimeline=tl;wrap._menuButton=btn;});
