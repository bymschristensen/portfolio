...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),n=e.querySelector(".cs-headline"),s=e.querySelectorAll(".cs-titles-inner div");i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"}),s.length&&gsap.set(s,{autoAlpha:0,y:20,filter:"blur(10px)"});if(n){const e="inline"===getComputedStyle(n).display,o=n.style.display,a=n.getBoundingClientRect().height||n.scrollHeight||0;a>0&&(n.style.minHeight=a+"px"),n.style.visibility="hidden",t.addLabel("headline",.35).call(()=>{n.style.visibility="visible";try{if(!window.SplitText||!window.splitAndMask||!window.animateLines){gsap.to(n,{y:0,opacity:1,duration:.6,ease:"power2.out"});return}const t=splitAndMask(n);animateLines(t.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?safelyRevertSplit:t.revert).call(t,t,n)}catch{}n.style.minHeight="",e&&(n.style.display="inline"),o&&(n.style.display=o)})}catch{gsap.to(n,{y:0,opacity:1,duration:.6,ease:"power2.out"}),n.style.minHeight="",e&&(n.style.display="inline"),o&&(n.style.display=o)}},null,"headline")}return i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),s.length&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
