...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.capabilities=function(e,t){e=e||document,t=t||{};var d=!!t.delayHero,n=gsap.timeline(),h=e.querySelector(".showreel-headline"),c=e.querySelector(".showreel-container");if(!h&&!c)return n;var s=d?.2:0;n.addLabel("heroStart",s);if(c){gsap.set(c,{autoAlpha:1,scale:0,transformOrigin:"100% 100%",willChange:"transform"});n.to(c,{scale:1,duration:1.15,ease:"power2.out"},"heroStart+=0.55")}if(h){gsap.set(h,{autoAlpha:1,clearProps:"visibility"});n.call(function(){var e=window.CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve();e.then(function(){if(window.SplitText&&window.splitAndMask&&window.animateLines&&window.safelyRevertSplit)try{var e=splitAndMask(h);animateLines(e.lines).eventCallback("onComplete",function(){safelyRevertSplit(e,h)})}catch(e){gsap.fromTo(h,{y:18,opacity:0,filter:"blur(10px)"},{y:0,opacity:1,filter:"blur(0px)",duration:.75,ease:"power2.out"})}else gsap.fromTo(h,{y:18,opacity:0,filter:"blur(10px)"},{y:0,opacity:1,filter:"blur(0px)",duration:.75,ease:"power2.out"})})},null,"heroStart")}return n};
