...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveViewToggle",stage:"main",namespaces:["archive"],selectors:[".archive-switch-block",".section-archive-playful",".section-archive-minimal"],init:function(r){r=r||document;var e=r.querySelector(".archive-switch-block");if(!e||e.__avBound)return;e.__avBound=1;var t=document.documentElement,a=window.matchMedia("(min-width: 1024px)"),i="archiveViewDesktop";function n(){if(a.matches){e.style.display="";var r=null;try{r=localStorage.getItem(i)}catch(e){}var n=r==="minimal"||r==="playful"?r:"playful";t.setAttribute("data-archive-view",n);e.setAttribute("data-view",n)}else e.style.display="none",t.removeAttribute("data-archive-view")}function l(r){r=r==="minimal"?"minimal":"playful";t.setAttribute("data-archive-view",r);try{localStorage.setItem(i,r)}catch(e){}e.setAttribute("data-view",r)}function o(){return t.getAttribute("data-archive-view")==="minimal"?"minimal":"playful"}n();a.addEventListener?a.addEventListener("change",n):a.addListener(n);e.addEventListener("click",function(){a.matches&&l(o()==="playful"?"minimal":"playful")},{passive:!0})}}),
			feature({id:"archiveController",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-playful-list",".archive-minimal-list",".archive-playful-item",".archive-minimal-item",".archive-categories .cms-categories",".archive-switch-block",".archive-switch",".archive-switch-dot","#archive-filter-all","#archive-filter-appswebapps","#archive-filter-websites","#archive-filter-ecommerce","#archive-filter-platforms-systems","#archive-filter-other","#nav-archive-filter-all","#nav-archive-filter-appswebapps","#nav-archive-filter-websites","#nav-archive-filter-ecommerce","#nav-archive-filter-platforms-systems","#nav-archive-filter-other"],init:function(e){if(!window.gsap)return;e=e||document;var t=e.querySelector('[data-barba="container"]')||e;if(t.__archiveControllerBound)return;t.__archiveControllerBound=1;var P=t.querySelector(".section-archive-playful"),M=t.querySelector(".section-archive-minimal"),pl=t.querySelector(".archive-playful-list"),ml=t.querySelector(".archive-minimal-list"),pi=[].slice.call(t.querySelectorAll(".archive-playful-item")),mi=[].slice.call(t.querySelectorAll(".archive-minimal-item")),blk=t.querySelector(".archive-switch-block"),dot=t.querySelector(".archive-switch-dot");if(!P||!M||!pl||!ml||!blk)return;function norm(s){return(s||"").toLowerCase().replace(/\s+/g," ").trim()}function cats(it){try{return[].slice.call(it.querySelectorAll(".archive-categories .cms-categories")).map(function(n){return norm(n.textContent)}).filter(Boolean)}catch(_){return[]}}var map=new WeakMap;pi.forEach(function(it){map.set(it,cats(it))});mi.forEach(function(it){map.set(it,cats(it))});var fA=["#archive-filter-all","#archive-filter-appswebapps","#archive-filter-websites","#archive-filter-ecommerce","#archive-filter-platforms-systems","#archive-filter-other"].map(function(s){return t.querySelector(s)}).filter(Boolean),fB=["#nav-archive-filter-all","#nav-archive-filter-appswebapps","#nav-archive-filter-websites","#nav-archive-filter-ecommerce","#nav-archive-filter-platforms-systems","#nav-archive-filter-other"].map(function(s){return t.querySelector(s)}).filter(Boolean);function keyFromId(id){id=(id||"").replace(/^#?/,"");return id.indexOf("appswebapps")>-1?"apps webapps":id.indexOf("platforms-systems")>-1?"platforms systems":id.indexOf("ecommerce")>-1?"ecommerce":id.indexOf("websites")>-1?"websites":id.indexOf("other")>-1?"other":"all"}function setActive(btn){fA.concat(fB).forEach(function(b){b&&b.classList.toggle("is-active",b===btn)})}function applyFilter(k){var all=k==="all";function show(it){var c=map.get(it)||[],ok=all||c.some(function(x){return x.indexOf(k)>-1||k.indexOf(x)>-1});it.style.display=ok?"":"none"}pi.forEach(show);mi.forEach(show)}function saveView(v){try{localStorage.setItem("archive:view",v)}catch(_){}}function loadView(){try{return localStorage.getItem("archive:view")}catch(_){return null}}function setHtmlView(v){try{document.documentElement.setAttribute("data-archive-view",v)}catch(_){}}function currentView(){return P.style.display==="none"?"minimal":"playful"}function moveDot(v,imm){if(!dot)return;var xp=v==="minimal"?100:0;imm?gsap.set(dot,{xPercent:xp}):gsap.to(dot,{xPercent:xp,duration:.22,ease:"power3.out",overwrite:!0})}function showView(v,imm){var toMin=v==="minimal",from=toMin?P:M,to=toMin?M:P;gsap.killTweensOf([from,to]);if(imm){from.style.display="none";to.style.display="";gsap.set([from,to],{autoAlpha:1,clearProps:"opacity,visibility,filter"});setHtmlView(v);moveDot(v,!0);return}to.style.display="";gsap.set(to,{autoAlpha:0});var tl=gsap.timeline({defaults:{overwrite:!0}});tl.to(from,{autoAlpha:0,duration:.18,ease:"power1.out"}).add(function(){from.style.display="none";gsap.set(from,{autoAlpha:1,clearProps:"opacity,visibility"});}).to(to,{autoAlpha:1,duration:.22,ease:"power1.out",clearProps:"opacity,visibility"},"<");setHtmlView(v);moveDot(v,!1)}function refreshAfterView(){var k="all";for(var i=0;i<fA.length;i++)if(fA[i].classList.contains("is-active")){k=keyFromId(fA[i].id);break}applyFilter(k)}function toggle(){var v=currentView()==="playful"?"minimal":"playful";saveView(v);showView(v,!1);refreshAfterView()}fA.concat(fB).forEach(function(btn){if(!btn||btn.__archiveFilterBound)return;btn.__archiveFilterBound=1;btn.addEventListener("click",function(ev){ev.preventDefault();setActive(btn);applyFilter(keyFromId(btn.id))},!1)});if(!blk.__archiveSwitchBound){blk.__archiveSwitchBound=1;var on=function(ev){var a=ev.target&&ev.target.closest&&ev.target.closest("a");if(a)return;if(ev.target&&ev.target.closest&&(ev.target.closest(".theme-switch")||ev.target.closest("[data-theme-toggle]")))return;ev.preventDefault();toggle()};blk.addEventListener("click",on,{passive:!1});blk.addEventListener("pointerup",on,{passive:!1});}var initV=loadView()||"playful";showView(initV,!0);moveDot(initV,!0);applyFilter("all");setTimeout(refreshAfterView,0)}}),
			feature({id:"archiveListMotion",stage:"main",namespaces:["archive"],selectors:[".archive-playful-item",".archive-minimal-item"],init:function(e){if(!window.gsap)return;e=e||document;var t=e.querySelector('[data-barba="container"]')||e;if(t.__almCleanup)try{t.__almCleanup()}catch(_){}var items=[].slice.call(t.querySelectorAll(".archive-playful-item,.archive-minimal-item"));if(!items.length)return;items.forEach(function(it){it.__almDone=0;gsap.killTweensOf(it);gsap.set(it,{autoAlpha:1,filter:"none",willChange:"transform,opacity,filter"})});var io=new IntersectionObserver(function(ents){ents.forEach(function(ent){var it=ent.target;if(!ent.isIntersecting||it.__almDone||it.style.display==="none")return;it.__almDone=1;gsap.fromTo(it,{y:18,autoAlpha:0,filter:"blur(10px)"},{y:0,autoAlpha:1,filter:"blur(0px)",duration:.55,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange"})})},{root:null,threshold:.15});items.forEach(function(it){io.observe(it)});t.__almCleanup=function(){try{io.disconnect()}catch(_){}t.__almCleanup=null}}})
		);
