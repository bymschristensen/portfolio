	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				namespaces: ['info'],
    			selectors: [],
				init: async r=>{if(window.__recoJumpInstalled)return;window.__recoJumpInstalled=1;const nn=()=>((document.querySelector('[data-barba="container"]')?.dataset?.barbaNamespace)||"")==="info",tab=()=>{const w=document.querySelector(".w-tabs");if(!w)return;const a=[...w.querySelectorAll(".w-tab-link")].find(el=>((el.getAttribute("data-w-tab")||el.textContent||"").trim().toLowerCase()).includes("recommend"));a&&a.click()},scroll=()=>{const d=document.getElementById("details")||document.querySelector(".section-details");if(!d)return;const y=d.getBoundingClientRect().top+window.pageYOffset-24;window.scrollTo({top:y,behavior:"smooth"})},go=()=>{tab();requestAnimationFrame(()=>requestAnimationFrame(scroll))},onHash=()=>{if(!nn())return;const h=(location.hash||"").toLowerCase();if(h==="#details"||h==="#recommendations")go()};onHash();addEventListener("hashchange",onHash,{passive:!0});document.addEventListener("click",e=>{const a=e.target&&(e.target.tagName==="A"?e.target:e.target.closest?.("a"));if(!a||a.classList.contains("w-tab-link"))return;const href=(a.getAttribute("href")||a.href||"")+"";const id=a.id||"";const onInfo=nn();const hit=/^recommendationsopen/i.test(id)||/#details$/i.test(href)||/#recommendations$/i.test(href)||(/\b\/info\/?$/i.test(href)&&onInfo);if(!hit)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();if(onInfo)go();else location.href="/info#details"},{capture:!0});}
			})
		);
