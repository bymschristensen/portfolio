...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.capabilities=function(e,t){e=e||document,t=t||{};var d=!!t.delayHero,o=gsap.timeline(),h=e.querySelector(".showreel-headline"),c=e.querySelector(".showreel-container");if(!h&&!c)return o;h&&gsap.set(h,{autoAlpha:0,visibility:"visible"});c&&(gsap.set(c,{autoAlpha:1,visibility:"visible"}),gsap.set(c,{overflow:"hidden"}),gsap.set(c,{clipPath:"polygon(100% 100%,100% 100%,100% 100%,100% 100%)",willChange:"clip-path"}));o.addLabel("hero",d?.2:0);h&&o.set(h,{autoAlpha:1},"hero").call(function(){var e=window.CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve();e.then(function(){if(window.SplitText&&window.splitAndMask&&window.animateLines&&window.safelyRevertSplit)try{var e=splitAndMask(h);animateLines(e.lines).eventCallback("onComplete",function(){safelyRevertSplit(e,h)})}catch(e){gsap.to(h,{autoAlpha:1,duration:.01})}else gsap.to(h,{autoAlpha:1,duration:.01})})},null,"hero");c&&o.to(c,{clipPath:"polygon(0% 0%,100% 0%,100% 100%,0% 100%)",duration:1.05,ease:"power3.inOut",clearProps:"willChange"},"hero+=0.45");return o};
