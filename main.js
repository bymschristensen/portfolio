	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({
				...
				init: async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero||hero.__heroShowreelBound)return;var container=hero.querySelector(".showreel-container");if(!container)return;hero.__heroShowreelBound=!0;container.style.setProperty("--hero-showreel-x","0px");container.style.setProperty("--hero-showreel-scale","1");var state={x:0,scale:1,targetX:0,targetScale:1,raf:0,normX:0,scrollScale:1},MAX_OFFSET_PX=160,BASE_SCROLL=.96,EXTRA_SCROLL=.3,LERP=.12,CURSOR_RANGE=.06;function clamp01(v){return v<0?0:v>1?1:v}function updateScrollScale(){var vh=window.innerHeight||document.documentElement.clientHeight||1,rect=hero.getBoundingClientRect(),mid=rect.top+rect.height/2,dist=Math.abs(mid-vh*.5),maxDist=vh*.8,f=1-dist/maxDist;f=clamp01(f);state.scrollScale=BASE_SCROLL+EXTRA_SCROLL*f}function applyTargets(){var cursorScale=1+Math.abs(state.normX)*CURSOR_RANGE;state.targetScale=state.scrollScale*cursorScale;state.raf||(state.raf=requestAnimationFrame(animate))}function animate(){var tx=state.targetX,ts=state.targetScale;state.x+=(tx-state.x)*LERP;state.scale+=(ts-state.scale)*LERP;container.style.setProperty("--hero-showreel-x",state.x.toFixed(2)+"px");container.style.setProperty("--hero-showreel-scale",state.scale.toFixed(3));Math.abs(tx-state.x)>.2||Math.abs(ts-state.scale)>.002?state.raf=requestAnimationFrame(animate):state.raf=0}function setTarget(normX){normX<-1?normX=-1:normX>1&&(normX=1);state.normX=normX;state.targetX=normX*MAX_OFFSET_PX;applyTargets()}function onPointerMove(e){var rect=hero.getBoundingClientRect();if(!rect.width)return;var x=e.clientX-rect.left,nx=x/rect.width*2-1;setTarget(nx)}function onPointerLeave(){setTarget(0)}function onScroll(){updateScrollScale();applyTargets()}function onResize(){updateScrollScale();applyTargets()}updateScrollScale();applyTargets();hero.addEventListener("pointermove",onPointerMove,{passive:!0});hero.addEventListener("pointerleave",onPointerLeave,{passive:!0});window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});hero.__heroShowreelDestroy__=function(){try{hero.removeEventListener("pointermove",onPointerMove)}catch(e){}try{hero.removeEventListener("pointerleave",onPointerLeave)}catch(e){}try{window.removeEventListener("scroll",onScroll)}catch(e){}try{window.removeEventListener("resize",onResize)}catch(e){}if(state.raf)try{cancelAnimationFrame(state.raf)}catch(e){}try{container.style.removeProperty("--hero-showreel-x"),container.style.removeProperty("--hero-showreel-scale")}catch(e){}hero.__heroShowreelBound=!1}},destroy:async function(root){root=root||document;try{var h=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");h&&h.__heroShowreelDestroy__&&h.__heroShowreelDestroy__()}catch(e){}}
			}),
			feature({
