		...
		registries.pages.archive.push(
			feature({
				...
				init: function(root){
					...
					// Motion Module
					(function(){if(!window.gsap){mod.scheduleMotionRefresh=function(){};mod.resetMotion=function(){};return}var io=0,scan=0,raf=0,to=0,run=0,tries=0,max=45,STAG=.08,MIN_AFTER=3,MIN_PAD=.06,MIN_DUR=.6,last=0,GAP=140,visRun=0;function cleanup(){try{io&&io.disconnect&&io.disconnect()}catch(e){}io=0;try{scan&&cancelAnimationFrame(scan)}catch(e){}scan=0;try{to&&clearTimeout(to)}catch(e){}to=0}unsubs.push(cleanup);function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(e){return!0}}function scope(){try{var v=mod.getView&&mod.getView();if(v==="playful"&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(v==="minimal"&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(e){}return el.host||host}function items(sc){return[].slice.call((sc||scope()).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function isMin(sc){try{return!!(sc&&sc.classList&&sc.classList.contains("section-archive-minimal"))}catch(e){return!1}}function minVis(sc){return sc&&(sc.querySelector(".archive-minimal-item-visual")||sc.querySelector(".archive-minimal-visual")||sc.querySelector(".archive-minimal-visual-wrap"))||null}function prep(it){if(!it)return;it.__almRun=run;it.__almDone=0;try{gsap.killTweensOf(it)}catch(e){}gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"})}function prepV(v){if(!v)return;try{gsap.killTweensOf(v)}catch(e){}gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}function reveal(it,d){if(!it||it.__almDone||!can(it))return;it.__almRun!==run&&prep(it);it.__almDone=1;gsap.to(it,{delay:d||0,y:0,autoAlpha:1,filter:"blur(0px)",duration:.75,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange,opacity,visibility"})}function revealV(sc,bd){var v=minVis(sc);if(!v||visRun===run)return;visRun=run;prepV(v);gsap.to(v,{delay:bd||0,autoAlpha:1,filter:"blur(0px)",duration:MIN_DUR,ease:"power2.out",overwrite:!0,clearProps:"filter,willChange,opacity,visibility"})}function batch(sc,els){els.sort(function(a,b){return a.getBoundingClientRect().top-b.getBoundingClientRect().top});for(var i=0;i<els.length;i++)reveal(els[i],STAG*i);if(isMin(sc)){var idx=Math.max(0,Math.min(MIN_AFTER,els.length)-1),dd=STAG*idx+MIN_PAD;revealV(sc,dd)}}function inView(it){if(!can(it)||it.__almDone)return!1;var r=it.getBoundingClientRect();return r.top<(innerHeight||0)&&r.bottom>0}function scanOnce(sc,list){for(var hits=[],i=0;i<list.length;i++)inView(list[i])&&hits.push(list[i]);hits.length&&batch(sc,hits)}function force(sc,list,frames){var c=0;function tick(){c++;scanOnce(sc,list);c<(frames||24)?scan=requestAnimationFrame(tick):scan=0}scan=requestAnimationFrame(tick)}function start(sc,list){cleanup();if("IntersectionObserver"in window){io=new IntersectionObserver(function(es){for(var h=[],i=0;i<es.length;i++)es[i].isIntersecting&&h.push(es[i].target);h.length&&batch(sc,h)},{threshold:.12,rootMargin:"0px 0px -10% 0px"});for(var i=0;i<list.length;i++)try{io.observe(list[i])}catch(e){}}force(sc,list,24)}function hard(sc){var list=items(sc);if(!list.length){if(tries++<max)return requestAnimationFrame(function(){hard(sc)});return}tries=0;run++;visRun=0;for(var i=0;i<list.length;i++)can(list[i])&&prep(list[i]);isMin(sc)&&prepV(minVis(sc));start(sc,list)}function soft(sc){var list=items(sc);if(!list.length)return;scanOnce(sc,list);force(sc,list,12)}mod.resetMotion=function(){tries=0;hard(scope())};mod.scheduleMotionRefresh=function(){var n=Date.now();if(n-last<GAP)return;last=n;if(raf)return;raf=requestAnimationFrame(function(){raf=0;tries=0;hard(scope());try{to&&clearTimeout(to)}catch(e){}to=setTimeout(function(){soft(scope())},180)})};on(document,"archive:view",mod.scheduleMotionRefresh,{passive:!0});on(document,"archive:filter:applied",mod.scheduleMotionRefresh,{passive:!0});requestAnimationFrame(mod.scheduleMotionRefresh)})();
