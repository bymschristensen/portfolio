	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!window.gsap||!window.ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capPin)return;e._capPin=1;const n=i===s.length-1?null:s[i+1];gsap.set(e,{filter:"contrast(100%) blur(0px)"});ScrollTrigger.create({id:"servicePin:"+i,trigger:e,start:"top top",endTrigger:n||e,end:n?"top top":"bottom bottom",pin:!0,pinSpacing:!0,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:t=>{let r=t.progress;r<=.15?r=0:r>=.85?r=1:r=(r-.15)/.7;const o=100-90*r,a=10*r;gsap.set(e,{filter:`contrast(${o}%) blur(${a}px)`})},onKill:()=>gsap.set(e,{clearProps:"filter"})})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
