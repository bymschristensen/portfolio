	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(e){if(!window.gsap)return;e=e||document;var c=e.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||e.querySelector('[data-barba="container"]')||e;if(c.__almCleanup)try{c.__almCleanup()}catch(_){}var io=null,run=0,tries=0,max=45;function canRun(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&("none"===s.display||"hidden"===s.visibility))}catch(_){return!0}}function getItems(root){return[].slice.call((root||c).querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function prep(list){run++;list.forEach(function(it){if(!canRun(it))return;it.__almRun=run;it.__almDone=0;gsap.killTweensOf(it);gsap.set(it,{y:100,autoAlpha:0,filter:"blur(12px)",willChange:"transform,opacity,filter"});var v=it.classList.contains("archive-minimal-item")?(it.querySelector(".archive-minimal-item-visual")||it.querySelector(".archive-minimal-visual")||it.querySelector(".archive-minimal-visual-wrap")):null;if(v){gsap.killTweensOf(v);gsap.set(v,{autoAlpha:0,filter:"blur(12px)",willChange:"opacity,filter"})}})}function revealBatch(batch){batch.sort(function(a,b){return a.top-b.top});batch.forEach(function(ent,i){var it=ent.el;if(!ent.in||!canRun(it)||it.__almDone||it.__almRun!==run)return;it.__almDone=1;var d=.08*i;gsap.to(it,{delay:d,y:0,autoAlpha:1,filter:"blur(0px)",duration:.75,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange"});var v=it.classList.contains("archive-minimal-item")?(it.querySelector(".archive-minimal-item-visual")||it.querySelector(".archive-minimal-visual")||it.querySelector(".archive-minimal-visual-wrap")):null;if(v)gsap.to(v,{delay:d+.06,autoAlpha:1,filter:"blur(0px)",duration:.6,ease:"power2.out",overwrite:!0,clearProps:"filter,willChange"})})}function start(list){io=new IntersectionObserver(function(ents){var batch=[];ents.forEach(function(ent){var it=ent.target;if(!ent.isIntersecting)return;batch.push({el:it,in:!0,top:it.getBoundingClientRect().top})});batch.length&&revealBatch(batch)},{threshold:.12,rootMargin:"0px 0px -10% 0px"});list.forEach(function(it){try{io.observe(it)}catch(_){}})}function boot(root){var list=getItems(root);if(!list.length&&tries++<max)return requestAnimationFrame(function(){boot(root)});if(!list.length)return;prep(list);start(list);c.__almCleanup=function(){try{io&&io.disconnect&&io.disconnect()}catch(_){}io=null;c.__almCleanup=null}}boot(c);window.__archiveListMotion=window.__archiveListMotion||{};window.__archiveListMotion.resetAndRun=function(root){try{c.__almCleanup&&c.__almCleanup()}catch(_){}c=root||c;tries=0;boot(c)};window.__archiveListMotion.destroy=function(){try{c.__almCleanup&&c.__almCleanup()}catch(_){}c.__almCleanup=null}}
			}),
			feature({
				...
				init: function(e){e=e||document;var d=e.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||e.querySelector('[data-barba="container"]')||e;if(d.__archiveControllerBound)return;d.__archiveControllerBound=1;var raf=0;function activeRoot(){var p=d.querySelector(".section-archive-playful"),m=d.querySelector(".section-archive-minimal");if(p&&getComputedStyle(p).display!=="none")return p;if(m&&getComputedStyle(m).display!=="none")return m;return d}function kick(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;try{window.__archiveListMotion&&__archiveListMotion.resetAndRun&&__archiveListMotion.resetAndRun(activeRoot())}catch(_){}})}d.addEventListener("archive:view",kick,{passive:!0});d.addEventListener("archive:filter:applied",kick,{passive:!0});kick();d.__archiveControllerDestroy=function(){try{d.removeEventListener("archive:view",kick)}catch(_){}try{d.removeEventListener("archive:filter:applied",kick)}catch(_){}raf=0;d.__archiveControllerBound=0;d.__archiveControllerDestroy=null}}
			})
		);
