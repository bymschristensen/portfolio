...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),o=e.querySelectorAll(".cs-titles-inner div"),n=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t))}catch{}};a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"}),i&&gsap.set(i,{autoAlpha:0,visibility:"hidden"}),o.length&&gsap.set(o,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(a),n(i),o.forEach(n),a&&t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),i&&t.addLabel("headline",.35).call(()=>{const r=()=>{try{const s=getComputedStyle(i),d=s.display||"block",w=s.whiteSpace||"pre-wrap",h=i.getBoundingClientRect().height||i.scrollHeight||0;i.style.height=h+"px";if(window.splitAndMask&&window.animateLines){const l=splitAndMask(i);try{l.lines.forEach(L=>{L.style.display="block",L.style.width="100%",L.style.whiteSpace=w});i.querySelectorAll(".text-mask").forEach(M=>{M.style.display="block",M.style.overflow="hidden",M.style.width="100%"});}catch{}gsap.set(i,{autoAlpha:1,visibility:"visible"}),animateLines(l.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:l.revert).call(l,l,i)}catch{}i.style.height="",d==="inline"&&(i.style.display="inline")})}else{gsap.set(i,{autoAlpha:1,visibility:"visible"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete:()=>{i.style.height="",d==="inline"&&(i.style.display="inline")}})}}catch{gsap.set(i,{autoAlpha:1,visibility:"visible"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>requestAnimationFrame(()=>requestAnimationFrame(r))).catch(r)},null,"headline"),o.length&&t.to(o,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t;}
		};
