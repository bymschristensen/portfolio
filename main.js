	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(e){if(!window.gsap)return;e=e||document;var c=e.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||e.querySelector('[data-barba="container"]')||e;if(c.__almCleanup)try{c.__almCleanup()}catch(_){}var r=0,m=45,io=null,run=0;function shown(t){if(!t)return!1;try{if(t.style&&"none"===t.style.display)return!1;var s=getComputedStyle(t);return s&&"none"!==s.display&&"hidden"!==s.visibility&&"0"!==s.opacity}catch(_){return!0}}function items(){return[].slice.call(c.querySelectorAll(".archive-playful-item,.archive-minimal-item"))}function prep(list){run++;list.forEach(function(it){if(!shown(it))return;it.__almRun=run;it.__almDone=0;gsap.killTweensOf(it);gsap.set(it,{y:18,autoAlpha:0,filter:"blur(10px)",willChange:"transform,opacity,filter"})})}function start(list){io=new IntersectionObserver(function(ents){ents.forEach(function(ent){var it=ent.target;if(!ent.isIntersecting||!shown(it)||it.__almDone||it.__almRun!==run)return;it.__almDone=1;gsap.fromTo(it,{y:18,autoAlpha:0,filter:"blur(10px)"},{y:0,autoAlpha:1,filter:"blur(0px)",duration:.6,ease:"power3.out",overwrite:!0,clearProps:"y,filter,willChange"})})},{threshold:.15});list.forEach(function(it){io.observe(it)})}function boot(){var list=items();if(!list.length&&r++<m)return requestAnimationFrame(boot);if(!list.length)return;prep(list);start(list);c.__almCleanup=function(){try{io&&io.disconnect&&io.disconnect()}catch(_){}io=null;c.__almCleanup=null}}boot();window.__archiveListMotion=window.__archiveListMotion||{};window.__archiveListMotion.resetAndRun=function(root){try{c.__almCleanup&&c.__almCleanup()}catch(_){}c=root||c;r=0;boot()};window.__archiveListMotion.destroy=function(){try{c.__almCleanup&&c.__almCleanup()}catch(_){}c.__almCleanup=null}}
			}),
			feature({
