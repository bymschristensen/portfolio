	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async e=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),i=e=>parseFloat(getComputedStyle(e).gap||"0")||0,a=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const r=(e.className||"").match(/sv-ratio-(\d+)/);if(r){const e=r[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const i=e.querySelector("img,video");if(i){const e=parseInt(i.getAttribute("width"),10)||i.naturalWidth||i.videoWidth||0,t=parseInt(i.getAttribute("height"),10)||i.naturalHeight||i.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5},s=(e,t)=>{const r=parseFloat(e.getAttribute("data-size")),i=t.closest(".section-single-service"),a=i&&i.clientHeight||innerHeight;return r&&a?Math.max(1,Math.round(a*r/100)):Math.round(.6*(a||innerHeight))},n=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,o=e=>{const t=i(e);let r=0;return e.childNodes.forEach(e=>{1===e.nodeType&&(r+=(e.getBoundingClientRect().width||0)+t)}),r},l=e=>{[...e.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())},d=(e,t)=>{const i=e.cloneNode(!0);i.setAttribute("data-clone","1"),i.dataset.key=e.dataset.key||"";const s=parseFloat(e.dataset.ratio)||a(e),n=+e.dataset.targetH||0,o=Math.max(1,Math.round(n*s));return i.style.overflow="hidden",i.style.flex="0 0 auto",i.style.willChange="height,transform",i.style.minWidth=o+"px",i.style.maxWidth=o+"px",r&&(i.style.aspectRatio=s+" / 1"),i.style.height=n?n+"px":"",t.appendChild(i),i};t.forEach(async e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0;const c=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!c.length)return;e.setAttribute("data-armed","measure"),await(e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const r=[],i=t.map(e=>{if(n(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),r=new Promise(t=>{const r=()=>{e.removeEventListener("load",r),t()};e.addEventListener("load",r,{once:!0})}),i=new Promise(t=>{let r=0;!function i(){if(n(e)||r++>80)return t();setTimeout(i,40)}()});return Promise.race([t,r,i])}),a=new Promise(e=>r.push(setTimeout(e,3500)));return Promise.race([Promise.all(i),a]).finally(()=>r.forEach(clearTimeout))})(t),c.forEach((e,i)=>{const n=a(e),o=s(e,t),l=Math.max(1,Math.round(o*n));e.dataset.key=String(i),e.dataset.ratio=String(n),e.dataset.targetH=String(o),e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.willChange="height,transform",e.style.minWidth=l+"px",e.style.maxWidth=l+"px",r&&(e.style.aspectRatio=n+" / 1"),e.style.height=o+"px"});const h="right"===(e.dataset.direction||"left").toLowerCase()?-1:1,u=parseFloat(e.dataset.speed)||.6,p=()=>{l(t),c.forEach(e=>d(e,t)),c.forEach(e=>d(e,t));let e=0;const r=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;o(t)<r&&e++<8;)c.forEach(e=>d(e,t))};p(),e.setAttribute("data-armed","1");let g=0;const m=(e,r=16.6667)=>{g-=h*((r||16.6667)/1e3*60)*u;const a=i(t),s=e=>(e.getBoundingClientRect().width||0)+a;let n=t.firstElementChild,o=0;for(;n&&g<-s(n)&&o++<50;)g+=s(n),t.appendChild(n),n=t.firstElementChild;let l=t.lastElementChild;for(o=0;l&&g>0&&o++<50;)g-=s(l),t.insertBefore(l,t.firstElementChild),l=t.lastElementChild;gsap.set(t,{x:g});const d=(innerWidth||0)/2;t.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),i=(d-(r.left+r.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*i+"px")})};gsap.ticker.add(m),CoreUtilities.Observers.addTicker(m);let f=!1;const y=CoreUtilities.Observers.addDom(new IntersectionObserver(r=>{r.forEach(r=>{if(r.target!==e||!r.isIntersecting||f)return;f=!0;const i=innerHeight,a=innerWidth,s=c.map(e=>{const t=e.getBoundingClientRect();return{w:e,area:Math.max(0,Math.min(t.bottom,i)-Math.max(t.top,0))*Math.max(0,Math.min(t.right,a)-Math.max(t.left,0)),dist:Math.abs(.5*(t.top+t.bottom)-.5*innerHeight)}}).filter(e=>e.area>1).sort((e,t)=>e.dist-t.dist).slice(0,7).map(e=>e.w);s.forEach((e,r)=>{const i=+e.dataset.targetH||e.clientHeight||0;i&&(gsap.set(e,{height:0}),gsap.to(e,{height:i,duration:.9,ease:"power2.out",delay:.12*r,onComplete:()=>{e.style.height=i+"px";const r=e.dataset.key||"";r&&t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${r}"]`).forEach(e=>e.style.height=i+"px")}}))}),y.unobserve(e)})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));y.observe(e);const v=new ResizeObserver(()=>{l(t),c.forEach(e=>{const i=parseFloat(e.dataset.ratio)||a(e),n=s(e,t);e.dataset.targetH=String(n);const o=Math.max(1,Math.round(n*i));e.style.minWidth=o+"px",e.style.maxWidth=o+"px",r&&(e.style.aspectRatio=i+" / 1"),e.style.height=n+"px"}),p()});v.observe(t),CoreUtilities.Observers.addDom(v)})}
			})
		);
