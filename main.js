...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({
				id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero||hero.__heroShowreelBound)return;var track=hero.querySelector(".showreel-track"),container=hero.querySelector(".showreel-container");if(!track||!container)return;hero.__heroShowreelBound=!0;container.style.setProperty("--hero-showreel-x","0px");container.style.setProperty("--hero-showreel-scale","1");var state={x:0,scale:1,targetX:0,targetScale:1,raf:0,normX:0,scrollScale:1,maxRange:0},LERP=.12,CURSOR_RANGE=.06,BASE_SCROLL=.98,EXTRA_SCROLL=.08;function measure(){var tw=track.clientWidth||0,cw=container.clientWidth||0,s=state.targetScale;state.maxRange=Math.max(0,(tw-cw*s)/2)}function clampX(x){var r=state.maxRange;return x<-r?-r:x>r?r:x}function animate(){var tx=state.targetX,ts=state.targetScale;state.x+=(tx-state.x)*LERP;state.scale+=(ts-state.scale)*LERP;container.style.setProperty("--hero-showreel-x",state.x.toFixed(2)+"px");container.style.setProperty("--hero-showreel-scale",state.scale.toFixed(3));if(Math.abs(tx-state.x)>.2||Math.abs(ts-state.scale)>.002){state.raf=requestAnimationFrame(animate)}else state.raf=0}function setFromNorm(n){n<-1?n=-1:n>1&&(n=1);state.normX=n;state.targetX=clampX(n*state.maxRange);state.targetScale=state.scrollScale*(1+Math.abs(n)*CURSOR_RANGE);state.raf||(state.raf=requestAnimationFrame(animate))}function updateScroll(){var rect=hero.getBoundingClientRect(),vh=window.innerHeight||document.documentElement.clientHeight||1,mid=rect.top+rect.height/2,dist=Math.abs(mid-.5*vh),f=1-Math.min(1,dist/(.8*vh));state.scrollScale=BASE_SCROLL+EXTRA_SCROLL*f;measure();state.targetX=clampX(state.normX*state.maxRange);state.targetScale=state.scrollScale*(1+Math.abs(state.normX)*CURSOR_RANGE);state.raf||(state.raf=requestAnimationFrame(animate))}function onPointerMove(e){var r=track.getBoundingClientRect();if(!r.width)return;var nx=(e.clientX-r.left)/r.width*2-1;setFromNorm(nx)}function onPointerLeave(){setFromNorm(0)}function onResize(){measure();updateScroll()}measure();updateScroll();setFromNorm(0);track.addEventListener("pointermove",onPointerMove,{passive:!0});track.addEventListener("pointerleave",onPointerLeave,{passive:!0});window.addEventListener("scroll",updateScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});hero.__heroShowreelDestroy__=function(){try{track.removeEventListener("pointermove",onPointerMove)}catch(a){}try{track.removeEventListener("pointerleave",onPointerLeave)}catch(a){}try{window.removeEventListener("scroll",updateScroll)}catch(a){}try{window.removeEventListener("resize",onResize)}catch(a){}if(state.raf)try{cancelAnimationFrame(state.raf)}catch(a){}try{container.style.removeProperty("--hero-showreel-x"),container.style.removeProperty("--hero-showreel-scale")}catch(a){}hero.__heroShowreelBound=!1}},destroy:async function(root){root=root||document;var h=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{h&&h.__heroShowreelDestroy__&&h.__heroShowreelDestroy__()}catch(a){}}
			}),
			feature({
