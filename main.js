	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				stage: "main",
				namespaces: ["archive"],
				selectors: [".section-archive-playful",".section-archive-minimal",".archive-playful-list",".archive-minimal-list",".archive-switch-block"],
				init: function(root){root=root||document;var main=root.querySelector("main.main")||root;if(main.__archiveCtlBound)return;main.__archiveCtlBound=1;var secPlay=main.querySelector(".section-archive-playful"),secMin=main.querySelector(".section-archive-minimal");if(!secPlay||!secMin)return;var playList=secPlay.querySelector(".archive-playful-list"),minList=secMin.querySelector(".archive-minimal-list");if(!playList||!minList)return;var mq=window.matchMedia("(min-width:1024px)"),IDS=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];function idKey(id){return(id||"").replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,"").trim()}function normLabel(t){t=(t||"").toLowerCase().trim().replace(/&/g," ").replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g,"-");return t==="apps-web-apps"||t==="apps-webapps"||t==="apps-web"?"appswebapps":t==="e-commerce"||t==="ecommerce"?"ecommerce":t==="platforms-systems"||t==="platforms-and-systems"?"platforms-systems":t==="websites"?"websites":t==="other"?"other":t==="all"?"all":t}function itemCats(it){for(var a=[],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var s=(n[i].textContent||"").trim();s&&a.push(normLabel(s))}return a}function matches(it,k){if(k==="all")return!0;for(var a=itemCats(it),i=0;i<a.length;i++)if(a[i]===k)return!0;return!1}var playItems=[].slice.call(playList.querySelectorAll(".archive-playful-item")),minItems=[].slice.call(minList.querySelectorAll(".archive-minimal-item")),state={view:"playful",cat:"all"};function setSections(v){var p=v==="playful";secPlay.style.display=p?"":"none";secMin.style.display=p?"none":"";secPlay.classList.toggle("is-active",p);secMin.classList.toggle("is-active",!p);secPlay.classList.toggle("is-inactive",!p);secMin.classList.toggle("is-inactive",p)}function apply(items,k){for(var i=0;i<items.length;i++){var it=items[i],show=matches(it,k);it.style.display=show?"":"none";it.setAttribute("aria-hidden",show?"false":"true")}}function syncFilters(){for(var i=0;i<IDS.length;i++){var id=IDS[i],el=main.querySelector("#"+id);if(!el)continue;var k=idKey(id),on=k===state.cat;el.classList.toggle("is-active",on);el.setAttribute("aria-pressed",on?"true":"false")}}function syncToggle(){var block=main.querySelector(".archive-switch-block"),dot=main.querySelector(".archive-switch-dot"),show=mq.matches;if(block)block.style.display=show?"":"none";if(dot)dot.classList.toggle("is-on",state.view==="minimal");if(block)block.setAttribute("aria-pressed",state.view==="minimal"?"true":"false")}function setView(v){mq.matches||(v="minimal");state.view=v;setSections(v);syncToggle();apply(playItems,state.cat);apply(minItems,state.cat)}function setCat(k){state.cat=k||"all";syncFilters();apply(playItems,state.cat);apply(minItems,state.cat)}function bindFilter(id){var el=main.querySelector("#"+id);if(!el||el.__archiveBound)return;el.__archiveBound=1;el.addEventListener("click",function(e){e.preventDefault();setCat(idKey(id))})}for(var i=0;i<IDS.length;i++)bindFilter(IDS[i]);var switchBlock=main.querySelector(".archive-switch-block");switchBlock&&!switchBlock.__archiveBound&&(switchBlock.__archiveBound=1,switchBlock.addEventListener("click",function(e){e.preventDefault();setView(state.view==="playful"?"minimal":"playful")}));function onMQ(){mq.matches?setView(state.view||"playful"):setView("minimal")}mq.addEventListener?mq.addEventListener("change",onMQ):mq.addListener(onMQ);setCat("all");setView(mq.matches?"playful":"minimal");main.__archiveAPI={setView:setView,setCategory:setCat,getState:function(){return{view:state.view,cat:state.cat}}}}
			})
