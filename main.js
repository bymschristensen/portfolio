...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),h=e.querySelector(".cs-headline"),n=e.querySelectorAll(".cs-titles-inner div"),f=()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));function S(E){if(!window.SplitText)return null;const s=getComputedStyle(E),w=s.whiteSpace||"normal",W=E.style.whiteSpace,D=E.style.display,I=getComputedStyle(E).display==="inline";E.style.whiteSpace=w,I&&(E.style.display="block"),E.clientWidth;const x=new SplitText(E,{type:"lines",linesClass:"line",reduceWhiteSpace:!1});x.lines.forEach(L=>{const a=L.getBoundingClientRect().height||L.offsetHeight||0,c=document.createElement("div");c.className="text-mask",c.style.cssText="overflow:hidden;display:block;height:"+a+"px",L.style.whiteSpace=w,L.style.display="block",L.parentNode.insertBefore(c,L),c.appendChild(L)});gsap.set(x.lines,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform,opacity"}),E.style.whiteSpace=W,E.style.display=D;return x}i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"}),h&&gsap.set(h,{autoAlpha:0}),n.length&&gsap.set(n,{autoAlpha:0,y:20,filter:"blur(10px)"}),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(h){const o=h.style.display,d=getComputedStyle(h).display==="inline";t.addLabel("headline",.35).set(h,{autoAlpha:1,visibility:"visible"},"headline").call(()=>{const run=()=>{try{const sp=S(h);if(!sp){gsap.to(h,{y:0,opacity:1,duration:.6,ease:"power2.out"});return}animateLines(sp.lines).eventCallback("onComplete",()=>{(window.safelyRevertSplit?window.safelyRevertSplit:sp.revert).call(sp,sp,h),d&&(h.style.display="inline"),h.style.display=h.style.display||o})}catch{gsap.to(h,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};CoreUtilities?.Fonts?.ready?.().then(f).then(run)},null,"headline")}return n.length&&t.to(n,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
