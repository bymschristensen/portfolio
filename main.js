...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async R=>{const H=R.querySelector(".hero-section-showreel");if(!H)return;const C=H.querySelector(".showreel-outer .container")||H.querySelector(".showreel-outer .container"),M=H.querySelector(".showreel-container"),A=[H.querySelector(".showreel-headline"),H.querySelector(".showreel-subtext"),H.querySelector(".toc-outer")].filter(Boolean);if(!C||!M)return;const L=(a,b,c)=>a+(b-a)*c,cl=a=>Math.max(0,Math.min(1,a));let vh=window.innerHeight,mouse=0.5,maxS=1,contW=0,baseW=0;const base=getComputedStyle(M).transform||"none",pref=base==="none"?"":base+" ";function measureBase(){const t=M.style.transform;M.style.transform=base;const w=M.getBoundingClientRect().width||1;M.style.transform=t;return w}function calcMaxScale(){return Math.max(1,(contW||1)/(baseW||1))}function progress(){const r=H.getBoundingClientRect(),span=r.height-vh;return span<=0?0:cl((-r.top)/span)}function readContW(){const r=C.getBoundingClientRect();contW=r.width||1}function apply(){const p=progress();const s=L(1,maxS,p);const curW=baseW*s;const headroom=Math.max(0,contW-curW);const dx=headroom>0?(mouse-0.5)*headroom:0;M.style.transform=`${pref}translate3d(${dx}px,0,0) scale(${s})`;const lift=-0.45*vh*p,op=1-p;for(let i=0;i<A.length;i++){const el=A[i];el.style.transform=`translate3d(0,${lift}px,0)`;el.style.opacity=op}}function onMove(e){const r=C.getBoundingClientRect(),x=Math.max(r.left,Math.min(e.clientX,r.right));mouse=(x-r.left)/(r.width||1)}function onResize(){vh=window.innerHeight;readContW();maxS=calcMaxScale();rafApply()}function rafApply(){queued||(queued=!0,requestAnimationFrame(()=>{queued=!1,apply()}))}baseW=measureBase();readContW();maxS=calcMaxScale();let queued=!1;const onScroll=()=>rafApply();window.addEventListener("scroll",onScroll,{passive:!0});window.addEventListener("resize",onResize,{passive:!0});H.addEventListener("mousemove",onMove,{passive:!0});const ro=new ResizeObserver(()=>onResize());try{ro.observe(C),ro.observe(M)}catch{};apply();document.__capHero&&document.__capHero.kill?.();document.__capHero={kill(){try{window.removeEventListener("scroll",onScroll)}catch{}try{window.removeEventListener("resize",onResize)}catch{}try{H.removeEventListener("mousemove",onMove)}catch{}try{ro.disconnect()}catch{}try{M.style.transform=base==="none"?"":base;A.forEach(el=>{el.style.transform="";el.style.opacity=""})}catch{}}}},destroy:async()=>{try{document.__capHero?.kill?.()}catch{}document.__capHero=null}),
			feature({
