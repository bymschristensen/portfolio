...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveController",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-switch-block",".archive-switch",".archive-switch-dot"],init:function(root){root=root||document;var KEY="archiveView",mq=matchMedia("(min-width: 1024px)"),playful=root.querySelector(".section-archive-playful"),minimal=root.querySelector(".section-archive-minimal"),btn=root.querySelector(".archive-switch-block")||root.querySelector(".archive-switch"),dot=root.querySelector(".archive-switch-dot");if(!playful||!minimal||root.__archiveViewBound)return;root.__archiveViewBound=1;function stored(){var v=localStorage.getItem(KEY);return v==="playful"||v==="minimal"?v:null}function bpDefault(){return mq.matches?"playful":"minimal"}function itemsFor(v){return v==="playful"?playful.querySelectorAll(".archive-playful-item"):minimal.querySelectorAll(".archive-minimal-item")}function setUI(v){var p=v==="playful";btn&&(btn.dataset.view=v,btn.setAttribute("aria-pressed",String(p)),btn.classList.toggle("is-playful",p),btn.classList.toggle("is-minimal",!p));dot&&(dot.dataset.view=v,dot.classList.toggle("is-playful",p),dot.classList.toggle("is-minimal",!p))}function showInstant(v,meta){var p=v==="playful";try{window.gsap&&gsap.set&&gsap.set([playful,minimal],{display:"none"})}catch(e){playful.style.display="none";minimal.style.display="none"};playful.style.display=p?"block":"none";minimal.style.display=p?"none":"block";try{window.gsap&&gsap.set&&gsap.set(p?playful:minimal,{autoAlpha:1,filter:"none",clearProps:"opacity,visibility"})}catch(e){}setUI(v);meta==="user"&&localStorage.setItem(KEY,v)}function animateSwap(nextV){if(!window.gsap)return showInstant(nextV,"user");var curV=(btn&&btn.dataset.view)||((playful.style.display!=="none")?"playful":"minimal");if(curV===nextV)return;var curSec=curV==="playful"?playful:minimal,nextSec=nextV==="playful"?playful:minimal;setUI(nextV);localStorage.setItem(KEY,nextV);gsap.killTweensOf([curSec,nextSec]);gsap.timeline({defaults:{ease:"power2.out"}}).to(curSec,{autoAlpha:0,filter:"blur(10px)",duration:.22,overwrite:!0},0).add(function(){curSec.style.display="none";nextSec.style.display="block";gsap.set(nextSec,{autoAlpha:1,filter:"none"});var its=[].slice.call(itemsFor(nextV)||[]);its.forEach(function(it){it.__almDone=1;gsap.killTweensOf(it);gsap.set(it,{y:18,autoAlpha:0,filter:"blur(10px)",willChange:"transform,opacity,filter"})})},">").to([].slice.call(itemsFor(nextV)||[]),{y:0,autoAlpha:1,filter:"blur(0px)",duration:.55,stagger:.06,overwrite:!0,clearProps:"y,filter,willChange"},">-=0.02").add(function(){try{window.ScrollTrigger&&ScrollTrigger.refresh&&ScrollTrigger.refresh()}catch(e){}},">") }function apply(v,meta){showInstant(v,meta)}var s=stored();apply(s||bpDefault(),s?"stored":"auto");function onMQ(){stored()||apply(bpDefault(),"auto")}mq.addEventListener?mq.addEventListener("change",onMQ):mq.addListener(onMQ);function onClick(e){e&&e.preventDefault&&e.preventDefault();var cur=(btn&&btn.dataset.view)||((playful.style.display!=="none")?"playful":"minimal");animateSwap(cur==="playful"?"minimal":"playful")}btn&&btn.addEventListener("click",onClick,{passive:!1});return root.__archiveViewDestroy=function(){try{mq.removeEventListener?mq.removeEventListener("change",onMQ):mq.removeListener(onMQ)}catch(e){}try{btn&&btn.removeEventListener("click",onClick)}catch(e){}root.__archiveViewBound=0;root.__archiveViewDestroy=null}}}),
