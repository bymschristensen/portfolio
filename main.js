...
// Navigation Manager
	window.NavigationManager = (function () {
		...
		function installLinkInterceptor(){if(state.installed)return;state.clickHandler=e=>{if(e.defaultPrevented)return;if(e.target?.closest('.nav-button-menu,.nav-button-filter,.nav-button-close-case,[data-router-ignore="true"]'))return;const n=isInternalAnchor(e);if(!n)return;const t=document.querySelector('[data-barba="container"]')?.dataset?.barbaNamespace||"";if(t!=="archive"&&t!=="resources"){const r=n.a.closest("[data-barba-prevent]");if(r&&r.getAttribute("data-barba-prevent")==="true")return}if(state.locks.size){dlog("blocked by lock(s):",[...state.locks]);e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();return}e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();console.info("[intercept→barba]",n.url.href,"locks:",[...state.locks]);window.barba?.go?barba.go(n.url.href):location.href=n.url.href};addEventListener("pointerdown",state.clickHandler,{capture:!0});addEventListener("click",state.clickHandler,{capture:!0});state.installed=!0;dlog("link interceptor installed")}
	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async r=>{try{const ns=r?.dataset?.barbaNamespace||r.getAttribute?.("data-barba-namespace")||"",D=document;D.querySelectorAll(".nav-primary-wrap").forEach(w=>{if(w.__navReady)return;w.__navReady=!0;const btn=w.querySelector(".nav-button-menu"),label=w.querySelector(".nav-button-text"),phone=w.querySelector(".phone-number"),mins=w.querySelectorAll(".button-minimal-darkmode"),links=w.querySelectorAll(".menu-link"),lineText=w.querySelector(".ta-one-menu"),wrap=w.querySelector(".menu-wrapper"),box=w.querySelector(".menu-container"),fBtn=w.querySelector(".nav-button-filter"),fBox=w.querySelector(".filters-container"),fItems=w.querySelectorAll(".modal-filters-item"),fCap=w.querySelector(".modal-filters-caption"),fl1=w.querySelector(".filter-line-1"),fl2=w.querySelector(".filter-line-2");if(!btn||!label||!wrap||!box){w._menuTimeline=null;w._filterTimeline=null;return}const lock=t=>{try{document.body.style.overflow=t?"hidden":""}catch{}},hardHideMenu=()=>{try{box&&(box.style.display="none",box.removeAttribute("data-open"));wrap&&(wrap.style.display="none",wrap.removeAttribute("data-open"));label&&label.dataset&&label.dataset.orig&&(label.textContent=label.dataset.orig);lock(!1)}catch{}},hardHideFilter=()=>{try{fBox&&(fBox.style.display="none",fBox.removeAttribute("data-open"));wrap&&(wrap.style.display="none",wrap.removeAttribute("data-open"));lock(!1)}catch{}};wrap.style.display="none";box.style.display="none";fBox&&(fBox.style.display="none");label.dataset.orig=label.textContent||"Menu";const showMenu=()=>{wrap.style.display="flex",box.style.display="flex",wrap.setAttribute("data-open","1"),box.setAttribute("data-open","1"),lock(!0)},showFilter=()=>{wrap.style.display="flex",wrap.setAttribute("data-open","1"),fBox&&(fBox.style.display="flex",fBox.setAttribute("data-open","1")),lock(!0)};let splitRan=!1;const playMenuTitle=()=>{if(!lineText||splitRan)return;splitRan=!0;try{if(window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){const s=splitAndMask(lineText);animateLines(s.lines).eventCallback("onComplete",()=>{safelyRevertSplit(s,lineText)})}}catch{}};const tl=gsap.timeline({paused:!0});tl.call(playMenuTitle,null,0);phone&&tl.from(phone,{opacity:0,duration:.35},">");mins.length&&tl.from(mins,{opacity:0,duration:.35,stagger:.12},"<");links.length&&tl.from(links,{opacity:0,yPercent:240,duration:.4,stagger:.1},"<");tl.to(label,{text:"Close",duration:.2},"<");tl.eventCallback("onReverseComplete",hardHideMenu);let guard=null;const arm=(t,hide)=>{guard&&gsap.ticker.remove(guard),guard=()=>{if(!t.reversed())return;(t.time()<=.02||t.progress()<=.02||!t.isActive())&&(hide(),gsap.ticker.remove(guard),guard=null)},gsap.ticker.add(guard),gsap.delayedCall(.6,()=>{t.reversed()&&(hide(),guard&&(gsap.ticker.remove(guard),guard=null))})};let state=null;btn.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if("menu"===state){tl.timeScale(2).reverse();arm(tl,hardHideMenu);return}if("filter"===state&&tlf){tlf.timeScale(2).reverse();arm(tlf,hardHideFilter);gsap.delayedCall(.02,()=>{state="menu",showMenu(),tl.timeScale(1).play(0)});return}state="menu",showMenu(),tl.timeScale(1).play(0)},{passive:!1});let tlf=null;if(fBtn&&fBox&&fItems.length){ns==="archive"?(fBtn.style.display="flex",gsap.to(fBtn,{opacity:1,duration:.2})):gsap.to(fBtn,{opacity:0,duration:.2,onComplete:()=>{fBtn.style.display="none"}});tlf=gsap.timeline({paused:!0}).to(fBox,{opacity:1,duration:.35,ease:"power2.out"},0);fl1&&tlf.to(fl1,{rotation:45,transformOrigin:"center",duration:.3},0);fl2&&tlf.to(fl2,{rotation:-45,marginTop:"-4px",transformOrigin:"center",duration:.3},0);fCap&&tlf.from(fCap,{opacity:0,duration:.4},"<");fItems.length&&tlf.from(fItems,{opacity:0,duration:.6,stagger:.15},"<");tlf.eventCallback("onReverseComplete",hardHideFilter);fBtn.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if("filter"===state){tlf.timeScale(2).reverse();arm(tlf,hardHideFilter);return}if("menu"===state){tl.timeScale(2).reverse();arm(tl,hardHideMenu);gsap.delayedCall(.02,()=>{state="filter",showFilter(),tlf.timeScale(1).play(0)});return}state="filter",showFilter(),tlf.timeScale(1).play(0)},{passive:!1})}D.addEventListener("keydown",ev=>{"Escape"===ev.key&&("menu"===state?(tl.timeScale(2).reverse(),arm(tl,hardHideMenu)):"filter"===state&&tlf&&(tlf.timeScale(2).reverse(),arm(tlf,hardHideFilter)))});w._menuTimeline=tl;w._filterTimeline=tlf||null});NavigationManager.attachMenuLocks(document)}catch(e){console.warn("[navigation:init] failed",e)}} 
			}),
