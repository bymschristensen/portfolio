...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveController",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-switch",".archive-switch-dot",".archive-switch-block"],init:function(root){root=root||document;var KEY="archiveView",mq=matchMedia("(min-width: 1024px)"),playful=root.querySelector(".section-archive-playful"),minimal=root.querySelector(".section-archive-minimal"),btn=root.querySelector(".archive-switch")||root.querySelector(".archive-switch-block"),dot=root.querySelector(".archive-switch-dot");if(!playful||!minimal||root.__archiveViewBound)return;root.__archiveViewBound=1;function stored(){var v=localStorage.getItem(KEY);return v==="playful"||v==="minimal"?v:null}function bpDefault(){return mq.matches?"playful":"minimal"}function apply(view,meta){var isPlayful=view==="playful";playful.style.display=isPlayful?"":"none";minimal.style.display=isPlayful?"none":"";btn&&(btn.setAttribute("aria-pressed",String(isPlayful)),btn.dataset.view=view);dot&&(dot.dataset.view=view);meta==="user"&&localStorage.setItem(KEY,view)}var s=stored();apply(s||bpDefault(),s?"stored":"auto");function onMQ(){stored()||apply(bpDefault(),"auto")}mq.addEventListener?mq.addEventListener("change",onMQ):mq.addListener(onMQ);function onClick(){var cur=(btn&&btn.dataset.view)||(playful.style.display!=="none"?"playful":"minimal");apply(cur==="playful"?"minimal":"playful","user")}btn&&btn.addEventListener("click",onClick);return root.__archiveViewDestroy=function(){try{mq.removeEventListener?mq.removeEventListener("change",onMQ):mq.removeListener(onMQ)}catch(e){}try{btn&&btn.removeEventListener("click",onClick)}catch(e){}root.__archiveViewBound=0;root.__archiveViewDestroy=null}}}),
