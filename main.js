...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroDrift",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async r=>{const s=r.querySelector(".hero-section-showreel"),o=s&&s.querySelector(".showreel-outer"),a=s&&s.querySelector(".showreel-outer .container"),n=s&&s.querySelector(".showreel-container");if(!s||!o||!a||!n)return;let t=window.innerHeight,e=1,i=.5;const l=getComputedStyle(n).transform||"none",c="none"===l?"":l+" ";function d(){const e=n.style.transform;n.style.transform=l;const t=n.getBoundingClientRect().width||1,r=a.clientWidth||1;return n.style.transform=e,Math.max(1,r/t)}function u(){const r=s.getBoundingClientRect(),o=r.height-t;if(o<=0)return 0;return Math.max(0,Math.min(1,(-r.top)/o))}function f(){const r=u();e=d();const t=a.clientWidth||1,l=n.getBoundingClientRect().width||1,c=Math.max(0,l-t);let f=(i-.5)*c*-1;n.style.transform=`${c?c:""}${c?c:""}${c?c:""}`,n.style.transform=`${c}${c}${c}`;n.style.transform=`${c}${c}${c}`;n.style.transform=`${c}${c}${c}`}function m(){const r=u(),t=Math.max(1,d()),l=a.clientWidth||1,c=n.getBoundingClientRect().width||1,p=Math.max(0,c-l),h=(i-.5)*p*-1;n.style.transform=`${c}translate3d(${h}px,0,0) scale(${t})`}function p(e){const t=a.getBoundingClientRect(),r=Math.max(t.left,Math.min(e.clientX,t.right));i=(r-t.left)/(t.width||1)}function h(){t=window.innerHeight,e=d(),m()}e=d();const g=()=>m();let v=!1,y=()=>{v||(v=!0,requestAnimationFrame(()=>{v=!1,m()}))};window.addEventListener("scroll",y,{passive:!0}),s.addEventListener("mousemove",p,{passive:!0}),window.addEventListener("resize",h),m(),document.__capDrift={offScroll:()=>window.removeEventListener("scroll",y),onResize:h,onMouseMove:p,container:s,media:n,base:l}},destroy:async()=>{const e=document.__capDrift;if(!e)return;try{e.offScroll&&e.offScroll()}catch{}try{e.container&&e.onMouseMove&&e.container.removeEventListener("mousemove",e.onMouseMove)}catch{}try{e.onResize&&window.removeEventListener("resize",e.onResize)}catch{}try{e.media&&(e.media.style.transform=e.base||"")}catch{}document.__capDrift=null}}),
			feature({
