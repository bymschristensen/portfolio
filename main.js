	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const mm=gsap.matchMedia();mm.add("(min-width:1024px)",()=>{const rail=r.querySelector('[data-imgscroll="photos"]'),bio=r.querySelector('[data-imgscroll="bio"]'),copy=r.querySelector('[data-imgscroll="copy"]'),stats=r.querySelector('[data-imgscroll="stats"]'),head=r.querySelector(".bio-text .headline-md");if(!rail||!bio||!copy||!stats||!head)return;const photos=[...rail.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(+a.getAttribute("data-photo")||0)-(+b.getAttribute("data-photo")||0));if(photos.length<3)return;const p1=photos.find(e=>e.getAttribute("data-photo")==="1"),p2=photos.find(e=>e.getAttribute("data-photo")==="2"),p3=photos.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const rc=e=>e.getBoundingClientRect(),reset=()=>gsap.set([p1,p2,p3],{x:0,y:0,scale:1,zIndex:i=>100-(+photos[i].getAttribute("data-photo")||0),willChange:"transform"}),anchor=()=>{const s=rc(stats),h=rc(head);return{left:s.left,top:h.top}},delta=(el,an,o)=>{const r=rc(el),dx=an.left-r.left+(o&&o.dx||0),dy=an.top-r.top+(o&&o.dy||0),sc=o&&o.scale!=null?o.scale:1;return{dx,dy,sc}},cfg={p1:{dx:0,dy:0,scale:.42},p2:{dx:18,dy:22,scale:.38},p3:{dx:44,dy:56,scale:.36}};reset();const startBase="top center",mk=(el,opts,st,ed)=>gsap.to(el,{x:()=>{const a=anchor();return delta(el,a,opts).dx},y:()=>{const a=anchor();return delta(el,a,opts).dy},scale:()=>{const a=anchor();return delta(el,a,opts).sc},ease:"power3.out",scrollTrigger:{trigger:rail,start:st,end:ed,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0},onStart:()=>el.classList.add("is-gathering")});const t3=mk(p3,cfg.p3,`${startBase}+=8%`, `${startBase}-=42%`),t2=mk(p2,cfg.p2,`${startBase}+=16%`,`${startBase}-=34%`),t1=mk(p1,cfg.p1,`${startBase}+=24%`,`${startBase}-=26%`);const ro=new ResizeObserver(()=>{reset();ScrollTrigger.refresh()});[rail,bio,copy,stats,head].forEach(n=>n&&ro.observe(n));return()=>{try{[t1,t2,t3].forEach(t=>t&&t.kill());ro.disconnect()}catch{}}})}
			}),
			feature({
