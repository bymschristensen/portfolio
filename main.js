	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async root=>{const ns=root.dataset?.barbaNamespace||root.getAttribute("data-barba-namespace")||"",S=document;S.querySelectorAll(".nav-primary-wrap").forEach(W=>{if(W._navInit)return;W._navInit=!0;const B=W.querySelector(".nav-button-menu"),L=W.querySelector(".nav-button-text"),P=W.querySelector(".phone-number"),M=W.querySelectorAll(".button-minimal-darkmode"),A=W.querySelectorAll(".menu-link"),T=W.querySelector(".ta-one-menu"),WR=W.querySelector(".menu-wrapper"),MC=W.querySelector(".menu-container"),BF=W.querySelector(".nav-button-filter"),FC=W.querySelector(".filters-container"),FI=W.querySelectorAll(".modal-filters-item"),FL1=W.querySelector(".filter-line-1"),FL2=W.querySelector(".filter-line-2"),FT=W.querySelectorAll(".filter-tuner"),FCap=W.querySelector(".modal-filters-caption");if(!(B&&L&&P&&M.length&&A.length&&T&&WR&&MC))return;const lock=e=>{document.body.style.overflow=e?"hidden":""},showMenu=()=>{WR.style.display="flex",WR.setAttribute("data-open","1"),MC.style.display="flex",MC.setAttribute("data-open","1"),lock(!0)},hideMenu=()=>{MC.style.display="none",MC.removeAttribute("data-open"),WR.style.display="none",WR.removeAttribute("data-open"),lock(!1)},showFilter=()=>{WR.style.display="flex",WR.setAttribute("data-open","1"),FC&&(FC.style.display="flex",FC.setAttribute("data-open","1")),lock(!0)},hideFilter=()=>{FC&&(FC.style.display="none",FC.removeAttribute("data-open")),WR.style.display="none",WR.removeAttribute("data-open"),lock(!1)};WR.style.display="none",MC.style.display="none",FC&&(FC.style.display="none");L.dataset.orig=L.textContent||"Menu";let sp=null;const tl=gsap.timeline({paused:!0}).call(()=>{sp=splitAndMask(T),animateLines(sp.lines).eventCallback("onComplete",()=>{safelyRevertSplit(sp,T),sp=null})},null,0).from(P,{opacity:0,duration:.4},">").from(M,{opacity:0,duration:.4,stagger:.15},"<").from(A,{opacity:0,yPercent:240,duration:.45,stagger:.12},"<").to(L,{text:"Close",duration:.25},"<");tl.eventCallback("onReverseComplete",()=>{L.textContent=L.dataset.orig,hideMenu()}),tl.eventCallback("onStart",()=>NavigationManager.setLock("menu",!0)),tl.eventCallback("onComplete",()=>NavigationManager.setLock("menu",!1)),tl.eventCallback("onReverseComplete",()=>NavigationManager.setLock("menu",!1));let watchdog=null,armWatch=(timeline,hideFn)=>{watchdog&&watchdog.kill&&watchdog.kill();watchdog=gsap.ticker.add(()=>{if(timeline.reversed()&&timeline.progress()<=.02){hideFn();gsap.ticker.remove(watchdog),watchdog=null}});gsap.delayedCall(.7,()=>{timeline.reversed()&&(hideFn(),watchdog&&gsap.ticker.remove(watchdog),watchdog=null)})};let state=null;B.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if(state==="menu"){tl.timeScale(2).reverse(),armWatch(tl,hideMenu);return}if(state==="filter"){tlf&&tlf.timeScale(2).reverse(),armWatch(tlf,hideFilter),gsap.delayedCall(.02,()=>{state="menu",showMenu(),tl.timeScale(1).play(0)});return}state="menu",showMenu(),tl.timeScale(1).play(0)},{passive:!1});let tlf=null;BF&&FC&&FI.length&&(ns==="archive"?(BF.style.display="flex",gsap.to(BF,{opacity:1,duration:.2})):gsap.to(BF,{opacity:0,duration:.2,onComplete:()=>BF.style.display="none"}),tlf=gsap.timeline({paused:!0}).to(FC,{opacity:1,duration:.4,ease:"power2.out"},0).to(FT,{opacity:0,duration:.15},"<").to(FL1,{rotation:45,transformOrigin:"center",duration:.35},"<").to(FL2,{rotation:-45,marginTop:"-4px",transformOrigin:"center",duration:.35},"<").from(FCap,{opacity:0,duration:.5},"<").from(FI,{opacity:0,duration:.8,stagger:.2},"<"),tlf.eventCallback("onReverseComplete",hideFilter),tlf.eventCallback("onStart",()=>NavigationManager.setLock("filter",!0)),tlf.eventCallback("onComplete",()=>NavigationManager.setLock("filter",!1)),tlf.eventCallback("onReverseComplete",()=>NavigationManager.setLock("filter",!1)),BF.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation();if(state==="filter"){tlf.timeScale(2).reverse(),armWatch(tlf,hideFilter);return}if(state==="menu"){tl.timeScale(2).reverse(),armWatch(tl,hideMenu),gsap.delayedCall(.02,()=>{state="filter",showFilter(),tlf.timeScale(1).play(0)});return}state="filter",showFilter(),tlf.timeScale(1).play(0)},{passive:!1}));S.addEventListener("keydown",ev=>{if(ev.key!=="Escape")return;if(state==="menu"){tl.timeScale(2).reverse(),armWatch(tl,hideMenu)}else if(state==="filter"&&tlf){tlf.timeScale(2).reverse(),armWatch(tlf,hideFilter)}})});NavigationManager.attachMenuLocks(document);}
			}),
