	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger)return;const c=r.closest(".barba-container")||document.querySelector(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;const BASE=getComputedStyle(c).backgroundColor,BG="var(--colors--background)",END="var(--colors--border)",setBG=t=>gsap.to(c,{backgroundColor:t,duration:.6,ease:"power1.inOut",overwrite:"auto"});["csbg:top","csbg:mid"].forEach(id=>{try{ScrollTrigger.getById(id)?.kill()}catch{}});ScrollTrigger.create({id:"csbg:top",trigger:d,start:"top bottom-=15%",endTrigger:m,end:"top bottom-=15%",onEnter:()=>setBG(BG),onEnterBack:()=>setBG(BG),onLeaveBack:()=>setBG(BASE),invalidateOnRefresh:!1,fastScrollEnd:1});ScrollTrigger.create({id:"csbg:mid",trigger:m,start:"top bottom-=15%",endTrigger:c,end:"bottom bottom",onEnter:()=>setBG(END),onEnterBack:()=>setBG(END),onLeaveBack:()=>setBG(BG),invalidateOnRefresh:!1,fastScrollEnd:1});}
			}),
			feature({
