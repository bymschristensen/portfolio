	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async e=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),a=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const r=(e.className||"").match(/sv-ratio-(\d+)/);if(r){const e=r[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const a=e.querySelector("img,video");if(a){const e=parseInt(a.getAttribute("width"),10)||a.naturalWidth||a.videoWidth||0,t=parseInt(a.getAttribute("height"),10)||a.naturalHeight||a.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5},i=e=>{let t=0;const r=parseFloat(getComputedStyle(e).gap||"0")||0;return e.childNodes.forEach(e=>{1===e.nodeType&&(t+=(e.getBoundingClientRect().width||0)+r)}),t},n=()=>Math.max(document.documentElement.clientWidth,window.innerWidth||0);t.forEach(async e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0,e.setAttribute("data-armed","measure");const s=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!s.length)return void e.setAttribute("data-armed","1");await(e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const r=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,a=[];return Promise.race([Promise.all(t.map(e=>{if(r(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),a=new Promise(t=>{const r=()=>{e.removeEventListener("load",r),t()};e.addEventListener("load",r,{once:!0})}),i=new Promise(t=>{let a=0;!function i(){if(r(e)||a++>80)return t();setTimeout(i,40)}()});return Promise.race([t,a,i])})),new Promise(e=>a.push(setTimeout(e,3500)))]).finally(()=>a.forEach(clearTimeout))})(t);const o=new Set,l="right"===(e.dataset.direction||"left").toLowerCase()?-1:1,d=parseFloat(e.dataset.speed)||.6;s.forEach((e,i)=>{const n=a(e),s=((e,t)=>{const r=parseFloat(e.getAttribute("data-size")),a=(e.className||"").match(/sv-size-(\d+)/),i=r||(a?parseFloat(a[1]):0),n=t.clientHeight||0;if(i>0&&n>0)return Math.max(1,Math.round(n*i/100));const s=e.style.height;e.style.height="";const o=parseFloat(getComputedStyle(e).height)||e.getBoundingClientRect().height||0;return e.style.height=s,o>8?Math.round(o):n>0?Math.round(.6*n):360})(e,t),o=Math.max(1,Math.round(s*n));e.dataset.key=String(i),e.dataset.ratio=String(n),e.dataset.targetH=String(s),e.dataset.revealed="0",e.dataset.revealing="",e.style.minWidth=o+"px",e.style.maxWidth=o+"px",r&&(e.style.aspectRatio=n+" / 1"),e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.willChange="height,transform",e.style.height="0px"});const c=()=>{[...t.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())},h=e=>{const i=e.cloneNode(!0),n=e.dataset.key||"",s=parseFloat(e.dataset.ratio)||a(e),l=+e.dataset.targetH||0,d=Math.max(1,Math.round(l*s));return i.setAttribute("data-clone","1"),i.dataset.key=n,i.style.overflow="hidden",i.style.flex="0 0 auto",i.style.willChange="height,transform",i.style.minWidth=d+"px",i.style.maxWidth=d+"px",r&&(i.style.aspectRatio=s+" / 1"),i.style.height=o.has(n)&&l>0?l+"px":"0px",t.appendChild(i),i},u=()=>{s.forEach(h)},m=()=>{c(),u(),u();let e=0;const r=3*n();for(;i(t)<r&&e++<8;)u()};m(),e.setAttribute("data-armed","1");let p=0,g=0,f=!1,y=0;const v=e=>{if(!f)return void cancelAnimationFrame(p);if((e||0)-g<32)return void(p=requestAnimationFrame(v));g=e||performance.now();const r=n(),a=-.05*r,i=1.05*r,s=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")].filter(e=>{const t=e.getBoundingClientRect();return t.right>a&&t.left<i}).filter(e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing).sort((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left);if(s.length){const e=gsap.timeline();s.forEach((r,a)=>{const i=+r.dataset.targetH||0,n=r.dataset.key||"";r.dataset.revealing="1",e.fromTo(r,{height:0},{height:i,duration:.9,ease:"power2.out",onComplete:()=>{r.style.height="",r.dataset.revealed="1",r.dataset.revealing="",o.add(n),t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${n}"]`).forEach(e=>e.style.height=i+"px")}},.12*a)})}p=requestAnimationFrame(v)},w=new IntersectionObserver(t=>{t.forEach(t=>{t.target===e&&(t.isIntersecting?(f=!0,cancelAnimationFrame(p),p=requestAnimationFrame(v)):(f=!1,cancelAnimationFrame(p)))})},{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});w.observe(e),CoreUtilities.Observers.addDom(w),CoreUtilities.Observers.addDom({disconnect(){f=!1,cancelAnimationFrame(p)}});const x=new ResizeObserver(()=>{c(),m(),s.forEach(e=>{const i=e.dataset.key||"",n=parseFloat(e.dataset.ratio)||a(e),s=+e.dataset.targetH||0,o=Math.max(1,Math.round(s*n));e.style.minWidth=o+"px",e.style.maxWidth=o+"px",r&&(e.style.aspectRatio=n+" / 1"),t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${i}"]`).forEach(e=>{e.style.minWidth=o+"px",e.style.maxWidth=o+"px",r&&(e.style.aspectRatio=n+" / 1")})})});x.observe(t),CoreUtilities.Observers.addDom(x);const C=(e,r)=>((e,r)=>{y-=l*((r||16.6667)/1e3)*(60*d);const a=parseFloat(getComputedStyle(t).gap||"0")||0,i=e=>(e.getBoundingClientRect().width||0)+a;let n=t.firstElementChild,s=0;for(;n&&y<-i(n)&&s++<50;)y+=i(n),t.appendChild(n),n=t.firstElementChild;let o=t.lastElementChild;for(s=0;o&&y>0&&s++<50;)y-=i(o),t.insertBefore(o,t.firstElementChild),o=t.lastElementChild;gsap.set(t,{x:y});const c=(innerWidth||0)/2;t.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),a=(c-(r.left+r.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*a+"px")})})(0,r);gsap.ticker.add(C),CoreUtilities.Observers.addTicker(C)})}
			})
		);
