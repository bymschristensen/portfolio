...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),s=e.querySelector(".cs-titles-inner .headline-m"),n=o=>{if(!o)return;try{o.removeAttribute("data-entry-hidden"),o.removeAttribute("hidden"),o.removeAttribute("aria-hidden"),o.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(r=>o.classList&&o.classList.remove(r))}catch{}};function l(){return Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>new Promise(r=>{if(window.SplitText)return r();let d=0;(function c(){window.SplitText?r():d++<40?setTimeout(c,50):r()})()}))}a&&(gsap.set(a,{autoAlpha:0,y:100,filter:"blur(10px)"}),n(a),t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.75,ease:"power2.out"},0));if(i){gsap.set(i,{autoAlpha:0,visibility:"hidden"}),n(i),s&&(gsap.set(s,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(s));t.addLabel("h",.15).call(()=>{l().then(()=>{requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=splitAndMask(i),r=(o.lines&&o.lines.length?o.lines.length:1),d=.8+.08*Math.max(0,r-1);gsap.set(i,{autoAlpha:1,visibility:"visible",display:"block"});s&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.5,ease:"power2.out"},"h+"+Math.max(0,d-.2));animateLines(o.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:o.revert).call(o,o,i)}catch{}"inline"===getComputedStyle(i).display&&(i.style.display="inline")})}));}))},null,"h")}return t}
		};
