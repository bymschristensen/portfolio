	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init:function(r){if(!window.gsap)return;r=r||document;if(r.__arRevealBound)return;r.__arRevealBound=1;var e=window.matchMedia&&matchMedia("(prefers-reduced-motion: reduce)").matches,t=0;function i(){var n=r.querySelectorAll(".archive-minimal-item,.archive-playful-item"),a=r.querySelectorAll(".archive-minimal-item-visual,.archive-playful-item-visual");if(!n.length){if(t++<30)return void setTimeout(i,50);return}var o=[].slice.call(n),s=[].slice.call(a);gsap.killTweensOf(o.concat(s));gsap.set(o,{autoAlpha:0,y:18,clearProps:"transform"});s.length&&gsap.set(s,{autoAlpha:0,scale:.98,filter:"blur(10px)",transformOrigin:"50% 50%"});function l(){var r=o.filter(function(r){var e=r.getBoundingClientRect();return e.bottom>0&&e.top<window.innerHeight*.92});r.length||(r=[o[0]]);if(e){gsap.set(r,{autoAlpha:1,y:0});s.length&&gsap.set(s,{autoAlpha:1,scale:1,filter:"blur(0px)"});return}var t=gsap.timeline({defaults:{ease:"power3.out"},onComplete:function(){u&&u.disconnect()}});t.to(r,{autoAlpha:1,y:0,duration:.65,stagger:.06},0);s.length&&t.to(s,{autoAlpha:1,scale:1,filter:"blur(0px)",duration:.85},.22)}var u=new IntersectionObserver(function(r){for(var e=0;e<r.length;e++)if(r[e].isIntersecting)return l()}, {threshold:.1,rootMargin:"0px 0px -15% 0px"});u.observe(o[0]);requestAnimationFrame(l)}i()}
			}),
			feature({
