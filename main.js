	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: r=>{if(!gsap||!ScrollTrigger)return;r=r||document;let LOCKED=!1;document.addEventListener("click",e=>{if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){LOCKED=!0;setTimeout(()=>{LOCKED=!1;ScrollTrigger.refresh()},350);}});const S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;S.forEach((sec,i)=>{const small=sec.offsetHeight<innerHeight,start=small?"top top":"bottom bottom";gsap.timeline({scrollTrigger:{trigger:sec,start:i===0?"top top":start,end:"bottom top",pin:!0,pinSpacing:!1,scrub:1,onUpdate:self=>{if(LOCKED)return self.progress(self.progress());}}}).to(sec,{filter:"contrast(10%) blur(10px)",ease:"none",startAt:{filter:"contrast(100%) blur(0px)"}})});
			}),
			feature({
				id: "productGallery",
				stage: "main",
