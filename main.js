...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		// Entry Animations
		var EntryAnimations = {};
		EntryAnimations.selected=function(e){e=e||document;const t=e.querySelector(".selected-container"),n=t&&t.querySelector(".selected-content"),i=gsap.timeline();if(!t||!n)return i;function r(){try{document.documentElement.removeAttribute("data-preloading")}catch{}[t,n].forEach(e=>{e&&(e.style.removeProperty("display"),e.style.visibility="visible")})}function o(e){if(e.__entryPrepared)return;const t=e.querySelector(".selected-visual"),n=e.querySelector(".selected-item-header .headline-m"),i=e.querySelector(".selected-item-details"),r=e.querySelectorAll(".selected-item-details .body-s");t&&gsap.set(t,{scaleY:0,transformOrigin:"bottom center",opacity:0}),n&&gsap.set(n,{opacity:0}),i&&gsap.set(i,{opacity:0,height:0}),r.length&&gsap.set(r,{opacity:0,y:20,filter:"blur(10px)"}),e.__entryPrepared=!0}function a(){const e=Array.from(n.querySelectorAll(".selected-item-outer"));if(!e.length)return i;e.forEach(o);const r=Math.max(document.documentElement.clientWidth,window.innerWidth||0),a=window.innerHeight||document.documentElement.clientHeight||0,l=e.map(e=>{const t=e.getBoundingClientRect(),n=Math.max(0,Math.min(t.right,r)-Math.max(t.left,0))*Math.max(0,Math.min(t.bottom,a)-Math.max(t.top,0));return{el:e,r:t,area:n,cx:.5*(t.left+t.right)}});let s=l.filter(e=>e.area>1).sort((e,t)=>e.r.left-t.r.left);if(!s.length){const e=r/2;s=l.slice().sort((t,n)=>Math.abs(t.cx-e)-Math.abs(n.cx-e)).slice(0,2).sort((e,t)=>e.r.left-t.r.left)}const d=new Set(s.map(e=>e.el));l.forEach(e=>{if(e.el.__entryDone||d.has(e.el))return;const t=e.el.querySelector(".selected-visual"),n=e.el.querySelector(".selected-item-header .headline-m"),i=e.el.querySelector(".selected-item-details"),r=e.el.querySelectorAll(".selected-item-details .body-s");t&&gsap.set(t,{scaleY:1,opacity:1}),n&&gsap.set(n,{opacity:1}),i&&gsap.set(i,{opacity:1,height:"auto"}),r.length&&gsap.set(r,{opacity:1,y:0,filter:"blur(0px)"}),e.el.__entryDone=!0}),s.forEach((e,t)=>{const n=e.el;if(n.__entryDone)return;const r=n.querySelector(".selected-visual"),o=n.querySelector(".selected-item-header .headline-m"),a=n.querySelector(".selected-item-details"),l=n.querySelectorAll(".selected-item-details .body-s"),s=.15*t;r&&i.set(r,{opacity:1},s).to(r,{scaleY:1,duration:.8,ease:"power2.out"},s),o&&i.set(o,{opacity:1},s+.2).call(()=>{if(window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){const e=splitAndMask(o);gsap.delayedCall(.15,()=>{animateLines(e.lines).eventCallback("onComplete",()=>safelyRevertSplit(e,o))})}else gsap.to(o,{y:0,opacity:1,duration:.55,ease:"power2.out"})},null,s+.2),a&&i.to(a,{opacity:1,height:"auto",duration:.4,ease:"power2.out"},s+.6),l.length&&i.to(l,{opacity:1,y:0,filter:"blur(0px)",duration:.4,ease:"power2.out",stagger:.15},s+.6),n.__entryDone=!0});return i}function l(){r();return a()}if(t.hasAttribute("data-loop-ready"))return l(),i;let s=!1;const d=()=>{s||(s=!0,n.removeEventListener("selected:loop-ready",d,!0),l())};return n.addEventListener("selected:loop-ready",d,!0),setTimeout(d,600),i};
