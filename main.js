	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger||!window.gsap)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill(),ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const s=v=>{v!==l&&(l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"}))};const sd=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>s(bg),onLeaveBack:()=>s(cs),refreshPriority:100}),sm=ScrollTrigger.create({id:"csbg:more",trigger:m,start:"top 80%",onEnter:()=>s(bd),onLeaveBack:()=>s(bg),refreshPriority:100});const y=()=>{const t=window.pageYOffset||document.documentElement.scrollTop||0;s(t>=sm.start?bd:t>=sd.start?bg:cs)};ScrollTrigger.addEventListener("refresh",y);requestAnimationFrame(()=>ScrollTrigger.refresh());setTimeout(()=>ScrollTrigger.refresh(),150);y()}
			}),
			feature({
