	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apxVM)return;host.__apxVM=1;if(!window.gsap||!window.ScrollTrigger){host.__apxVM=0;return}var gs=window.gsap,ST=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches){host.__apxVM=0;return}var P=host.querySelector(".section-archive-playful");if(!P){host.__apxVM=0;return}var un=[],io=0,raf=0,run=0,live=new Map,all=[],onScr=0,st=0;function on(n,e,f,o){n&&n.addEventListener(e,f,o),un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function pickParts(it){var pd=qs(it,".archive-playful-data-product"),pe=qs(it,".archive-playful-data-expertise");if(!pd.length&&!pe.length){pd=qs(it,".archive-playful-data-inner .headline-xxs");pe=qs(it,".archive-playful-data-inner .body-sm")}return pd.concat(pe)}function pickLink(it){return q(it,".archive-playful-item-data-outer .case-study-url")||q(it,".archive-playful-item-data .shortcut-link")}function mkTL(it){var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickParts(it),ln=pickLink(it),tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.fromTo(imgs,{yPercent:-15},{yPercent:15,duration:1},0);parts.length&&tl.to(parts,{opacity:0,duration:.08},.70);di&&tl.to(di,{marginTop:0,marginBottom:0,duration:.08},.70);parts.length&&tl.to(parts,{height:0,duration:.08},.75);ln&&tl.to(ln,{opacity:0,duration:.08},.75);ln&&tl.to(ln,{height:0,duration:.08},.80);imgs.length&&tl.to(imgs,{scale:1.5,duration:.20},.80);v&&tl.to(v,{height:0,duration:.20},.80);return tl}function prep(it){if(it.__apxP)return;it.__apxP=1;var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickParts(it),ln=pickLink(it);try{imgs.length&&gs.set(imgs,{willChange:"transform"});v&&gs.set(v,{willChange:"height",overflow:"hidden"});di&&gs.set(di,{willChange:"margin"});parts.length&&gs.set(parts,{willChange:"opacity,height",overflow:"hidden"});ln&&gs.set(ln,{willChange:"opacity,height",overflow:"hidden"})}catch(_){}}function getP(it){var vh=innerHeight||0;try{var r=it.getBoundingClientRect();return vh?Math.max(0,Math.min(1,1-(r.top/vh))):0}catch(_){return 0}}function tick(){raf=0;if(!mm.matches||!vis()||!live.size)return;run++;live.forEach(function(o,it){if(!o||!o.tl)return;var p=getP(it);if(o.p!==p){o.p=p;try{o.tl.progress(p)}catch(_){}}})}function sched(){raf||(raf=requestAnimationFrame(tick))}function mount(it){if(live.has(it))return;prep(it);var tl=mkTL(it);live.set(it,{tl:tl,p:-1});try{tl.progress(getP(it))}catch(_){}}function unmount(it){var o=live.get(it);if(!o)return;try{o.tl&&o.tl.kill&&o.tl.kill()}catch(_){ }live.delete(it)}function killAll(){try{io&&io.disconnect&&io.disconnect()}catch(_){ }io=0;live.forEach(function(_,it){unmount(it)});live.clear();all.length=0;try{st&&st.kill&&st.kill()}catch(_){ }st=0}function initIO(){killAll();if(!mm.matches||!vis())return;all=[].slice.call(P.querySelectorAll(".archive-playful-item"));if(!all.length)return;io=new IntersectionObserver(function(es){for(var i=0;i<es.length;i++){var e=es[i],it=e.target;if(e.isIntersecting)mount(it);else unmount(it)}sched()},{threshold:0,rootMargin:"200% 0px 200% 0px"});for(var i=0;i<all.length;i++)try{io.observe(all[i])}catch(_){ }st=ST.create({trigger:P,start:"top top",end:"bottom bottom",onUpdate:sched,onRefresh:sched});sched()}function sync(){if(!mm.matches||!vis())killAll();else initIO()}on(document,"archive:view",sync,{passive:!0});on(document,"archive:filter:applied",sync,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sync):mm.addListener(sync)}catch(_){ }un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sync):mm.removeListener(sync)}catch(_){}});requestAnimationFrame(function(){setTimeout(sync,0)});return function destroy(){try{killAll()}catch(_){ }for(var i=0;i<un.length;i++)un[i]();un.length=0;try{host.__apxVM=0}catch(_){}}}
			})
		);
