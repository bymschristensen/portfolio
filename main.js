...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),h=e.querySelector(".cs-headline"),s=e.querySelectorAll(".cs-titles-inner div"),u=o=>{if(!o)return;try{o.removeAttribute("data-entry-hidden"),o.removeAttribute("hidden"),o.removeAttribute("aria-hidden"),o.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(c=>o.classList&&o.classList.remove(c))}catch{}};i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"});h&&gsap.set(h,{autoAlpha:0});s.length&&gsap.set(s,{autoAlpha:0,y:20,filter:"blur(10px)"});u(i);u(h);s.forEach(u);i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(h){t.addLabel("headline",.35).set(h,{autoAlpha:1},"headline").call(()=>{const r=()=>{try{if(!window.splitAndMask||!window.animateLines){gsap.to(h,{y:0,opacity:1,duration:.6,ease:"power2.out"});return}const cs=getComputedStyle(h),origWS=h.style.whiteSpace||"",origDisp=h.style.display||"",wasInline=(cs.display||"").includes("inline");if(wasInline)h.style.display="block";h.style.whiteSpace=cs.whiteSpace||"pre-wrap";const sp=splitAndMask(h);try{gsap.set(sp.lines,{display:"block",whiteSpace:cs.whiteSpace||"pre-wrap"});}catch{}animateLines(sp.lines).eventCallback("onComplete",()=>{try{window.safelyRevertSplit?safelyRevertSplit(sp,h):sp.revert&&sp.revert()}catch{}try{h.style.whiteSpace=origWS}catch{}try{h.style.display=origDisp}catch{}})}catch{gsap.to(h,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>{requestAnimationFrame(()=>requestAnimationFrame(r))}).catch(r)},null,"headline")}s.length&&t.to(s,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t}
		};
