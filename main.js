...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async e=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),a=e=>parseFloat(getComputedStyle(e).gap||"0")||0,i=e=>{const t=parseFloat(e.getAttribute("data-ratio"));if(t&&t>0)return t;const r=(e.className||"").match(/sv-ratio-(\d+)/);if(r){const e=r[1];if("169"===e)return 16/9;if("133"===e||"43"===e)return 4/3;if("11"===e)return 1}const a=e.querySelector("img,video");if(a){const e=parseInt(a.getAttribute("width"),10)||a.naturalWidth||a.videoWidth||0,t=parseInt(a.getAttribute("height"),10)||a.naturalHeight||a.videoHeight||0;if(e>0&&t>0)return e/t}return 1.5},s=(e,t)=>{const r=parseFloat(e.getAttribute("data-size")),a=t.closest(".section-single-service"),i=a&&a.clientHeight||innerHeight;if(r&&i)return Math.max(1,Math.round(i*r/100));const s=e.style.height;e.style.height="";const o=Math.round(parseFloat(getComputedStyle(e).height)||e.getBoundingClientRect().height||0);return e.style.height=s,o>8?o:Math.round(.6*(i||innerHeight))},o=e=>{const t=a(e);let r=0;return e.childNodes.forEach(e=>{1===e.nodeType&&(r+=(e.getBoundingClientRect().width||0)+t)}),r},n=(e,t)=>{const a=e.cloneNode(!0);a.setAttribute("data-clone","1"),a.dataset.key=e.dataset.key||"";const s=parseFloat(e.dataset.ratio)||i(e),o=+e.dataset.targetH||0,n=Math.max(1,Math.round(o*s));return a.style.overflow="hidden",a.style.flex="0 0 auto",a.style.willChange="height,transform",a.style.minWidth=n+"px",a.style.maxWidth=n+"px",r&&(a.style.aspectRatio=s+" / 1"),a.style.height="0px",t.appendChild(a),a},l=e=>{[...e.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove())};t.forEach(async e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0,e.setAttribute("data-armed","measure");const d=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!d.length)return void e.setAttribute("data-armed","1");await(e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const r=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,a=[],i=t.map(e=>{if(r(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),a=new Promise(t=>{const r=()=>{e.removeEventListener("load",r),t()};e.addEventListener("load",r,{once:!0})}),i=new Promise(t=>{let a=0;!function i(){if(r(e)||a++>80)return t();setTimeout(i,40)}()});return Promise.race([t,a,i])}),s=new Promise(e=>a.push(setTimeout(e,3500)));return Promise.race([Promise.all(i),s]).finally(()=>a.forEach(clearTimeout))})(t),d.forEach((e,a)=>{const o=i(e),n=s(e,t),l=Math.max(1,Math.round(n*o));e.dataset.key=String(a),e.dataset.ratio=String(o),e.dataset.targetH=String(n),e.dataset.revealed="1"===e.dataset.revealed?"1":"0",e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.willChange="height,transform",e.style.minWidth=l+"px",e.style.maxWidth=l+"px",r&&(e.style.aspectRatio=o+" / 1"),"1"!==e.dataset.revealed&&(e.style.height="0px")});const c="right"===(e.dataset.direction||"left").toLowerCase()?-1:1,h=parseFloat(e.dataset.speed)||.6,u=()=>{l(t),d.forEach(e=>n(e,t)),d.forEach(e=>n(e,t));let e=0;const r=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;o(t)<r&&e++<8;)d.forEach(e=>n(e,t))};u(),e.setAttribute("data-armed","1");let g=0;const p=(e,r=16.6667)=>{g-=c*((r||16.6667)/1e3*60)*h;const i=a(t),s=e=>(e.getBoundingClientRect().width||0)+i;let o=t.firstElementChild,n=0;for(;o&&g<-s(o)&&n++<50;)g+=s(o),t.appendChild(o),o=t.firstElementChild;let l=t.lastElementChild;for(n=0;l&&g>0&&n++<50;)g-=s(l),t.insertBefore(l,t.firstElementChild),l=t.lastElementChild;gsap.set(t,{x:g});const d=(innerWidth||0)/2;t.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),a=(d-(r.left+r.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*a+"px")})};gsap.ticker.add(p),CoreUtilities.Observers.addTicker(p);const f=(e,r=0)=>{if(!e||"1"===e.dataset.revealed)return;const a=+e.dataset.targetH||0;if(!a)return e.dataset.revealed="1",void(e.style.height="");gsap.fromTo(e,{height:0},{height:a,duration:.9,ease:"power2.out",delay:r,onComplete:()=>{e.style.height="",e.dataset.revealed="1";const r=e.dataset.key||"";r&&t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${r}"]`).forEach(e=>e.style.height=a+"px")}})};let m=!1;const y=CoreUtilities.Observers.addDom(new IntersectionObserver(r=>{r.forEach(r=>{r.target===e&&r.isIntersecting&&(m||(m=!0,(()=>{const e=innerHeight,r=d.filter(e=>"1"!==e.dataset.revealed).map(t=>{const r=t.getBoundingClientRect();return{w:t,area:Math.max(0,Math.min(r.bottom,e)-Math.max(r.top,0))*Math.max(0,Math.min(r.right,innerWidth)-Math.max(r.left,0)),dist:Math.abs(.5*(r.top+r.bottom)-.5*innerHeight)}}).filter(e=>e.area>1).sort((e,t)=>e.dist-t.dist),a=r.slice(0,7),i=r.slice(7);a.forEach((e,t)=>f(e.w,.12*t)),i.forEach(e=>{const r=+e.w.dataset.targetH||0;e.w.style.height="",e.w.dataset.revealed="1";const a=e.w.dataset.key||"";a&&t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${a}"]`).forEach(e=>e.style.height=r+"px")})})()),y.unobserve(e))})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));y.observe(e);const v=CoreUtilities.Observers.addDom(new IntersectionObserver((e,t)=>{e.forEach(e=>{const r=e.target;e.isIntersecting&&("1"!==r.dataset.revealed&&f(r,0),t.unobserve(r))})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));d.forEach(e=>v.observe(e));const w=new ResizeObserver(()=>{l(t),d.forEach(e=>{const a=parseFloat(e.dataset.ratio)||i(e),o=s(e,t);e.dataset.targetH=String(o);const n=Math.max(1,Math.round(o*a));e.style.minWidth=n+"px",e.style.maxWidth=n+"px",r&&(e.style.aspectRatio=a+" / 1")}),u(),d.forEach(e=>{if("1"===e.dataset.revealed){const r=+e.dataset.targetH||0,a=e.dataset.key||"";a&&t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${a}"]`).forEach(e=>e.style.height=r+"px")}})});w.observe(t),CoreUtilities.Observers.addDom(w)})}
			})
		);
