...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy=function(root){var tl=gsap.timeline(),hero=root.querySelector(".cs-hero-image"),headWrap=root.querySelector(".cs-headline"),headEl=headWrap&&(headWrap.querySelector("h1, .headline-xl, .headline-lg, .headline, [data-split], [role='heading'][aria-level='1']")||[].slice.call(headWrap.querySelectorAll("*")).find(function(n){return((n.textContent||"").trim().length&&getComputedStyle(n).display!=="none")})||headWrap),sub=root.querySelector(".cs-titles-inner .headline-m"),unhide=function(e){if(!e)return;try{e.removeAttribute("data-entry-hidden");e.removeAttribute("hidden");e.removeAttribute("aria-hidden");e.style.visibility="visible";["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(function(c){e.classList&&e.classList.remove(c)})}catch(_){} };if(hero){gsap.set(hero,{autoAlpha:0,y:100,filter:"blur(10px)"});unhide(hero);tl.to(hero,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.7,ease:"power2.out"},0)}if(headEl){gsap.set(headEl,{autoAlpha:0,visibility:"hidden",display:"block"});unhide(headEl);if(sub){gsap.set(sub,{autoAlpha:0,y:20,filter:"blur(10px)"});unhide(sub)}var fontsReady=(window.CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready&&CoreUtilities.Fonts.ready())||Promise.resolve();var splitReady=new Promise(function(res){var t=0;(function wait(){if(window.SplitText&&window.animateLines)return res();t++;if(t>40)return res();setTimeout(wait,50)})()});tl.addLabel("h",.15).call(function(){Promise.race([fontsReady,splitReady]).then(function(){requestAnimationFrame(function(){requestAnimationFrame(function(){var split; if(window.splitAndMask){split=splitAndMask(headEl)}else{var w=document.createElement("div"),line=document.createElement("div");w.className="text-mask";w.style.overflow="hidden";line.className="split-line";line.innerHTML=headEl.innerHTML;w.appendChild(line);headEl.innerHTML="";headEl.appendChild(w);gsap.set(line,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform"});split={lines:[line],revert:function(){try{headEl.innerHTML=line.innerHTML}catch(_){}}}}var lines=(split&&split.lines&&split.lines.length)?split.lines:[headEl];var headlineDur=.8+.08*Math.max(0,lines.length-1);gsap.set(headEl,{autoAlpha:1,visibility:"visible"});if(sub){tl.to(sub,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.5,ease:"power2.out"},"h+"+Math.max(0,headlineDur-.2))}var anim=window.animateLines?animateLines(lines):gsap.to(lines,{yPercent:0,rotation:0,duration:headlineDur,ease:"power2.out",stagger:.08});anim.eventCallback("onComplete",function(){try{(window.safelyRevertSplit?window.safelyRevertSplit:split.revert).call(split,split,headEl)}catch(_){} try{if(getComputedStyle(headEl).display==="inline"){headEl.style.display="inline"}}catch(_){} })})})})},null,"h")}return tl}
		};
