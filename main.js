	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||document.querySelector(".barba-container")||r;if(!c||c.__csbgInit)return;c.__csbgInit=1;const d=c.querySelector(".cs-details"),mw=c.querySelector(".cs-morework");if(!d||!mw){c.__csbgInit=0;return}const more=mw.querySelector(".cs-morework-outer")||mw,cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let last="";c.style.transition="background-color .6s ease";const set=v=>{if(v===last)return;last=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})},kill=()=>{try{ScrollTrigger.getById("csbg:details")?.kill(),ScrollTrigger.getById("csbg:more")?.kill()}catch{}},rf=()=>requestAnimationFrame(()=>ScrollTrigger.refresh());kill();ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>set(bg),onLeaveBack:()=>set(cs),invalidateOnRefresh:!0,refreshPriority:100});ScrollTrigger.create({id:"csbg:more",trigger:more,start:"top 80%",onEnter:()=>set(bd),onLeaveBack:()=>set(bg),invalidateOnRefresh:!0,refreshPriority:110});rf(),setTimeout(rf,160),setTimeout(rf,520),setTimeout(rf,1100),window.addEventListener("load",rf,{once:!0});try{document.fonts&&document.fonts.ready&&document.fonts.ready.then(rf).catch(()=>{})}catch{}const g=c.querySelector(".cs-gallery");if(g){const imgs=[...g.querySelectorAll("img")];if(imgs.length){let n=0;const done=()=>{if(++n<imgs.length)return;rf(),setTimeout(rf,160)};imgs.forEach(im=>{if(im.complete&&im.naturalWidth>0)return void done();im.addEventListener("load",done,{once:!0}),im.addEventListener("error",done,{once:!0})})}}return()=>{kill();try{delete c.__csbgInit}catch{}}}
			}),
			feature({
