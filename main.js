...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.info=function(e){e=e||document;var t=gsap.timeline(),n=e.querySelector(".hero-section-text h1"),i=e.querySelector(".hero-subtext"),o=[...e.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((e,t)=>(+e.getAttribute("data-photo")||0)-(+t.getAttribute("data-photo")||0));function r(e){try{var t=getComputedStyle(e).height;return!!t&&"auto"!==t&&e.getBoundingClientRect().height>0}catch{return!1}}function a(e){gsap.set(e,{overflow:"hidden",transformOrigin:"bottom center",willChange:"transform,height,opacity"});var t=e.querySelector("img,video,.bio-photo,.service-visual,.photo")||e.firstElementChild;t&&gsap.set(t,{willChange:"transform,opacity"})}n&&(gsap.set(n,{autoAlpha:0}),t.addLabel("h",0).set(n,{autoAlpha:1},"h").call((function(){(CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve()).then((function(){if(window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){var e=splitAndMask(n);animateLines(e.lines).eventCallback("onComplete",(function(){safelyRevertSplit(e,n)}))}else gsap.to(n,{y:0,opacity:1,duration:.6,ease:"power2.out"})}))}),null,"h"));i&&(gsap.set(i,{autoAlpha:0,y:20,filter:"blur(10px)"}),t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},"h+=0.48"));if(o.length){o.forEach(a);var s=o.every(r),l=s?{from:{height:"0%"},to:{height:"100%"}}:{from:{scaleY:0},to:{scaleY:1}};o.forEach((e=>{gsap.set(e,{...l.from,opacity:0})})),t.to(o.map((e=>e)),{opacity:1,duration:.1,ease:"none"},"h+=0.8").to(o.map((e=>e)),{...l.to,duration:.8,ease:"power2.out",stagger:.12,clearProps:"transform,height"},">0.02")}return t};
