...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(root){if(!gsap||!ScrollTrigger)return;root=root||document;var sections=[].slice.call(root.querySelectorAll(".section-single-service"));if(!sections.length)return;var pins=[];ScrollTrigger._ppsPins=pins;sections.forEach(function(sec){var small=sec.offsetHeight<window.innerHeight,tl=gsap.timeline({scrollTrigger:{trigger:sec,start:small?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}}).to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0);tl.scrollTrigger&&pins.push(tl.scrollTrigger)});if(!pins.length)return;ScrollTrigger._ppsEventsBound||(ScrollTrigger._ppsEventsBound=!0,ScrollTrigger.addEventListener("refreshInit",function(){var list=ScrollTrigger._ppsPins||[];list.forEach(function(st){if(!st)return;st._ppsPrevProgress=st.progress;st.animation&&st.animation.progress(0);st.trigger&&gsap.set(st.trigger,{filter:"contrast(100%) blur(0px)"});st.disable()})}),ScrollTrigger.addEventListener("refresh",function(){var list=ScrollTrigger._ppsPins||[];list.forEach(function(st){if(!st)return;st.enable(!1,!1);st.animation&&st.animation.progress(0)})}))}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
