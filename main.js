	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async r=>{const t=".appear-in-line",o=":scope > *",n=.15,i=.8;function a(e){if(e.__ailArmed)return;e.__ailArmed=!0;const t=[],a=[];Array.from(e.querySelectorAll(o)).forEach(o=>{const r=getComputedStyle(o),s=parseInt(r.columnCount,10)||1,l=o.getBoundingClientRect();if(s>1){try{gsap.set(o,{opacity:0});const r=new SplitText(o,{type:"lines",linesClass:"split-line"});t.push(r);const c=l.width/s,u=Array.from({length:s},()=>[]);r.lines.forEach(t=>{const r=t.getBoundingClientRect().left-l.left,o=Math.min(Math.floor(r/c),s-1);u[o].push(t),gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})}),u.forEach(e=>a.push(e))}catch{gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),a.push([o])}}else gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),a.push([o])});const s=new IntersectionObserver((t,o)=>{t.forEach(t=>{if(!t.isIntersecting)return;o.unobserve(t.target),a.forEach((t,o)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:o*n})});const e=(a.length-1)*n+i;gsap.delayedCall(e+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})})},{root:null,rootMargin:"0px 0px -10% 0px",threshold:0});CoreUtilities.Observers.addDom(s);s.observe(e);const l=e.getBoundingClientRect();if(l.top<window.innerHeight&&l.bottom>0){s.unobserve(e),a.forEach((t,e)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:e*n})});const o=(a.length-1)*n+i;gsap.delayedCall(o+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})}const c=r.querySelector(".w-tabs");if(c){const r=e.closest(".w-tab-pane");if(r){const f=new MutationObserver(()=>{if(!r.classList.contains("w--tab-active"))return;s.observe(e);const o=e.getBoundingClientRect();if(o.top<window.innerHeight&&o.bottom>0){s.unobserve(e),a.forEach((t,e)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:e*n})});const r=(a.length-1)*n+i;gsap.delayedCall(r+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})}});CoreUtilities.Observers.addDom(f);f.observe(r,{attributes:!0,attributeFilter:["class"]})}}}r.querySelectorAll(t).forEach(a)}
			}),
