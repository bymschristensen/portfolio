	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init:async r=>{const s=".appear-in-line";(()=>{if(document.getElementById("ail-style"))return;const e=document.createElement("style");e.id="ail-style",e.textContent='.appear-in-line[data-ail-armed="1"]>:not(:empty){opacity:0;transform:translateY(100px);filter:blur(10px);will-change:transform,opacity}',document.head.appendChild(e)})();function a(e){if(e.__ailArmed)return;e.__ailArmed=!0,e.setAttribute("data-ail-armed","1");let t=[],i=[];function n(){try{t.forEach(e=>e.revert())}catch{}t=[],i=[],[...e.querySelectorAll(":scope > *:not(:empty)")].forEach(o=>{const r=getComputedStyle(o),s=parseInt(r.columnCount,10)||1,c=o.getBoundingClientRect();if(s>1){const r=new SplitText(o,{type:"lines",linesClass:"split-line"});t.push(r);const l=c.width/s,u=Array.from({length:s},()=>[]);r.lines.forEach(t=>{const i=t.getBoundingClientRect().left-c.left,n=Math.min(s-1,Math.max(0,Math.floor(i/l)));u[n].push(t)}),u.forEach(d=>{d.length&&(gsap.set(d,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),i.push(d))})}else gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),i.push([o])})}function o(){if(e.__ailPlaying)return;e.__ailPlaying=!0,e.__ailDone=!1,n(),i.forEach((e,t)=>gsap.to(e,{y:0,opacity:1,filter:"blur(0px)",duration:.8,ease:"power2.out",delay:.12*t}));const o=.12*(i.length-1)+.85;gsap.delayedCall(o,()=>{try{t.forEach(e=>e.revert())}catch{}e.removeAttribute("data-ail-armed"),e.__ailPlaying=!1,e.__ailDone=!0})}const c=CoreUtilities.Observers.addDom(new IntersectionObserver((t,i)=>{t.forEach(t=>{t.isIntersecting&&(i.unobserve(e),o())})},{root:null,rootMargin:"0px 0px -12% 0px",threshold:0}));c.observe(e);const l=e.getBoundingClientRect();l.top<innerHeight&&l.bottom>0&&(c.unobserve(e),requestAnimationFrame(o)),e.__ailReplay=(()=>{e.__ailPlaying||(e.setAttribute("data-ail-armed","1"),e.__ailDone=!1,o())}),e.__ailTearDown=(()=>{try{t.forEach(e=>e.revert())}catch{}t=[],i=[],e.removeAttribute("data-ail-armed")})}r.querySelectorAll(s).forEach(a),r.querySelectorAll(".w-tabs .w-tab-pane").forEach(e=>{CoreUtilities.Observers.addDom(new MutationObserver(()=>{e.classList.contains("w--tab-active")&&e.querySelectorAll(s).forEach(t=>{a(t),t.__ailReplay&&t.__ailReplay()})}).observe(e,{attributes:!0,attributeFilter:["class"]})})}
			}),
