...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy:e=>{const t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),n=e.querySelector(".cs-headline"),o=[...e.querySelectorAll(".cs-titles-inner div")],r=e=>{if(!e)return;try{e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList&&e.classList.remove(t))}catch{}};i&&gsap.set(i,{autoAlpha:0,y:80,filter:"blur(10px)"});n&&gsap.set(n,{opacity:0});o.length&&gsap.set(o,{autoAlpha:0,y:20,filter:"blur(10px)"});r(i),r(n),o.forEach(r);i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0);if(n){t.addLabel("headline",.35).call(()=>{const e=()=>{try{const t=getComputedStyle(n),a=t.display,s=t.whiteSpace||"normal",l=n.getBoundingClientRect().height|0,u={display:n.style.display,width:n.style.width,whiteSpace:n.style.whiteSpace};if(a&&a.indexOf("flex")>-1){n.style.display="block";n.style.width="100%"}n.style.whiteSpace=s;n.style.height=l+"px";if(window.splitAndMask&&window.animateLines){const e=splitAndMask(n);try{e.lines&&e.lines.forEach(d=>{d.style.whiteSpace=s;d.style.display="block";d.style.width="100%"})}catch{}gsap.set(n,{opacity:1});animateLines(e.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:e.revert).call(e,e,n)}catch{}requestAnimationFrame(()=>{n.style.height=l+"px";requestAnimationFrame(()=>{n.style.height="";n.style.display=u.display||"";n.style.width=u.width||"";n.style.whiteSpace=u.whiteSpace||""})})})}else{gsap.to(n,{opacity:1,y:0,duration:.6,ease:"power2.out",onComplete:()=>{n.style.height="";n.style.display=u.display||"";n.style.width=u.width||"";n.style.whiteSpace=u.whiteSpace||""}})}}catch{gsap.to(n,{opacity:1,y:0,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>new Promise(t=>requestAnimationFrame(()=>requestAnimationFrame(t)))).then(e).catch(e)},null,"headline")}return o.length&&t.to(o,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6),t}
		};
