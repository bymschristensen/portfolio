	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apxBound)return;host.__apxBound=1;if(!window.gsap||!window.ScrollTrigger)return host.__apxBound=0,void 0;var gs=window.gsap,ST=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches)return host.__apxBound=0,void 0;var P=host.querySelector(".section-archive-playful");if(!P)return host.__apxBound=0,void 0;var un=[],io=0,raf=0,live=new Map;function on(n,e,f,o){n&&n.addEventListener(e,f,o),un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function getItems(){return[].slice.call(P.querySelectorAll(".archive-playful-item"))}function prep(it,idx){var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),hx=q(it,".archive-playful-data-inner .headline-xxs"),bx=q(it,".archive-playful-data-inner .body-sm"),sl=q(it,".archive-playful-item-data .shortcut-link");v&&gs.set(v,{height:0,opacity:0,overflow:"hidden",willChange:"height,opacity"});di&&gs.set(di,{marginTop:0,willChange:"margin-top"});hx&&gs.set(hx,{opacity:0,height:0,overflow:"hidden",willChange:"height,opacity"});bx&&gs.set(bx,{opacity:0,height:0,overflow:"hidden",willChange:"height,opacity"});sl&&gs.set(sl,{opacity:0,height:0,overflow:"hidden",y:0,willChange:"transform,opacity,height"});imgs.length&&imgs.forEach(function(im){try{gs.set(im,{scale:1.5,yPercent:-15,transformOrigin:"50% 50%",willChange:"transform"})}catch(_){}});if(idx===0){v&&gs.set(v,{height:"45vh",opacity:1});di&&gs.set(di,{marginTop:"1rem"});hx&&gs.set(hx,{opacity:1,height:"1rem"});bx&&gs.set(bx,{opacity:1,height:"3rem"});sl&&gs.set(sl,{opacity:1,height:"1rem",y:0})}}function makeTL(it){var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),hx=q(it,".archive-playful-data-inner .headline-xxs"),bx=q(it,".archive-playful-data-inner .body-sm"),sl=q(it,".archive-playful-item-data .shortcut-link");var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});v&&tl.set(v,{opacity:1},.01);imgs.length&&tl.to(imgs,{yPercent:15,duration:1},0).to(imgs,{keyframes:[{scale:1.5},{scale:1.1},{scale:1.1},{scale:1.5}],times:[0,.30,.70,1],duration:1},0);v&&tl.to(v,{height:"45vh",duration:.30},.30).to(v,{height:"45vh",duration:.40},.30).to(v,{height:0,duration:.30},.70).to(v,{opacity:0,duration:0},1);di&&tl.to(di,{marginTop:"1rem",duration:.70},.10).to(di,{marginTop:"1rem",duration:.10},.80).to(di,{marginTop:0,duration:.20},.80);hx&&tl.set(hx,{opacity:0,height:0},.10).to(hx,{opacity:1,height:"1rem",duration:.18},.28).to(hx,{opacity:0,height:0,duration:.08},.92);bx&&tl.set(bx,{opacity:0,height:0},.15).to(bx,{opacity:1,height:"3rem",duration:.18},.30).to(bx,{opacity:0,height:0,duration:.10},.88);sl&&tl.set(sl,{opacity:0,height:0,y:"2rem"},.15).to(sl,{opacity:1,height:"1rem",y:0,duration:.18},.30).to(sl,{opacity:1,height:"1rem",duration:.40},.30).to(sl,{opacity:0,height:0,y:"2rem",duration:.10},.88).to(sl,{y:0,duration:0},1);return tl}function mount(it,idx){if(live.has(it))return;prep(it,idx);var tl=makeTL(it);var st=ST.create({trigger:it,start:"top 100%",end:"top 0%",scrub:!0,onUpdate:function(self){try{tl.progress(self.progress)}catch(_){}}});live.set(it,{tl:tl,st:st})}function unmount(it){var o=live.get(it);if(!o)return;try{o.st&&o.st.kill&&o.st.kill()}catch(_){ }try{o.tl&&o.tl.kill&&o.tl.kill()}catch(_){ }live.delete(it)}function killAll(){try{live.forEach(function(_,it){unmount(it)})}catch(_){ }live.clear();try{io&&io.disconnect&&io.disconnect()}catch(_){ }io=0}function initIO(){killAll();if(!mm.matches||!vis())return;var list=getItems();io=new IntersectionObserver(function(es){for(var i=0;i<es.length;i++){var e=es[i],it=e.target;if(e.isIntersecting){mount(it,list.indexOf(it))}else{unmount(it)}}},{root:null,threshold:0,rootMargin:"150% 0px 150% 0px"});for(var i=0;i<list.length;i++)try{io.observe(list[i])}catch(_){}try{ST.refresh()}catch(_){}}function sched(){raf||(raf=requestAnimationFrame(function(){raf=0;initIO()}))}function onView(){sched()}function onMQ(){sched()}on(document,"archive:view",onView,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",onMQ):mm.addListener(onMQ)}catch(_){ }un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",onMQ):mm.removeListener(onMQ)}catch(_){}});requestAnimationFrame(initIO);return function destroy(){try{killAll()}catch(_){ }un.forEach(function(fn){fn()});un.length=0;try{host.__apxBound=0}catch(_){}}}
			})
		);
