	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(r){r=r||document;var h=r.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||r.querySelector('[data-barba="container"]')||r;if(h.__apx3Bound)return;h.__apx3Bound=1;if(!window.gsap||!window.ScrollTrigger)return void(h.__apx3Bound=0);var g=window.gsap,S=window.ScrollTrigger,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches)return void(h.__apx3Bound=0);var P=h.querySelector(".section-archive-playful");if(!P)return void(h.__apx3Bound=0);var un=[],st=0,raf=0,items=[],cfg=[],built=!1;function on(n,e,f,o){n&&n.addEventListener(e,f,o),un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(_){return!0}}function list(){return[].slice.call(P.querySelectorAll(".archive-playful-item"))}function prep(it){if(!it||it.__apx3Prep)return;it.__apx3Prep=1;var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),dp=qs(it,".archive-playful-data-inner .archive-playful-data-product, .archive-playful-data-inner .archive-playful-data-expertise"),ln=q(it,".archive-playful-item-data-outer .case-study-url");v&&g.set(v,{willChange:"height"});imgs.length&&imgs.forEach(function(im){try{g.set(im,{willChange:"transform"})}catch(_){}});di&&g.set(di,{willChange:"margin-top,margin-bottom"});dp.length&&dp.forEach(function(el){try{g.set(el,{willChange:"height,opacity"})}catch(_){}});ln&&g.set(ln,{willChange:"height,opacity"})}function makeTL(it){var v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),dp=qs(it,".archive-playful-data-inner .archive-playful-data-product, .archive-playful-data-inner .archive-playful-data-expertise"),ln=q(it,".archive-playful-item-data-outer .case-study-url"),t=g.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&t.to(imgs,{yPercent:15,duration:1},0);dp.length&&t.to(dp,{opacity:0,duration:.08},.70);di&&t.to(di,{marginTop:"0rem",marginBottom:0,duration:.08},.70);dp.length&&t.to(dp,{height:"0rem",duration:.08},.75);ln&&t.to(ln,{opacity:0,duration:.08},.75);ln&&t.to(ln,{height:"0rem",duration:.08},.80);imgs.length&&t.to(imgs,{scale:1.5,duration:.20},.80);v&&t.to(v,{height:"0vh",duration:.20},.80);return t}function measure(){cfg.length=0;var y=window.pageYOffset||document.documentElement.scrollTop||0,vh=window.innerHeight||0;for(var i=0;i<items.length;i++){var it=items[i];if(!can(it))continue;var r=it.getBoundingClientRect(),top=r.top+y,start=top-vh,end=top;cfg.push({it:it,s:start,e:end,inv:1/Math.max(1,(end-start)),tl:cfg[i]&&cfg[i].tl?cfg[i].tl:null})}for(i=0;i<cfg.length;i++)cfg[i].tl||(cfg[i].tl=makeTL(cfg[i].it));cfg.sort(function(a,b){return a.s-b.s})}function drive(y){for(var i=0;i<cfg.length;i++){var c=cfg[i],p=(y-c.s)*c.inv;p<0?p=0:p>1&&(p=1);try{c.tl.progress(p)}catch(_){}}}function kill(){try{st&&st.kill&&st.kill()}catch(_){ }st=0;cfg.length=0;items.length=0;built=!1;try{raf&&cancelAnimationFrame(raf)}catch(_){ }raf=0}function build(){if(built)return;items=list();if(!items.length)return;for(var i=0;i<items.length;i++)prep(items[i]);measure();st=S.create({trigger:P,start:0,end:function(){var m=0;for(var i=0;i<cfg.length;i++)cfg[i].e>m&&(m=cfg[i].e);return m||"+=1"},onUpdate:function(self){drive(self.scroll())},onRefresh:function(){measure();drive((window.pageYOffset||document.documentElement.scrollTop||0))},invalidateOnRefresh:!0});built=!0}function sync(){if(!mm.matches||!vis())return kill();build();try{S.refresh()}catch(_){}}function sched(){raf||(raf=requestAnimationFrame(function(){raf=0;sync()}))}on(document,"archive:view",function(){sched()},{passive:!0});on(document,"archive:filter:applied",function(){sched()},{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sched):mm.addListener(sched)}catch(_){ }un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sched):mm.removeListener(sched)}catch(_){}});on(window,"load",function(){setTimeout(function(){try{sync()}catch(_){}} ,120)},{once:!0,passive:!0});requestAnimationFrame(function(){sync();setTimeout(function(){try{S.refresh()}catch(_){}} ,350)});return function(){try{kill()}catch(_){ }un.forEach(function(fn){fn()});un.length=0;try{h.__apx3Bound=0}catch(_){}}}
			})
		);
