...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;if(!matchMedia("(pointer:fine)").matches)return;root=root||document;var hero=root.querySelector(".hero-section-showreel");if(!hero)return;var track=hero.querySelector(".showreel-track"),box=hero.querySelector(".showreel-container");if(!track||!box)return;box.style.setProperty("--hero-showreel-x","0px");box.style.setProperty("--hero-showreel-scale","1");hero.__heroShowreelDestroy__&&hero.__heroShowreelDestroy__();var state={nx:1,scrollScale:1,x:0,scale:1,raf:0},heroRect,trackRect,boxRect,baseMinX=0,baseMaxX=0,POINTER_SCALE_RANGE=.06,SCROLL_SCALE_RANGE=.14,LERP=.12;function clamp(e,t,r){return e<t?t:e>r?r:e}function measureRects(){heroRect=hero.getBoundingClientRect();trackRect=track.getBoundingClientRect();boxRect=box.getBoundingClientRect();if(!trackRect||!boxRect)return;baseMinX=trackRect.left-boxRect.left;baseMaxX=trackRect.right-boxRect.right}function updateScrollScale(){var e=window.innerHeight||1,t=hero.getBoundingClientRect(),r=.85*e,o=.25*e,n=(r-t.top)/(r-o);n=clamp(n,0,1),state.scrollScale=1+n*SCROLL_SCALE_RANGE}function ensureRAF(){state.raf||(state.raf=requestAnimationFrame(tick))}function tick(){state.raf=0;if(!trackRect||!boxRect)measureRects();if(!trackRect||!boxRect)return;var e=1+Math.abs(state.nx)*POINTER_SCALE_RANGE,t=state.scrollScale*e,r=boxRect.width*t-boxRect.width;r<0&&(r=0);var o=r/2,n=baseMinX+o,a=baseMaxX-o;n> a&&(n=a=baseMaxX);var i=(state.nx+1)*.5* (a-n)+n;state.x+=(i-state.x)*LERP,state.scale+=(t-state.scale)*LERP,box.style.setProperty("--hero-showreel-x",state.x.toFixed(2)+"px"),box.style.setProperty("--hero-showreel-scale",state.scale.toFixed(3)),Math.abs(i-state.x)>.2||Math.abs(t-state.scale)>.002?state.raf=requestAnimationFrame(tick):state.raf=0}function onPointerMove(e){heroRect|| (heroRect=hero.getBoundingClientRect());var t=heroRect.width||1,r=(e.clientX-heroRect.left)/t*2-1;r=clamp(r,-1,1),state.nx=r,ensureRAF()}function onPointerLeave(){state.nx=1,ensureRAF()}function onScroll(){updateScrollScale(),ensureRAF()}function onResize(){heroRect=null,measureRects(),updateScrollScale(),ensureRAF()}measureRects(),updateScrollScale(),ensureRAF(),hero.addEventListener("pointermove",onPointerMove),hero.addEventListener("pointerleave",onPointerLeave),window.addEventListener("scroll",onScroll,{passive:!0}),window.addEventListener("resize",onResize,{passive:!0});var ro=new ResizeObserver(function(){measureRects(),ensureRAF()});try{ro.observe(track),ro.observe(box)}catch(e){}hero.__heroShowreelDestroy__=function(){try{cancelAnimationFrame(state.raf)}catch(e){}try{hero.removeEventListener("pointermove",onPointerMove)}catch(e){}try{hero.removeEventListener("pointerleave",onPointerLeave)}catch(e){}try{window.removeEventListener("scroll",onScroll)}catch(e){}try{window.removeEventListener("resize",onResize)}catch(e){}try{ro.disconnect()}catch(e){}try{box.style.removeProperty("--hero-showreel-x"),box.style.removeProperty("--hero-showreel-scale")}catch(e){}hero.__heroShowreelDestroy__=null}},destroy:async function(){try{var e=document.querySelector(".hero-section-showreel");e&&e.__heroShowreelDestroy__&&(e.__heroShowreelDestroy__(),e.__heroShowreelDestroy__=null)}catch(t){}}}),
			feature({
