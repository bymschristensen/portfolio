	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init:async r=>{const e=".appear-in-line",t=":scope > *",o=.15,i=.8;function n(a){if(a.__ailArmed)return;a.__ailArmed=!0;const e=[],n=[];Array.from(a.querySelectorAll(t)).forEach(t=>{const s=getComputedStyle(t),l=parseInt(s.columnCount,10)||1,c=t.getBoundingClientRect();if(l>1){try{gsap.set(t,{opacity:0});const s=new SplitText(t,{type:"lines",linesClass:"split-line"});e.push(s);const r=c.width/l,d=Array.from({length:l},()=>[]);s.lines.forEach(e=>{const n=e.getBoundingClientRect().left-c.left,t=Math.min(Math.floor(n/r),l-1);d[t].push(e),gsap.set(e,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})}),d.forEach(e=>n.push(e))}catch(u){gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),n.push([t])}}else gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),n.push([t])});const s=CoreUtilities.Observers.addDom(new IntersectionObserver((t,s)=>{t.forEach(a=>{if(!a.isIntersecting)return;s.unobserve(a.target),n.forEach((e,t)=>{gsap.to(e,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:t*o})});const t=(n.length-1)*o+i;gsap.delayedCall(t+.05,()=>{try{e.forEach(e=>e.revert())}catch(u){}})})},{root:null,rootMargin:"0px 0px -10% 0px",threshold:0}));a._appearData={splits:e,groups:n,observer:s},s.observe(a),a.setAttribute("data-ail-armed","1");const l=a.getBoundingClientRect();if(l.top<window.innerHeight&&l.bottom>0){s.unobserve(a),n.forEach((e,t)=>{gsap.to(e,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:t*o})});const t=(n.length-1)*o+i;gsap.delayedCall(t+.05,()=>{try{e.forEach(e=>e.revert())}catch(u){}})}const c=r.querySelector(".w-tabs");if(c){const e=a.closest(".w-tab-pane");e&&CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!e.classList.contains("w--tab-active"))return;s.observe(a);const t=a.getBoundingClientRect();if(t.top<window.innerHeight&&t.bottom>0){s.unobserve(a),n.forEach((e,t)=>{gsap.to(e,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:t*o})});const t=(n.length-1)*o+i;gsap.delayedCall(t+.05,()=>{try{e.forEach(e=>e.revert())}catch(u){}})}}).observe(e,{attributes:!0,attributeFilter:["class"]})}}r.querySelectorAll(e).forEach(n)}
			}),
