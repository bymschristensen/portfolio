	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				//init: async r=>{r=r||document;const ns=r.dataset?.barbaNamespace||r.getAttribute?.("data-barba-namespace")||document.querySelector('[data-barba="container"]')?.dataset?.barbaNamespace||"";const enableRefresh=ns!=="capabilities";const L=[...r.querySelectorAll(".accordion-list")];if(!L.length)return;let t;const rf=()=>{if(!enableRefresh||!window.ScrollTrigger)return;clearTimeout(t);t=setTimeout(()=>{try{ScrollTrigger.refresh()}catch(e){}},120)};L.forEach(l=>{const A=[...l.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];if(!A.length)return;A.forEach(it=>{if(it._accBound)return;it._accBound=1;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;h.classList.remove("accordion-active");gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:null});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":null,duration:.5,onComplete:()=>gsap.set(ct,{maxHeight:"none"})},0);isQ&&qi&&tl.from(qi,{opacity:0,duration:.4},0);it._accTL=tl;tl.eventCallback("onComplete",rf);tl.eventCallback("onReverseComplete",rf);it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");A.forEach(o=>{if(o!==it){const oh=o.querySelector(".accordion-header"),ot=o._accTL,oc=o.querySelector(".accordion-content");if(oh&&ot&&oh.classList.contains("accordion-active")&&!ot.isActive()){oh.classList.remove("accordion-active");oc&&gsap.set(oc,{maxHeight:oc.offsetHeight});ot.reverse();}}});open?(gsap.set(ct,{maxHeight:ct.offsetHeight}),tl.reverse()):(gsap.set(ct,{maxHeight:0}),tl.play());h.classList.toggle("accordion-active",!open);});});});}
				init: async r=>{const L=[...(r||document).querySelectorAll(".accordion-list")];if(!L.length)return;let t;const rf=()=>{if(!ScrollTrigger)return;clearTimeout(t);t=setTimeout(()=>{window.__capAccordionAdjusting=!0;requestAnimationFrame(()=>{try{ScrollTrigger.refresh()}catch(e){}setTimeout(()=>{window.__capAccordionAdjusting=!1},40)})},120)};L.forEach(l=>{const A=[...l.querySelectorAll(".accordion-subservice,.accordion-mindset,.accordion-quote")];if(!A.length)return;A.forEach(it=>{if(it._accBound)return;it._accBound=1;const h=it.querySelector(".accordion-header"),cr=it.querySelector(".cross-line-animating"),ct=it.querySelector(".accordion-content"),qi=it.querySelector(".accordion-icon-quote"),isQ=it.classList.contains("accordion-quote");if(!h||!cr||!ct)return;h.classList.remove("accordion-active");gsap.set(ct,{maxHeight:0,opacity:0,paddingBottom:0,paddingTop:isQ?0:null});const tl=gsap.timeline({paused:!0,defaults:{ease:"power2.out"}}).to(h,{paddingTop:"2rem",duration:.4},0).to(cr,{rotation:0,duration:.4},0).to(ct,{maxHeight:600,opacity:1,paddingBottom:isQ?"0rem":"2rem",paddingTop:isQ?"2rem":null,duration:.5,onComplete:()=>gsap.set(ct,{maxHeight:"none"})},0);isQ&&qi&&tl.from(qi,{opacity:0,duration:.4},0);it._accTL=tl;tl.eventCallback("onComplete",rf);tl.eventCallback("onReverseComplete",rf);it.addEventListener("click",()=>{if(tl.isActive())return;const open=h.classList.contains("accordion-active");A.forEach(o=>{if(o!==it){const oh=o.querySelector(".accordion-header"),ot=o._accTL,oc=o.querySelector(".accordion-content");if(oh&&ot&&oh.classList.contains("accordion-active")&&!ot.isActive()){oh.classList.remove("accordion-active");oc&&gsap.set(oc,{maxHeight:oc.offsetHeight});ot.reverse();}}});open?(gsap.set(ct,{maxHeight:ct.offsetHeight}),tl.reverse()):(gsap.set(ct,{maxHeight:0}),tl.play());h.classList.toggle("accordion-active",!open);});});});}
			}),
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				//init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!window.gsap||!window.ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capPin)return;e._capPin=1;const t=e.offsetHeight<window.innerHeight,n=gsap.timeline({defaults:{ease:"none"}}).fromTo(e,{filter:"contrast(100%) blur(0px)"},{filter:"contrast(10%) blur(10px)",duration:1},0);ScrollTrigger.create({id:"servicePin:"+i,trigger:e,start:t?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,animation:n,onKill:()=>gsap.set(e,{clearProps:"filter"})})});}
				//init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!gsap||!ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capPin)return;e._capPin=1;gsap.set(e,{filter:"contrast(100%) blur(0px)"});const t=e.offsetHeight<innerHeight;ScrollTrigger.create({id:"servicePin:"+i,trigger:e,start:t?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:o=>{if(window.__capAccordionAdjusting)return;const p=o.progress,c=100-90*p,b=10*p;gsap.set(e,{filter:`contrast(${c}% ) blur(${b}px)`})},onKill:()=>gsap.set(e,{clearProps:"filter"})})})}
				init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!window.gsap||!window.ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capBlur)return;e._capBlur=1;gsap.set(e,{filter:"contrast(100%) blur(0px)"});const t=gsap.timeline({defaults:{ease:"none"}}).fromTo(e,{filter:"contrast(100%) blur(0px)"},{filter:"contrast(10%) blur(10px)",duration:1},0);ScrollTrigger.create({id:"serviceBlur:"+i,trigger:e,start:"top top",end:"bottom top",scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,animation:t,onKill:()=>gsap.set(e,{clearProps:"filter"})})});try{ScrollTrigger.refresh(!0)}catch{}}
			}),
			feature({
				id: "productGallery",
				stage: "main",
