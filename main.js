	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init:async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches||!gsap||!ScrollTrigger)return;r=r||document;const s=[...r.querySelectorAll(".section-single-service")];if(!s.length)return;s.forEach((e,i)=>{if(e._capPin)return;e._capPin=1;const t=ScrollTrigger.getById("servicePin:"+i);t&&t.kill();const n=e.offsetHeight<innerHeight,o=s[i+1]||null;ScrollTrigger.create({id:"servicePin:"+i,trigger:e,start:n?"top top":"bottom bottom",endTrigger:o||e,end:o?"top top":"bottom top",pin:!0,pinSpacing:!0,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:self=>{const p=Math.max(0,Math.min(1,(self.progress-.15)/.85)),c=100-90*p,b=10*p;gsap.set(e,{filter:`contrast(${c}%) blur(${b}px)`})},onKill:()=>gsap.set(e,{clearProps:"filter"})})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
