	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.ScrollTrigger||!window.gsap)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let last="";c.style.transition="background-color .6s ease";const set=v=>{if(v===last)return;last=v;gsap.set(c,{backgroundColor:v,overwrite:"auto"})};const stD=ScrollTrigger.create({id:"csbg:details",trigger:d,start:"top bottom",onEnter:()=>set(bg),onLeaveBack:()=>set(cs),refreshPriority:100});const stM=ScrollTrigger.create({id:"csbg:more",trigger:m,start:"top bottom",onEnter:()=>set(bd),onLeaveBack:()=>set(bg),refreshPriority:100});const sync=()=>{const y=ScrollTrigger.scroll();set(y>=stM.start?bd:y>=stD.start?bg:cs)};ScrollTrigger.addEventListener("refresh",sync);sync()}
			}),
			feature({
