...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveController",stage:"main",namespaces:["archive"],selectors:[".section-archive-playful",".section-archive-minimal",".archive-playful-list",".archive-minimal-list",".archive-playful-item",".archive-minimal-item",".archive-switch-block",".archive-switch",".archive-switch-dot","#archive-filter-all","#archive-filter-appswebapps","#archive-filter-websites","#archive-filter-ecommerce","#archive-filter-platforms-systems","#archive-filter-other","#nav-archive-filter-all","#nav-archive-filter-appswebapps","#nav-archive-filter-websites","#nav-archive-filter-ecommerce","#nav-archive-filter-platforms-systems","#nav-archive-filter-other"],init:function(r){r=r||document;if(!r||r.__archiveCtlBound)return;var e=r.querySelector(".section-archive-playful"),t=r.querySelector(".section-archive-minimal"),a=r.querySelector(".archive-playful-list"),i=r.querySelector(".archive-minimal-list");if(!e||!t||!a||!i)return;r.__archiveCtlBound=1;var n=[].slice.call(a.querySelectorAll(".archive-playful-item")),o=[].slice.call(i.querySelectorAll(".archive-minimal-item")),l=function(s){return(s||"").toLowerCase().replace(/\s+/g," ").trim()},c=function(u){try{return l((u.querySelector(".archive-categories .cms-categories")||{}).textContent||"")}catch(_) {return""}},d=new WeakMap;n.forEach(function(u){d.set(u,c(u))}),o.forEach(function(u){d.set(u,c(u))});var f={all:"all",appswebapps:"appswebapps",websites:"websites",ecommerce:"ecommerce","platforms-systems":"platforms-systems",other:"other"},m=null,v=null,p=function(u){return u&&u.id?u.id.replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,""):""},g=function(u){var h=p(u);return f[h]||null},y=function(u){return u==="all"||!u},b=function(u,h){if(!u)return;u.classList.toggle("is-active",!!h);u.setAttribute("aria-pressed",h?"true":"false")},w=function(u){if(!u)return;var h=r.querySelector("#archive-filter-"+u),q=r.querySelector("#nav-archive-filter-"+u);b(h,1);b(q,1)},k=function(u){["all","appswebapps","websites","ecommerce","platforms-systems","other"].forEach(function(h){var q=r.querySelector("#archive-filter-"+h),x=r.querySelector("#nav-archive-filter-"+h);b(q,h===u);b(x,h===u)})},S=function(u){var h=r.querySelector(".archive-switch-block"),q=r.querySelector(".archive-switch"),x=r.querySelector(".archive-switch-dot");if(h)b(h,1);if(q)q.setAttribute("aria-checked",u==="minimal"?"true":"false");if(x)x.style.transform=u==="minimal"?"translateX(100%)":"translateX(0%)"},E=function(u){var h=u==="minimal";t.style.display=h?"":"none";e.style.display=h?"none":"";S(u)},A=function(u,h){for(var q=0;q<u.length;q++){var x=u[q],T=h==="all"||h==null?1:d.get(x).indexOf(h)>-1;x.style.display=T?"":"none"}},C=function(u){k(u);var h=v==="minimal"?o:n;var q=v==="minimal"?n:o;q.forEach(function(x){x.style.display="none"});A(h,u)},L=function(u){if(m===u)return;m=u;window.clearTimeout(L._t);L._t=window.setTimeout(function(){try{history.replaceState(null,"",location.pathname+(u&&u!=="all"?"?filter="+encodeURIComponent(u):""))}catch(h){}},50)},P=function(){var u=null;try{var h=new URL(location.href).searchParams.get("filter");u=h?l(h):null}catch(q){};if(!u||!f[u])u="all";return u},I=function(){var u=null;try{var h=localStorage.getItem("archiveView");u=h==="minimal"?"minimal":"playful"}catch(q){u="playful"}return u},M=function(u){v=u;try{localStorage.setItem("archiveView",u)}catch(h){};E(u);C(m||"all")};m=P();v=I();E(v);k(m);C(m);var R=function(u){var h=g(u.target&&u.target.closest?u.target.closest("[id^='archive-filter-'],[id^='nav-archive-filter-']"):null);if(!h)return;u.preventDefault();var q=h==="all"?"all":h;C(q);L(q)},V=function(u){var h=u.target&&u.target.closest?u.target.closest(".archive-switch,.archive-switch-block"):null;if(!h)return;u.preventDefault();M(v==="minimal"?"playful":"minimal")};r.addEventListener("click",function(u){R(u);V(u)},!0);r.addEventListener("keydown",function(u){var h=u.target;if(!h)return;var q=h.closest&&h.closest("[id^='archive-filter-'],[id^='nav-archive-filter-']");if(q&&(u.key==="Enter"||u.key===" ")){u.preventDefault();var x=g(q);if(x){C(x);L(x)}}var T=h.closest&&h.closest(".archive-switch,.archive-switch-block");if(T&&(u.key==="Enter"||u.key===" ")){u.preventDefault();M(v==="minimal"?"playful":"minimal")}},!0)}}),
