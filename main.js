	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init:async function(root){root=root||document;if(!window.matchMedia)return;var S=root.__archiveMinimalExperience||(root.__archiveMinimalExperience={});if(S._bound)return;S._bound=1;S.sel={section:".section-archive-minimal",list:".archive-minimal-list",item:".archive-minimal-item",hiddenImg:"img.archive-minimal-item-hidden-img",stickyImg:".archive-minimal-visual-img",viewport:".archive-minimal-visual-viewport",stack:".archive-minimal-visual-stack"};var mqD=matchMedia("(min-width:1024px) and (pointer:fine)"),mqRM=matchMedia("(prefers-reduced-motion: reduce)");function $(q,c){return(c||root).querySelector(q)}function $$(q,c){return[].slice.call((c||root).querySelectorAll(q))}function vis(el){if(!el)return!1;var cs=getComputedStyle(el);if(!cs||cs.display==="none"||cs.visibility==="hidden")return!1;return el.offsetParent!==null}S.section=$(S.sel.section)||root;S.list=$(S.sel.list,S.section)||S.section;S.stickyImg=$(S.sel.stickyImg,root);S.viewport=$(S.sel.viewport,root)||S.stickyImg&&S.stickyImg.parentElement||null;S.stack=$(S.sel.stack,root)||S.viewport||null;if(!S.list||!S.stickyImg){S.destroy=function(){S._bound=0;try{delete root.__archiveMinimalExperience}catch(e){}};return}var gs=window.gsap,canAnim=!!gs&&!mqRM.matches;function keyFromImg(img){return img&&(img.currentSrc||img.src)||""}function copyAttrs(srcImg,dstImg){if(!srcImg||!dstImg)return!1;var src=srcImg.currentSrc||srcImg.src;if(!src)return!1;dstImg.alt=srcImg.alt||"";var ss=srcImg.getAttribute("srcset"),sz=srcImg.getAttribute("sizes");ss?dstImg.setAttribute("srcset",ss):dstImg.removeAttribute("srcset");sz?dstImg.setAttribute("sizes",sz):dstImg.removeAttribute("sizes");dstImg.src=src;return!0}function waitImg(img){try{if(img.decode)return img.decode().catch(function(){})}catch(e){}return new Promise(function(r){if(!img||img.complete)return r();var d=0;function done(){if(d)return;d=1;img.removeEventListener&&img.removeEventListener("load",done);img.removeEventListener&&img.removeEventListener("error",done);r()}try{img.addEventListener("load",done,{once:!0});img.addEventListener("error",done,{once:!0});setTimeout(done,1200)}catch(e){setTimeout(done,80)}})}function ensureStackLayout(){if(!S.stack)return;try{S.stack.style.position=S.stack.style.position||"relative"}catch(e){}try{S.viewport&&getComputedStyle(S.viewport).overflow!=="hidden&&(S.viewport.style.overflow="hidden")}catch(e){}}ensureStackLayout();var current=S.stickyImg,currentKey="",hoverActive=!1,firstSet=!1,tl=0,swapSeq=0;function instantSwap(srcImg){if(!srcImg)return!1;var k=keyFromImg(srcImg);if(k&&k===currentKey)return!0;copyAttrs(srcImg,current);currentKey=k;firstSet=!0;return!0}function animSwap(srcImg){if(!srcImg)return!1;var k=keyFromImg(srcImg);if(k&&k===currentKey)return!0;if(!canAnim||!gs||!S.stack||!current||!vis(current))return instantSwap(srcImg);if(!firstSet){instantSwap(srcImg);return!0}swapSeq++;var my=swapSeq;var incoming=new Image();incoming.decoding="async";incoming.loading="eager";incoming.className=(current.className||"")+" ame-layer";incoming.setAttribute("aria-hidden","true");incoming.style.position="absolute";incoming.style.inset="0";incoming.style.width="100%";incoming.style.height="100%";incoming.style.objectFit=getComputedStyle(current).objectFit||"cover";incoming.style.transformOrigin="50% 50%";incoming.style.pointerEvents="none";incoming.style.willChange="transform";copyAttrs(srcImg,incoming);S.stack.appendChild(incoming);var outgoing=current;function cleanup(){try{incoming&&incoming.parentNode&&incoming.parentNode.removeChild(incoming)}catch(e){} }waitImg(incoming).then(function(){if(my!==swapSeq){cleanup();return}try{tl&&tl.kill&&tl.kill()}catch(e){}try{gs.set(incoming,{yPercent:-12,scale:1.12})}catch(e){}try{gs.set(outgoing,{yPercent:0,scale:1})}catch(e){}tl=gs.timeline({defaults:{ease:"power3.out"},onComplete:function(){if(my!==swapSeq)return;try{outgoing&&outgoing.parentNode&&outgoing.parentNode.removeChild(outgoing)}catch(e){}current=incoming;currentKey=k;try{gs.set(current,{clearProps:"willChange"})}catch(e){}try{gs.set(S.stack,{clearProps:"willChange"})}catch(e){}}});try{gs.set(S.stack,{willChange:"transform"})}catch(e){}tl.to(incoming,{yPercent:0,scale:1,duration:.65,overwrite:!0},0).to(outgoing,{yPercent:12,scale:.98,duration:.65,overwrite:!0},0)})}function getVisibleItems(){return $$(S.sel.item,S.list).filter(vis)}function hiddenFromItem(it){return it?$(S.sel.hiddenImg,it):null}function setToFirstVisible(anim){var first=getVisibleItems()[0];var img=hiddenFromItem(first);if(!img)return;anim?animSwap(img):instantSwap(img)}function onEnter(e){if(!mqD.matches)return;var img=hiddenFromItem(e.currentTarget);if(!img)return;hoverActive=!0;animSwap(img)}function onLeave(){if(!mqD.matches)return;hoverActive=!1}function bindHover(){for(var items=$$(S.sel.item,S.list),i=0;i<items.length;i++){var it=items[i];if(it.__ame_h)continue;it.__ame_h=1;it.addEventListener("mouseenter",onEnter,{passive:!0});it.addEventListener("mouseleave",onLeave,{passive:!0})}}function refresh(){var allowAnim=mqD.matches&&!mqRM.matches;(!allowAnim||!hoverActive)?setToFirstVisible(allowAnim):0;bindHover()}setToFirstVisible(!1);bindHover();var mo=0;try{mo=new MutationObserver(function(){refresh()});mo.observe(S.list,{subtree:!0,childList:!0,attributes:!0,attributeFilter:["style","class"]})}catch(e){}S.destroy=function(){try{mo&&mo.disconnect&&mo.disconnect()}catch(e){}try{tl&&tl.kill&&tl.kill()}catch(e){}tl=0;swapSeq++;for(var items=$$(S.sel.item,S.list),i=0;i<items.length;i++){var it=items[i];if(!it.__ame_h)continue;it.removeEventListener("mouseenter",onEnter);it.removeEventListener("mouseleave",onLeave);it.__ame_h=0}try{S.stack&&S.stack.querySelectorAll&&[].slice.call(S.stack.querySelectorAll("img.ame-layer")).forEach(function(n){try{n.parentNode&&n.parentNode.removeChild(n)}catch(e){}})}catch(e){}S._bound=0;try{delete root.__archiveMinimalExperience}catch(e){}};S.refresh=function(){try{refresh()}catch(e){}};try{document.addEventListener("archive:view",function(){setTimeout(function(){refresh()},0)},{passive:!0})}catch(e){}try{document.addEventListener("archive:filter:applied",function(){setTimeout(function(){refresh()},0)},{passive:!0})}catch(e){}}
			})
		);
