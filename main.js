...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"capabilitiesHeroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async function(R){const S=R.querySelector(".hero-section-showreel");if(!S)return;const F=S.querySelector(".showreel-outer .container")||S.querySelector(".showreel-inner")||S;const V=S.querySelector(".showreel-container");const C=[S.querySelector(".showreel-headline"),S.querySelector(".showreel-subtext"),S.querySelector(".toc-outer")].filter(Boolean);if(!F||!V)return;function L(a,b,c){return a+(b-a)*c}function P(e){return Math.max(0,Math.min(1,e))}let H=window.innerHeight;let k=.5;let B=0;let W=1;let z=0;const T=getComputedStyle(V).transform||"none";const X=T==="none"?"":T+" ";function q(){const t=V.style.transform;V.style.transform=T;const w=V.getBoundingClientRect().width||1;V.style.transform=t;return w}function N(){const r=F.getBoundingClientRect();B=r.width||1}function Y(){W=Math.max(1,(B||1)/(z||1))}function U(){const r=S.getBoundingClientRect();const h=r.height-H;return h<=0?0:P((-r.top)/h)}function Z(){const p=U();const s=L(1,W,p);const cur=z*s;const room=Math.max(0,B-cur);const dx=(k-.5)*room;V.style.transform=X+"translate3d("+dx+"px,0,0) scale("+s+")";const lift=-0.45*H*p;const op=1-p;for(var i=0;i<C.length;i++){var el=C[i];if(el){el.style.transform="translate3d(0,"+lift+"px,0)";el.style.opacity=op}}}function G(e){const r=F.getBoundingClientRect();const x=Math.max(r.left,Math.min(e.clientX,r.right));k=(x-r.left)/(r.width||1)}function J(){H=window.innerHeight;N();z=q();Y();Raf()}let f=false;function Raf(){if(f)return;f=true;requestAnimationFrame(function(){f=false;Z()})}z=q();N();Y();window.addEventListener("scroll",Raf,{passive:!0});window.addEventListener("resize",J,{passive:!0});S.addEventListener("mousemove",G,{passive:!0});const ro=new ResizeObserver(J);try{ro.observe(F);ro.observe(V)}catch(e){}Z();if(document.__capHero&&document.__capHero.kill){document.__capHero.kill()}document.__capHero={kill:function(){try{window.removeEventListener("scroll",Raf)}catch(e){}try{window.removeEventListener("resize",J)}catch(e){}try{S.removeEventListener("mousemove",G)}catch(e){}try{ro.disconnect()}catch(e){}try{V.style.transform=T==="none"?"":T;for(var i=0;i<C.length;i++){C[i].style.transform="";C[i].style.opacity=""}}catch(e){}}}},destroy:async function(){try{if(document.__capHero&&document.__capHero.kill){document.__capHero.kill()}}catch(e){}document.__capHero=null}),
			feature({
