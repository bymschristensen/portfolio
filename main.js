	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const mm=gsap.matchMedia();mm.add("(min-width: 1024px)",(()=>{const sec=r.querySelector('[data-imgscroll="photos"]'),bio=r.querySelector('[data-imgscroll="bio"]'),copy=r.querySelector('[data-imgscroll="copy"]'),stats=r.querySelector('[data-imgscroll="stats"]'),head=r.querySelector(".bio-text .headline-md");if(!sec||!bio||!copy||!stats||!head)return;const list=[...sec.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(+a.getAttribute("data-photo")||0)-(+b.getAttribute("data-photo")||0)),p1=list.find(e=>e.getAttribute("data-photo")==="1"),p2=list.find(e=>e.getAttribute("data-photo")==="2"),p3=list.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const IMGSEL='img,video,.bio-photo,.service-visual,.photo';const i1=p1.querySelector(IMGSEL)||p1.firstElementChild,i2=p2.querySelector(IMGSEL)||p2.firstElementChild,i3=p3.querySelector(IMGSEL)||p3.firstElementChild;gsap.set([p1,p2,p3],{willChange:"transform",overflow:"hidden"});gsap.set([i1,i2,i3].filter(Boolean),{willChange:"transform",force3D:!0});const g=e=>e.getBoundingClientRect(),reset=()=>{gsap.set([p1,p2,p3],{x:0,y:0,scale:1});gsap.set([i1,i2,i3].filter(Boolean),{y:0})},base=()=>{const s=g(stats),h=g(head);return{left:s.left,top:h.top}},d=(el,b,o)=>{const a=g(el);return{x:b.left-a.left+(o.dx||0),y:b.top-a.top+(o.dy||0),scale:o.scale==null?1:o.scale}};let tl;const build=()=>{tl&&tl.kill();reset();tl=gsap.timeline({defaults:{ease:"none"},scrollTrigger:{trigger:sec,start:"top 35%",end:"+=140%",scrub:1,anticipatePin:1,invalidateOnRefresh:!0,onRefreshInit:reset}});const x3=()=>{const b=base();return d(p3,b,{dx:44,dy:56,scale:.36})},x2=()=>{const b=base();return d(p2,b,{dx:18,dy:22,scale:.38})},x1=()=>{const b=base(),v=d(p1,b,{scale:.79});return v.scale=Math.max(.79,v.scale||.79),v};const P1=-8,P2=-16,P3=-24;tl.addLabel("g",0).to(p3,{x:()=>x3().x,y:()=>x3().y,scale:()=>x3().scale},"g").to(p2,{x:()=>x2().x,y:()=>x2().y,scale:()=>x2().scale},"g+=0.06").to(p1,{x:()=>x1().x,y:()=>x1().y,scale:()=>x1().scale},"g+=0.12");i3&&tl.to(i3,{y:P3},"g");i2&&tl.to(i2,{y:P2},"g+=0.04");i1&&tl.to(i1,{y:P1},"g+=0.08")};build();const ro=new ResizeObserver(()=>{try{ScrollTrigger.refresh()}catch{}});[sec,bio,copy,stats,head].forEach(el=>el&&ro.observe(el));return()=>{try{ro.disconnect()}catch{}try{tl&&tl.kill()}catch{}}}))}
			}),
			feature({
