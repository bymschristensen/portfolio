...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var S=[].slice.call(r.querySelectorAll(".section-single-service"));if(!S.length)return;S.forEach(function(sec){var inner=sec.querySelector(".single-service-wrapper");if(!inner)return;gsap.timeline({scrollTrigger:{trigger:sec,start:"top top",end:"bottom top",pin:!0,pinSpacing:!1,scrub:1,anticipatePin:1,invalidateOnRefresh:!0}}).to(inner,{filter:"contrast(10%) blur(10px)",ease:"none",startAt:{filter:"contrast(100%) blur(0px)"}})});ScrollTrigger.refresh()}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
