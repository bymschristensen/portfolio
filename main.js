...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			// feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(e){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;e=e||document;var t=e.querySelector(".hero-section-showreel");if(!t||t.__hsBound)return;var r=t.querySelector(".showreel-track"),n=t.querySelector(".showreel-container");if(!r||!n)return;t.__hsBound=1;var i=window.CSS&&CSS.supports&&CSS.supports("aspect-ratio:16/9"),o={x:0,px:null,last:0,raf:0,w:0,tw:0,left:0,right:0,baseC:0,hasMove:!1,vx:0,sq:0},L=2.2,M=.30,V=1.3e3,S=.16,H={base:.88,min:.80,max:1};function c(){var e=n.getBoundingClientRect(),t=r.getBoundingClientRect();o.w=e.width||n.offsetWidth||0,o.tw=t.width||r.offsetWidth||0,o.left=t.left,o.right=t.right,o.baseC=e.left+e.width/2-o.x,!i&&o.w&&(n.style.height=o.w*9/16+"px")}function u(e,t,r){return e<t?t:e>r?r:e}function l(e){var t=u(e,o.left,o.right),i=t+o.w/2,c=o.left+o.w/2,l=o.right-o.w/2;c>l&&(c=l=(o.left+o.right)/2);var a=u(i,c,l);return a-o.baseC}function a(e){var t=e.clientX;o.hasMove&&(o.vx+=t-(null==o.px?t:o.px)),o.hasMove=!0,o.px=t}function s(e){o.last||(o.last=e);if(!o.hasMove||null==o.px){o.last=e,o.raf=requestAnimationFrame(s);return}var t=Math.min(.05,(e-o.last)/1e3);o.last=e,c();var r=l(o.px),i=1-Math.exp(-L*t);o.x+=i*(r-o.x),o.vx*=.94,Math.abs(o.vx)<.02&&(o.vx=0);var a=0;o.vx&&(a=-o.vx/V,a>M?a=M:a<-M&&(a=-M));o.sq+=S*(a-o.sq);var d=H.base+o.sq;d<H.min?d=H.min:d>H.max&&(d=H.max),n.style.transform="translate3d("+o.x+"px,0,0) scale("+d+")",o.raf=requestAnimationFrame(s)}c(),addEventListener("pointermove",a,{passive:!0}),addEventListener("resize",c,{passive:!0}),o.raf=requestAnimationFrame(s),t.__hsDestroy=function(){try{removeEventListener("pointermove",a)}catch(e){}try{removeEventListener("resize",c)}catch(e){}try{o.raf&&cancelAnimationFrame(o.raf)}catch(e){}}},destroy:async function(e){e=e||document;var t=e.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{t&&t.__hsDestroy&&t.__hsDestroy()}catch(e){}}}),
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var sec=root.querySelector(".hero-section-showreel");if(!sec||sec.__hsBound)return;var track=sec.querySelector(".showreel-track"),container=sec.querySelector(".showreel-container");if(!track||!container)return;sec.__hsBound=1;var hasAR=window.CSS&&CSS.supports&&CSS.supports("aspect-ratio:16/9"),st={x:0,px:null,last:0,raf:0,w:0,tw:0,left:0,right:0,baseC:0,hasMove:!1,vx:0,sq:0,wbA:0,wbP:0,wasMin:!1,wasMax:!1},L=2.2,M=.3,V=1300,S=.16,H={base:.88,min:.8,max:1},D=4,F=14,WOBBLE_AMP=.045,WOBBLE_PAD=.03;function measure(){var cb=container.getBoundingClientRect(),tb=track.getBoundingClientRect();st.w=cb.width||container.offsetWidth||0;st.tw=tb.width||track.offsetWidth||0;st.left=tb.left;st.right=tb.right;st.baseC=cb.left+cb.width/2-st.x;!hasAR&&st.w&&(container.style.height=st.w*9/16+"px")}function clamp(v,a,b){return v<a?a:v>b?b:v}function project(x){var xClamped=clamp(x,st.left,st.right),centerTarget=xClamped+st.w/2,minCenter=st.left+st.w/2,maxCenter=st.right-st.w/2;minCenter>maxCenter&&(minCenter=maxCenter=(st.left+st.right)/2);var centerClamped=clamp(centerTarget,minCenter,maxCenter);return centerClamped-st.baseC}function onMove(ev){var cx=ev.clientX;st.hasMove&&(st.vx+=cx-(null==st.px?cx:st.px));st.hasMove=!0;st.px=cx}function tick(ts){st.last||(st.last=ts);if(!st.hasMove||null==st.px){st.last=ts;st.raf=requestAnimationFrame(tick);return}var dt=Math.min(.05,(ts-st.last)/1e3);st.last=ts;measure();var targetX=project(st.px),posAlpha=1-Math.exp(-L*dt);st.x+=posAlpha*(targetX-st.x);st.vx*=.94;Math.abs(st.vx)<.02&&(st.vx=0);var vNorm=0;st.vx&&(vNorm=-st.vx/V,vNorm>M?vNorm=M:vNorm<-M&&(vNorm=-M));st.sq+=S*(vNorm-st.sq);var baseScale=H.base+st.sq,hitMin=baseScale<=H.min,hitMax=baseScale>=H.max;hitMin&&!st.wasMin?(st.wbA=WOBBLE_AMP,st.wbP=0):hitMax&&!st.wasMax&&(st.wbA=WOBBLE_AMP,st.wbP=0);st.wasMin=hitMin;st.wasMax=hitMax;st.wbA&&(st.wbA*=Math.exp(-D*dt),st.wbP+=F*dt);var wobble=st.wbA?st.wbA*Math.sin(st.wbP):0,rawScale=baseScale+wobble,minAllowed=H.min-WOBBLE_PAD,maxAllowed=H.max+WOBBLE_PAD,finalScale=clamp(rawScale,minAllowed,maxAllowed);container.style.transform="translate3d("+st.x+"px,0,0) scale("+finalScale+")";st.raf=requestAnimationFrame(tick)}measure();addEventListener("pointermove",onMove,{passive:!0});addEventListener("resize",measure,{passive:!0});st.raf=requestAnimationFrame(tick);sec.__hsDestroy=function(){try{removeEventListener("pointermove",onMove)}catch(e){}try{removeEventListener("resize",measure)}catch(e){}try{st.raf&&cancelAnimationFrame(st.raf)}catch(e){}}},destroy:async function(root){root=root||document;var sec=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{sec&&sec.__hsDestroy&&sec.__hsDestroy()}catch(e){}}}),
			feature({
