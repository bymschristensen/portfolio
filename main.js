...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({id:"archiveFiltersController",stage:"early",namespaces:["archive"],selectors:["#archive-filter-all","#archive-filter-appswebapps","#archive-filter-websites","#archive-filter-ecommerce","#archive-filter-platforms-systems","#archive-filter-other","#nav-archive-filter-all","#nav-archive-filter-appswebapps","#nav-archive-filter-websites","#nav-archive-filter-ecommerce","#nav-archive-filter-platforms-systems","#nav-archive-filter-other",".section-archive-playful",".section-archive-minimal"],init:function(r){r=r||document;var KEY="archiveFilter",doc=document,root=r.querySelector("main.main")||r;if(root.__archiveFiltersBound)return root.__archiveFiltersDestroy||function(){};root.__archiveFiltersBound=1;var ids=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];function baseId(e){return(e||"").replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,"").trim()}function norm(e){return e=(e||"").toLowerCase().trim().replace(/&/g," ").replace(/[–—]/g,"-").replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g,"-"),"apps-web-apps"===e||"apps-webapps"===e||"apps-web"===e?"appswebapps":"e-commerce"===e||"ecommerce"===e?"ecommerce":"platforms-systems"===e||"platforms-and-systems"===e?"platforms-systems":"websites"===e?"websites":"other"===e?"other":"all"===e?"all":e}function readCats(it){if(it.__afCats)return it.__afCats;for(var a=["all"],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var t=(n[i].textContent||"").trim();t&&(t=norm(t))&&-1===a.indexOf(t)&&a.push(t)}return it.__afCats=a,a}function getAllItems(){var a=[],p=root.querySelector(".section-archive-playful"),m=root.querySelector(".section-archive-minimal");return p&&(a=a.concat([].slice.call(p.querySelectorAll(".archive-playful-item")))),m&&(a=a.concat([].slice.call(m.querySelectorAll(".archive-minimal-item")))),a}function setActive(cat){for(var i=0;i<ids.length;i++){var id=ids[i],el=root.querySelector("#"+id)||doc.getElementById(id);if(!el)continue;var on=baseId(id)===cat;el.classList.toggle("is-active",on);try{el.setAttribute("aria-current",on?"true":"false")}catch(e){}try{el.setAttribute("aria-pressed",on?"true":"false")}catch(e){}}}function stored(){try{var v=localStorage.getItem(KEY);return v?norm(v):null}catch(e){}return null}function apply(cat,meta){cat=norm(cat||"all")||"all";setActive(cat);var items=getAllItems();for(var i=0;i<items.length;i++){var it=items[i],cats=readCats(it),show="all"===cat||-1!==cats.indexOf(cat);it.style.display=show?"":"none"}if("user"===meta)try{localStorage.setItem(KEY,cat)}catch(e){}try{root.dispatchEvent(new CustomEvent("archive:filter:applied",{bubbles:!0,detail:{filter:cat}}))}catch(e){}}function onClick(e){var t=e.target&&e.target.closest?e.target.closest('[id^="archive-filter-"],[id^="nav-archive-filter-"]'):null;if(!t||!t.id)return;e.preventDefault&&e.preventDefault();apply(baseId(t.id),"user")}function onExternalFilter(e){try{var d=e&&e.detail||{},cat=d.filter||d.category||d.id||"";cat=baseId(String(cat||""));cat&&(apply(cat,"user"))}catch(_){}}function onView(){apply(stored()||"all","auto")}root.addEventListener("click",onClick,{passive:!1});doc.addEventListener("archive:filter",onExternalFilter,{passive:!0});root.addEventListener("archive:view",onView,{passive:!0});apply(stored()||"all","auto");var destroy=function(){try{root.removeEventListener("click",onClick)}catch(e){}try{doc.removeEventListener("archive:filter",onExternalFilter)}catch(e){}try{root.removeEventListener("archive:view",onView)}catch(e){}root.__archiveFiltersBound=0;root.__archiveFiltersDestroy=null};root.__archiveFiltersDestroy=destroy;return destroy}}),
