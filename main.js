...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				id:"productGallery",stage:"main",namespaces:["capabilities"],selectors:[".infinite-gallery"],init:async t=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=[...t.querySelectorAll(".infinite-gallery")];if(!e.length)return;const r=!!(window.CSS&&CSS.supports&&CSS.supports("aspect-ratio: 1/1")),i=t=>parseFloat(getComputedStyle(t).gap||"0")||0,a=t=>{const e=parseFloat(t.getAttribute("data-ratio"));if(e&&e>0)return e;const r=(t.className||"").match(/sv-ratio-(\d+)/);if(r){const t=r[1];if("169"===t)return 16/9;if("133"===t||"43"===t)return 4/3;if("11"===t)return 1}const i=t.querySelector("img,video");if(i){const t=parseInt(i.getAttribute("width"),10)||i.naturalWidth||i.videoWidth||0,e=parseInt(i.getAttribute("height"),10)||i.naturalHeight||i.videoHeight||0;if(t>0&&e>0)return t/e}return 1.5},o=(t,e)=>{const r=parseFloat(t.getAttribute("data-size")),i=e.closest(".section-single-service"),a=i&&i.clientHeight||innerHeight;return r&&a?Math.max(1,Math.round(a*r/100)):Math.round(.6*(a||innerHeight))},s=t=>{const e=i(t);let r=0;return t.childNodes.forEach(t=>{1===t.nodeType&&(r+=(t.getBoundingClientRect().width||0)+e)}),r},n=t=>{[...t.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(t=>t.remove())},l=(t,e)=>{const i=t.cloneNode(!0);i.setAttribute("data-clone","1"),i.dataset.key=t.dataset.key||"";const o=parseFloat(t.dataset.ratio)||a(t),s=+t.dataset.targetH||0,n=Math.max(1,Math.round(s*o));return i.style.overflow="hidden",i.style.flex="0 0 auto",i.style.willChange="height,transform",i.style.minWidth=n+"px",i.style.maxWidth=n+"px",r&&(i.style.aspectRatio=o+" / 1"),i.style.height=s?s+"px":"",e.appendChild(i),i};e.forEach(async t=>{const e=t.querySelector(".infinite-gallery-wrapper");if(!e||e.__inited)return;e.__inited=!0,t.setAttribute("data-armed","measure");const d=[...e.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!d.length)return void t.setAttribute("data-armed","1");await(t=>{const e=[...t.querySelectorAll("img,video")];if(!e.length)return Promise.resolve();const r=t=>t.naturalWidth>0&&t.naturalHeight>0||t.videoWidth>0&&t.videoHeight>0,i=[],a=e.map(t=>{if(r(t))return Promise.resolve();const e="function"==typeof t.decode?t.decode().catch(()=>{}):Promise.resolve(),i=new Promise(e=>{const r=()=>{t.removeEventListener("load",r),e()};t.addEventListener("load",r,{once:!0})}),a=new Promise(e=>{let i=0;!function a(){if(r(t)||i++>80)return e();setTimeout(a,40)}()});return Promise.race([e,i,a])}),o=new Promise(t=>i.push(setTimeout(t,3500)));return Promise.race([Promise.all(a),o]).finally(()=>i.forEach(clearTimeout))})(e),d.forEach((t,i)=>{const s=a(t),n=o(t,e),l=Math.max(1,Math.round(n*s));t.dataset.key=String(i),t.dataset.ratio=String(s),t.dataset.targetH=String(n),t.style.overflow="hidden",t.style.flex="0 0 auto",t.style.willChange="height,transform",t.style.minWidth=l+"px",t.style.maxWidth=l+"px",r&&(t.style.aspectRatio=s+" / 1"),t.style.height=""});const c="right"===(t.dataset.direction||"left").toLowerCase()?-1:1,h=parseFloat(t.dataset.speed)||.6,u=()=>{n(e),d.forEach(t=>l(t,e)),d.forEach(t=>l(t,e));let t=0;const r=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;s(e)<r&&t++<8;)d.forEach(t=>l(t,e))};u(),t.setAttribute("data-armed","1");let m=0;const p=(t,r=16.6667)=>{m-=c*((r||16.6667)/1e3*60)*h;const a=i(e),o=t=>(t.getBoundingClientRect().width||0)+a;let s=e.firstElementChild,n=0;for(;s&&m<-o(s)&&n++<50;)m+=o(s),e.appendChild(s),s=e.firstElementChild;let l=e.lastElementChild;for(n=0;l&&m>0&&n++<50;)m-=o(l),e.insertBefore(l,e.firstElementChild),l=e.lastElementChild;gsap.set(e,{x:m});const d=(innerWidth||0)/2;e.querySelectorAll(".service-visual").forEach(t=>{const e=t.closest(".service-visual-wrapper");if(!e)return;const r=e.getBoundingClientRect(),i=(d-(r.left+r.width/2))/(innerWidth||1);t.style.setProperty("--drift",40*i+"px")})};gsap.ticker.add(p),CoreUtilities.Observers.addTicker(p);let g=!1;const f=CoreUtilities.Observers.addDom(new IntersectionObserver(r=>{r.forEach(r=>{if(r.target!==t||!r.isIntersecting||g)return;g=!0;const i=innerHeight,a=d.map(t=>{const e=t.getBoundingClientRect();return{w:t,area:Math.max(0,Math.min(e.bottom,i)-Math.max(e.top,0))*Math.max(0,Math.min(e.right,innerWidth)-Math.max(e.left,0)),dist:Math.abs(.5*(e.top+e.bottom)-.5*innerHeight)}}).filter(t=>t.area>1).sort((t,e)=>t.dist-e.dist);a.slice(0,7).forEach((t,r)=>((t,r=0)=>{const i=+t.dataset.targetH||0;i&&gsap.fromTo(t,{height:0},{height:i,duration:.9,ease:"power2.out",delay:r,onComplete:()=>{t.style.height="";const r=t.dataset.key||"";r&&e.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${r}"]`).forEach(t=>t.style.height=i+"px")}})})(t.w,.12*r)),f.unobserve(t)})},{root:null,rootMargin:"0px 0px -5% 0px",threshold:0}));f.observe(t);const y=new ResizeObserver(()=>{n(e),d.forEach(t=>{const i=parseFloat(t.dataset.ratio)||a(t),s=o(t,e);t.dataset.targetH=String(s);const n=Math.max(1,Math.round(s*i));t.style.minWidth=n+"px",t.style.maxWidth=n+"px",r&&(t.style.aspectRatio=i+" / 1")}),u()});y.observe(e),CoreUtilities.Observers.addDom(y)})}
			})
		);
