	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var s=[].slice.call(r.querySelectorAll(".section-single-service"));if(!s.length)return;var p=ScrollTrigger._ppsPins||[];ScrollTrigger._ppsPins=p;s.forEach(function(sec){var small=sec.offsetHeight<window.innerHeight,tl=gsap.timeline({scrollTrigger:{trigger:sec,start:small?"top top":"bottom bottom",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}}).to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0);tl.scrollTrigger&&p.indexOf(tl.scrollTrigger)===-1&&p.push(tl.scrollTrigger)});if(ScrollTrigger._ppsHooksInstalled)return;ScrollTrigger._ppsHooksInstalled=!0;ScrollTrigger.addEventListener("refreshInit",function(){var list=ScrollTrigger._ppsPins||[];list.forEach(function(st){st&&(st._ppsPrevProgress=st.progress)})});ScrollTrigger.addEventListener("refresh",function(){var list=ScrollTrigger._ppsPins||[];list.forEach(function(st){if(!st||!st.animation)return;var pr=st._ppsPrevProgress;null!=pr&&st.animation.progress(pr).pause()})})}
			}),
			feature({
				id: "productGallery",
				stage: "main",
