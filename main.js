...
// Navigation Manager Test
	window.NavigationManager = (function () {
		...
		function isInternalAnchor(t){if(t.defaultPrevented||0!==t.button||t.metaKey||t.ctrlKey||t.shiftKey||t.altKey)return null;const e=t.target&&("A"===t.target.tagName?t.target:t.target.closest?.("a"));if(!e)return null;if(e.hasAttribute("download")||"_blank"===e.target||"external"===e.getAttribute("rel"))return null;if(e.closest('[data-router-ignore="true"], .w-lightbox'))return null;const r=(e.getAttribute("href")||e.href||"").trim();if(!r||"#"===r||r.startsWith("javascript:"))return null;let n;try{n=new URL(r,location.href)}catch{return null}const l=n.pathname.replace(/\/+$/,"")===location.pathname.replace(/\/+$/,"");return n.origin!==location.origin||l&&n.hash?null:{a:e,url:n}}
		function installLinkInterceptor(){state.installed||(state.clickHandler=t=>{if(t.defaultPrevented)return;if(t.target&&t.target.closest&&t.target.closest('[id^="archive-filter-"],[id^="nav-archive-filter-"],.modal-filters-item,.filters-container,.nav-button-filter,.archive-switch-block,.archive-switch,.archive-switch-dot'))return;const e=t.target&&("A"===t.target.tagName?t.target:t.target.closest?.("a"));if(e){const t=(e.getAttribute("href")||"").trim();if(!t||"#"===t||t.startsWith("javascript:"))return}const a=document.querySelector('[data-barba="container"]')?.dataset?.barbaNamespace||"";if("info"===a&&t.target?.closest('a[id^="recommendationsOpen"]'))return;if(t.target?.closest('.nav-button-menu,.nav-button-close-case,[data-router-ignore="true"]'))return;const r=isInternalAnchor(t);if(r){if("archive"!==a&&"resources"!==a){const t=r.a.closest("[data-barba-prevent]");if(t&&"true"===t.getAttribute("data-barba-prevent"))return}if(state.locks.size)return dlog("blocked by lock(s):",[...state.locks]),t.preventDefault(),t.stopPropagation(),void t.stopImmediatePropagation();t.preventDefault(),t.stopPropagation(),t.stopImmediatePropagation(),console.info("[intercept→barba]",r.url.href,"locks:",[...state.locks]),window.barba?.go?barba.go(r.url.href):location.href=r.url.href}},addEventListener("click",state.clickHandler,{capture:!0}),state.installed=!0,dlog("link interceptor installed"))}
	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(r){
					r=r||document;var KEY="archiveFilter",doc=document,root=(r.querySelector&&r.querySelector("main.main"))||r;if(doc.__afDocBound)return;doc.__afDocBound=1;
					var ids=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];
					function baseId(e){return(e||"").replace(/^nav-archive-filter-/,"").replace(/^archive-filter-/,"").trim()}
					function norm(e){return e=(e||"").toLowerCase().trim().replace(/&/g," ").replace(/[–—]/g,"-").replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g,"-"),"apps-web-apps"===e||"apps-webapps"===e||"apps-web"===e?"appswebapps":"e-commerce"===e||"ecommerce"===e?"ecommerce":"platforms-systems"===e||"platforms-and-systems"===e?"platforms-systems":"websites"===e?"websites":"other"===e?"other":"all"===e?"all":e}
					function getScope(){var c=doc.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||doc.querySelector('[data-barba="container"]');return c||doc}
					function readCats(it){if(it.__afCats)return it.__afCats;for(var a=["all"],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var t=(n[i].textContent||"").trim();t&&(t=norm(t))&&-1===a.indexOf(t)&&a.push(t)}return it.__afCats=a,a}
					function getAllItems(scope){var a=[],p=scope.querySelector(".section-archive-playful"),m=scope.querySelector(".section-archive-minimal");return p&&(a=a.concat([].slice.call(p.querySelectorAll(".archive-playful-item")))),m&&(a=a.concat([].slice.call(m.querySelectorAll(".archive-minimal-item")))),a}
					function setActive(cat){for(var i=0;i<ids.length;i++){var id=ids[i],el=doc.getElementById(id);if(!el)continue;var on=baseId(id)===cat;el.classList.toggle("is-active",on);try{el.setAttribute("aria-current",on?"true":"false")}catch(e){}try{el.setAttribute("aria-pressed",on?"true":"false")}catch(e){}}}
					function stored(){try{var v=localStorage.getItem(KEY);return v?norm(v):null}catch(e){}return null}
					...
					function onDocClick(e){var t=e.target&&e.target.closest?e.target.closest('[id^="archive-filter-"],[id^="nav-archive-filter-"]'):null;if(!t||!t.id)return;e.preventDefault&&e.preventDefault();apply(baseId(t.id),"user")}
					function onExternal(e){try{var d=e&&e.detail||{},cat=d.filter||d.category||d.id||"";cat=baseId(String(cat||""));cat&&apply(cat,"user")}catch(_){}} 
					function onView(){apply(stored()||"all","auto")}
					doc.addEventListener("click",onDocClick,!0);
					doc.addEventListener("archive:filter",onExternal,{passive:!0});
					doc.addEventListener("archive:view",onView,{passive:!0});
					apply(stored()||"all","auto");
				}
			}),
			feature({
...
// Debug Core
	...
	(function () {
		function boot() {
			...
			waitFor((function(){return!!(window.gsap&&window.InitManager&&window.EntryOrchestrator)}),(function(){try{waitFor((function(){return!(!window.barba||"function"!=typeof barba.init)}),(function(){try{EntryOrchestrator.init()}catch(n){console.warn("[BOOT] EntryOrchestrator.init failed",n),fallback()}}),80),setTimeout((function(){if(!window.__barbaInited&&!window.__HARD_ENTRY_DONE){window.__HARD_ENTRY_DONE=1;try{var n=document.querySelector('[data-barba="container"]')||document.body;n&&EntryOrchestrator.runEntryFlow(n,{withCoverOut:!1}).catch((function(n){console.warn("[BOOT] hard entry runEntryFlow failed",n)}))}catch(n){console.warn("[BOOT] hard entry setup failed",n)}}}),300)}catch(n){console.warn("[BOOT] step4 failed",n)}setTimeout((function(){window.__barbaInited||fallback()}),600)}),120);
		}
