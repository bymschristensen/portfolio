...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(root){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;root=root||document;var section=root.querySelector(".hero-section-showreel");if(!section||section.__hsBound)return;var track=section.querySelector(".showreel-track"),container=section.querySelector(".hero-section-showreel .showreel-container");if(!track||!container)return;section.__hsBound=1;var supportsAR=CSS.supports&&CSS.supports("aspect-ratio:16/9"),st={x:0,px:null,last:0,raf:0,w:0,tw:0,left:0,right:0,baseC:0,hasMove:!1,vx:0,sq:0,wob:0,wobP:0,prevSq:0,scroll:0,ch:0},L=2.2,M=.3,V=1300,S=.13,H={base:1,min:.85,max:1.2},WF=5,WS=.5;function clamp(v,a,b){return v<a?a:v>b?b:v}function measure(){var cr=container.getBoundingClientRect(),tr=track.getBoundingClientRect();st.w=cr.width;st.ch=cr.height;st.tw=tr.width;st.left=tr.left;st.right=tr.right;st.baseC=cr.left+cr.width/2-st.x;if(!supportsAR&&st.w)container.style.height=st.w*9/16+"px"}function targetX(px){var t=clamp(px,st.left,st.right),mid=t+st.w/2,lC=st.left+st.w/2,rC=st.right-st.w/2;if(lC>rC)lC=rC=(st.left+st.right)/2;var cm=clamp(mid,lC,rC);return cm-st.baseC}function ptr(ev){var x=ev.clientX;if(st.hasMove)st.vx+=x-(st.px==null?x:st.px);st.hasMove=!0;st.px=x}function scrollLayout(){var r=section.getBoundingClientRect(),vh=innerHeight||document.documentElement.clientHeight,h=r.height||section.offsetHeight;if(!h||vh>=h)st.scroll=0;else st.scroll=clamp(-r.top/(h-vh),0,1);if(st.tw>0){var s=.1,e=.7,g=(st.scroll-s)/(e-s);g=g<0?0:g>1?1:g;container.style.width=40+60*g+"%"}}function resize(){measure();scrollLayout()}function loop(ts){if(!st.last)st.last=ts;var dt=(ts-st.last)/1e3;if(dt<0)dt=0;if(dt>.05)dt=.05;st.last=ts;measure();var sc=st.scroll,base=H.base+(1-H.base)*sc,minS=H.min+(base-H.min)*sc,maxS=H.max-(H.max-base)*(.9*sc);if(!st.hasMove||st.px==null){container.style.transform="translate3d("+st.x+"px,0,0) scale("+base+")";st.raf=requestAnimationFrame(loop);return}var tx=targetX(st.px),k=1-Math.exp(-L*dt);st.x+=k*(tx-st.x);st.vx*=.96;if(Math.abs(st.vx)<.02)st.vx=0;var tilt=0;if(st.vx){tilt=-st.vx/V;if(tilt>M)tilt=M;else if(tilt<-M)tilt=-M}st.sq+=S*(tilt-st.sq);var scale=base+st.sq;if(scale<minS)scale=minS;else if(scale>maxS)scale=maxS;if(sc>.25){st.wob=0;st.wobP=0}var extra=1,d=scale-base,pr=st.prevSq,A=Math.abs(d),B=Math.abs(pr),tIn=.02,tOut=.03,trig=A<tIn&&B>tOut;if(trig&&!st.wob&&sc<.25){st.wob=1;st.wobP=0}if(st.wob>0){st.wobP+=WF*dt;extra=1+WS*st.wob*Math.sin(st.wobP);st.wob-=dt/.9;if(st.wob<=.001){st.wob=0;st.wobP=0;extra=1}}st.prevSq=d;var finalS=scale*extra,minX=st.left+st.w/2-st.baseC,maxX=st.right-st.w/2-st.baseC;if(minX>maxX){var mid=(minX+maxX)/2;minX=maxX=mid}st.x=clamp(st.x,minX,maxX);container.style.transform="translate3d("+st.x+"px,0,0) scale("+finalS+")";st.raf=requestAnimationFrame(loop)}measure();container.style.transform="translate3d(0,0,0) scale("+H.base+")";addEventListener("pointermove",ptr,{passive:!0});addEventListener("resize",resize,{passive:!0});addEventListener("scroll",scrollLayout,{passive:!0});scrollLayout();st.raf=requestAnimationFrame(loop);section.__hsDestroy=function(){removeEventListener("pointermove",ptr);removeEventListener("resize",resize);removeEventListener("scroll",scrollLayout);st.raf&&cancelAnimationFrame(st.raf)}},destroy:async function(root){root=root||document;var s=root.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");s&&s.__hsDestroy&&s.__hsDestroy()}}),
			feature({
