	...
	window.InitManager = (function () {
		...
		registries.pages.archive.push(
			...
			feature({
				...
				init: function(root){root=root||document;var host=root.querySelector('[data-barba="container"][data-barba-namespace="archive"]')||root.querySelector('[data-barba="container"]')||root;if(host.__apxLiveBound)return;host.__apxLiveBound=1;if(!window.gsap){host.__apxLiveBound=0;return}var gs=window.gsap,mm=matchMedia("(min-width:1024px)"),pr=matchMedia("(prefers-reduced-motion: reduce)");if(pr.matches){host.__apxLiveBound=0;return}var P=host.querySelector(".section-archive-playful");if(!P){host.__apxLiveBound=0;return}var un=[],items=[],raf=0,active=!1,clearT=0,buildRaf=0;function on(n,e,f,o){if(!n)return;n.addEventListener(e,f,o);un.push(function(){try{n.removeEventListener(e,f,o)}catch(_){}})}function q(a,s){return a?a.querySelector(s):null}function qs(a,s){return a?[].slice.call(a.querySelectorAll(s)):[]}function vis(){try{return!!(P&&getComputedStyle(P).display!=="none")}catch(_){return!0}}function can(it){if(!it)return!1;try{if(it.style&&it.style.display==="none")return!1;var s=getComputedStyle(it);return!(s&&s.display==="none")}catch(_){return!0}}function pickDataParts(it){var pd=qs(it,".archive-playful-data-product"),pe=qs(it,".archive-playful-data-expertise");if(!pd.length&&!pe.length){pd=qs(it,".archive-playful-data-inner .headline-xxs");pe=qs(it,".archive-playful-data-inner .body-sm")}return pd.concat(pe)}function pickLink(it){return q(it,".archive-playful-item-data-outer .case-study-url")||q(it,".archive-playful-item-data .shortcut-link")}function kill(){active=!1;raf&&cancelAnimationFrame(raf);raf=0;for(var i=0;i<items.length;i++){var d=items[i];try{d&&d.tl&&d.tl.kill&&d.tl.kill()}catch(_){}}items.length=0}function hardClearEntry(){try{var list=[].slice.call(P.querySelectorAll(".archive-playful-item"));for(var i=0;i<list.length;i++){var it=list[i];if(!can(it))continue;try{it.__almDone=1}catch(_){}try{gs.killTweensOf(it)}catch(_){}try{gs.set(it,{y:0,autoAlpha:1,filter:"blur(0px)",clearProps:"filter,opacity,visibility"})}catch(_){}}}catch(_){}}function build(){kill();if(!mm.matches||!vis())return;var list=[].slice.call(P.querySelectorAll(".archive-playful-item")).filter(can);if(!list.length)return;for(var i=0;i<list.length;i++){var it=list[i],v=q(it,".archive-playful-visuals"),imgs=qs(it,".archive-playful-visual-img"),di=q(it,".archive-playful-data-inner"),parts=pickDataParts(it),ln=pickLink(it);try{imgs.length&&gs.set(imgs,{willChange:"transform"});v&&gs.set(v,{willChange:"height",overflow:"hidden"});di&&gs.set(di,{willChange:"margin"});parts.length&&gs.set(parts,{willChange:"opacity,height"});ln&&gs.set(ln,{willChange:"opacity,height",overflow:"hidden"})}catch(_){}var tl=gs.timeline({paused:!0,defaults:{ease:"none"}});imgs.length&&tl.fromTo(imgs,{yPercent:-15},{yPercent:15,duration:1},0);parts.length&&tl.to(parts,{opacity:0,duration:.08},.70);di&&tl.to(di,{marginTop:0,marginBottom:0,duration:.08},.70);parts.length&&tl.to(parts,{height:0,duration:.08},.75);ln&&tl.to(ln,{opacity:0,duration:.08},.75);ln&&tl.to(ln,{height:0,duration:.08},.80);imgs.length&&tl.to(imgs,{scale:1.5,duration:.20},.80);v&&tl.to(v,{height:0,duration:.20},.80);items.push({it:it,tl:tl})}hardClearEntry();active=!0;tick()}function clamp(n){return n<0?0:n>1?1:n}function tick(){if(!active){raf=0;return}raf=requestAnimationFrame(tick);var vh=innerHeight||0;if(!vh)return;for(var i=0;i<items.length;i++){var d=items[i],it=d.it,tl=d.tl;if(!it||!tl||!can(it))continue;var r;try{r=it.getBoundingClientRect()}catch(_){continue}if(r.bottom<-200||r.top>vh+200)continue;var p=clamp((vh-r.top)/vh);try{tl.progress(p)}catch(_){}}}function schedClear(){clearT&&clearTimeout(clearT);clearT=setTimeout(function(){clearT=0;try{hardClearEntry()}catch(_){}},120)}function schedBuild(){if(buildRaf)return;buildRaf=requestAnimationFrame(function(){buildRaf=0;syncNow()})}function syncNow(){if(!mm.matches||!vis()){kill();return}build();schedClear()}function sync(){schedBuild()}on(document,"archive:view",sync,{passive:!0});on(document,"archive:filter:applied",sync,{passive:!0});try{mm.addEventListener?mm.addEventListener("change",sync):mm.addListener(sync)}catch(_){}un.push(function(){try{mm.removeEventListener?mm.removeEventListener("change",sync):mm.removeListener(sync)}catch(_){}});requestAnimationFrame(function(){setTimeout(sync,0)});return function destroy(){try{kill()}catch(_){}clearT&&clearTimeout(clearT);clearT=0;buildRaf&&cancelAnimationFrame(buildRaf);buildRaf=0;for(var i=0;i<un.length;i++)un[i]();un.length=0;try{host.__apxLiveBound=0}catch(_){}}}
