	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async root=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const sections=root.querySelectorAll(".infinite-gallery");if(!sections.length)return;const waitImgs=async(el,ms=3500)=>{const imgs=[...el.querySelectorAll("img")];if(!imgs.length)return;const hasDim=i=>i.naturalWidth>0&&i.naturalHeight>0;const timers=[];try{await Promise.race([Promise.all(imgs.map(img=>{if(hasDim(img))return Promise.resolve();const decoded=typeof img.decode==="function"?img.decode().catch(()=>{}):Promise.resolve();const onload=new Promise(res=>{const h=()=>{img.removeEventListener("load",h);res()};img.addEventListener("load",h,{once:!0})});const poll=new Promise(res=>{let tries=0;const tick=()=>{if(hasDim(img)||tries++>80)return res();setTimeout(tick,40)};tick()});return Promise.race([decoded,onload,poll])})),new Promise(res=>timers.push(setTimeout(res,ms)))])}finally{timers.forEach(clearTimeout)}};sections.forEach(section=>{const track=section.querySelector(".infinite-gallery-wrapper");if(!track||track.__inited)return;track.__inited=!0;section.setAttribute("data-armed","0");const originals=[...track.querySelectorAll(".service-visual-wrapper")];if(!originals.length){section.setAttribute("data-armed","1");return}const revealed=new Set;let running=!1,rafId=0,lastTs=0,offsetX=0;const dir=((section.dataset.direction||"left").toLowerCase()==="right"?-1:1),spd=parseFloat(section.dataset.speed)||.6;const slotW=el=>(el.getBoundingClientRect().width||0)+(parseFloat(getComputedStyle(track).gap||"0")||0);const removeClones=()=>track.querySelectorAll('.service-visual-wrapper[data-clone="1"]').forEach(n=>n.remove());const measureOriginals=()=>{originals.forEach((wrap,idx)=>{wrap.style.height="";const w=wrap.getBoundingClientRect().width||wrap.offsetWidth||0;let h=parseFloat(getComputedStyle(wrap).height)||wrap.getBoundingClientRect().height||0;if(h<=0){const img=wrap.querySelector("img");if(img&&img.naturalWidth>0&&img.naturalHeight>0&&w>0)h=img.naturalHeight/img.naturalWidth*w}const key=String(idx);wrap.dataset.key=key;wrap.dataset.targetH=String(Math.max(0,Math.round(h)));wrap.style.minWidth=w+"px";wrap.style.maxWidth=w+"px";wrap.style.overflow="hidden";if(revealed.has(key)){wrap.style.height="";wrap.dataset.revealed="1";wrap.dataset.revealing=""}else{wrap.style.height="0px";wrap.dataset.revealed="0";wrap.dataset.revealing=""}})};const cloneOnce=()=>{originals.forEach(wrap=>{const key=wrap.dataset.key||"";const h=+wrap.dataset.targetH||0;const c=wrap.cloneNode(!0);c.setAttribute("data-clone","1");c.dataset.key=key;c.style.minWidth=wrap.style.minWidth;c.style.maxWidth=wrap.style.maxWidth;c.style.height=h>0?h+"px":"0px";c.style.overflow="hidden";track.appendChild(c)})};const totalWidth=()=>{let sum=0;track.childNodes.forEach(n=>{if(n.nodeType===1)sum+=slotW(n)});return sum};const ensureBreadth=()=>{removeClones();cloneOnce();cloneOnce();let guard=0;const need=3*section.clientWidth;while(totalWidth()<need&&guard++<8)cloneOnce()};const startReveal=()=>{section.setAttribute("data-armed","1");const r=section.getBoundingClientRect();const vh=innerHeight||document.documentElement.clientHeight||0;if(r.bottom>0&&r.top<vh){running=!0;cancelAnimationFrame(rafId);rafId=requestAnimationFrame(revealTick)}};const revealTick=ts=>{if(!running){cancelAnimationFrame(rafId);return}if(ts-lastTs<80){rafId=requestAnimationFrame(revealTick);return}lastTs=ts||performance.now();const vw=innerWidth||document.documentElement.clientWidth||0;const leftG=-.05*vw,rightG=1.05*vw;const candidates=[...track.querySelectorAll('.service-visual-wrapper:not([data-clone])')].filter(el=>{const b=el.getBoundingClientRect();return b.right>leftG&&b.left<rightG}).filter(el=>el.dataset.revealed!=="1"&&el.dataset.revealing!=="1").sort((a,b)=>a.getBoundingClientRect().left-b.getBoundingClientRect().left);if(candidates.length){const tl=gsap.timeline();candidates.forEach((el,i)=>{const h=+el.dataset.targetH||0;const key=el.dataset.key||"";el.dataset.revealing="1";tl.to(el,{height:h,duration:.9,ease:"power2.out",onComplete:()=>{el.style.height="";el.dataset.revealed="1";el.dataset.revealing="";revealed.add(key);track.querySelectorAll('.service-visual-wrapper[data-clone][data-key="'+key+'"]').forEach(k=>k.style.height=h+"px")}},.12*i)})}rafId=requestAnimationFrame(revealTick)};const tickMove=(_,delta)=>{const dt=(delta||16.6667)/1000;const pps=60*spd;offsetX-=dir*pps*dt;let first=track.firstElementChild,guard=0;for(;first&&offsetX<-slotW(first)&&guard++<50;){offsetX+=slotW(first);track.appendChild(first);first=track.firstElementChild}let last=track.lastElementChild;guard=0;for(;last&&offsetX>0&&guard++<50;){offsetX-=slotW(last);track.insertBefore(last,track.firstElementChild);last=track.lastElementChild}gsap.set(track,{x:offsetX});const cx=(innerWidth||0)/2;track.querySelectorAll(".service-visual").forEach(v=>{const wrap=v.closest(".service-visual-wrapper");if(!wrap)return;const br=wrap.getBoundingClientRect();const drift=(cx-(br.left+br.width/2))/((innerWidth)||1);v.style.setProperty("--drift",40*drift+"px")})};const onIntersect=ents=>{ents.forEach(ent=>{if(ent.target!==section)return;if(ent.isIntersecting){running=!0;cancelAnimationFrame(rafId);rafId=requestAnimationFrame(revealTick)}else{running=!1;cancelAnimationFrame(rafId)}})};const ro=new ResizeObserver(()=>{measureOriginals();removeClones();ensureBreadth();revealed.forEach(key=>{const src=originals.find(o=>o.dataset.key===key);const h=(src&&+src.dataset.targetH)||0;track.querySelectorAll('.service-visual-wrapper[data-clone][data-key="'+key+'"]').forEach(c=>c.style.height=h+"px")});startReveal()});(async()=>{await waitImgs(track);measureOriginals();ensureBreadth();startReveal();const io=new IntersectionObserver(onIntersect,{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});io.observe(section);CoreUtilities.Observers.addDom(io);CoreUtilities.Observers.addDom({disconnect(){running=!1;cancelAnimationFrame(rafId)}});const ticker=(t,d)=>tickMove(t,d);gsap.ticker.add(ticker);CoreUtilities.Observers.addTicker(ticker);ro.observe(track);CoreUtilities.Observers.addDom(ro)})()})}
			})
		);
...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		EntryAnimations.caseStudy=function(e){e=e||document;var t=gsap.timeline(),i=e.querySelector(".cs-hero-image"),n=e.querySelector(".cs-headline"),a=n&&(n.querySelector("h1, .headline-xl, .headline-lg, .headline, [data-split], [role='heading'][aria-level='1']")||n),l=e.querySelector(".cs-titles-inner .headline-m");i&&gsap.set(i,{autoAlpha:0,y:100,filter:"blur(10px)"}),a&&gsap.set(a,{autoAlpha:0}),l&&gsap.set(l,{autoAlpha:0,y:20,filter:"blur(10px)"}),i&&t.to(i,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.7,ease:"power2.out"},0),a&&t.addLabel("cs_h",.15).set(a,{autoAlpha:1},"cs_h").call((function(){(CoreUtilities&&CoreUtilities.Fonts&&CoreUtilities.Fonts.ready?CoreUtilities.Fonts.ready():Promise.resolve()).then((function(){if(window.SplitText&&window.splitAndMask&&window.animateLines&&window.safelyRevertSplit){var e=splitAndMask(a);animateLines(e.lines).eventCallback("onComplete",(function(){safelyRevertSplit(e,a)}))}else gsap.to(a,{y:0,autoAlpha:1,duration:.6,ease:"power2.out"})}))}),null,"cs_h"),l&&t.to(l,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.5,ease:"power2.out"},"cs_h+=0.6");return t};
