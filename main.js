...
// Core Utilities
	window.CoreUtilities = (function () {
		...
		const Text={splitAndMask(e){if(!e)return null;if(e._originalHTML||=e.innerHTML,e._split)return e._split;if(void 0===window.SplitText)return gsap.set(e,{autoAlpha:1,clearProps:"yPercent, rotation, willChange"}),e._split={lines:[e],revert(){}},e._split;const t=getComputedStyle(e).whiteSpace||"normal",i=e.style.whiteSpace,l=e.style.display;e.style.whiteSpace=t,"inline"===getComputedStyle(e).display&&(e.style.display="block"),e.clientWidth;const n=new SplitText(e,{type:"lines",linesClass:"line",reduceWhiteSpace:!1});return n.lines.forEach((e=>{const i=e.getBoundingClientRect().height||e.offsetHeight||0,l=document.createElement("div");l.className="text-mask",l.style.overflow="hidden",l.style.display="block",l.style.height=i+"px",e.style.whiteSpace=t,e.style.display="block",e.parentNode.insertBefore(l,e),l.appendChild(e)})),gsap.set(n.lines,{yPercent:100,rotation:10,transformOrigin:"0 10%",willChange:"transform,opacity"}),e.style.whiteSpace=i,e.style.display=l,e._split=n,n},safelyRevertSplit(e,t){if(e&&t){try{e.revert&&e.revert()}catch{}t._originalHTML&&(t.innerHTML=t._originalHTML,delete t._originalHTML),delete t._split}},animateLines:e=>gsap.to(e,{yPercent:0,rotation:0,duration:.8,ease:"power2.out",stagger:.08})};
	...
	window.InitManager = (function () {
		...
		registries.common.push(
			...
			feature({
				...
				init: async r=>{const t=".appear-in-line",o=":scope > *",n=.15,i=.8;function a(e){if(e.__ailInit)return;e.__ailInit=!0;const t=[],a=[];e.setAttribute("data-ail","1"),[...e.querySelectorAll(o)].forEach(o=>{const r=getComputedStyle(o),s=parseInt(r.columnCount,10)||1,l=o.getBoundingClientRect();if(s>1){try{const r=new SplitText(o,{type:"lines",linesClass:"split-line"});t.push(r);const c=l.width/s,u=Array.from({length:s},()=>[]);r.lines.forEach(t=>{const r=t.getBoundingClientRect().left-l.left,o=Math.min(Math.floor(r/c),s-1);u[o].push(t),gsap.set(t,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"})}),u.forEach(e=>a.push(e))}catch{gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),a.push([o])}}else gsap.set(o,{y:100,opacity:0,filter:"blur(10px)",willChange:"transform,opacity"}),a.push([o])});const s=new IntersectionObserver((t,o)=>{t.forEach(t=>{if(!t.isIntersecting)return;o.unobserve(t.target),e.removeAttribute("data-ail"),a.forEach((t,o)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:o*n})});const r=(a.length-1)*n+i;gsap.delayedCall(r+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})})},{root:null,rootMargin:"0px 0px -12% 0px",threshold:0});CoreUtilities.Observers.addDom(s),s.observe(e);const l=e.getBoundingClientRect();if(l.top<innerHeight&&l.bottom>0){s.unobserve(e),e.removeAttribute("data-ail"),a.forEach((t,e)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:e*n})});const o=(a.length-1)*n+i;gsap.delayedCall(o+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})}const c=r.querySelector(".w-tabs");if(c){const r=e.closest(".w-tab-pane");r&&CoreUtilities.Observers.addDom(new MutationObserver(()=>{if(!r.classList.contains("w--tab-active"))return;s.observe(e);const o=e.getBoundingClientRect();if(o.top<innerHeight&&o.bottom>0){s.unobserve(e),e.removeAttribute("data-ail"),a.forEach((t,e)=>{gsap.to(t,{y:0,opacity:1,filter:"blur(0px)",duration:i,ease:"power2.out",delay:e*n})});const r=(a.length-1)*n+i;gsap.delayedCall(r+.05,()=>{try{t.forEach(e=>e.revert())}catch{}})}}).observe(r,{attributes:!0,attributeFilter:["class"]})}}r.querySelectorAll(t).forEach(a)}
			}),
