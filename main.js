	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: r=>{if(!gsap||!ScrollTrigger)return;r=r||document;const S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;S.forEach((sec,i)=>{const tall=!(sec.offsetHeight<innerHeight),start=i===0?"top top":tall?"bottom bottom":"top top";gsap.timeline({scrollTrigger:{trigger:sec,start:start,end:"bottom top",pin:!0,pinSpacing:!1,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:self=>{if(Math.abs(self.getVelocity())<5)return;}}}).to(sec,{filter:"contrast(10%) blur(10px)",ease:"none",startAt:{filter:"contrast(100%) blur(0px)"}})});try{ScrollTrigger.refresh()}catch(e){}document.addEventListener("click",e=>{if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){const all=ScrollTrigger.getAll();all.forEach(st=>st.disable());setTimeout(()=>{all.forEach(st=>st.enable());ScrollTrigger.refresh()},350);}});}
			}),
			feature({
				id: "productGallery",
				stage: "main",
