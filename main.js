	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async r=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;if(!matchMedia("(pointer:fine)").matches||matchMedia("(max-width:1023px)").matches)return;const root=r||document,sections=[...root.querySelectorAll(".section-single-service")];if(!sections.length)return;sections.forEach((s,i)=>{if(s._pinBound)return;s._pinBound=1;const last=i===sections.length-1;ScrollTrigger.create({id:"servicePin:"+i,trigger:s,start:"top top",end:last?"bottom top":"bottom+=20% top",pin:!last,pinSpacing:!0,scrub:!0,invalidateOnRefresh:!0,anticipatePin:1,onUpdate:self=>{const p=self.progress;gsap.to(s,{filter:`saturate(${1+.4*p}) contrast(${1+.3*p})`,duration:.15,overwrite:"auto"})},onLeave:()=>gsap.to(s,{filter:"none",duration:.2}),onLeaveBack:()=>gsap.to(s,{filter:"none",duration:.2}),onKill:()=>gsap.set(s,{clearProps:"filter"})});});}
			}),
			feature({
				id: "productGallery",
				stage: "main",
