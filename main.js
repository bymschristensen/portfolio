	...
	window.InitManager = (function () {
		...
		registries.pages.info.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const m=gsap.matchMedia();m.add("(min-width:1024px)",(()=>{const sec=r.querySelector('[data-imgscroll="photos"]'),bio=r.querySelector('[data-imgscroll="bio"]'),cpy=r.querySelector('[data-imgscroll="copy"]'),sts=r.querySelector('[data-imgscroll="stats"]'),hd=r.querySelector(".bio-text .headline-md");if(!sec||!bio||!cpy||!sts||!hd)return;const items=[...sec.querySelectorAll('.bio-photo-wrapper[data-photo]')].sort((a,b)=>(+a.getAttribute("data-photo")||0)-(+b.getAttribute("data-photo")||0)),p1=items.find(e=>e.getAttribute("data-photo")==="1"),p2=items.find(e=>e.getAttribute("data-photo")==="2"),p3=items.find(e=>e.getAttribute("data-photo")==="3");if(!p1||!p2||!p3)return;const rc=e=>e.getBoundingClientRect(),base=()=>{gsap.set([p1,p2,p3],{x:0,y:0,scale:1,transformOrigin:"0 0",willChange:"transform",zIndex:i=>300-(+items[i].getAttribute("data-photo")||0)})},anchor=()=>({left:rc(sts).left,top:rc(hd).top}),delta=(el,an,o)=>{o=o||{};const b=rc(el);return{x:an.left-b.left+(o.dx||0),y:an.top-b.top+(o.dy||0),scale:o.scale==null?1:o.scale}},cfg={p3:{dx:44,dy:56,scale:.36},p2:{dx:18,dy:22,scale:.38},p1:{dx:0,dy:0,scale:.42}};let tls=[];const rebuild=()=>{base();const an=anchor();tls.forEach(t=>{try{t.kill()}catch{}}),tls=[];const mk=(el,opt,st,en)=>gsap.to(el,{...delta(el,an,opt),ease:"power3.out",overwrite:"auto",immediateRender:!1,scrollTrigger:{trigger:bio,start:st,end:en,scrub:1,fastScrollEnd:!0,anticipatePin:1,invalidateOnRefresh:!0}}),s="center center";tls.push(mk(p3,cfg.p3,`${s}+=0%`,`${s}-=40%`));tls.push(mk(p2,cfg.p2,`${s}+=6%`,`${s}-=34%`));tls.push(mk(p1,cfg.p1,`${s}+=12%`,`${s}-=28%`))};rebuild();const ro=new ResizeObserver(()=>gsap.delayedCall(.03,rebuild));[sec,bio,cpy,sts,hd].forEach(e=>e&&ro.observe(e));return()=>{tls.forEach(t=>{try{t.kill()}catch{}});try{ro.disconnect()}catch{}}}))}
			}),
			feature({
