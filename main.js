...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-container"],init:async function(root){const sec=root.querySelector(".hero-section-showreel");const win=root.querySelector(".hero-section-showreel .showreel-container");if(!sec||!win)return;let track=win.querySelector(".showreel-track");if(!track){track=document.createElement("div");track.className="showreel-track";while(win.firstChild)track.appendChild(win.firstChild);win.appendChild(track);}const area=root.querySelector(".hero-section-showreel .showreel-inner")||root.querySelector(".hero-section-showreel .container")||sec;let nx=.5,ny=.5,tx=.5,ty=.5,raf=0,active=!1;const XR=80,YR=40,LERP=.12;function lerp(a,b,t){return a+(b-a)*t}function onMove(e){const r=area.getBoundingClientRect();const x=Math.max(r.left,Math.min(e.clientX,r.right));const y=Math.max(r.top,Math.min(e.clientY,r.bottom));nx=(x-r.left)/Math.max(1,r.width);ny=(y-r.top)/Math.max(1,r.height);kick();}function kick(){if(active)return;active=!0;raf=requestAnimationFrame(tick);}function tick(){tx=lerp(tx,nx,LERP);ty=lerp(ty,ny,LERP);const dx=(tx-.5)*XR*2;const dy=(ty-.5)*YR*2;track.style.setProperty("--sx",dx.toFixed(2)+"px");track.style.setProperty("--sy",dy.toFixed(2)+"px");const still=Math.abs(nx-tx)<.001&&Math.abs(ny-ty)<.001;active=!still;raf=active?requestAnimationFrame(tick):0;}function onResize(){kick();}sec.addEventListener("mousemove",onMove,{passive:!0});sec.addEventListener("pointermove",onMove,{passive:!0});addEventListener("resize",onResize,{passive:!0});kick();if(document.__heroShowreel&&document.__heroShowreel.kill)document.__heroShowreel.kill();document.__heroShowreel={kill:function(){try{sec.removeEventListener("mousemove",onMove);}catch{}try{sec.removeEventListener("pointermove",onMove);}catch{}try{removeEventListener("resize",onResize);}catch{}try{raf&&cancelAnimationFrame(raf);}catch{}raf=0;}}},destroy:async function(){try{document.__heroShowreel&&document.__heroShowreel.kill&&document.__heroShowreel.kill();}catch{}document.__heroShowreel=null;}),
			feature({
