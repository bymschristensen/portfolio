...
// Entry Orchestrator
	window.EntryOrchestrator = window.EntryOrchestrator || (function () {
		...
		const EntryAnimations = {
			...
			caseStudy(e){const t=gsap.timeline(),a=e.querySelector(".cs-hero-image"),i=e.querySelector(".cs-headline"),o=e.querySelectorAll(".cs-titles-inner div"),n=e=>{if(!e)return;try{e.removeAttribute("data-entry-hidden"),e.removeAttribute("hidden"),e.removeAttribute("aria-hidden"),e.style.visibility="visible",["hidden","is-hidden","u-hidden","opacity-0","visually-hidden"].forEach(t=>e.classList?.remove(t))}catch{}};a&&gsap.set(a,{autoAlpha:0,y:80,filter:"blur(10px)"}),i&&gsap.set(i,{autoAlpha:0,visibility:"hidden"}),o.length&&gsap.set(o,{autoAlpha:0,y:20,filter:"blur(10px)"}),n(a),n(i),o.forEach(n),a&&t.to(a,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out"},0),i&&t.addLabel("headline",.35).call(()=>{const r=()=>{try{const e=i.getBoundingClientRect().height;/* lock only height to prevent jump, keep display/width untouched */i.style.minHeight=Math.max(1,Math.round(e))+"px",i.style.visibility="visible",i.style.willChange="transform,opacity";/* make sure wrapping stays normal for SplitText */const d=i.style.whiteSpace;i.style.whiteSpace="normal";if(window.splitAndMask&&window.animateLines){const s=splitAndMask(i);gsap.set(i,{autoAlpha:1});animateLines(s.lines).eventCallback("onComplete",()=>{try{(window.safelyRevertSplit?window.safelyRevertSplit:s.revert).call(s,s,i)}catch{}i.style.willChange="",i.style.minHeight="",i.style.whiteSpace=d})}else gsap.set(i,{autoAlpha:1}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out",onComplete(){i.style.willChange="",i.style.minHeight="",i.style.whiteSpace=d}})}catch{gsap.set(i,{autoAlpha:1,visibility:"visible"}),gsap.to(i,{y:0,opacity:1,duration:.6,ease:"power2.out"})}};Promise.resolve(CoreUtilities?.Fonts?.ready?.()).then(()=>requestAnimationFrame(()=>requestAnimationFrame(r))).catch(r)},null,"headline"),o.length&&t.to(o,{autoAlpha:1,y:0,filter:"blur(0px)",duration:.6,ease:"power2.out",stagger:.05},.6);return t;}
		};
