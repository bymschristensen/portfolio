...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"heroShowreel",stage:"main",namespaces:["capabilities"],selectors:[".hero-section-showreel .showreel-track",".hero-section-showreel .showreel-container"],init:async function(e){if(matchMedia("(prefers-reduced-motion: reduce)").matches||!matchMedia("(pointer:fine)").matches)return;e=e||document;var t=e.querySelector(".hero-section-showreel");if(!t||t.__hsBound)return;var r=t.querySelector(".showreel-track"),n=t.querySelector(".hero-section-showreel .showreel-container");if(!r||!n)return;var o=t.querySelector(".showreel-outer"),i=t.querySelector(".showreel-track-wrapper"),l=t.querySelector(".showreel-headline-wrapper"),a=t.querySelector(".showreel-options");t.__hsBound=1;var c=window.CSS&&CSS.supports&&CSS.supports("aspect-ratio:16/9"),s={x:0,px:null,last:0,raf:0,w:0,tw:0,left:0,right:0,baseC:0,hasMove:!1,vx:0,sq:0,wP:0,prevSq:0,wob:0,scroll:0},u=2.2,d=.3,f=1300,h=.13,p={base:.8,min:.65,max:1},v=5,m=.5,g=.08,y=.002,w=0,b=0,_=0;i&&(_=parseFloat(getComputedStyle(i).marginTop)||0);function x(e,t,r){return e<t?t:e>r?r:e}function C(){var e=n.getBoundingClientRect(),t=r.getBoundingClientRect();s.w=e.width||n.offsetWidth||0;s.tw=t.width||r.offsetWidth||0;s.left=t.left;s.right=t.right;s.baseC=e.left+e.width/2-s.x;s.w&&!w&&(w=s.w);s.tw&&(b=s.tw);!c&&s.w&&(n.style.height=s.w*9/16+"px")}function M(e){var t=x(e,s.left,s.right),o=t+s.w/2,i=s.left+s.w/2,l=s.right-s.w/2;i>l&&(i=l=(s.left+s.right)/2);var a=x(o,i,l);return a-s.baseC}function S(e){var t=e.clientX;s.hasMove&&(s.vx+=t-(null==s.px?t:s.px));s.hasMove=!0;s.px=t}function T(){if(!o)return;var e=t.getBoundingClientRect(),r=window.innerHeight||document.documentElement.clientHeight,n=e.height||t.offsetHeight||0;if(!n||r>=n)s.scroll=0;else{var c=x(-e.top/(n-r),0,1);s.scroll=c}var u=s.scroll||0;if(l||a){var d=o?o.clientHeight||0:0,f=-u*d*.55;l&&(l.style.transform="translate3d(0,"+f+"px,0)");a&&(a.style.transform="translate3d(0,"+f+"px,0)")}if(i){var h=_*(1-(s.scroll||0));i.style.marginTop=h+"px"}}function k(e){s.last||(s.last=e);var t=(e-s.last)/1e3;t<0&&(t=0);t>.05&&(t=.05);s.last=e;C();var o=s.scroll||0;var l=p.base+(1-p.base)*o;var a=p.min+(l-p.min)*o;var c=p.max-(p.max-l)*(.5*o);var u=w||s.w;var d=b||s.tw||u;var f=0;if(o>.1){f=(o-.1)/.8;f>1&&(f=1)}else f=0;var h=u+(d-u)*f;h>0&&(n.style.width=h+"px");if(!s.hasMove||null==s.px){n.style.transform="translate3d("+s.x+"px,0,0) scale("+l+")";s.raf=requestAnimationFrame(k);return}var v=M(s.px),m=1-Math.exp(-u*t);s.x+=m*(v-s.x);s.vx*=.96;Math.abs(s.vx)<.02&&(s.vx=0);var g=0;s.vx&&(g=-s.vx/f,h);g>d?g=d:g<-d&&(g=-d);s.sq+=h*(g-s.sq);var y=l+s.sq;y<a?y=a:y>c&&(y=c);var w=1,_=y-l,C=s.prevSq,M=Math.abs(_),T=Math.abs(C),k=.02,E=.03,A=M<k&&T>E;if(A&&!s.wob){s.wob=1;s.wP=0}if(s.wob>0){s.wP+=v*t;var L=m*s.wob;w=1+L*Math.sin(s.wP);s.wob-=t/.9;if(s.wob<=.001){s.wob=0;s.wP=0;w=1}}s.prevSq=_;var H=y*w;n.style.transform="translate3d("+s.x+"px,0,0) scale("+H+")";s.raf=requestAnimationFrame(k)}C();n.style.transform="translate3d(0,0,0) scale("+p.base+")";addEventListener("pointermove",S,{passive:!0});addEventListener("resize",C,{passive:!0});addEventListener("scroll",T,{passive:!0});T();s.raf=requestAnimationFrame(k);t.__hsDestroy=function(){try{removeEventListener("pointermove",S)}catch(e){}try{removeEventListener("resize",C)}catch(e){}try{removeEventListener("scroll",T)}catch(e){}try{s.raf&&cancelAnimationFrame(s.raf)}catch(e){}}},destroy:async function(e){e=e||document;var t=e.querySelector(".hero-section-showreel")||document.querySelector(".hero-section-showreel");try{t&&t.__hsDestroy&&t.__hsDestroy()}catch(e){}}}),
			feature({
