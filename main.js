...
// Init Manager
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({id:"productPinnedSections",stage:"main",namespaces:["capabilities"],selectors:[".section-single-service"],init:function(r){if(!gsap||!ScrollTrigger)return;r=r||document;var S=[].slice.call(r.querySelectorAll(".section-single-service"));if(!S.length)return;var activeST=[];function disableAll(){activeST.forEach(function(st){st.disable()})}function enableAll(){activeST.forEach(function(st){st.enable()});ScrollTrigger.refresh()}document.addEventListener("click",function(e){if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){disableAll();setTimeout(enableAll,350)}});S.forEach(function(sec){var tl=gsap.timeline({scrollTrigger:{trigger:sec,start:"bottom bottom",end:"bottom top",pin:!0,pinSpacing:!1,scrub:1,invalidateOnRefresh:!0}});activeST.push(tl.scrollTrigger);tl.to(sec,{ease:"none",startAt:{filter:"contrast(100%) blur(0px)"},filter:"contrast(10%) blur(10px)"},0)});}}),
			feature({
				id: "productGallery",
				stage: "main",
				namespaces: ["capabilities"],
