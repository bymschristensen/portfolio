		...
		registries.pages.archive.push(
			feature({
				...
				init: function(root){
					...
					// Filters Module
					(function(){var KEY=state.KEY_FILTER||"archiveFilter",doc=el.doc,ids=["archive-filter-all","archive-filter-appswebapps","archive-filter-websites","archive-filter-ecommerce","archive-filter-platforms-systems","archive-filter-other","nav-archive-filter-all","nav-archive-filter-appswebapps","nav-archive-filter-websites","nav-archive-filter-ecommerce","nav-archive-filter-platforms-systems","nav-archive-filter-other"];function setActive(cat){for(var i=0;i<ids.length;i++){var id=ids[i],x=doc.getElementById(id);if(!x)continue;var onv=baseId(id)===cat;x.classList.toggle("is-active",onv);try{x.setAttribute("aria-current",onv?"true":"false")}catch(_){}try{x.setAttribute("aria-pressed",onv?"true":"false")}catch(_){}}}function stored(){try{var v=localStorage.getItem(KEY);return v?normCat(v):null}catch(_){return null}}function getAll(){var a=[],p=el.P,m=el.M;p&&(a=a.concat([].slice.call(p.querySelectorAll(".archive-playful-item"))));m&&(a=a.concat([].slice.call(m.querySelectorAll(".archive-minimal-item"))));return a}function readCats(it){if(it.__asCats)return it.__asCats;for(var a=["all"],n=it.querySelectorAll(".archive-categories .cms-categories"),i=0;i<n.length;i++){var t=(n[i].textContent||"").trim();t&&(t=normCat(t))&&-1===a.indexOf(t)&&a.push(t)}return it.__asCats=a,a}function applyOnce(cat){cat=normCat(cat||"all")||"all";state.filter=cat;setActive(cat);for(var items=getAll(),shown=0,i=0;i<items.length;i++){var it=items[i],cats=readCats(it),show="all"===cat||-1!==cats.indexOf(cat);it.style.display=show?"":"none";show&&shown++}return{cat:cat,shown:shown}}function emit(cat){try{doc.dispatchEvent(new CustomEvent("archive:filter:applied",{bubbles:!0,detail:{filter:cat}}))}catch(_){}}function apply(cat,meta){var res=applyOnce(cat);if("all"!==res.cat&&0===res.shown){try{localStorage.removeItem(KEY)}catch(_){}res=applyOnce("all")}if("user"===meta)try{localStorage.setItem(KEY,res.cat)}catch(_){}emit(res.cat);try{mod.scheduleMotionRefresh&&mod.scheduleMotionRefresh()}catch(_){}}function onClick(ev){var t=ev&&ev.target&&ev.target.closest?ev.target.closest('[id^="archive-filter-"],[id^="nav-archive-filter-"]'):null;if(!t||!t.id)return;ev.preventDefault&&ev.preventDefault();apply(baseId(t.id),"user")}function onExternal(ev){try{var d=ev&&ev.detail||{},cat=d.filter||d.category||d.id||"";cat=baseId(String(cat||""));cat&&apply(cat,"user")}catch(_){}}function onView(){apply(stored()||"all","auto")}mod.applyFilter=function(cat,save){apply(cat,save?"user":"auto")};mod.getFilter=function(){return state.filter||stored()||"all"};on(doc,"click",onClick,!0);on(doc,"archive:filter",onExternal,{passive:!0});on(doc,"archive:view",onView,{passive:!0});apply(stored()||"all","auto")})();

					// Motion Scheduler
					(function(){var raf=0;function active(){try{var v=mod.getView&&mod.getView();if("playful"===v&&el.P&&getComputedStyle(el.P).display!=="none")return el.P;if("minimal"===v&&el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){}try{if(el.P&&getComputedStyle(el.P).display!=="none")return el.P;if(el.M&&getComputedStyle(el.M).display!=="none")return el.M}catch(_){}return el.host||host}mod.resetMotion=function(){try{window.__archiveListMotion&&__archiveListMotion.resetAndRun&&__archiveListMotion.resetAndRun(active())}catch(_){}};mod.scheduleMotionRefresh=function(){if(raf)return;raf=requestAnimationFrame(function(){raf=0;mod.resetMotion&&mod.resetMotion()})};on(document,"archive:view",mod.scheduleMotionRefresh,{passive:!0});on(document,"archive:filter:applied",mod.scheduleMotionRefresh,{passive:!0})})();
