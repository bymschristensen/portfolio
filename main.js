	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async r=>{if(!gsap||!ScrollTrigger)return;const S=[...r.querySelectorAll(".section-single-service")];if(!S.length)return;S.forEach((sec,i)=>{const tall=!(sec.offsetHeight<innerHeight),start=tall?"bottom bottom":"top top";const tl=gsap.timeline({scrollTrigger:{trigger:sec,start:i===0?"top top":start,end:"bottom top",pin:!0,pinSpacing:!1,scrub:!0,anticipatePin:1,invalidateOnRefresh:!0,onUpdate:self=>{if(Math.abs(self.getVelocity())<5)return;}}});tl.to(sec,{filter:"contrast(10%) blur(10px)",ease:"none",startAt:{filter:"contrast(100%) blur(0px)"}})});try{ScrollTrigger.refresh()}catch{};document.addEventListener("click",e=>{if(e.target.closest(".accordion")||e.target.closest("[data-accordion]")){ScrollTrigger.getAll().forEach(st=>st.disable());setTimeout(()=>{ScrollTrigger.getAll().forEach(st=>st.enable());ScrollTrigger.refresh();},350);}})
			}),
			feature({
				id: "productGallery",
				stage: "main",
