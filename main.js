	...
	window.InitManager = (function () {
		...
		registries.pages.capabilities.push(
			...
			feature({
				...
				init: async e=>{if(matchMedia("(prefers-reduced-motion: reduce)").matches)return;const t=[...e.querySelectorAll(".infinite-gallery")];if(!t.length)return;const r=e=>{let t=0;const r=parseFloat(getComputedStyle(e).gap||"0")||0;return e.childNodes.forEach(e=>{1===e.nodeType&&(t+=(e.getBoundingClientRect().width||0)+r)}),t};t.forEach(async e=>{const t=e.querySelector(".infinite-gallery-wrapper");if(!t||t.__inited)return;t.__inited=!0,e.setAttribute("data-armed","measure");const i=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")];if(!i.length)return void e.setAttribute("data-armed","1");await(e=>{const t=[...e.querySelectorAll("img,video")];if(!t.length)return Promise.resolve();const r=e=>e.naturalWidth>0&&e.naturalHeight>0||e.videoWidth>0&&e.videoHeight>0,i=[];return Promise.race([Promise.all(t.map(e=>{if(r(e))return Promise.resolve();const t="function"==typeof e.decode?e.decode().catch(()=>{}):Promise.resolve(),i=new Promise(t=>{const r=()=>{e.removeEventListener("load",r),t()};e.addEventListener("load",r,{once:!0})}),a=new Promise(t=>{let i=0;!function a(){if(r(e)||i++>80)return t();setTimeout(a,40)}()});return Promise.race([t,i,a])})),new Promise(e=>i.push(setTimeout(e,3500)))]).finally(()=>i.forEach(clearTimeout))})(t);const a=new Set,n="right"===(e.dataset.direction||"left").toLowerCase()?-1:1,o=parseFloat(e.dataset.speed)||.6;i.forEach((e,t)=>{e.style.height="";const r=e.getBoundingClientRect(),i=parseFloat(getComputedStyle(e).height)||r.height||0,a=r.width;e.dataset.key=String(t),e.dataset.targetH=String(i),e.dataset.revealed="0",e.dataset.revealing="",e.style.minWidth=a+"px",e.style.maxWidth=a+"px",e.style.overflow="hidden",e.style.flex="0 0 auto",e.style.willChange="height,transform",e.style.height="0px"});const l=()=>[...t.querySelectorAll('.service-visual-wrapper[data-clone="1"]')].forEach(e=>e.remove()),s=e=>{const r=e.cloneNode(!0),i=e.dataset.key||"",n=+e.dataset.targetH||0;return r.setAttribute("data-clone","1"),r.dataset.key=i,r.style.overflow="hidden",r.style.flex="0 0 auto",r.style.willChange="height,transform",r.style.minWidth=e.style.minWidth,r.style.maxWidth=e.style.maxWidth,r.style.height=a.has(i)&&n>0?n+"px":"0px",t.appendChild(r),r},d=()=>{i.forEach(s)},c=()=>{l(),d(),d();let e=0;const i=3*Math.max(document.documentElement.clientWidth,window.innerWidth||0);for(;r(t)<i&&e++<8;)d()};c(),e.setAttribute("data-armed","1");let h=0,u=0,m=0;const g=e=>{if(!f)return void cancelAnimationFrame(h);if((e||0)-u<32)return void(h=requestAnimationFrame(g));u=e||performance.now();const r=innerWidth||document.documentElement.clientWidth||0,i=-.05*r,n=1.05*r,o=[...t.querySelectorAll(".service-visual-wrapper:not([data-clone])")].filter(e=>{const t=e.getBoundingClientRect();return t.right>i&&t.left<n}).filter(e=>"1"!==e.dataset.revealed&&"1"!==e.dataset.revealing).sort((e,t)=>e.getBoundingClientRect().left-t.getBoundingClientRect().left);if(o.length){const e=gsap.timeline();o.forEach((r,i)=>{const n=+r.dataset.targetH||0,o=r.dataset.key||"";r.dataset.revealing="1",e.fromTo(r,{height:0},{height:n,duration:.9,ease:"power2.out",onComplete:()=>{r.style.height="",r.dataset.revealed="1",r.dataset.revealing="",a.add(o),t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${o}"]`).forEach(e=>e.style.height=n+"px")}},.12*i)})}h=requestAnimationFrame(g)};let f=!1;const p=new IntersectionObserver(t=>{t.forEach(t=>{t.target===e&&(t.isIntersecting?(f=!0,cancelAnimationFrame(h),h=requestAnimationFrame(g)):(f=!1,cancelAnimationFrame(h)))})},{root:null,threshold:0,rootMargin:"0px 0px -5% 0px"});p.observe(e),CoreUtilities.Observers.addDom(p),CoreUtilities.Observers.addDom({disconnect(){f=!1,cancelAnimationFrame(h)}});const y=new ResizeObserver(()=>{l(),c(),i.forEach(e=>{const r=e.dataset.key||"",i=+e.dataset.targetH||0;t.querySelectorAll(`.service-visual-wrapper[data-clone="1"][data-key="${r}"]`).forEach(e=>e.style.height=i+"px")})});y.observe(t),CoreUtilities.Observers.addDom(y);const v=(e,r)=>((e,r)=>{m-=n*((r||16.6667)/1e3)*(60*o);const i=parseFloat(getComputedStyle(t).gap||"0")||0,a=e=>(e.getBoundingClientRect().width||0)+i;let l=t.firstElementChild,s=0;for(;l&&m<-a(l)&&s++<50;)m+=a(l),t.appendChild(l),l=t.firstElementChild;let d=t.lastElementChild;for(s=0;d&&m>0&&s++<50;)m-=a(d),t.insertBefore(d,t.firstElementChild),d=t.lastElementChild;gsap.set(t,{x:m});const c=(innerWidth||0)/2;t.querySelectorAll(".service-visual").forEach(e=>{const t=e.closest(".service-visual-wrapper");if(!t)return;const r=t.getBoundingClientRect(),i=(c-(r.left+r.width/2))/(innerWidth||1);e.style.setProperty("--drift",40*i+"px")})})(0,r);gsap.ticker.add(v),CoreUtilities.Observers.addTicker(v)})}
			})
		);
