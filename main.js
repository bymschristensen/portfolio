	...
	window.InitManager = (function () {
		...
		registries.pages.caseStudy.push(
			feature({
				...
				init: async r=>{if(!window.gsap||!window.ScrollTrigger)return;const c=r.closest(".barba-container")||r,d=c.querySelector(".cs-details"),m=c.querySelector(".cs-morework");if(!d||!m)return;let sc=window;const findScroller=e=>{for(let n=e;n&&n!==document.documentElement;n=n.parentElement){const s=getComputedStyle(n),oy=s.overflowY;if((oy==="auto"||oy==="scroll")&&n.scrollHeight>n.clientHeight+2)return n}return window};sc=findScroller(d);try{ScrollTrigger.getById("csbg:details")?.kill();ScrollTrigger.getById("csbg:more")?.kill()}catch{}const cs=getComputedStyle(c).backgroundColor,bg="var(--colors--background)",bd="var(--colors--border)";let l="";c.style.transition="background-color .6s ease";const set=v=>{if(v===l)return;l=v,gsap.set(c,{backgroundColor:v,overwrite:"auto"})};ScrollTrigger.create({id:"csbg:details",scroller:sc,trigger:d,start:"top bottom",onEnter:()=>set(bg),onLeaveBack:()=>set(cs),invalidateOnRefresh:!0,refreshPriority:100});ScrollTrigger.create({id:"csbg:more",scroller:sc,trigger:m,start:"top 80%",onEnter:()=>set(bd),onLeaveBack:()=>set(bg),invalidateOnRefresh:!0,refreshPriority:110});const ref=()=>requestAnimationFrame(()=>ScrollTrigger.refresh());ref(),setTimeout(ref,160),setTimeout(ref,520),setTimeout(ref,1100);const g=c.querySelector(".cs-gallery");if(g&&"ResizeObserver"in window){let h=g.getBoundingClientRect().height,ro=new ResizeObserver(()=>{const nh=g.getBoundingClientRect().height;if(Math.abs(nh-h)>1){h=nh,ref(),setTimeout(ref,160)}});try{ro.observe(g)}catch{}setTimeout(()=>{try{ro.disconnect()}catch{}},2e4);return()=>{try{ro.disconnect()}catch{}}}}
			}),
			feature({
